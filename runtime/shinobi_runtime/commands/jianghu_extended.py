...
from shinobi_runtime.martial_world.physical_presence import active_combat_for_person, effective_person_presence, physical_unavailable_person_refs, same_effective_location
...
from shinobi_runtime.martial_world.exact_combat import canonical_player_action_kind, combat_default_targeting_intent, currently_visible_enemies, default_action_for, default_target_for, default_weapon_for_action, improvised_weapon_state_from_scene, initialize_combat, resolve_exchange, attempt_disengage
from shinobi_runtime.martial_world.scene_sessions import SESSION_PATH as _SCENE_SESSION_PATH, active_scene_session, scene_history_record
from shinobi_runtime.martial_world.health import functional_capacity_factors, record_current_wound
from shinobi_runtime.martial_world.retinue_support import emergency_field_stabilization_policy
...
from shinobi_runtime.martial_world.npc_judgment import (
    build_npc_judgment_event, institutional_office_judgment_events,
    option_authors_player_intent, selected_option, remove_pending_judgment,
)
...
_COMBAT_TERMINAL_STATUSES=frozenset({'dead','unconscious','incapacitated','escaped'})


def _legacy_route_temporary_combat_refs(
    *,
    ...
    """Recover old route-party companions without inventing combat membership."""
    standing={str(ref) for ref in standing_refs if isinstance(ref,str)} | {str(actor_ref)}
    actor_side_refs:set[str]=set()
    sides=combat.get('sides',{}) if isinstance(combat.get('sides'),Mapping) else {}
    for side_refs in sides.values():
        if isinstance(side_refs,list) and actor_ref in side_refs:
            actor_side_refs={str(ref) for ref in side_refs if isinstance(ref,str)}
            break
    route_candidates=movement.get('escort_refs')
    if not isinstance(route_candidates,list):
        route_candidates=movement.get('participant_refs',[])
    return [
        str(ref) for ref in route_candidates
        if isinstance(ref,str)
        and ref not in standing
        and (not actor_side_refs or ref in actor_side_refs)
    ]

def _combat_terminal_snapshot(combat:Mapping[str,Any])->tuple[tuple[str,tuple[str,...]],...]:
    states=combat.get('combatants',{}) if isinstance(combat.get('combatants'),Mapping) else {}
    return tuple((str(ref),tuple(sorted({str(x) for x in row.get('status_families',[]) if isinstance(x,str)}&_COMBAT_TERMINAL_STATUSES))) for ref,row in sorted(states.items()) if isinstance(ref,str) and isinstance(row,Mapping))

def _combat_pressure_metrics(combat:Mapping[str,Any])->tuple[int,int]:
    positions=combat.get('positions',{}) if isinstance(combat.get('positions'),Mapping) else {}; sides=combat.get('sides',{}) if isinstance(combat.get('sides'),Mapping) else {}
    active_by_side=[]
    states=combat.get('combatants',{}) if isinstance(combat.get('combatants'),Mapping) else {}
    for refs in sides.values():
        if not isinstance(refs,list): continue
        active=[]
        for ref in refs:
            if not isinstance(ref,str): continue
            row=states.get(ref,{}) if isinstance(states.get(ref),Mapping) else {}; statuses={str(x) for x in row.get('status_families',[]) if isinstance(x,str)}
            if not statuses&_COMBAT_TERMINAL_STATUSES: active.append(ref)
        active_by_side.append(active)
    pairs=pressure=0
    for i,left in enumerate(active_by_side):
        for right in active_by_side[i+1:]:
            for a in left:
                pa=positions.get(a);
                if not isinstance(pa,Mapping): continue
                for b in right:
                    pb=positions.get(b)
                    if not isinstance(pb,Mapping) or pa.get('zone_ref')!=pb.get('zone_ref'): continue
                    dx=int(pa.get('x_mm',0))-int(pb.get('x_mm',0)); dy=int(pa.get('y_mm',0))-int(pb.get('y_mm',0)); d=math.isqrt(dx*dx+dy*dy)
                    if d<=12000: pairs+=1; pressure+=12000-d
    return pairs,pressure

def _combat_material_progress_event(event:Mapping[str,Any])->bool:
    if str(event.get('result') or '') in {'dead','incapacitated','escaped','withdrew_from_combat','support_treatment_completed'}: return True
    physiology=event.get('physiology') if isinstance(event.get('physiology'),Mapping) else {}
    if str(physiology.get('status') or '') in {'dead','incapacitated','unconscious'}: return True
    damage=event.get('damage') if isinstance(event.get('damage'),Mapping) else {}; wound=damage.get('wound') if isinstance(damage.get('wound'),Mapping) else {}
    if wound:
        return max(0,int(wound.get('severity',0) or 0))>=20 or max(0,int(wound.get('bleeding_ml_per_min',0) or 0))>0 or max(0,int(wound.get('function_loss_pct',0) or 0))>0 or max(max(0,int(wound.get('fracture',0) or 0)),max(0,int(wound.get('tendon_damage',0) or 0)),max(0,int(wound.get('nerve_damage',0) or 0)),max(0,int(wound.get('organ_trauma',0) or 0)))>0
    return False

def _combat_resolution_progress(before:Mapping[str,Any],after:Mapping[str,Any],events:Sequence[Mapping[str,Any]])->bool:
    if str(after.get('status') or '')!='active' or _combat_terminal_snapshot(before)!=_combat_terminal_snapshot(after): return True
    if any(_combat_material_progress_event(event) for event in events if isinstance(event,Mapping)): return True
    before_pairs,before_pressure=_combat_pressure_metrics(before); after_pairs,after_pressure=_combat_pressure_metrics(after)
    return after_pairs<before_pairs or (before_pressure>0 and after_pressure<=before_pressure*4//5)


def _player_poison_instruction(payload:Mapping[str,Any])->tuple[str|None,bool]:
    """Return (explicit poison ref, doctrine-auto) for one player combat payload.

    Finite consumables require explicit authority. Omission and ``none`` mean no
    poison; only the explicit ``auto`` token delegates poison selection/spending
    to the current doctrine for this command span.
    """
    supplied='poison_ref' in payload
    raw=payload.get('poison_ref') if supplied else None
    normalized=str(raw or '').lower()
    auto=bool(supplied and normalized=='auto')
    explicit=None if normalized in {'','none','auto'} else str(raw)
    return explicit,auto


def _resolve_player_combat_span_base(
    *, combat: Mapping[str, Any], people: Mapping[str, Mapping[str, Any]],
    ...
    movement_intent: str | None = None,
    maneuver_only: bool = False,
    rally_allies: bool = False,
    ally_orders: Sequence[Mapping[str, Any]] | None = None,
    player_improvised_weapon_state: Mapping[str, Any] | None = None,
    npc_judgments: Mapping[str, Mapping[str, Any]] | None = None,
    require_gm_npc_judgments: bool = False,
    ...
    while str(combat_cursor.get('status') or '')=='active' and not requested_scope_complete():
        if exchanges>=max(1,int(frontier_exchanges)):
            ...
            break
        guard_only = raw_action_kind == 'hold'
        if guard_only:
        ...
        if guard_only:
            action_kind='hold'
            weapon_ref='body_unarmed'
            resolved_intent='disable'
        elif maneuver_only:
            if raw_weapon_ref in {'','auto'}:
                try:
                    weapon_ref=default_weapon_for_action(
        ...
        resolved=resolve_exchange(
            ...
            player_poison_ref=(None if maneuver_only else explicit_poison_ref),
            player_qi_allocation_milli=explicit_qi_allocation_milli,
            player_auto_qi=bool(qi_auto),player_auto_poison=bool(poison_auto and not maneuver_only),
            player_action_detail_origin=(
                "player" if raw_target_ref not in {"","auto"} and raw_action_kind not in {"attack","auto"} and raw_weapon_ref not in {"","auto"}
                else "player_adaptive"
            ),
            player_movement_intent=movement_intent,
            player_rally_allies=bool(rally_allies and exchanges==0),
            player_ally_orders=ally_orders,
            ...
            npc_judgments=npc_judgments,require_gm_npc_judgments=require_gm_npc_judgments,
        )
        ...
        for raw_event in resolved.get('events',[]):
            ...
            if str(event.get('actor_ref') or '')==player_ref and str(event.get('action_kind') or '')!='maneuver':
        ...
        for raw_beat in projection.get('beats',[]) if isinstance(projection.get('beats'),list) else []:
            if isinstance(raw_beat,Mapping):
                narrative_beats.append(copy.deepcopy(dict(raw_beat)))
        for rule in projection.get('narration_rules',[]) if isinstance(projection.get('narration_rules'),list) else []:
            if isinstance(rule,str) and rule not in narrative_rules:
                narrative_rules.append(rule)
        exchanges+=1
        if require_gm_npc_judgments and str(combat_cursor.get('status') or '')=='active':
            # One GM judgment packet describes one exact decision frontier. Never
            # reuse those human choices after geometry, wounds, readiness, or
            # perception changes. The standing player intent may be continued by
            # the GM after a fresh play-context/judgment projection.
            stop_reason='gm_judgment_refresh'
            break
        ...
        for event in events:
            if str(event.get('actor_ref') or '')!=player_ref or str(event.get('result') or '') in rejected_results or str(event.get('action_kind') or '')=='maneuver':
                continue
    ...
    elif stop_reason=='scope_complete' and requested_scope_complete():
        stop_reason='scope_complete'
    if (
        bool(until_resolution) and exchange_count is None and duration_seconds is None
        and stop_reason=='execution_frontier' and str(combat_cursor.get('status') or '')=='active'
        and not _combat_resolution_progress(combat,combat_cursor,[event for event in all_events if isinstance(event,Mapping)])
    ):
        stop_reason='stagnation_checkpoint'
        rule='The bounded combat span did not reduce opposing contact pressure, produce a material injury, complete treatment, or change a terminal combat state; return control instead of auto-continuing.'
        if rule not in narrative_rules: narrative_rules.append(rule)
    continuation_required=(stop_reason in {'execution_frontier','gm_judgment_refresh'} and str(combat_cursor.get('status') or '')=='active')
    if explicit_lethal_pursuit and isinstance(people_cursor.get(player_ref),dict):
    ...
    protected=[row for row in narrative_beats if row.get('salience')=='protected']
    ordinary=[row for row in narrative_beats if row.get('salience')!='protected']
    slots=max(0,48-len(protected))
    if len(ordinary)<=slots:
        sampled_ordinary=[copy.deepcopy(dict(row)) for row in ordinary]
    elif slots<=0:
        sampled_ordinary=[]
    elif slots==1:
        sampled_ordinary=[copy.deepcopy(dict(ordinary[-1]))]
    else:
        indices=[]
        for slot in range(slots):
            idx=(slot*(len(ordinary)-1)+((slots-1)//2))//(slots-1)
            if idx not in indices:indices.append(idx)
        sampled_ordinary=[copy.deepcopy(dict(ordinary[idx])) for idx in sorted(indices)]
    bounded_beats=sorted(
        [*protected,*sampled_ordinary],
        key=lambda row:(int(row.get('at_ms',0) or 0),0 if row.get('salience')=='protected' else 1),
    )
    last_projection=last_resolved.get('narrative_projection') if isinstance(last_resolved.get('narrative_projection'),Mapping) else {}
    span_projection={
        ...
        'beats':bounded_beats,
        'material_beat_count':len(narrative_beats),
        'material_beats_truncated':len(bounded_beats)<len(narrative_beats),
        'omitted_material_beat_count':max(0,len(narrative_beats)-len(bounded_beats)),
        'coverage':'stratified_beginning_middle_end_with_protected_beats',
        'protected_salience_count':len(protected),
        'current_visibility':copy.deepcopy(dict(last_projection.get('current_visibility',{}))) if isinstance(last_projection.get('current_visibility'),Mapping) else {},
        'narration_rules':narrative_rules,
    }
    return {
        **last_resolved,
        ...
        'narrative_projection':span_projection,
    }


def _resolve_player_combat_span(**kwargs: Any) -> dict[str, Any]:
    """Canonical production combat-span composition with no runtime monkey patching."""
    from shinobi_runtime.commands.combat_adaptation import adaptive_standing_span
    from shinobi_runtime.commands.combat_span_safety import bounded_standing_span, _normalize_production_span_information

    if kwargs.get('player_improvised_weapon_state') is not None:
        result=bounded_standing_span(_resolve_player_combat_span_base,**kwargs)
    else:
        result=adaptive_standing_span(
            _resolve_player_combat_span_base,
            fallback=bounded_standing_span,
            max_elapsed_ms=300_000,
            standing_exchange_limit=16,
            **kwargs,
        )
    result=_normalize_production_span_information(result,kwargs)
    if str(result.get('scope_stop_reason') or '')=='tactical_stagnation':
        normalized=copy.deepcopy(dict(result)); normalized['scope_stop_reason']='stagnation_checkpoint'; normalized['continuation_required']=False
        projection=normalized.get('narrative_projection')
        if isinstance(projection,dict): projection['scope_stop_reason']='stagnation_checkpoint'
        return normalized
    return dict(result)
...
class JianghuExtendedCommandsMixin:
    ...
    def _cleanup_command_deaths(self,writes:Mapping[str,Any],dead_refs:set[str],current_time:CampaignTime)->dict[str,Any]:
        ...
        schedule=copy.deepcopy(out.get(_SCHEDULE) or self.repository.read_json(_SCHEDULE))
        staged_judgment=False
        ...
        succession_by_faction:dict[str,dict[str,Any]]={}
        for fid,refs in faction_dead.items():
            ...
            succession_by_faction[fid]=copy.deepcopy(dict(succession))
        ...
        if dead_civic_rows:
            ...
            civic_rows,civilian_after,civic_judgments=appoint_civic_successors(
            ...
            for index, judgment_context in enumerate(civic_judgments):
                if not isinstance(judgment_context,Mapping):continue
                options=[row for row in judgment_context.get('options',[]) if isinstance(row,Mapping)]
                event=build_npc_judgment_event(
                    ...
                    source_event_id=f'command-death-civic:{index}:{judgment_context.get("predecessor_ref","")}',
                )
                schedule=upsert_one_off_event(schedule,event); staged_judgment=True
        ...
        for fid in sorted(faction_dead):
            ...
            for event in institutional_office_judgment_events(
                faction_ref=fid,at_iso=at_iso,office_result=office_result,
                succession=succession_by_faction.get(fid),
                source_event_id=f'command-death:{fid}:{at_iso}',
            ):
                schedule=upsert_one_off_event(schedule,event); staged_judgment=True
        ...
        if isinstance(dep_rows,dict) and isinstance(commit_rows,dict) and isinstance(person_index,dict):
            for dep_ref,row in list(dep_rows.items()):
                ...
                if not survivors:
                    ...
                    commitments=release_resources(commitments,activity_ref=str(dep_ref))
    ...
    def _jianghu_medicine_resolution(self,command:CommandEnvelope,meta:Mapping[str,Any],current_time:CampaignTime):
        ...
        if action=='field_stabilize':
            if set(command.payload)!={'action','subject_ref'}:
                raise CommandRejectedError('jianghu_medicine_resolution_payload_fields_invalid')
            subject_ref=str(command.payload.get('subject_ref') or '')
            _apath,_aroster,_aordinal,actor=self._person(command.actor_id)
            rpath,roster,ordinal,subject=self._person(subject_ref)
            if not is_living_and_conscious(actor):
                raise CommandRejectedError('jianghu_field_medicine_actor_unusable')
            subject_health=subject.get('health',{}) if isinstance(subject.get('health'),Mapping) else {}
            if subject_health.get('status')=='dead':
                raise CommandRejectedError('jianghu_field_medicine_subject_dead')
            active_refs=self._active_combat_person_refs()
            if command.actor_id in active_refs or subject_ref in active_refs:
                raise CommandRejectedError('jianghu_field_medicine_active_combat_requires_support_order')
            if subject_ref!=command.actor_id and not self._same_effective_location(command.actor_id,subject_ref):
                raise CommandRejectedError('jianghu_field_medicine_subject_not_present')
            injuries=subject_health.get('injuries',[]) if isinstance(subject_health,Mapping) else []
            if not isinstance(injuries,list) or not any(isinstance(row,Mapping) for row in injuries):
                raise CommandRejectedError('jianghu_field_medicine_no_treatable_injury')
            policy=emergency_field_stabilization_policy()
            ledger=hydrate_equipment_ledger(self.repository.read_json(_EQUIPMENT))
            load=effective_person_loadout(ledger,command.actor_id)
            held=load.get('items',{}) if isinstance(load.get('items'),Mapping) else {}
            kit_ref=str(policy['physician_kit_item_ref']); supply_ref=str(policy['medical_supply_item_ref'])
            supply_cost=max(1,int(policy['medical_supply_consumed_per_attempt']))
            if max(0,int(held.get(kit_ref,0)))<=0:
                raise CommandRejectedError('jianghu_field_medicine_physician_kit_unavailable')
            if max(0,int(held.get(supply_ref,0)))<supply_cost:
                raise CommandRejectedError('jianghu_field_medicine_medical_supply_unavailable')
            # Emergency aftercare is a physical ten-second action in the same
            # route/contact context. Use the chronology authority directly,
            # then rebase the treatment onto its after-image; unlike ordinary
            # infirmary work it must not reject a traveler merely for still
            # belonging to the route movement that produced the casualty.
            seconds=max(1,(max(1,int(policy['minimum_continuous_access_ms']))+999)//1000)
            time_plan=self._time_plan_exact(command,meta,current_time,seconds=seconds)
            post_roster=self._time_after_record(time_plan,rpath,roster)
            rows=post_roster.get('people',[]) if isinstance(post_roster,Mapping) else []
            idx=next((i for i,row in enumerate(rows) if isinstance(row,Mapping) and row.get('person_id')==subject_ref),None)
            if idx is None:
                raise CommandRejectedError('jianghu_field_medicine_subject_missing_after_time')
            post_subject=copy.deepcopy(dict(rows[idx])); health=post_subject.get('health',{}) if isinstance(post_subject.get('health'),Mapping) else {}
            attrs=actor.get('attributes',{}) if isinstance(actor.get('attributes'),Mapping) else {}
            prof=actor.get('professional_skills',{}) if isinstance(actor.get('professional_skills'),Mapping) else {}
            shock=max(0,int(health.get('shock',0)))
            patient_condition=max(max(1,int(policy['patient_condition_floor_milli'])),1000-shock*max(0,int(policy['shock_penalty_milli_per_point'])))
            score=wound_treatment_score(
                medicine=max(0,int(prof.get('medicine',0))), dexterity=max(0,int(attrs.get('dexterity',0))),
                intelligence=max(0,int(attrs.get('intelligence',0))), perception=max(0,int(attrs.get('perception',0))),
                physician_kit=True, medical_supply=True, environment_milli=max(1,int(policy['environment_milli'])),
                treatment_minutes=max(1,int(policy['treatment_minutes_for_score'])), patient_condition_milli=patient_condition,
            )
            before_bleeding=sum(max(0,int(row.get('bleeding_ml_per_min',0))) for row in health.get('injuries',[]) if isinstance(row,Mapping)) if isinstance(health.get('injuries'),list) else 0
            post_subject['health']=stabilize_wounds(health,treatment_score_value=max(0,int(score['treatment_score'])),advanced_procedure_enabled=bool(score['advanced_procedure_enabled']),medical_supply_available=True)
            after_health=post_subject.get('health',{}) if isinstance(post_subject.get('health'),Mapping) else {}
            after_bleeding=sum(max(0,int(row.get('bleeding_ml_per_min',0))) for row in after_health.get('injuries',[]) if isinstance(row,Mapping)) if isinstance(after_health.get('injuries'),list) else 0
            rows[idx]=post_subject; post_roster['people']=rows
            post_ledger=hydrate_equipment_ledger(self._time_after_record(time_plan,_EQUIPMENT,ledger))
            load_row=post_ledger.setdefault('person_loadouts',{}).setdefault(command.actor_id,{'items':{},'condition_milli':{}})
            load_items=load_row.setdefault('items',{})
            if max(0,int(load_items.get(supply_ref,0)))<supply_cost:
                raise CommandRejectedError('jianghu_field_medicine_medical_supply_lost_during_time')
            load_items[supply_ref]=max(0,int(load_items.get(supply_ref,0))-supply_cost)
            if load_items[supply_ref]<=0:
                load_items.pop(supply_ref,None); load_row.setdefault('condition_milli',{}).pop(supply_ref,None)
            claim=provenance_claim(post_ledger,command.actor_id,supply_ref)
            if isinstance(claim,Mapping):
                updated=set_nonholder_claim(
                    post_ledger,holder_ref=command.actor_id,item_ref=supply_ref,owner_ref=str(claim.get('owner_ref') or command.actor_id),
                    quantity=max(0,int(claim.get('quantity',0))-supply_cost),
                ...
                post_ledger=hydrate_equipment_ledger(updated)
            extras={rpath:post_roster,_EQUIPMENT:compact_equipment_ledger(post_ledger)}
            return self._combine_time_plan(command,time_plan,extra_records=extras,code='jianghu_field_stabilization_completed',result={
                'command_type':command.command_type,'action':'field_stabilize','subject_ref':subject_ref,'elapsed_seconds':seconds,
                'treatment_score':max(0,int(score['treatment_score'])),'bleeding_ml_per_min_before':before_bleeding,
                'bleeding_ml_per_min_after':after_bleeding,'medical_supply_consumed':supply_cost,
                'care_scope':'emergency_field_stabilization_not_infirmary_care',
            })
        if action=='qi_purge':
    ...
    def _jianghu_property_transfer_resolution(self,command:CommandEnvelope,meta:Mapping[str,Any],current_time:CampaignTime):
        ...
        if action in {'give_cash','seize_cash','accept_cash_offer'}:
            amount=int(command.payload.get('cash',0) or 0)
            if amount<=0:
                raise CommandRejectedError('jianghu_property_cash_invalid')
            offer_ref=None
            if action=='accept_cash_offer':
                offer_ref=str(command.payload.get('basis_ref') or '')
                if not offer_ref:
                    raise CommandRejectedError('jianghu_property_cash_offer_basis_required')
                offer=scene_history_record(self.repository.read_json,offer_ref)
                if not isinstance(offer,Mapping):
                    raise CommandRejectedError('jianghu_property_cash_offer_not_found')
                if (
                    str(offer.get('speaker_ref') or '')!=other_ref
                    or str(offer.get('speech_kind') or '')!='nonbinding_proposal'
                    or offer.get('authority') is not False
                    or offer.get('mechanical_consequence_authority') is not False
                ):
                    raise CommandRejectedError('jianghu_property_cash_offer_basis_invalid')
                terms=offer.get('proposal_terms')
                if not isinstance(terms,Mapping):
                    raise CommandRejectedError('jianghu_property_cash_offer_terms_missing')
                offered_cash=terms.get('cash')
                if isinstance(offered_cash,bool) or not isinstance(offered_cash,int) or offered_cash<=0 or amount>offered_cash:
                    raise CommandRejectedError('jianghu_property_cash_offer_exceeds_terms')
                recipient_ref=terms.get('recipient_ref')
                if recipient_ref is not None and str(recipient_ref)!=command.actor_id:
                    raise CommandRejectedError('jianghu_property_cash_offer_wrong_recipient')
                offer_session=str(offer.get('session_ref') or '')
                active_combat=active_combat_for_person(self.repository.read_json,command.actor_id)
                if active_combat is not None:
                    live_session=str(active_combat[0])
                else:
                    live_scene=active_scene_session(self.repository.read_json)
                    live_session=str(live_scene.get('session_ref') or '') if isinstance(live_scene,Mapping) else ''
                if not live_session or offer_session!=live_session:
                    raise CommandRejectedError('jianghu_property_cash_offer_not_live')
                basis={'kind':'recorded_cash_offer','speech_ref':offer_ref,'offered_cash':int(offered_cash)}
    ...
    def _jianghu_diplomacy_resolution(self,command:CommandEnvelope,meta:Mapping[str,Any],current_time:CampaignTime):
        ...
        advisory=evaluate_proposal(edge,proposal_value_cash=value,proposal_cost_cash=cost,strategic_fit=fit,risk=risk)
        target_response=str(command.payload.get('target_response') or '')
        if target_response not in {'accept','counteroffer','reject'}:
            raise CommandRejectedError('jianghu_diplomacy_target_response_required')
        ...
        record={'proposal_id':pid,'from_faction':source,'to_faction':target,'kind':proposal,'terms':terms,'created_at':str(current_time).removeprefix('SE-'),'decision':{'accept':'accepted','counteroffer':'counteroffer','reject':'rejected'}[target_response],'advisory_score':int(advisory['score']),'decision_origin':'gm_npc_judgment'}
        writes={}
        ...
        if authorization_ref and isinstance(delegated_row,Mapping):
            ...
            closed=close_institutional_operation(
                ...
                extra_report={
                    ...
                    'diplomacy_decision':decision_label,'diplomacy_score':int(record.get('advisory_score',0)),
    ...
    def _jianghu_family_resolution(self,command:CommandEnvelope,meta:Mapping[str,Any],current_time:CampaignTime):
        action=str(command.payload.get('action')); other_ref=str(command.payload.get('other_ref')); response=str(command.payload.get('counterparty_response') or ''); _ap,_,_,actor=self._person(command.actor_id); _op,_,_,other=self._person(other_ref)
        if response not in {'accept','refuse'}:raise CommandRejectedError('jianghu_family_counterparty_response_required')
        ...
        if response=='refuse':
            return self._simple_plan(command,meta,current_time,writes_records={},code='jianghu_family_proposal_refused',result={'command_type':command.command_type,'action':action,'other_ref':other_ref,'counterparty_response':'refuse','decision_origin':'gm_npc_judgment'})
        ...
        if action=='courtship':
            ...
            if not marriage_eligible(age_a=age_a,age_b=age_b,mutual_consent=True,relationship_stage='courtship'):raise CommandRejectedError('jianghu_marriage_requirements_not_met')
    ...
    def _jianghu_npc_judgment_resolution(self,command:CommandEnvelope,meta:Mapping[str,Any],current_time:CampaignTime):
        """Commit one exact GM-authored NPC intent without advancing time.

        The decision itself is durable causal input, not the domain consequence.
        The same-timestamp scheduler frontier consumes this committed intent and
        revalidates/executes it through the owning domain reducer.
        """
        event_id=str(command.payload.get('event_id') or '')
        option_ref=str(command.payload.get('option_ref') or '')
        schedule=copy.deepcopy(self.repository.read_json(_SCHEDULE))
        one_off=schedule.get('one_off',{}) if isinstance(schedule,Mapping) else {}
        event=one_off.get(event_id) if isinstance(one_off,Mapping) else None
        if not isinstance(event,Mapping) or str(event.get('kind') or '')!='npc_judgment_due' or event.get('requires_gm_judgment') is not True:
            raise CommandRejectedError('jianghu_npc_judgment_event_unavailable')
        if str(event.get('due_at') or '')!=str(current_time).removeprefix('SE-'):
            raise CommandRejectedError('jianghu_npc_judgment_not_current_frontier')
        judgment_kind=str(event.get('judgment_kind') or '')
        if judgment_kind not in {'faction_hostile_strategy','faction_investment','faction_aid_or_diplomacy','family_courtship','family_marriage','faction_office_appointment','faction_membership_departure','civic_office_appointment','captor_ransom_policy','captive_response_policy','route_interception_policy','institutional_evolution_policy','retinue_assignment_policy','faction_contract_acceptance','house_assignment_offer_policy','house_career_offer_policy','government_enforcement_policy','government_contact_response_policy','strategic_raid_aftermath_policy','tournament_participation_policy'}:
            raise CommandRejectedError('jianghu_npc_judgment_kind_unsupported')
        actor_ref=str(event.get('actor_ref') or '')
        if actor_ref and actor_ref==str(meta.get('player_id') or ''):
            raise CommandRejectedError('jianghu_npc_judgment_cannot_choose_player_intent')
        try:
            choice=selected_option(event,option_ref)
        except ValueError as exc:
            raise CommandRejectedError('jianghu_npc_judgment_option_unavailable') from exc
        if option_authors_player_intent(
            judgment_kind=judgment_kind, option=choice, player_ref=str(meta.get('player_id') or ''),
        ):
            raise CommandRejectedError('jianghu_npc_judgment_cannot_choose_player_intent')
        schedule=remove_pending_judgment(schedule,event_id)
        digest=hashlib.sha256((event_id+'|'+option_ref).encode()).hexdigest()[:24]
        committed={
            'event_id':f'npc_judgment_committed:{digest}',
            'kind':'npc_judgment_committed',
            'due_at':str(current_time).removeprefix('SE-'),
            'owner_ref':str(event.get('owner_ref') or actor_ref),
            'actor_ref':actor_ref,
            ...
            'decision_context':copy.deepcopy(dict(event.get('decision_context') or {})),
            'decision_origin':'gm_npc_judgment',
            'requires_player_decision':False,
        }
        schedule=upsert_one_off_event(schedule,committed)
        return self._simple_plan(
            command,meta,current_time,writes_records={_SCHEDULE:schedule},
            code='jianghu_npc_judgment_committed',
            result={
                'command_type':command.command_type,'event_id':event_id,
                'committed_event_id':committed['event_id'],'decision_ref':committed['decision_ref'],
                'judgment_kind':committed['judgment_kind'],'option_ref':option_ref,
                'decision_origin':'gm_npc_judgment','continuation_required':True,
                'continuation_reason':'apply_committed_npc_judgment',
            },
        )
    ...
    def _jianghu_crime_report_resolution(self,command:CommandEnvelope,meta:Mapping[str,Any],current_time:CampaignTime):
        ...
        try:
            ...
            subject={}; place=''; jurisdiction=''
        extra_writes={}
        funded_bounty=prior_bounty
        pending_enforcement=None
        if bounty>0 and jurisdiction:
            schedule=copy.deepcopy(self.repository.read_json(_SCHEDULE))
            existing=(gov.get('warrants',{}) or {}).get('warrant:'+subject_ref,{}) if isinstance(gov.get('warrants',{}),Mapping) else {}
            event=build_npc_judgment_event(
                judgment_kind='government_enforcement_policy', at_iso=str(current_time).removeprefix('SE-'),
                actor_ref=f'government:{jurisdiction}', owner_ref=subject_ref,
                options=[
                    {'intent':'maintain' if existing else 'no_action','subject_ref':subject_ref,'jurisdiction_ref':jurisdiction},
                    {'intent':'issue_warrant','subject_ref':subject_ref,'jurisdiction_ref':jurisdiction,'offense':offense,'bounty_cash':bounty,'evidence_ref':evidence_ref},
                ],
                decision_context={
                    'subject_ref':subject_ref,'jurisdiction_ref':jurisdiction,'attention':attention,'prior_offenses':prior+1,
                    'offense':offense,'confidence':confidence,'evidence_ref':evidence_ref,
                    'existing_warrant':bool(existing),'existing_bounty_cash':max(0,int(existing.get('bounty_escrow_cash',0))) if isinstance(existing,Mapping) else 0,
                },
                source_event_id=f'crime_report:{evidence_hash}',
            )
            schedule=upsert_one_off_event(schedule,event); extra_writes[_SCHEDULE]=schedule
            pending_enforcement=event['decision_ref']
        ...
        return self._simple_plan(command,meta,current_time,writes_records=writes,code='jianghu_crime_report_committed',result={'command_type':command.command_type,'subject_ref':subject_ref,'attention':attention,'bounty_cash':funded_bounty,'suggested_bounty_cash':bounty,'government_enforcement_judgment_ref':pending_enforcement,'government_audience_ref':audience})
    ...
    def _jianghu_combat_core_resolution(self,command:CommandEnvelope,meta:Mapping[str,Any],current_time:CampaignTime):
        ...
        raw_movement=command.payload.get('movement_intent')
        if raw_movement not in (None,''):
            if action not in {'exchange','maneuver'}:
                raise CommandRejectedError('jianghu_combat_tactical_movement_exchange_only')
            if not isinstance(raw_movement,str) or raw_movement not in {'close','chase','lateral','lateral_left','lateral_right'}:
                raise CommandRejectedError('jianghu_combat_tactical_movement_invalid')
            if action=='maneuver' and raw_movement not in {'close','chase'}:
                raise CommandRejectedError('jianghu_combat_maneuver_intent_invalid')
            if command.payload.get('exchange_count') not in (None,1):
            ...
            if bool(command.payload.get('until_resolution')):
                raise CommandRejectedError('jianghu_combat_tactical_movement_scope_invalid')
        state=copy.deepcopy(self.repository.read_json(_COMBATS)); combats=state.setdefault('combats',{})
        if action=='start':
        ...
        if action in {'exchange','maneuver'}:
                ...
                if isinstance(movement_ref,str) and movement_ref:
                    ...
                    if isinstance(movement,Mapping):
                        ...
                        if not isinstance(temporary,list):
                            temporary=_legacy_route_temporary_combat_refs(
                                movement=movement,
                                combat=combat,
                                actor_ref=command.actor_id,
                                standing_refs=player_retinue_context.get('member_refs',[]),
                            )
            ...
            try:
                raw_action_kind=canonical_player_action_kind(
                    str(command.payload.get('action_kind') or 'attack'), allow_auto=True,
                )
            except ValueError as exc:
                raise CommandRejectedError('jianghu_combat_action_invalid') from exc
            ...
            explicit_poison,poison_auto=_player_poison_instruction(command.payload)
            ...
            if 'movement_intent' in command.payload:
                raw_movement=command.payload.get('movement_intent')
                if not isinstance(raw_movement,str) or raw_movement not in {'close','chase','lateral','lateral_left','lateral_right'}:
                    raise CommandRejectedError('jianghu_combat_tactical_movement_invalid')
                movement_intent=raw_movement
            maneuver_only=(action=='maneuver')
            if maneuver_only:
                if movement_intent not in {'close','chase'}:
                    raise CommandRejectedError('jianghu_combat_maneuver_intent_invalid')
                raw_action_kind='maneuver'
                resolved_hit_zone='auto'
                target_structure_ref=None
                explicit_poison=None
                poison_auto=False
            ally_orders:list[dict[str,str]]=[]
            if 'ally_orders' in command.payload:
                raw_orders=command.payload.get('ally_orders')
                if not isinstance(raw_orders,Sequence) or isinstance(raw_orders,(str,bytes,bytearray)) or len(raw_orders)>8:
                    raise CommandRejectedError('jianghu_combat_ally_orders_invalid')
                player_side=next((str(side) for side,side_refs in combat.get('sides',{}).items() if isinstance(side_refs,list) and command.actor_id in side_refs),None)
                same_side={str(ref) for ref in combat.get('sides',{}).get(player_side,[]) if isinstance(ref,str)} if player_side else set()
                commanded=set()
                if isinstance(player_retinue_context,Mapping):
                    commanded.update(str(x) for x in player_retinue_context.get('member_refs',[]) if isinstance(x,str))
                    commanded.update(str(x) for x in player_retinue_context.get('temporary_member_refs',[]) if isinstance(x,str))
                for raw in raw_orders:
                    if not isinstance(raw,Mapping) or set(raw)!={'actor_ref','task','target_ref'}:
                        raise CommandRejectedError('jianghu_combat_ally_orders_invalid')
                    actor_ref=str(raw.get('actor_ref') or ''); target_ref=str(raw.get('target_ref') or ''); task=str(raw.get('task') or '')
                    if actor_ref not in commanded or actor_ref not in same_side or target_ref not in same_side or task not in {'reach','protect','extract','treat'}:
                        raise CommandRejectedError('jianghu_combat_ally_order_not_authorized')
                    ally_orders.append({'actor_ref':actor_ref,'task':task,'target_ref':target_ref})
            npc_judgments=thaw_json(command.payload.get('npc_judgments'))
            if not isinstance(npc_judgments,Mapping):
                raise CommandRejectedError('jianghu_combat_npc_judgments_required')
            if any(not isinstance(ref,str) or not isinstance(row,Mapping) for ref,row in npc_judgments.items()):
                raise CommandRejectedError('jianghu_combat_npc_judgments_invalid')
            ...
            try:
                resolved=_resolve_player_combat_span(
                    ...
                    movement_intent=movement_intent,maneuver_only=maneuver_only,
                    rally_allies=rally_allies,ally_orders=ally_orders,player_improvised_weapon_state=improvised_prop_state,
                    npc_judgments=npc_judgments,require_gm_npc_judgments=True,
                )
            except ValueError as exc:
            ...
            resolved_people=resolved.get('people_after',{}) if isinstance(resolved.get('people_after'),Mapping) else {}
            for ev in resolved.get('events',[]):
                ...
                # Cumulative encounter memory is not proof that the intended
                # target identified this exact attack. Attribute the attacker to
                # the target only when that attack was detected, or the attacker
                # is freshly visible after the exchange. This prevents a hidden
                # projectile/melee action from creating omniscient social memory.
                defense=ev.get('defense') if isinstance(ev.get('defense'),Mapping) else {}
                actual_ref=str(ev.get('actual_ref') or '')
                detected_this_attack=bool(actual_ref==b and defense.get('detected') is True)
                freshly_visible=False
                if b in resolved_people and a in resolved_people and b in combatants_after and a in combatants_after:
                    freshly_visible=a in currently_visible_enemies(combat_after,actor_ref=b,enemy_refs=[a],people=resolved_people)
                if (detected_this_attack or freshly_visible) and (b,a) not in seen_pairs:
                ...
                for dead_ref in sorted(newly_dead):
                    ...
                    for relative_ref in sorted(close_family_refs(family_after,dead_ref)):
                        ...
                        visible_before=set(currently_visible_enemies(
                            combat,actor_ref=relative_ref,enemy_refs=[killer_ref],people=people,
                        ))
                        visible_after=set(currently_visible_enemies(
                            combat_after,actor_ref=relative_ref,enemy_refs=[killer_ref],people=resolved['people_after'],
                        ))
                        if killer_ref not in (visible_before|visible_after):continue
        ...
        if action=='disengage':
            ...
            if r.get('escaped'):
                objective=combat.get('objective',{}) if isinstance(combat.get('objective'),Mapping) else {}
                movement_ref=str(objective.get('movement_ref') or '')
                if movement_ref:
                    try:
                        route_current=self.repository.read_json(_ROUTE_OPERATIONS)
                    except FileNotFoundError:
                        route_current=None
                    if isinstance(route_current,Mapping):
                        route_after=self._time_after_record(time_plan,_ROUTE_OPERATIONS,route_current)
                        movements=route_after.get('movements',{}) if isinstance(route_after.get('movements'),Mapping) else {}
                        movement=movements.get(movement_ref) if isinstance(movements,Mapping) else None
                        if isinstance(movement,Mapping):
                            participants={str(x) for x in movement.get('participant_refs',[]) if isinstance(x,str)} if isinstance(movement.get('participant_refs'),list) else set()
                            movement_combat_ref=str(movement.get('combat_ref') or '')
                            if command.actor_id in participants and movement_combat_ref in {'',combat_ref}:
                                if not isinstance(movements,dict):
                                    raise CommandRejectedError('jianghu_route_state_invalid')
                                updated=copy.deepcopy(dict(movement))
                                separated=[str(x) for x in updated.get('separated_person_refs',[]) if isinstance(x,str)] if isinstance(updated.get('separated_person_refs'),list) else []
                                if command.actor_id not in separated:
                                    separated.append(command.actor_id)
                                updated['separated_person_refs']=separated
                                movements[movement_ref]=updated
                                extra_records[_ROUTE_OPERATIONS]=route_after
        ...
        if action=='end':
            ...
            route_objective_kinds={'protect_cargo','retain_seized_people_and_cargo','protect_party','preserve_route_mission','preserve_escort_mission','survive_interception'}
            if objective.get('movement_ref') or objective_kind in route_objective_kinds:
                raise CommandRejectedError('jianghu_combat_pending_route_resolution')
            refs=[r for side in combat.get('sides',{}).values() if isinstance(side,list) for r in side if isinstance(r,str)]
