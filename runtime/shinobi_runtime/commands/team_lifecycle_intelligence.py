...
class TeamLifecycleIntelligenceMixin:
    ...
    MAX_AUTONOMOUS_TEAMS_PER_OWNER = 8
    ...
    def _autonomy_policy_book(self) -> AutonomousPolicyBook:
        ...
        for faction_id, raw in book.faction_assignments.items():
            ...
            if isinstance(spec, Mapping):
                clean = dict(spec)
                # Active exact-team membership is exclusive. Autonomous policy
                # may not silently borrow someone from another standing team.
                clean.pop("allow_existing_team_members", None)
                clean.setdefault("purpose_kind", "standing")
                clean.setdefault("replacement_policy", "maintain_strength")
                clean.setdefault("max_active_teams", 1 if isinstance(clean.get("team_id"), str) else 4)
    ...
    def _active_autonomous_team(
        ...
        limit = getattr(self, "_team_form_capacity_limit", None)
        if isinstance(limit, int) and not isinstance(limit, bool) and limit > 0 and len(teams) < limit:
            # During a team-form decision, returning None tells the lower layer
            # that another bounded standing team may be created. Outside that
            # narrow call, the first active team remains the deterministic
            # mission-routing choice.
            return None
        ...
        *,
        ...
        if getattr(decision, "kind", None) == "team_form":
            ...
            if isinstance(spec, Mapping):
                ...
                clean_spec.pop("allow_existing_team_members", None)
                raw_limit = clean_spec.get("max_active_teams", 1 if isinstance(clean_spec.get("team_id"), str) else 4)
                if isinstance(raw_limit, int) and not isinstance(raw_limit, bool):
                    limit = max(1, min(self.MAX_AUTONOMOUS_TEAMS_PER_OWNER, raw_limit))
                clean_spec["max_active_teams"] = limit or 1
        ...
        if limit is not None:
            self._team_form_capacity_limit = limit
        try:
            ...
            if hasattr(self, "_team_form_capacity_limit"):
                delattr(self, "_team_form_capacity_limit")
