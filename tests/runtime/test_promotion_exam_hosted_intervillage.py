        ...
        "foreign_delegations": [
            {
                ...
                "home_place_ref": "place.suna.residential",
                "service_pool_ref": "pool.suna.shinobi_service",
                "force_ref": "force.suna.shinobi",
                "delegation_size": 3,
                "candidate_pool_refs": ["canon_gaara", "canon_kankuro", "canon_temari"],
            }
        ],
        "arrival_buffer_days": 2,
        "generated_candidate_calibration": {
            "force_baseline_offset": -8,
            "spread": 7,
            "minimum": 18,
            "maximum": 120,
        },
...
def test_common_schedule_expands_so_slow_invited_delegation_can_register(monkeypatch):
    monkeypatch.setattr(hosted, "_ORIGINAL_ELIGIBLE", lambda *args, **kwargs: [])
    monkeypatch.setattr(hosted.scheduler, "registered_candidate_refs", lambda *args, **kwargs: ())
    ...
    assert hosted.arrival_lead_days(PROFILE) == 10.0
    assert hosted.required_arrival_lead_days(planner.repository, PROFILE) == 13
    assert hosted.effective_arrival_lead_days(planner.repository, PROFILE) == 13.0
