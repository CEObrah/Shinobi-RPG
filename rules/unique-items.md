...
Numerical authority for chakra interfaces, named-item base links, unique modules, and named-item mechanical profiles lives in `data/mechanics/unique-items.json`. Custody and condition live in `state/reg/named-items.json`. Ordinary physical weapon/armor mechanics continue to resolve from the global item registry.
