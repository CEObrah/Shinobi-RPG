...
def test_existing_exact_and_person_owners_share_one_core_interface():
    ...
    toma = core_from_person(
        load("state/person/ht/023.json"),
        component_ref="profile.legacy",
        source_ref="state/person/ht/023.json",
    )
...
def test_rostered_sheet_requires_baseline_and_keeps_namespaces_separate():
    ...
    sheet = assemble_sheet(
        core,
        cohort_baseline={"stats": {"sword": 80}},
        components={},
    )
...
def test_house_tang_roster_target_conserves_current_thirty_two_people():
    ...
    exact = tuple(
        person_id
        for cohort in house["cohorts"]
        for person_id in cohort["members"]
    ) + tuple(house["unassigned_members"]) + tuple(house["externally_assigned_members"])
...
def test_house_roster_registry_has_one_core_per_roster_reference():
    ...
    for person_id, record in registry["people"].items():
        ...
        assert record["resolved_through"] == load("state/meta.json")["time"]
    ...
    for cohort in house["cohorts"]:
        ...
        for baseline in baselines:
            for group, values in baseline["category_values"].items():
                for value in values:
                    actual[f"{group}:{value}"] = actual.get(f"{group}:{value}", 0) + 1
...
def test_deterministic_materialization_selects_real_ids_and_conserves_total():
    ...
    first_rng = CounterRNG(
        world_seed="seed", transaction_id="tx.casualty", stream="house.instructors"
    )
    replay_rng = CounterRNG(
        world_seed="seed", transaction_id="tx.casualty", stream="house.instructors"
    )
...
def test_repository_resolver_reuses_owner_index_reads_within_one_call():
    ...
    sheet = RepositoryPersonSheetResolver(repository)("ht.core.001")
    assert sheet["core"]["person_id"] == "ht.core.001"
    assert repository.read_counts["state/index/owners.json"] == 1
    assert repository.read_counts["state/index/owners/ht.json"] == 1
    assert repository.read_counts["state/index/owners/house.json"] == 1


def test_every_indexed_exact_shinobi_character_resolves_to_a_logical_sheet():
    owner_index = load("state/index/owners.json")
    indexed = {}
    for shard_path in owner_index["prefix_index"].values():
        shard = load(shard_path)
        for owner_id, owner_path in shard["owners"].items():
            owner = load(owner_path)
            if owner.get("schema") == "shinobi_character":
                assert owner_id not in indexed
                indexed[owner_id] = (owner_path, owner)

    assert indexed
    resolver = RepositoryPersonSheetResolver(RepositoryStore(ROOT))
    for owner_id, (owner_path, owner) in sorted(indexed.items()):
        sheet = resolver(owner_id)
        assert sheet is not None, (owner_id, owner_path)
        assert sheet["core"]["person_id"] == owner_id
        assert sheet["core"]["representation"] == "exact"
        assert sheet["core"]["life_status"] in {"alive", "dead", "missing", "unknown"}
        assert sheet["components"]["profile.exact"]["life_status"] == owner["life_status"]
        if owner.get("birth_date") is not None:
            assert sheet["core"]["birth_date"] == owner["birth_date"]
