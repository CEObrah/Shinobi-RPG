...
def _combat_ready_person(person: dict) -> dict:
    """Build a synthetic combatant independent of current campaign wounds."""
...
def test_npc_exact_combat_allocates_and_spends_real_qi():
    ...
    player = _combat_ready_person(roster[0])
    npc = _combat_ready_person(roster[3])
...
def test_simultaneous_defenses_share_one_authoritative_qi_clock_interval(monkeypatch):
    ...
    base = _combat_ready_person(load('state/martial-world/people/house_tang.json')['people'][0])
