...
def test_exchange_boundary_and_repeated_failure_do_not_manufacture_player_menu():
    contract = combat_contract_text(ROOT)
    normalized = contract.replace("**", "")

    assert "not a player-decision boundary" in normalized
    assert "Repeated mechanically similar exchanges must be compressed" in contract
    assert "Do not create a \"sharper choice\" merely because a tactic is currently ineffective" in contract
    assert "Mechanical frustration is not itself a protected decision" in contract
