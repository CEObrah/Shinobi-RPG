...
def settle_institutional_offices(
    ...
    """Prune objectively invalid offices and expose vacancy judgment envelopes.

    Eligibility, service, trust and relevant capability are runtime evidence.
    Selecting the person who should receive a consequential office is a human
    institutional judgment and is therefore never committed here.
    """
    ...
    required = list(required_institutional_offices(faction, out))
    required_names = {office for office, _skill in required}
    for idx, raw in enumerate(people):
        ...
        if kept != offices:
            person = copy.deepcopy(dict(raw)); person["standing_offices"] = kept; people[idx] = person
    ...
    leader_refs = [
        ...
        and "leader" in [_office_key(x) for x in (person.get("standing_offices", []) if isinstance(person.get("standing_offices"), list) else [])]
    ]
    trust = {
    ...
    for office, skill in required:
        if office in occupied:
            continue
        options=[]
        for person in eligible:
            ...
            capability=max(int(prof.get(skill,0) or 0),int(martial.get(skill,0) or 0))
            options.append({
                "person_ref":ref,"relevant_skill":skill,"capability":capability,
                "service_days":max(0,int(service.get(ref,0))),"leader_trust":int(trust.get(ref,0)),
                "membership_grade":str(person.get("membership_grade") or ""),
            })
        options.sort(key=lambda row:(-int(row["capability"]),-int(row["service_days"]),-int(row["leader_trust"]),str(row["person_ref"])))
        judgments.append({"office":office,"relevant_skill":skill,"requires_gm_judgment":True,"options":options[:32]})
    out["people"] = people
    return {"roster": out, "appointments": [], "appointment_judgments": judgments}


def apply_institutional_office_judgment(
    faction: Mapping[str, Any], roster: Mapping[str, Any], *, year: int,
    social: Mapping[str, Any], office: str, person_ref: str | None,
    player_ref: str | None = None, unavailable_refs: Sequence[str] = (),
) -> dict[str, Any]:
    """Revalidate and apply one exact GM-authored institutional appointment."""
    current = settle_institutional_offices(
        faction, roster, year=year, social=social, player_ref=player_ref,
        unavailable_refs=unavailable_refs,
    )
    out = current["roster"]
    vacancy = next((row for row in current.get("appointment_judgments", []) if row.get("office") == office), None)
    if vacancy is None:
        raise ValueError("institutional office is not currently vacant")
    ref = str(person_ref or "")
    if not ref:
        return {"roster": out, "appointment": None, "result": "office_left_vacant"}
    options = vacancy.get("options", []) if isinstance(vacancy, Mapping) else []
    if not any(isinstance(row, Mapping) and str(row.get("person_ref") or "") == ref for row in options):
        raise ValueError("institutional appointment candidate unavailable")
    people = out.get("people")
    if not isinstance(people, list):
        raise ValueError("jianghu roster people invalid")
    found = False
    for idx, raw in enumerate(people):
        if not isinstance(raw, Mapping) or str(raw.get("person_id") or "") != ref:
            continue
        person = copy.deepcopy(dict(raw))
        offices = [str(value) for value in person.get("standing_offices", []) if isinstance(value, str)]
        if office not in offices:
            offices.append(office)
        person["standing_offices"] = sorted(set(offices))
        people[idx] = person
        found = True
        break
    if not found:
        raise ValueError("institutional appointment candidate missing")
    return {
        "roster": out,
        "appointment": {"office": office, "person_ref": ref, "decision_origin": "gm_npc_judgment"},
        "result": "office_appointed",
    }
...
__all__ = ["advance_institution", "apply_autonomous_clinical_treatment", "institutional_status", "settle_institutional_offices", "apply_institutional_office_judgment"]
