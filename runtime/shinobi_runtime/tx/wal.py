...
class WriteAheadLog:
    ...
    def _migrate_legacy_flat_records(self) -> None:
        """Move validated pre-partition WAL files into the bounded layout.

        Old Shinobi deployments stored every WAL directly under ``wal/``.  The
        migration is local runtime evidence only: it never mutates campaign
        state and refuses ambiguous/corrupt collisions.  ``committed`` records
        remain pending because receipt/remote finalization may still be due.
        """
        for legacy in sorted(self.directory.glob("*.json")):
            record = self._read_path(legacy)
            transaction_id = record["transaction_id"]
            target = (
                self.terminal_directory
                if record["status"] == "rolled_back"
                else self.pending_directory
            ) / self._record_name(transaction_id)
            if target.exists():
                if target.read_bytes() != legacy.read_bytes():
                    raise WalError("legacy WAL collides with partitioned WAL evidence")
                legacy.unlink()
                continue
