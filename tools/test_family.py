...
def all_routes():
    m=rj('runtime/contracts/repository-map.json');out=dict(m.get('routes',{}))
...
# Routing and canonical human contract.
router=rj('runtime/contracts/rule-router.json').get('domains',{})
...
h=(R/'plugins/shinobi-rpg/skills/shinobi-game-master/references/repository-map.md').read_text(encoding='utf-8').lower()
for phrase in ('family/succession','use family system contracts'):
    if phrase not in h:err('family_human_map_missing:'+phrase)
