...
def person_settlement(person: Mapping[str,Any], *, faction_headquarters: str, sites: Mapping[str,Any]) -> str:
    ...
    if loc:
        ...
        # Strategic place refs are allowed as exact current locations.
...
def attendance_threshold_milli(person: Mapping[str,Any], *, site_type: str, hour: int) -> int:
    ...
    # Time-of-day makes public presence plausible without an hourly schedule.
...
def person_attends_site(
    ...
    # Four six-hour blocks keep the answer stable during a scene while allowing
    # lawful movement over a day without any persisted schedule.
    block=at.hour//6
...
def derived_site_attendance(
    *, site_ref: str, site: Mapping[str,Any], faction_people: Sequence[tuple[str,Sequence[Mapping[str,Any]]]],
    ...
    unavailable_refs: set[str]|frozenset[str]=frozenset(), exclude_refs: set[str]|frozenset[str]=frozenset(), limit: int=16,
    ...
    for faction_ref,people in faction_people:
        ...
        for person in people:
            ...
            if person_attends_site(person,site_ref=site_ref,site=site,faction_headquarters=hq,sites=sites,at=at,unavailable_refs=unavailable_refs):
                # High social/martial standing is more likely to be useful to a
                # bounded context, but attendance itself is already decided.
    ...
    rows.sort()
    return [pid for _standing,pid in rows[:max(0,int(limit))]]
