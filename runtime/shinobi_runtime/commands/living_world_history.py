...
class LivingWorldHistoryMixin:
    ...
    def _faction_memory(self, faction_id: str, *, at: CampaignTime, record_writes: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if path in record_writes:
            ...
            if raw is None:
                memory = {"schema":"faction-operational-memory","faction_id":faction_id,"as_of":str(at),"active_mission_team_refs":{},"team_performance":{},"recent_report_refs":[],"work_queue":{"pending_refs":[],"active_refs":[],"completed_recent_refs":[],"cursor":0}}
                ...
                record_writes[path] = memory
        if not isinstance(memory.get("active_mission_team_refs"), dict) or not isinstance(memory.get("team_performance"), dict) or not isinstance(memory.get("recent_report_refs"), list) or not isinstance(memory.get("work_queue"), dict):
            raise CommandRejectedError("faction_operational_memory_invalid")
        memory["as_of"] = str(at)
        return memory
