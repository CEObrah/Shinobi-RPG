...
def hostile_action_options(
    ...
    """Return feasible hostile choices with advisory pressure, never a choice.

    The runtime may calculate capability, institutional pressure and lawful
    affordances. Selecting whether/whom/how to attack is a consequential human
    judgment and therefore remains unselected for the GM.
    """
    ...
    for edge in edges:
        ...
        if not kind:
            continue
        if kind == "faction_raid" and eligible_raid_target_refs is not None and target not in eligible_raid_target_refs:
            continue
        ...
        if kind == "faction_war_strike":
            urgency = min(1000, max(0, 90 + (hostility - 65) * 10 + risk * 2 + political_pressure * 2 + (120 if coalition_pressure else 0)))
        elif kind == "faction_raid":
            urgency = min(1000, max(0, (hostility - 45) * 14 + risk * 2 + political_pressure))
        else:
            urgency = min(1000, max(0, (hostility - 25) * 8 + risk + political_pressure // 2))
            urgency = min(1000, urgency * formal_challenge_pressure_milli(datetime(year, month, 1)) // 1000)
        preferred_intent = (
            outlaw_intent(str(outlaw_subtype), stage=stage)
            if str(faction_type) == "outlaw_faction" else
            ("honor_challenge" if kind == "formal_challenge" else "punitive_expedition")
        )
        if kind == "faction_raid" and str(faction_type) == "outlaw_faction":
            intent_options = [preferred_intent] + [
                value for value in ("robbery", "kidnapping", "cargo_seizure", "extortion", "cargo_diversion")
                if value != preferred_intent
            ]
        elif kind == "faction_war_strike" and str(faction_type) == "outlaw_faction":
            intent_options = [preferred_intent, "punitive_expedition"] if preferred_intent != "punitive_expedition" else [preferred_intent]
        else:
            intent_options = [preferred_intent]
        rows.append({
            "action": kind,
            ...
            "advisory_operation_intent": preferred_intent,
            "operation_intent_options": intent_options,
            "urgency_milli": int(urgency),
            "coalition_pressure": coalition_pressure,
        })
    return sorted(rows, key=lambda row: (-int(row.get("urgency_milli", 0)), -int(row.get("hostility", 0)), str(row.get("target_faction_ref", "")), str(row.get("action", ""))))


def friendly_aid_options(
    edges: Sequence[Mapping[str, Any]], *, faction_ref: str, year: int, month: int,
    cash_reserve_months: int,
) -> list[dict[str, Any]]:
    """Return feasible aid counterparts with advisory obligation strength."""
    ...
    for edge in edges:
        ...
        if not target or target == faction_ref:
            continue
        trust = int(edge.get("trust", 0)); respect = int(edge.get("respect", 0)); hostility = int(edge.get("hostility", 0)); obligation = max(0, int(edge.get("obligation", 0)))
        if hostility > 12 or (trust < 18 and obligation <= 0):
            continue
        strength = trust * 3 + max(0, respect) + obligation * 5
        rows.append({"target_faction_ref": target, "support_pressure": int(strength), "trust": trust, "respect": respect, "obligation": obligation, "hostility": hostility})
    return sorted(rows, key=lambda row: (-int(row.get("support_pressure", 0)), str(row.get("target_faction_ref", ""))))


# Backward-compatible names now return option envelopes, not selected intents.
def choose_hostile_action(*args: Any, **kwargs: Any) -> dict[str, Any] | None:
    options = hostile_action_options(*args, **kwargs)
    return {"requires_gm_judgment": True, "options": options} if options else None


def choose_friendly_aid_target(*args: Any, **kwargs: Any) -> dict[str, Any] | None:
    options = friendly_aid_options(*args, **kwargs)
    return {"requires_gm_judgment": True, "options": options} if options else None
...
def investment_priority_options(
    ...
    """Return feasible strategic investments with advisory bottleneck scores."""
    ...
    if housing>0 and pop*100>=housing*78 and int(buildings.get("residential_compound",0))>0:
        current=facilities.get("residential_compound",{}) if isinstance(facilities,Mapping) else {}; footprint=max(0,int(current.get("footprint_m2",0))) if isinstance(current,Mapping) else 0
        rows.append({"kind":"expand_building","building_type":"residential_compound","additional_footprint_m2":max(200,footprint//10),"urgency_milli":min(1000,pop*1000//max(1,housing))})
    if training>0 and pop*2>training and int(buildings.get("training_grounds",0))>0:
        current=facilities.get("training_grounds",{}) if isinstance(facilities,Mapping) else {}; footprint=max(0,int(current.get("footprint_m2",0))) if isinstance(current,Mapping) else 0
        rows.append({"kind":"expand_building","building_type":"training_grounds","additional_footprint_m2":max(250,footprint//12),"urgency_milli":min(1000,pop*1000//max(1,training))})
    for order,building_type in enumerate(("main_hall","training_hall","training_grounds","qi_hall","armory_workshop","infirmary_apothecary","storehouse","library_records","transport_yard","walls_gate")):
        level=max(0,int(buildings.get(building_type,0)))
        if 0<level<5:
            rows.append({"kind":"upgrade_building","building_type":building_type,"target_level":level+1,"urgency_milli":max(100,500-order*20-level*20)})
    for enterprise_type in ("escort_service","trade_merchant_business","crafting_workshop","medicine_apothecary","agriculture_landholding"):
        ...
        if level>0:
            rows.append({"kind":"expand_enterprise","enterprise_type":enterprise_type,"additional_scale":1,"urgency_milli":200+min(300,level*30)})
    return sorted(rows,key=lambda row:(-int(row.get("urgency_milli",0)),str(row.get("kind","")),str(row.get("building_type") or row.get("enterprise_type") or "")))


def choose_investment_priority(*args: Any, **kwargs: Any) -> dict[str, Any] | None:
    options=investment_priority_options(*args,**kwargs)
    return {"requires_gm_judgment":True,"options":options} if options else None


__all__ = [
    "friendly_aid_options", "hostile_action_options", "investment_priority_options", "choose_friendly_aid_target", "tournament_travel_interested", "tournament_entrant_interested", "tournament_spectator_interested", "choose_hostile_action", "choose_investment_priority", "stable_permille",
]
