...
_RESOURCE_FIELDS = {"qi_conservation", "fatigue_reserve", "qi_trigger_threat_percent", "poison_trigger_threat_percent"}
...
_RETINUE_TOP_LEVEL_FIELDS = {"principal", "allocation", "formation", "temporary_members", "resource_discipline"}
...
_RETINUE_RESOURCE_FIELDS = {"qi_conservation", "qi_trigger_threat_percent", "poison_trigger_threat_percent", "outnumbered_override", "principal_overflow_override", "formal_nonlethal_poison"}
_RETINUE_ENUMS = {
...
def _bounded_threat_percent(value: Any, *, field: str) -> int:
    if not isinstance(value, int) or isinstance(value, bool) or not 0 <= value <= 200:
        raise ValueError(f"combat doctrine {field} must be integer 0..200")
    return value
...
def validate_player_retinue_doctrine(doctrine: Mapping[str, Any]) -> dict[str, Any]:
    ...
    resources = _closed_mapping(doctrine.get("resource_discipline"), _RETINUE_RESOURCE_FIELDS, field="resource_discipline")
    out = {
        "principal": {
        ...
        "resource_discipline": {
            "qi_conservation": _bounded_int(resources["qi_conservation"], field="qi_conservation"),
            "qi_trigger_threat_percent": _bounded_threat_percent(resources["qi_trigger_threat_percent"], field="qi_trigger_threat_percent"),
            "poison_trigger_threat_percent": _bounded_threat_percent(resources["poison_trigger_threat_percent"], field="poison_trigger_threat_percent"),
            "outnumbered_override": resources["outnumbered_override"],
            "principal_overflow_override": resources["principal_overflow_override"],
            "formal_nonlethal_poison": resources["formal_nonlethal_poison"],
        },
    }
    for key in ("guards_first", "break_for_pursuit", "inherit_retinue_coordination", "outnumbered_override", "principal_overflow_override", "formal_nonlethal_poison"):
        if key == "guards_first":
            container = out["allocation"]
        elif key == "break_for_pursuit":
            container = out["formation"]
        elif key == "inherit_retinue_coordination":
            container = out["temporary_members"]
        else:
            container = out["resource_discipline"]
...
def validate_individual_doctrine(doctrine: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if resources:
        ...
        for key, value in resources.items():
            if key in {"qi_trigger_threat_percent", "poison_trigger_threat_percent"}:
                normalized_resources[key] = _bounded_threat_percent(value, field=key)
            else:
                normalized_resources[key] = _bounded_int(value, field=key)
        out["resource_discipline"] = normalized_resources
...
def resolve_faction_force_intent(doctrine: Mapping[str, Any] | None, context: str) -> str:
    """Resolve institutional lethal restraint for actors without personal policy.

    The authored ``lethality_threshold`` is a 0..100 institutional propensity:
    values below 50 retain disable intent even in otherwise lethal hostile
    contexts; values at or above 50 accept lethal intent.  Formal, tournament
    and capture contexts remain nonlethal.  Missing doctrine preserves the
    generic legacy context default.
    """
    base = resolve_force_intent(None, context)
    if base != "lethal" or not isinstance(doctrine, Mapping):
        return base
    raw = doctrine.get("lethality_threshold")
    if not isinstance(raw, int) or isinstance(raw, bool):
        return base
    return "lethal" if max(0, min(100, raw)) >= 50 else "disable"
