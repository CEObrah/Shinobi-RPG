...
def initialize_combat(*, combat_ref: str, side_a_refs: Sequence[str], side_b_refs: Sequence[str], people: Mapping[str, Mapping[str, Any]], zone_ref: str, started_at: str, objective: Mapping[str, Any], awareness_mode: str = "mutual", initial_range_band: int = 1, obstacles: Sequence[Mapping[str, Any]] = (), awareness_evidence: Mapping[str, Any] | None = None, equipment_ledger: Mapping[str, Any] | None = None, initial_ready_weapons: Mapping[str, str] | None = None, mount_assignments: Mapping[str, Mapping[str, Any]] | None = None, environment: Mapping[str, Any] | None = None, reinforcement_delays_ms: Mapping[str, int] | None = None) -> dict[str, Any]:
    ...
    for ref in a+b:
        ...
        state[ref]={"defense_state":{"load_milli":0,"last_at_ms":-1_000_000_000,"recent_attackers":{}},"balance_milli":1000,"limb_commitment_milli":0,"recovery_until_ms":0,"weapon_position":"guard","ready_weapon_ref":ready_ref,"status_families":status_families,"reinforcement_at_ms":reinforce,"surprise_milli":700 if surprised else 0,"observed_refs":([] if reinforce>0 else observed),"current_contact_refs":initial_contacts,"awareness_confidence_milli":0 if reinforce>0 else (1000 if observed else 0),"qi_allocation_milli":{},"environment_movement_milli":max(250,int(env.get("movement_milli",1000))),"environment_mounted_milli":max(250,int(env.get("mounted_milli",1000))),"environment_visibility_milli":max(200,int(env.get("visibility_milli",1000))),"concealment_milli":max(0,int(env.get("concealment_milli",0)))}
...
def _ally_support_step(*, combat: dict[str, Any], actor_ref: str, target_ref: str, task: str,
                       people: Mapping[str, Mapping[str, Any]], equipment_ledger: Mapping[str, Any],
                       start_ms: int, duration_ms: int = 1000) -> dict[str, Any]:
        ...
        if remaining>desired:
            ...
            if not isinstance(support_state,dict):
                support_state={
                    ...
                    "issued_by_ref":"player","issued_at_ms":max(0,int(start_ms)),
                }
...
def _combat_narrative_projection(*, combat_before: Mapping[str, Any], combat_after: Mapping[str, Any], people_before: Mapping[str, Mapping[str, Any]], people_after: Mapping[str, Mapping[str, Any]], events: Sequence[Mapping[str, Any]], player_ref: str, combat_information: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for raw in events:
        ...
        wound=raw.get("wound") if isinstance(raw.get("wound"),Mapping) else {}
        structure=raw.get("target_structure_contacted") or wound.get("structure_ref")
    ...
    for ref in sorted(allied):
        ...
        if newly_critical:
            priority.append({
                "at_ms":max(0,int(combat_after.get("elapsed_ms",0))),"kind":"critical_ally_casualty",
    ...
    # Preserve every protected casualty beat; trim ordinary transaction detail first.
    protected=[row for row in merged if row.get("salience")=="protected"]
    ordinary=[row for row in merged if row.get("salience")!="protected"][-10:]
    final=sorted([*protected,*ordinary],key=lambda row:(int(row.get("at_ms",0)),0 if row.get("salience")=="protected" else 1))
    return {
...
def resolve_exchange(*, combat: Mapping[str, Any], people: Mapping[str, Mapping[str, Any]], equipment_ledger: Mapping[str, Any], doctrines: Mapping[str, Mapping[str, Any]], player_ref: str, player_action_kind: str, player_target_ref: str, player_weapon_ref: str, player_hit_zone: str = "chest", player_target_structure_ref: str | None = None, player_targeting_intent: str = "disable", player_poison_ref: str | None = None, player_qi_allocation_milli: Mapping[str, int] | None = None, player_qi_reserve_milli: int | None = None, player_auto_qi: bool = False, player_auto_poison: bool = False, player_action_detail_origin: str = "player", npc_targeting_intent: str | None = None, martial_familiarity: Mapping[str, Any] | None = None, player_retinue_context: Mapping[str, Any] | None = None, player_improvised_weapon_state: Mapping[str, Any] | None = None, player_rally_allies: bool = False, player_ally_orders: Sequence[Mapping[str, Any]] | None = None, equipment_ledger_hydrated: bool = False, compact_equipment_result: bool = True, mutate_equipment_ledger: bool = False, mutate_state: bool = False) -> dict[str, Any]:
    ...
    for actor_ref,order in ally_order_map.items():
        if actor_ref not in active_at_declaration:
            ...
            continue
        event=_ally_support_step(combat=out,actor_ref=actor_ref,target_ref=order["target_ref"],task=order["task"],people=persons,equipment_ledger=ledger,start_ms=declared_exchange_ms)
    ...
    for actor_ref in active_at_declaration:
        if actor_ref in withdrawing or actor_ref in support_actors: continue
        ...
        if actor_ref==player_ref:
            ...
            # A designated field medic is a support asset, not an assault slot.
            # This role comes from the exact retinue authority rather than the
            # latency-gated team plan, so a communications delay cannot briefly
            # turn the medic into an autonomous needle/weapon attacker.  The
            # medic can still defend reactively when another scheduled attack
            # reaches them, withdraw under ordinary survival doctrine, or act
            # through an explicit reach/protect/extract/treat ally order above.
            if retinue_member_roles.get(actor_ref)=="field_medic":
                declaration_events.append({
                    "actor_ref":actor_ref,
                    "result":"holding_medical_support_position",
                    "decision_origin":"player_retinue_role",
                    "role":"field_medic",
                })
                continue
...
__all__ = ["attempt_disengage", "capability_from_person", "combatant_active", "currently_visible_enemies", "default_action_for", "default_target_for", "default_weapon_for_action", "initialize_combat", "resolve_exchange"]
