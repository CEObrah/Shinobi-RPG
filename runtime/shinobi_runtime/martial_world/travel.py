...
def _edge_hours(edge: Mapping[str, Any], mode: str) -> float:
    ...
    terrain = float(travel["terrain_time_milli"][edge["terrain"]]) / 1000.0
...
def travel_plan(
    *,
    ...
    geo = _load("geography.json")
    edge_by_id = {e["id"]: e for e in geo["routes"]}
    travel = _load("travel.json")
    now = start_at
    total_hours = 0.0
    segments = []
    for edge_index, edge_id in enumerate(route["edges"]):
        ...
        weather = weather_snapshot(world_seed=world_seed, at=now, place_id=origin)
        ...
        hours = base * weather_factor * ground_factor * load_factor / speed_factor
        hours = round(hours, 4)
        segments.append({
            "edge_id": edge_id,
