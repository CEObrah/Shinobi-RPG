...
_ROUTES_PATH = "state/world/routes-and-settlements.json"
...
class OperationalWorldCommandsMixin:
    ...
    def _mission_market_resolution(self, command: CommandEnvelope, meta: Mapping[str, Any], current_time: CampaignTime) -> _BuiltPlan:
        ...
        market_row=next((row for row in rows if isinstance(row,Mapping) and row.get("id")==market_ref),None)
        owner_ref=market_row.get("owner_id") if isinstance(market_row,Mapping) else None
        place_ref=market_row.get("place_ref") if isinstance(market_row,Mapping) else None
        if not isinstance(owner_ref,str) or not isinstance(place_ref,str): raise CommandRejectedError("mission_market_world_invalid")
        try: location_graph=LocationGraph(self.repository.read_json(_ROUTES_PATH))
        except (FileNotFoundError,ValueError) as exc: raise CommandRejectedError("mission_market_world_invalid") from exc
        if location_graph.place(place_ref) is None: raise CommandRejectedError("mission_market_world_invalid")
        self._operational_authority(command.actor_id,owner_ref)
        ...
        return _BuiltPlan(code="mission_market_resolution_ready",affected_refs=expected_paths,writes=writes,result={"command_type":command.command_type,"market_ref":market_ref,"place_ref":place_ref,"axis":axis,"signal_ref":key,"score":scores[key],"evidence_ref":evidence_ref,"semantic_event_id":event_id},validator=validate)
