...
Development history is not campaign state. The packaged save is the current canonical campaign checkpoint at revision 143 and contains only current campaign truth. Gameplay idempotency receipts are runtime-private and are created only by future committed commands.

## Current campaign baseline

- Campaign revision: 143.
- Player: Tang Wei.
...
The packaged campaign baseline is revision 143 at `SE-0061-09-27T22:58:33`. Treat the repository ZIP as source plus baseline state; deployment state becomes authoritative only after the intended Railway volume reset/deploy.
