    ...
    if isinstance(people, list):
        for person in people:
            ...
            if person.get("person_id") in {"pc_wei_tang", "char.zhu"}:
                ...
                # This is a synthetic isolated-combat fixture. Current live
                # wounds must not make the regression depend on save timing.
                person["health"] = {
...
def test_installer_extends_existing_exchange_contract_only() -> None:
    ...
    assert exchange.payload_hints["movement_intent"] == "<close|chase|lateral|lateral_left|lateral_right>"
...
def test_command_preview_accepts_lateral_melee_entry_end_to_end(tmp_path: Path) -> None:
    ...
    exchange = CommandEnvelope(
        ...
        payload={
            ...
            "weapon_ref": "auto",
            "movement_intent": "lateral",
            "npc_judgments": {"char.zhu": {"intent": "hold"}},
        },
        mode="gameplay",
    )
...
def test_command_preview_accepts_move_only_close_maneuver_end_to_end(tmp_path: Path) -> None:
    root = _copy_runtime_repository(tmp_path)
    repo = RepositoryStore(root)
    planner = RepositoryCommandPlanner(repo)
    meta = repo.read_json("state/meta.json")
    combat_ref = "combat.test.move-only-close"

    start = CommandEnvelope(
        campaign_id=meta["campaign_id"],
        request_id="test.move-only-close.start",
        actor_id=meta["player_id"],
        command_type="jianghu_combat_resolution",
        expected_revision=meta["revision"],
        submitted_at="2026-09-06T10:00:00Z",
    ...
    current = repo.read_json("state/meta.json")
    maneuver = CommandEnvelope(
        campaign_id=current["campaign_id"],
        request_id="test.move-only-close.maneuver",
        actor_id=current["player_id"],
        command_type="jianghu_combat_resolution",
        expected_revision=current["revision"],
        submitted_at="2026-09-06T10:00:01Z",
    ...
    preview = planner.preview(maneuver)
    assert preview.status == "ready"
    assert preview.code == "jianghu_combat_exchange_resolved"
    plan = planner.plan(maneuver)
    player_events = [
        row for row in plan.result["events"] if row.get("actor_ref") == current["player_id"]
    ]
    assert any(row.get("action_kind") == "maneuver" for row in player_events)
    assert not any(
        row.get("actor_ref") == current["player_id"] and row.get("result") in {"contact", "mount_contact", "mount_disabled"}
