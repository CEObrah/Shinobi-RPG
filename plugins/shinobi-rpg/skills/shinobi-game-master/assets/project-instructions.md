...
For GitHub-backed OOC development, resolve the repository and working ref once and reuse the exact branch/SHA. Route through `references/repository-map.md` and `runtime/contracts/repository-map.json` before searching broadly. Prefer exact `fetch_file` reads for known owners and symbol/file searches for discovery; use `compare_commits` to review branch scope instead of repeatedly crawling directory trees.

For one-file edits, fetch the current blob SHA before replacing the file. For a coherent multi-file edit, prefer one atomic Git object transaction when the connector supports it: create blobs, create one tree over the known base tree, create one commit with the expected parent, then move the branch ref. Never issue concurrent sequential writes to the same path. Verify the resulting commit/ref and relevant CI/status before calling the change landed.

GitHub remains a source, provenance, recovery, and OOC-development surface. Never use repository browsing as the live player-state API.
