...
from shinobi_runtime.martial_world.escort_living_world import (
    best_route_observer, interception_attack_options, interception_decision, interception_force_size, observed_escort_strength,
...
class JianghuInformationCommandsMixin:
    def _jianghu_public_disclosure_resolution(self, command: CommandEnvelope, meta: Mapping[str,Any], current_time: CampaignTime):
        ...
        # generated after-images are written only if the tip reaches a durable
        # faction judgment frontier below; otherwise the candidate never becomes
        # save state.
        ...
        for fid in sorted(set(local_fids)):
            ...
            candidate_refs=[str(p.get("person_id")) for p in available if isinstance(p.get("person_id"),str)]
            attack_options=interception_attack_options(
                attacker_faction_ref=fid,target_movement_ref=movement_ref,route_ref=target_route,
                candidate_attacker_refs=candidate_refs,decision=decision,player_ref=str(meta.get("player_id") or ""),
            )
            if not attack_options:
                continue
            event=build_npc_judgment_event(
                judgment_kind="route_interception_policy",at_iso=at_iso,actor_ref=fid,owner_ref=movement_ref,
                options=[
                    {"intent":"ignore","attacker_faction_ref":fid,"target_movement_ref":movement_ref,"route_ref":target_route},
                    {"intent":"observe","attacker_faction_ref":fid,"target_movement_ref":movement_ref,"route_ref":target_route},
                    *attack_options,
                ],
                decision_context={
                    "attacker_faction_ref":fid,"target_movement_ref":movement_ref,"route_ref":target_route,
                    "opportunity_kind":"public_disclosure","stop_place_ref":settlement,
                    "observer_ref":observer_ref,"candidate_attacker_refs":candidate_refs,
                    "visible_escort_count":max(0,int(observed.get("visible_escort_count",0))),
                    "estimated_escort_combat_index":max(0,int(observed.get("estimated_combat_index",0))),
                    "advisory":private_interception_decision_context(decision),
                },
                source_event_id=f"public-disclosure:{disclosure_ref}",
            )
            ...
            if transient_civic_ref and str(observer.get("person_id") or "")==transient_civic_ref:
                # Reaching a durable institutional judgment is the promotion
                # boundary: the informant has now caused a real causal fact even
                # if the faction ultimately observes or declines violence.
        ...
        # Disclosure can create a same-timestamp judgment frontier, never an
        # attack/pursuit by itself. The route reducer applies a later committed
        # option and revalidates people, training, resources and target movement.
