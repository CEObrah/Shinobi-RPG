...
def run(state, *, social=None, relations=None, sites=None, events=None):
    ...
    result=settle_autonomous_institutional_evolution(
        ...
        events=events if events is not None else [{"schedule_class":"faction_annual","owner_ref":"f.final"}],
    ...
    result["_pending_judgments"] = pending
    return result,writes


def commit_event(event):
    accept=next(row for row in event["options"] if row.get("intent")=="accept")
    return {
        "event_id":"npc_judgment_committed:test",
        "kind":"npc_judgment_committed",
        "due_at":event["due_at"],
        "owner_ref":event["owner_ref"],
        "actor_ref":event["actor_ref"],
        "judgment_kind":event["judgment_kind"],
        "decision_ref":event["decision_ref"],
        "selected_option":copy.deepcopy(accept),
        "decision_context":copy.deepcopy(event.get("decision_context",{})),
    }


def test_autonomous_dissolution_requires_gm_judgment_before_moving_person():
    ...
    assert review["kind"]=="institutional_evolution_judgment_due"
    assert review["proposal"]["kind"]=="autonomous_faction_dissolution"
    event=result["_pending_judgments"][0]
    assert event["actor_ref"]=="p.one"
    result,writes=run(state,events=[commit_event(event)])
...
def test_autonomous_merger_requires_gm_judgment_before_moving_assets():
    ...
    assert writes=={}
    review=result["reviews"][0]
    assert review["proposal"]["kind"]=="autonomous_faction_merger"
    event=result["_pending_judgments"][0]
    result,writes=run(state,relations=rel,events=[commit_event(event)])
    transition=result["reviews"][0]["transition"]
    source,target=transition["source_faction_ref"],transition["target_faction_ref"]
...
def test_autonomous_split_requires_gm_judgment_before_breakaway():
    ...
    sites={estate_site:{"site_type":"martial_headquarters","parent_place_ref":"place.branch"}}
    result,writes=run(state,social=social,sites=sites)
    assert writes=={}
    review=result["reviews"][0]
    assert review["proposal"]["kind"]=="autonomous_faction_split"
    assert review["actor_ref"]=="p.1"
    event=result["_pending_judgments"][0]
    result,writes=run(state,social=social,sites=sites,events=[commit_event(event)])
    transition=result["reviews"][0]["transition"]
    new=transition["new_faction_ref"]
    assert set(transition["member_refs"])=={"p.1","p.2"}
...
def test_autonomous_foundation_requires_gm_judgment_before_cash_or_members_move():
    ...
    assert writes=={}
    review=result["reviews"][0]
    assert review["proposal"]["kind"]=="autonomous_faction_foundation"
    event=result["_pending_judgments"][0]
    result,writes=run(state,social=social,sites=sites,events=[commit_event(event)])
    transition=result["reviews"][0]["transition"]
    new=transition["faction_ref"]
    assert transition["startup_cash"]==1000
