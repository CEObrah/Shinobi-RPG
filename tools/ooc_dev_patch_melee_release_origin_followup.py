...
old = r'''def test_melee_strike_lane_launches_from_post_approach_position(monkeypatch):
...
'''
new = r'''def test_melee_strike_lane_launches_from_post_approach_position():
    attacker, defender = _people_pair()
    people = {attacker["person_id"]: attacker, defender["person_id"]: defender}
    ledger = _jian_ledger(attacker["person_id"])
    combat = exact.initialize_combat(
...
'''
if text.count(old) != 1:
    raise SystemExit("post-approach regression harness match missing or ambiguous")
path.write_text(text.replace(old, new, 1), encoding="utf-8")
print("post-approach regression isolated to one scheduled action")
