...
def _judgment_due(result, kind):
    return next(
        row for row in result["schedule_after"].get("one_off", {}).values()
        if isinstance(row, dict) and row.get("kind") == "npc_judgment_due" and row.get("judgment_kind") == kind
    )


def _committed_judgment(due, intent):
    option = next(row for row in due["options"] if row.get("intent") == intent)
    return {
        "event_id": "npc_judgment_committed:test:" + str(due.get("decision_ref") or "").split(":")[-1],
        "kind": "npc_judgment_committed", "due_at": due["due_at"], "owner_ref": due["owner_ref"],
        "actor_ref": due.get("actor_ref", ""), "judgment_kind": due["judgment_kind"],
        "decision_ref": due["decision_ref"], "selected_option": copy.deepcopy(option),
...
def test_raid_return_reaches_real_hideout_before_captor_can_demand_ransom():
    ...
    unchanged = after["state/martial-world/custody.json"]["records"][0]
    assert "ransom_demand_cash" not in unchanged
    due = _judgment_due(second, "captor_ransom_policy")
    committed = _committed_judgment(due, "demand")
    third = settle_martial_world_frontier(
        read_json=reader(after), schedule=second["schedule_after"], events=[committed], at=review_at,
    )
    ...
    message_events = [
        row for row in third["schedule_after"].get("one_off", {}).values()
...
def test_npc_ransom_moves_real_cash_then_returns_exact_captive_home():
    ...
    assert row["responses"][0]["result"] == "response_judgment_required"
    assert after["state/martial-world/custody.json"]["records"]
    assert int(after[f"state/martial-world/factions/{responder_fid}.json"]["treasury_cash"]) == cash_before
    due = _judgment_due(first, "captive_response_policy")
    committed = _committed_judgment(due, "pay_ransom")
    paid = settle_martial_world_frontier(
        read_json=reader(after), schedule=first["schedule_after"], events=[committed], at=at,
    )
    after.update({str(k): copy.deepcopy(v) for k, v in paid["writes"].items()})
    payment_review = next(x for x in paid["reviews"] if x.get("kind") == "captive_response_policy")
    assert payment_review["result"] == "ransom_paid"
    op_ref = payment_review["operation_ref"]
    assert after["state/martial-world/custody.json"]["records"] == []
    ...
    departure_event = next(
        row for row in paid["schedule_after"].get("one_off", {}).values()
    ...
    second = settle_martial_world_frontier(
        read_json=reader(after), schedule=paid["schedule_after"],
...
def test_returning_raiders_can_be_intercepted_and_custody_follows_exact_combat(monkeypatch):
    ...
    assert call == {}
    due = _judgment_due(result, "route_interception_policy")
    committed = _committed_judgment(due, "attack")
    decided = settle_martial_world_frontier(
        read_json=reader(after), schedule=result["schedule_after"], events=[committed], at=at,
    )
    after.update({str(k): copy.deepcopy(v) for k, v in decided["writes"].items()})
    wake = next(
        row for row in decided["schedule_after"].get("one_off", {}).values()
        if isinstance(row, dict) and row.get("kind") == "route_activity_cycle"
        and row.get("decision_origin") == "gm_npc_judgment"
    )
    resolved = settle_martial_world_frontier(
        read_json=reader(after), schedule=decided["schedule_after"], events=[wake], at=at,
    )
    after.update({str(k): copy.deepcopy(v) for k, v in resolved["writes"].items()})
...
def test_nonambulatory_captive_does_not_pay_ransom_until_self_travel_is_possible():
    ...
    assert response["result"] == "response_judgment_required"
    due = _judgment_due(result, "captive_response_policy")
    committed = _committed_judgment(due, "pay_ransom")
    decided = settle_martial_world_frontier(
        read_json=reader(after), schedule=result["schedule_after"], events=[committed], at=at,
    )
    after.update({str(k): copy.deepcopy(v) for k, v in decided["writes"].items()})
    payment = next(x for x in decided["reviews"] if x.get("kind") == "captive_response_policy")
    assert payment["result"] == "ransom_waiting_for_recovery"
    ...
    rechecks = [
        event for event in decided["schedule_after"].get("one_off", {}).values()
