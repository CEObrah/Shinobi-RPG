...
from shinobi_runtime.martial_world.family_simulation import advance_npc_relationships, apply_npc_relationship_judgment, review_conceptions, resolve_birth, advance_annual_life_course
...
def test_demographic_pairing_target_uses_full_adult_cohort_not_unmarried_remainder(monkeypatch):
    ...
    assert result['courtships_started'] == []
    request = next(row for row in result['judgment_requests'] if row['judgment_kind']=='family_courtship')
    start = next(row for row in request['options'] if row['intent']=='start')
    pair = start['pair_ref']
    assert not {'a','b'} & set(pair.split('|'))
    applied = family_simulation.apply_npc_relationship_judgment(
        result['family_after'], result['social_after'], faction_ref='f', roster_people=people,
        at_iso='0061-12-01T00:00:00', judgment_kind='family_courtship', option=start, residence_ref='site.f',
    )
    assert pair in applied['social_after']['courtships']
...
def test_relationship_locality_uses_parent_place_not_exact_site():
    ...
    assert local['courtships_started']==[]
    request=next(row for row in local['judgment_requests'] if row['judgment_kind']=='family_courtship')
    assert next(row for row in request['options'] if row['intent']=='start')['pair_ref']=='a|b'
...
def test_existing_courtship_waits_for_physical_reunion_before_marriage():
    ...
    assert reunited['marriages_created']==[]
    request=next(row for row in reunited['judgment_requests'] if row['judgment_kind']=='family_marriage')
    marry=next(row for row in request['options'] if row['intent']=='marry')
    applied=apply_npc_relationship_judgment(
        reunited['family_after'], reunited['social_after'], faction_ref='f', roster_people=[a,b_home],
        at_iso='0061-10-01T00:00:00', judgment_kind='family_marriage', option=marry, residence_ref='site.home', site_rows=sites,
    )
    assert applied['marriage_ref']
    assert 'a|b' not in applied['social_after']['courtships']
