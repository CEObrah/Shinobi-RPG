...
def test_current_campaign_uses_persisted_causal_scheduler_without_legacy_frontier():
    ...
    assert record["schema"] == "causal-scheduler-index"
    assert scheduler.hosts
    assert len(scheduler.queue.snapshot()) == record["pending_event_count"]
...
def test_catchup_accepts_more_than_legacy_1024_total_hosts_when_they_are_safe() -> None:
    start = time("SE-0061-02-07T00:00:00")
    target = time("SE-0061-02-07T01:00:00")
    hosts = {
        f"host.safe.{idx:04d}": HostState(
            host_id=f"host.safe.{idx:04d}",
            kind="test",
            resolved_through=start,
            safe_through=target,
            handler_ref="test",
            rng_namespace=f"safe.{idx}",
            next_due=None,
        )
        for idx in range(1025)
    }
    result = CatchUpEngine({}).settle(hosts=hosts, queue=EventQueue(), target=target)
    assert result.reached_time == target
    assert result.budget_exhausted is False
    assert result.unsafe_host_ids == ()


def test_catchup_accepts_more_than_legacy_20000_pending_events_without_world_rejection() -> None:
    start = time("SE-0061-02-07T00:00:00")
    target = time("SE-0061-02-07T00:00:01")
    first_due = time("SE-0061-02-07T00:00:02")
    host = HostState(
        host_id="host.large-queue",
        kind="test",
        resolved_through=start,
        safe_through=target,
        handler_ref="test",
        rng_namespace="large-queue",
        next_due=first_due,
    )
    events = [
        event(
            f"event.large.{idx:05d}",
            f"SE-0061-02-07T00:{(idx // 60) % 60:02d}:{2 + (idx % 58):02d}",
            target_host="host.large-queue",
            source_host="host.large-queue",
        )
        for idx in range(20001)
    ]
    # The helper timestamps above repeat. Event IDs make ordering deterministic;
    # all are after target, so this tests validity rather than throughput.
    queue = EventQueue(events)
    host.next_due = queue.peek().due_at
    result = CatchUpEngine({}).settle(hosts={host.host_id: host}, queue=queue, target=target)
    assert result.reached_time == target
    assert len(queue) == 20001
    assert result.budget_exhausted is False


def test_causal_host_work_budget_returns_resumable_frontier_instead_of_rejecting_world() -> None:
    start = time("SE-0061-02-07T00:00:00")
    target = time("SE-0061-02-07T00:00:10")
    due_values = [
        time("SE-0061-02-07T00:00:02"),
        time("SE-0061-02-07T00:00:04"),
        time("SE-0061-02-07T00:00:06"),
    ]
    hosts = {}
    events = []
    for idx, due in enumerate(due_values):
        host_id = f"host.chunk.{idx}"
        hosts[host_id] = HostState(
            host_id=host_id,
            kind="test",
            resolved_through=start,
            safe_through=start,
    ...
    def handler(_event, _host):
        return EventOutcome(safe_through=target, next_due=None)

    queue = EventQueue(events)
    first = CatchUpEngine({"test": handler}).settle(
        hosts=hosts, queue=queue, target=target, causal_host_budget=2
    )
    assert first.budget_exhausted is True
    assert first.reached_time == due_values[1]
    assert first.processed_event_ids == ("event.chunk.0", "event.chunk.1")
    assert queue.peek().event_id == "event.chunk.2"

    second = CatchUpEngine({"test": handler}).settle(
        hosts=hosts, queue=queue, target=target, causal_host_budget=2
    )
    assert second.budget_exhausted is False
    assert second.reached_time == target
    assert second.processed_event_ids == ("event.chunk.2",)
    assert len(queue) == 0
