...
OLD_BAD_TEAM = {"mw.person.house_tang.5000", "mw.person.house_tang.1037", "mw.person.house_tang.1001"}
...
def _bad_assignment_read():
    ...
    row["member_refs"] = ["mw.person.house_tang.5000", "mw.person.house_tang.1037", "mw.person.house_tang.1001"]
    row["member_roles"] = {
    ...
    def read(rel: str):
        return copy.deepcopy(deployments) if rel == DEPLOYMENTS else load(rel)
...
def _assert_corrected_team(row):
    ...
    for ref in row["member_refs"]:
        ...
        assert 16 <= age <= 35
        joined_year = people[ref].get("joined_year")
        assert not isinstance(joined_year, int) or joined_year <= 60
...
def test_bad_multi_generation_team_is_recomputed_from_same_conserved_roster():
    ...
    result = plan_permanent_team_cohort_v3_migration(read)
...
def test_permanent_team_migration_is_idempotent_after_corrected_after_image():
    read, _ = _bad_assignment_read()
    first = plan_permanent_team_cohort_v3_migration(read)
    assert first["writes"]

    def read_after(rel: str):
        return copy.deepcopy(first["writes"][rel]) if rel in first["writes"] else read(rel)

    second = plan_permanent_team_cohort_v3_migration(read_after)
...
def test_current_save_is_either_repairable_now_or_already_repaired():
    result = plan_permanent_team_cohort_v3_migration(load)
    if result["writes"]:
...
def test_retinue_migration_after_image_passes_registered_deployment_contracts():
    ...
    result = plan_permanent_team_cohort_v3_migration(repository.read_json)
    if not result["writes"]:
        assert result["reason"] == "already_current"
        return
    meta = repository.read_json("state/meta.json")
    command = CommandEnvelope(
        campaign_id=meta["campaign_id"], request_id="test.permanent-team-cohort-v3.maintenance",
        actor_id=meta["player_id"], command_type="maintenance_permanent_travel_team_cohort_v3",
        expected_revision=meta["revision"], submitted_at="2026-08-22T00:00:00Z", mode="maintenance",
        payload={"migration": "permanent_travel_team_cohort_v3"},
    )
    writes = {path: (json.dumps(value, ensure_ascii=False, indent=2) + "\n").encode("utf-8")
              for path, value in result["writes"].items()}
    manifest = TransactionPlanner(repository).plan(
        command, transaction_id="tx.maintenance.test.permanent-team-cohort-v3",
        created_at="2026-08-22T00:00:00Z", writes=writes)
