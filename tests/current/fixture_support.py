...
def make_person_activity_ready_fixture(person: dict[str, Any]) -> dict[str, Any]:
    """Normalize one disposable person row for tests unrelated to live injury state.

    The campaign save is allowed to advance into injury, fatigue, or incapacitation.
    Tests that clone a live person only as an identity/capability fixture must opt in
    to a clean activity-ready body rather than inheriting unrelated campaign damage.
    This mutates only the caller-owned disposable row and never writes campaign state.
    """
    if not isinstance(person, dict):
        raise TypeError("person fixture must be a mutable dict")
