...
Treat every remote letter, ransom demand, House report, mission message, summons, institutional answer, faction notice, and courier dispatch as a physical information handoff. `Prepared`, `decided`, `written`, `sent`, or `dispatched` is not `received`. A remote faction learning or deciding something does not automatically make it Wei's knowledge. Immediate receipt requires exact co-location or a runtime-established immediate signal; otherwise the lawful travel/message route must settle, and a moved recipient must cause rerouting rather than delivery to an obsolete address. An informational message that has already been resolved offscreen is not by itself a protected player decision or hard chronology stop.
