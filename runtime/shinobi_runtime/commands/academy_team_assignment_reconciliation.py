...
def _outside_active_members(planner: Any, repair_refs: set[str], member_refs: set[str]) -> set[str]:
    occupied: set[str] = set()
    for person_ref in sorted(member_refs):
        try:
            team_refs = team_refs_for_member(planner.repository, person_ref)
        except ValueError as exc:
            raise CommandRejectedError("academy_team_reconciliation_membership_routes_invalid") from exc
        for team_ref in team_refs:
            if team_ref in repair_refs:
                continue
            try:
...
def _academy_team_assignment_reconciliation(
    ...
    occupied_elsewhere = _outside_active_members(self, repair_refs, all_desired_members)
