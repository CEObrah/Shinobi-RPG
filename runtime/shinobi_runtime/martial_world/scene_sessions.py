...
def relevant_scene_continuity(
    read_json: Callable[[str], Any], *, subject_refs: Sequence[str] = (), location_ref: str | None = None, limit: int = 12
) -> list[dict[str, Any]]:
    """Return bounded long-lived literary continuity for the current scene.

    This is an authority-false hot index over already recorded continuity notes.
    It never upgrades a note into objective truth.
    """
    head = history_head(read_json)
...
def append_scene_history_record(read_json: Callable[[str], Any], *, row: Mapping[str, Any]) -> dict[str, Mapping[str, Any]]:
    ...
    if any(_history_record_ref(x) == record_ref for x in recent if isinstance(x, Mapping) and (x.get("speech_ref") or x.get("fact_ref") or x.get("continuity_ref"))):
        return {}
    ...
    if any(_history_record_ref(x) == record_ref for x in records if isinstance(x, Mapping) and (x.get("speech_ref") or x.get("fact_ref") or x.get("continuity_ref"))):
        return {}
    ...
    _update_continuity_hot_index(head, row)
...
__all__ = [
    "SESSION_PATH", "ATTEMPT_LEDGER_PATH", "HISTORY_HEAD_PATH", "SESSION_KINDS", "SPEECH_KINDS", "SCENE_FACT_KINDS", "CONTINUITY_NOTE_KINDS",
    ...
    "inspect_history_object", "relevant_scene_continuity", "safe_ref", "bounded_text",
]
