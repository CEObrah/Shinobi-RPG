...
def hosted_config(profile: Mapping[str, Any]) -> Mapping[str, Any] | None:
    ...
    if (
        ...
        raise CommandRejectedError("promotion_exam_rules_invalid")
    calibration = config.get("generated_candidate_calibration")
    ...
    configured_villages: set[str] = set()
    for row in delegations:
        ...
        size = row.get("delegation_size")
        village = str(row["service_village"]).lower()
        if (
            ...
            or not 1 <= size <= 16
            or village not in {value.lower() for value in villages}
            or village == host_village.lower()
            or village in configured_villages
        ):
            raise CommandRejectedError("promotion_exam_rules_invalid")
        configured_villages.add(village)
    expected_foreign = {value.lower() for value in villages} - {host_village.lower()}
    arrival_buffer = config.get("arrival_buffer_days", 0)
    if (
        configured_villages != expected_foreign
        or isinstance(arrival_buffer, bool)
        or not isinstance(arrival_buffer, int)
        or arrival_buffer < 0
    ):
        raise CommandRejectedError("promotion_exam_rules_invalid")
    return config
...
def required_arrival_lead_days(repository: Any, profile: Mapping[str, Any]) -> int:
    """Return the minimum common registration window for every invited village.

    The window is derived from the slowest lawful minimum route plus the
    configured arrival buffer.  A configured foreign delegation with no lawful
    route is a rules defect, not a silently ineligible delegation.
    """
    config = hosted_config(profile)
    if config is None:
        return int(math.ceil(arrival_lead_days(profile)))
    host_place = str(config["host_arrival_place_ref"])
    buffer_days = int(config.get("arrival_buffer_days", 0))
    required = 0.0
    for delegation in foreign_delegations(profile):
        home = str(delegation["home_place_ref"])
        route_days = minimum_route_days(repository, home, host_place)
        if route_days is None:
            raise CommandRejectedError("promotion_exam_attendance_route_invalid")
        required = max(required, route_days + buffer_days)
    return int(math.ceil(required))


def effective_arrival_lead_days(repository: Any, profile: Mapping[str, Any]) -> float:
    """Published arrival lead after applying common-exam travel feasibility."""
    return float(max(math.ceil(arrival_lead_days(profile)), required_arrival_lead_days(repository, profile)))
...
def eligible_hosted_registrations(
    ...
    *,
    ...
    cycle_id: str,
    player_id: str,
    record_writes: dict[str, dict[str, Any]] | None = None,
    at: CampaignTime | None = None,
) -> list[dict[str, Any]]:
    if _ORIGINAL_ELIGIBLE is None:
    ...
    host_place = str(config["host_arrival_place_ref"])
    lead_days = effective_arrival_lead_days(self.repository, profile)
    for delegation in foreign_delegations(profile):
        village = str(delegation["service_village"])
        size = int(delegation["delegation_size"])
        candidates: list[str] = []
        ...
        needed = size - len(candidates)
        if needed > 0 and record_writes is not None and at is not None:
            from shinobi_runtime.commands.promotion_exam_delegation_materialization import (
                exactify_home_village_genin,
            )
            generated = exactify_home_village_genin(
                self,
                ...
                record_writes=record_writes,
                count=needed,
                calibration=config["generated_candidate_calibration"],
            )
            candidates.extend(ref for ref, _person in generated)

        if len(candidates) != size:
            # A configured invitation is real. If the home village cannot field
            # its authored delegation from conserved eligible Genin, fail closed
            # rather than silently dropping that village from the common exam.
...
__all__ = [
    ...
    "delegation_by_team",
    "effective_arrival_lead_days",
    "eligible_hosted_registrations",
    "foreign_delegations",
    "hosted_config",
    "minimum_route_days",
    "required_arrival_lead_days",
