...
# Formation partition/merge mechanics are deterministic and anti-reroll.
part=rj('game/data/mechanics/formation-partition.json')
if part.get('schema')!='formation_partition_mechanics':err('formation_partition_v1')
...
# Split/merge evidence is part of ordinary semantic transaction receipts; no parallel formation transaction registry is authoritative.
# Formation model and aggregate computation.
um=rj('game/data/organization/formation-model.json'); ur=rj('game/data/mechanics/formation-resolution.json')
if um.get('schema')!='formation_model':err('formation_model_v1')
if ur.get('schema')!='formation_resolution_mechanics':err('formation_resolution_v1')
if 'one scalar power score' not in str(ur.get('anti_overcompression','')).lower():err('anti_scalar_missing')
if 'aggregate' not in str(ur.get('principle','')).lower():err('aggregate_formation_missing')
# Formation split/merge boundaries are mechanical here. Equipment standards and
# refit are deliberately owned by inventory/loadout authority rather than copied
# into the formation model.
joined=json.dumps(um).lower()
if 'split' not in joined:err('formation_boundary_missing:split')
loadout_rules=(R/'game/rules/text/loadouts.md').read_text(encoding='utf-8').lower()
org_rules=(R/'game/rules/text/org.md').read_text(encoding='utf-8').lower()
inv_contract=json.dumps(rj('runtime/contracts/system-contracts/inventory_loadouts.json')).lower()
if 'standard loadout' not in loadout_rules:err('loadout_standard_boundary_missing')
if 'refit' not in loadout_rules or 'refit' not in org_rules or 'refit' not in inv_contract:err('refit_inventory_boundary_missing')
if 'declaring a standard never creates equipment' not in loadout_rules:err('loadout_conservation_rule_missing')
# Support semantics.
...
if is_shinobi:
    ...
    # Capability-source classifications are not operational formations and are never a second conserved personnel partition.
    ...
    for rel in ['game/rules/text/characters.md','game/rules/text/world.md','state/life/identity-life-course.json','state/cap/internal-unit-combat-kernels.json','state/prog/sword-manor-progression.json']:
        t=(R/rel).read_text(encoding='utf-8').lower()
        for bad in ('dormant_and_latent_identities','unnamed named-person placeholders','no personal name until deepening','latent canon identity'):
            if bad in t:err(f'stale_character_model:{rel}:{bad}')
if errs:
    print('FORMATION MODEL TEST FAILED')
    for e in errs[:200]:print('-',e)
    sys.exit(1)
print('FORMATION MODEL TEST OK')
