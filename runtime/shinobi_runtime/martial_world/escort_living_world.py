...
from .exact_combat import capability_from_person, combat_capability_index
...
def person_combat_index(person: Mapping[str, Any]) -> int:
    return combat_capability_index(person)
...
def interception_decision(
    *, attacker_faction_type: str, relation: Mapping[str, Any] | None,
    ...
    result = {
        ...
        "ransom_value_cash": max(0, int(ransom_value_cash)),
    }
    if not attack:
...
def interception_motive_options(decision: Mapping[str, Any]) -> list[dict[str, Any]]:
    """Return bounded feasible hostile motives without choosing one.

    The utility model may advise whether violence looks attractive, but motive
    is a human intent.  Candidate construction therefore exposes every motive
    supported by the actor's current evidence rather than copying the model's
    single advisory motive into the only attack option.
    """
    if not isinstance(decision, Mapping):
        return [{"contact_intent":"hostile_interception","motive_kind":"grievance"}]
    cargo = max(0, int(decision.get("cargo_value_cash", 0)))
    ransom = max(0, int(decision.get("ransom_value_cash", 0)))
    hostility = max(0, int(decision.get("hostility", 0)))
    criminal = bool(decision.get("criminal"))
    identity_reward = max(0, int(decision.get("recognized_target_reward_score", 0)))
    rows: list[dict[str, Any]] = []
    if ransom > 0:
        rows.append({"contact_intent":"kidnap_principal","motive_kind":"ransom"})
    if cargo > 0:
        rows.append({"contact_intent":"rob_cargo","motive_kind":"loot"})
    if hostility >= 55:
        rows.append({"contact_intent":"revenge","motive_kind":"grievance"})
    if identity_reward > 0:
        rows.append({"contact_intent":"hostile_interception","motive_kind":"recognized_notable_target"})
    if criminal:
        rows.append({"contact_intent":"hostile_interception","motive_kind":"opportunistic_predation"})
    if not rows:
        rows.append({"contact_intent":"hostile_interception","motive_kind":"grievance"})
    unique: list[dict[str, Any]] = []
    seen: set[tuple[str, str]] = set()
    for row in rows:
        key = (str(row["contact_intent"]), str(row["motive_kind"]))
        if key not in seen:
            seen.add(key); unique.append(row)
    return unique


def interception_attack_options(
    *, attacker_faction_ref: str, target_movement_ref: str, route_ref: str,
    candidate_attacker_refs: Sequence[str], decision: Mapping[str, Any],
    player_ref: str = "", maximum_options: int = 32,
) -> list[dict[str, Any]]:
    """Enumerate bounded exact route-attack choices without selecting a party/motive."""
    candidates = [
        str(ref) for ref in candidate_attacker_refs
        if isinstance(ref, str) and ref and str(ref) != str(player_ref or "")
    ]
    candidates = list(dict.fromkeys(candidates))
    if not candidates:
        return []
    pool = candidates[: min(len(candidates), 9)]
    preferred = max(1, min(len(pool), int(decision.get("advisory_force_count", len(pool)))))
    sizes = sorted({
        size for size in (
            max(1, preferred // 2), preferred,
            min(len(pool), preferred + max(1, preferred // 2)),
        ) if 0 < size <= len(pool)
    })
    party_options: list[dict[str, Any]] = []
    # Keep the envelope bounded, but vary complete membership rather than
    # silently fixing every non-leader slot to the strongest prefix.
    for size in sizes:
        for combo in itertools.combinations(pool, size):
            for leader_ref in combo[: min(3, len(combo))]:
                party_options.append({"attacker_refs":list(combo),"leader_ref":leader_ref})
                if len(party_options) >= max(8, int(maximum_options)):
                    break
            if len(party_options) >= max(8, int(maximum_options)):
                break
        if len(party_options) >= max(8, int(maximum_options)):
    ...
    for party in party_options:
        for motive in interception_motive_options(decision):
            out.append({
                "intent":"attack", "attacker_faction_ref":str(attacker_faction_ref),
                "target_movement_ref":str(target_movement_ref), "route_ref":str(route_ref),
                **copy.deepcopy(party), **copy.deepcopy(motive),
                "combat_policy":{
                    "decision_origin":"gm_npc_judgment", "engagement_intent":"intercept_and_disable",
                    "targeting_intent":"disable", "target_policy":"nearest_visible",
                    "resource_policy":"none", "withdrawal_policy":"no_voluntary_withdrawal",
                },
            })
            if len(out) >= max(1, int(maximum_options)):
                return out
    return out
...
_PRIVATE_INTERCEPTION_DECISION_KEYS = (
    ...
    "cargo_value_cash", "ransom_value_cash", "advisory_force_count",
)
...
def route_extortion_settlement_terms(
    *, attacker_faction_type: str, attacker_intent: str, motive_kind: str,
    observed_escort_count: int, cargo_value_cash: int = 0,
) -> dict[str, int] | None:
    """Derive one deterministic road-extortion bargaining floor.

    The terms are private mechanical intent, not player knowledge and not NPC
    dialogue.  They exist so a spoken demand can be grounded in a real hard
    settlement authority instead of becoming an unenforceable narrative promise.
    Only ordinary outlaw robbery/extortion is eligible; ransom, grievance and
    mission violence require their own consequence owners.
    """
    criminal = str(attacker_faction_type) == "outlaw_faction"
    intent = str(attacker_intent or "")
    motive = str(motive_kind or "")
    if not criminal or intent in {"kidnap_principal", "revenge"} or motive in {"ransom", "grievance", "recognized_notable_target"}:
        return None
    if intent not in {"hostile_interception", "extortion", "robbery", "rob_cargo", "cargo_seizure"}:
        return None
    count = max(1, int(observed_escort_count))
    # Routine road shakedown: a fixed company-risk premium plus a bounded
    # per-visible-traveler amount. Manifest cargo can raise the floor, but with
    # diminishing leverage so payment does not become automatic cargo purchase.
    minimum = 1000 + count * 250 + min(20_000, max(0, int(cargo_value_cash)) // 20)
    minimum = ((minimum + 99) // 100) * 100
    opening = ((minimum * 5 // 4 + 499) // 500) * 500
    return {
        "minimum_cash": max(500, minimum),
        "opening_demand_cash": max(500, opening),
    }
...
def interception_force_size(
    *, available_count: int, observed_escort_count: int, hostility: int,
    ...
    """Allocate a plausible mission detachment instead of the whole faction.

    ``available_count`` is capacity, not the desired encounter size.  Ordinary
    predation starts from a modest edge over the visible party and keeps a real
    reserve for camp security, other work, casualties, and parallel operations.
    Organization scale improves the faction's ability to field/reinforce bands;
    it does not add bodies linearly to every roadside contact.  Exceptional
    value, grievance, or risk appetite may justify deeper commitment, including
    near-total mobilization when the stakes are genuinely strategic.
    """
    import math
    ...
    if available <= 0:
        return 0
    visible = max(1, int(observed_escort_count))
    hostility = max(0, min(100, int(hostility)))
    scale = max(0, int(criminal_scale))
    risk = max(0, min(100, int(risk_tolerance)))
    value = max(0, int(known_value_cash))
    criminal = str(attacker_faction_type) == "outlaw_faction"

    # Mission requirement, not victory guarantee.  A routine outlaw band seeks
    # a useful but not absurd numerical edge; ordinary institutions require a
    # serious grievance and are slightly more conservative about ad-hoc road
    # violence.
    base_ratio_milli = 1250 if criminal else 1150
    desired = max(2, (visible * base_ratio_milli + 999) // 1000)

    # Better organization lets a faction coordinate a somewhat larger band, but
    # the effect is sub-linear and bounded by the size of the target party.
    if criminal and scale > 0:
        desired += min(max(1, visible // 4), math.isqrt(scale))
    elif not criminal and hostility > 55:
        desired += min(max(0, visible // 3), (hostility - 55) // 15)

    # Risk appetite changes how much margin the mission commander wants.
    if risk >= 70:
        desired += max(1, visible * (risk - 60) // 160)
    elif risk <= 30:
        desired = max(2, desired - max(1, visible * (40 - risk) // 160))

    # Valuable cargo/notable captives can justify more bodies, but with strongly
    # diminishing returns.  Ten times the value never means ten times the men.
    if value >= 10_000:
        desired += min(max(1, visible // 2 + 2), math.isqrt(max(1, value // 10_000)))

    # Preserve a faction/local-presence reserve during routine operations. High
    # stakes progressively relax that reserve. Only truly exceptional grievance
    # or value can justify committing effectively everyone who is mobilizable.
    reserve_milli = 400 if criminal else 500
    if value >= 100_000 or hostility >= 75:
        reserve_milli -= 125
    if value >= 500_000 or hostility >= 90:
        reserve_milli -= 150
    if risk >= 80:
        reserve_milli -= 75
    if value >= 2_000_000 or hostility >= 98:
        reserve_milli = 0
    reserve_milli = max(0, min(700, reserve_milli))
    reserve = (available * reserve_milli + 999) // 1000
    mobilizable_for_this_mission = max(1, available - reserve)

    # A tiny local band may have no practical reserve. That does not make the
    # attack wise: interception_decision still evaluates the resulting odds and
    # can decline the contact entirely.
    if available <= 4:
        mobilizable_for_this_mission = available

    return max(1, min(available, desired, mobilizable_for_this_mission))
...
__all__ = [
    ...
    "interception_attack_options",
    "interception_motive_options",
    ...
    "route_extortion_settlement_terms",
]
