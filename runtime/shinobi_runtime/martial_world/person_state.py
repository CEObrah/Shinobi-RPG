...
from .character_rules import martial_discipline_keys, professional_skill_keys
...
def _reject_noncanonical_person_fields(person: Mapping[str, Any]) -> None:
    for field in _NONCANONICAL_PERSON_FIELDS:
        if field in person:
            raise ValueError(f"jianghu person contains noncanonical per-person field: {field}")


@lru_cache(maxsize=1)
def _skill_keys() -> tuple[tuple[str, ...], tuple[str, ...]]:
    return martial_discipline_keys(), professional_skill_keys()
...
def _compact_injury_row(value: Any) -> dict[str, Any]:
    """Strip injury defaults while preserving irreversible current anatomy.

    Injury rows are current clinical state, not contact receipts.  False/empty
    permanence and recovery flags are deterministic defaults and therefore do
    not belong in every acute wound.  True permanent outcomes/effects and true
    stabilized/healed facts remain authoritative because downstream function
    and recovery mechanics consume them.
    """
    if not isinstance(value, Mapping):
        return {}
    row = copy.deepcopy(dict(value))
    if not bool(row.get("permanent", False)):
        row.pop("permanent", None)
    if row.get("permanent_outcome") in (None, ""):
        row.pop("permanent_outcome", None)
    permanent = _prune_zero_map(row.get("permanent_effects"))
    if permanent:
        row["permanent_effects"] = permanent
    else:
        row.pop("permanent_effects", None)
    if not bool(row.get("stabilized", False)):
        row.pop("stabilized", None)
    if not bool(row.get("healed", False)):
        row.pop("healed", None)
    return row


def _encode_training_residual_vector(value: Any) -> list[int]:
    """Encode current fractional training carry in canonical domain order.

    Training carry is current progression state, not an extensible storage
    envelope. Unknown domains are invalid because no current training mechanic
    can consume them.
    """
    ...
    for domain, amount in residual.items():
        if domain not in _TRAINING_RESIDUAL_INDEX:
            raise ValueError(f"jianghu training carry has unknown domain: {domain}")
        if amount < 0 or amount >= 1000:
            raise ValueError("jianghu training residual carry must be between 0 and 999")
        if amount:
            highest = max(highest, _TRAINING_RESIDUAL_INDEX[domain])
    vector = [0] * (highest + 1)
    for domain, amount in residual.items():
        if amount:
            vector[_TRAINING_RESIDUAL_INDEX[domain]] = amount
...
def _encode_training_carry_milli(value: Any) -> str:
    """Encode current fractional training carry as one compact readable line."""
    vector = _encode_training_residual_vector(value)
    return ",".join(str(amount) for amount in vector)
...
def hydrate_person_state(
    ...
    *,
    ...
    _reject_noncanonical_person_fields(out)
    ...
    qi_capacity_milli = max(0, int(out.get("qi", 0))) * 1000
    current_qi_milli = out.get("current_qi_milli")
    if current_qi_milli is None:
        current_qi_milli = max(0, int(out.get("current_qi", out.get("qi", 0)))) * 1000
    current_qi_milli = max(0, min(qi_capacity_milli, int(current_qi_milli)))
    out["current_qi_milli"] = current_qi_milli
    out["current_qi"] = current_qi_milli // 1000
    ...
    if isinstance(training, Mapping):
        ...
        if "residual_vector" in state:
            raise ValueError("jianghu training residual_vector is not current state")
    ...
    *,
    ...
    if isinstance(health_raw, Mapping):
        ...
        if isinstance(injuries, list):
            compacted_injuries = [_compact_injury_row(row) for row in injuries if isinstance(row, Mapping)]
            compacted_injuries = [row for row in compacted_injuries if row]
            health["injuries"] = compacted_injuries
    ...
    qi_capacity_milli = qi * 1000
    current_qi_milli = out.get("current_qi_milli")
    if current_qi_milli is None:
        current_qi_milli = max(0, int(out.get("current_qi", qi))) * 1000
    current_qi_milli = max(0, min(qi_capacity_milli, int(current_qi_milli)))
    current_qi = current_qi_milli // 1000
    if current_qi_milli == qi_capacity_milli:
        out.pop("current_qi", None)
        out.pop("current_qi_milli", None)
    else:
        out["current_qi"] = current_qi
        if current_qi_milli == current_qi * 1000:
            out.pop("current_qi_milli", None)
        else:
            out["current_qi_milli"] = current_qi_milli
    ...
    if isinstance(burdens, Mapping):
        compact_burdens = {str(k): max(0, int(v)) for k, v in burdens.items() if max(0, int(v)) > 0}
        if compact_burdens: out["poison_burdens"] = compact_burdens
        else: out.pop("poison_burdens", None)
    else:
        out.pop("poison_burdens", None)
    ...
    if isinstance(pending, Mapping):
        ...
        for key, raw in pending.items():
            ...
            activates_at = raw.get("activates_at")
            if burden <= 0 or not isinstance(activates_at, str) or not activates_at:
                continue
            row = {"burden": burden, "activates_at": activates_at}
            poison_ref = raw.get("poison_ref")
            if isinstance(poison_ref, str) and poison_ref:
                row["poison_ref"] = poison_ref
            peaks_at = raw.get("peaks_at")
            if isinstance(peaks_at, str) and peaks_at:
                row["peaks_at"] = peaks_at
            stage = raw.get("stage")
            if stage in {"onset", "peak"}:
                row["stage"] = str(stage)
            compact_pending[str(key)] = row
        if compact_pending:
            out["pending_poison_burdens"] = compact_pending
        else:
            out.pop("pending_poison_burdens", None)
    else:
        out.pop("pending_poison_burdens", None)
    ...
    if isinstance(training, Mapping):
        ...
        for domain, amount in named_residual.items():
            ...
            if prior is not None and prior != amount:
                raise ValueError("jianghu training residual storage conflicts")
            packed_residual[domain] = amount
        residual_packed = _encode_training_carry_milli(packed_residual)
        ...
        # Current institutional availability is derived from authoritative finite
        # activity owners. Never persist the derived pause projection on a person.
...
def reconcile_faction_population(faction: Mapping[str, Any], roster: Mapping[str, Any]) -> dict[str, Any]:
    ...
    Population is a transient projection, not an independent writable owner.
    Any path that changes life/death or membership can call this helper when it
    needs a hydrated calculation view; canonical faction writes strip the field.
    """
