...
def test_coalition_pressure_only_boosts_an_already_lawful_war_target():
    ...
    base = choose_hostile_action(
        [war_edge], faction_ref="faction.a", year=61, month=10,
    ...
    assert base is not None and coordinated is not None
    assert len(base["options"]) == 1 and len(coordinated["options"]) == 1
    base_option = base["options"][0]
    coordinated_option = coordinated["options"][0]
    assert base_option["action"] == coordinated_option["action"] == "faction_war_strike"
    assert base_option["coalition_pressure"] is False
    assert coordinated_option["coalition_pressure"] is True
    assert coordinated_option["urgency_milli"] > base_option["urgency_milli"]
...
def test_social_command_promise_forgiveness_renunciation_and_heard_claim_use_real_writes(tmp_path):
    ...
    remove_people_from_route_fixture(root, "pc_wei_tang")
    remove_people_from_active_combat_fixture(root, "pc_wei_tang")
    # Put the synthetic social scene back at House Tang after removing the live
    # road/combat owners. Presence-sensitive social rules should be tested with
    # an explicit physical arrangement, not a stale scene projection.
    ...
    scene["present_person_ids"] = ["pc_wei_tang", "char.zhu"]
    scene["visible_person_ids"] = ["pc_wei_tang", "char.zhu"]
