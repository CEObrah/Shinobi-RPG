    ...
    if isinstance(people, list):
        for person in people:
            if isinstance(person, dict) and person.get("person_id") in {"pc_wei_tang", "char.zhu"}:
                ...
                # The committed campaign may be mid-fight or wounded. These
                # wrapper regressions own a synthetic combat fixture, so reset
                # participant physiology only inside the temporary copy.
                person["health"] = {
                    "status": "ready", "injuries": [], "blood_lost_ml": 0,
                    "shock": 0, "consciousness": 100, "toxicity_milli": 0,
                }
...
def _hold_npc_judgments(repo: RepositoryStore, combat_ref: str, player_ref: str) -> dict[str, dict[str, str]]:
    """Author an explicit no-attack choice for every synthetic NPC fighter.

    These wrapper regressions exercise player command plumbing rather than NPC
    tactics.  Live combat deliberately has no deterministic NPC-intent fallback,
    so the fixture must still supply a real GM-authored choice.
    """
    combat = repo.read_json("state/martial-world/combats.json")["combats"][combat_ref]
    return {
        str(ref): {"intent": "hold"}
        for refs in combat.get("sides", {}).values()
        if isinstance(refs, list)
        for ref in refs
        if isinstance(ref, str) and ref != player_ref
    }
...
def test_live_combat_start_and_bare_attack_exchange_preview_and_plan_end_to_end(tmp_path):
    ...
    exchange = CommandEnvelope(
        ...
        payload={
            "action": "exchange", "combat_ref": combat_ref,
            "npc_judgments": _hold_npc_judgments(repo, combat_ref, current["player_id"]),
        },
        mode="gameplay",
    )
...
def test_high_level_attack_scope_returns_at_fresh_npc_judgment_frontier(tmp_path):
    root = _copy_runtime_repository(tmp_path)
    repo = RepositoryStore(root)
    planner = RepositoryCommandPlanner(repo)
    meta = repo.read_json("state/meta.json")
    ...
    attack = CommandEnvelope(
        ...
        payload={
            "action": "exchange", "combat_ref": combat_ref, "exchange_count": 3,
            "npc_judgments": _hold_npc_judgments(repo, combat_ref, current["player_id"]),
        },
        mode="gameplay",
    )
    ...
    if plan.result["combat_status"] == "active":
        assert plan.result["scope_stop_reason"] == "gm_judgment_refresh"
    ...
    assert len([row for row in plan.result["events"] if row.get("actor_ref") == current["player_id"]]) >= 1
    assert plan.result["world_time"] > current["time"]
...
def test_bare_attack_respects_nonlethal_vow_but_explicit_lethal_intent_can_break_it(tmp_path):
    ...
    lethal=CommandEnvelope(
        ...
        payload={
            "action":"exchange", "combat_ref":combat_ref2, "targeting_intent":"lethal",
            "npc_judgments": _hold_npc_judgments(repo2, combat_ref2, current2["player_id"]),
        }, mode="gameplay",
    )
...
def test_duration_scoped_attack_preserves_scope_but_refreshes_npc_judgment(tmp_path):
    ...
    assert int(combat_after["elapsed_ms"])-elapsed_before > 0
    assert plan.result["exchanges_resolved"] == 1
    if plan.result["combat_status"] == "active":
        assert plan.result["scope_stop_reason"] == "gm_judgment_refresh"
        assert plan.result["continuation_required"] is True
...
def test_until_resolution_scope_finishes_or_returns_explicit_continuation_frontier(tmp_path):
    ...
    cmd=CommandEnvelope(
        ...
        payload={
            "action":"exchange","combat_ref":combat_ref,"until_resolution":True,
            "npc_judgments": _hold_npc_judgments(repo, combat_ref, current["player_id"]),
        },mode="gameplay",
    )
    plan=planner.plan(cmd)
...
def test_bounded_combat_does_not_convert_internal_type_errors_into_partial_success(monkeypatch):
    ...
    with pytest.raises(TypeError, match="internal invariant bug"):
        extended._resolve_player_combat_span(
            ...
            raw_target_ref="enemy", raw_action_kind="auto", raw_weapon_ref="auto", hit_zone="auto",
...
def test_scene_prop_promotes_through_real_command_wrapper_without_inventory_minting(tmp_path):
    ...
    exchange = CommandEnvelope(
        ...
        payload={
            "action":"exchange", "combat_ref":combat_ref, "improvised_prop_fact_ref":prop_ref,
            "npc_judgments": _hold_npc_judgments(repo, combat_ref, player_ref),
        },
        mode="gameplay",
    )
    preview = planner.preview(exchange)
