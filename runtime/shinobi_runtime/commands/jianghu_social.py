...
def _physically_colocated(runtime: Any, left_ref: str, right_ref: str) -> bool:
    try:
        _lp, _lr, _lo, left = runtime._person(left_ref)
        _rp, _rr, _ro, right = runtime._person(right_ref)
    except (FileNotFoundError, KeyError, ValueError, CommandRejectedError):
        return False
    return same_effective_location(
        runtime.repository.read_json, left_ref, right_ref, left_person=left, right_person=right,
    )
...
class JianghuSocialCommandsMixin:
    def _jianghu_social_resolution(
        ...
        if action == "promise":
            ...
            if command.mode == "gameplay" and not _physically_colocated(self, command.actor_id, other_ref):
        ...
        if action == "renounce_obligation":
            ...
            counterparty_ref = str(row.get("counterparty_ref") or "")
            if command.mode == "gameplay" and not _physically_colocated(self, command.actor_id, counterparty_ref):
        ...
        if action == "hear_claim":
            ...
            if command.mode == "gameplay" and not _physically_colocated(self, command.actor_id, source_ref):
