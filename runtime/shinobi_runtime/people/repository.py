...
class RepositoryPersonSheetResolver:
    ...
    def __call__(self, person_id: str) -> Optional[Mapping[str, Any]]:
        ...
        if isinstance(cohort_ref, str) and cohort_ref.startswith("pool."):
        ...
        # Rostered people are not frozen when their sparse core is cold.  Their
        # routine mechanical baseline remains cohort-backed, while exact
        # calendar facts such as age are derived from the stable birth anchor
        # at read time.  With no individual divergence component there is no
        # causal work to persist merely to say the person is current.
        try:
