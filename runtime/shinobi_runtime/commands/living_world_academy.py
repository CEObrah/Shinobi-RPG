...
def _configured_refs(spec: Mapping[str, Any], key: str) -> list[str]:
    raw = spec.get(key, [])
...
class LivingWorldAcademyMixin:
    def _academy_dynamic_candidate_refs(
        ...
        *,
        ...
        """
        graduates = _configured_refs(spec, "graduate_candidate_refs")
        instructors = _configured_refs(spec, "instructor_candidate_refs")
        ...
        return (
            list(dict.fromkeys(graduates)),
            list(dict.fromkeys(instructors)),
        )
    ...
    def _apply_academy_team_form(
        ...
        *,
        ...
        if not isinstance(formed, list):
            raise CommandRejectedError("faction_owner_invalid")
        # No fictional team-count ceiling. Available graduates, instructors,
        # and exclusive active-team membership are the lawful constraints.
