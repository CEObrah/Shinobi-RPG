...
from shinobi_runtime.release_baseline import verify_release_baseline_core
...
def _retire_recovery_store(
    ...
    *,
    ...
    """Archive private recovery evidence when a certified baseline changes.

    Completed WAL records and idempotency receipts are scoped to the retired
    campaign lineage. They must never answer requests inside a replacement
    release baseline merely because the persistent volume survived deployment.
    """
    sources = [
        settings.runtime_root / name
        for name in ("wal", "receipts")
        if (settings.runtime_root / name).exists()
    ]
    if not sources:
        return
...
def _release_baseline_id_at_commit(
    settings: CheckoutSettings,
    commit: str,
) -> Optional[str]:
    raw = _commit_text(
    ...
    if any(character in baseline_id for character in ("\x00", "\r", "\n")):
        raise BootstrapError("release baseline id contains invalid control characters")
    identity = _campaign_identity_revision(settings, commit)
    if identity is None or identity[0] != campaign_id:
        raise BootstrapError(
            "release campaign-state contract campaign id disagrees with committed state"
        )
    return baseline_id.strip()
...
def _semantic_campaign_authority_matches(
    ...
    for path in changed_paths:
        # Current mutable campaign truth is JSON under state/.
...
def _campaign_authority_matches(
    ...
    or state path-set difference still fails closed.
    """
...
def ensure_checkout(settings: CheckoutSettings) -> Path:
    ...
    if dirty:
        if local_head != remote_head or not _has_recoverable_wal(settings):
            raise BootstrapError(
                "dirty campaign checkout is not provable WAL recovery state"
            )
        return settings.campaign_root
    if local_head == remote_head:
        return settings.campaign_root
    # A certified release_baseline_id change is an explicit reset boundary.
    # This is the one safe case where a lower remote campaign revision may
    # replace a newer persistent-volume revision: the remote tree must verify
    # exactly against its packaged baseline contract, the campaign identity
    # must stay the same, and no recoverable transaction may be pending.
    local_baseline = _release_baseline_id_at_commit(settings, local_head)
    remote_baseline = _release_baseline_id_at_commit(settings, remote_head)
    if remote_baseline is not None and remote_baseline != local_baseline:
        ...
        if local_campaign is None or remote_campaign is None:
            raise BootstrapError(
                "release baseline reset requires readable local and remote campaign identity"
            )
        if local_campaign[0] != remote_campaign[0]:
            raise BootstrapError(
                "release baseline reset cannot switch campaign identity"
            )
        if _has_recoverable_wal(settings):
            raise BootstrapError(
                "release baseline reset blocked by recoverable transaction WAL"
            )
        ...
        if baseline_errors:
            _run(settings, ("reset", "--hard", local_head), cwd=settings.campaign_root)
            _assert_clean(settings)
            raise BootstrapError(
                "replacement release baseline failed verification: "
        ...
        try:
            ...
            raise
        print(
            "Runtime bootstrap: adopted certified replacement release baseline "
            f"{remote_baseline}",
            file=sys.stderr,
        )
        return settings.campaign_root
    ...
    if _campaign_authority_matches(settings, local_head, remote_head):
        return _adopt_remote_history(
            settings,
            remote_ref,
            "Runtime bootstrap: adopted remote Git history after verifying "
    ...
    if local_campaign is not None and remote_campaign is not None:
        ...
        if remote_revision > local_revision:
            return _adopt_remote_history(
                settings,
                remote_ref,
                "Runtime bootstrap: adopted replaced remote history with newer "
...
def main() -> int:
    ...
    try:
        ...
        raise BootstrapError(f"deployment source freshness check failed: {exc}") from exc
