...
Review every live gameplay turn and append exactly one compact `OOC QA:` footer after the IC response. Use `PASS`, `WARN`, or `FAIL`. `PASS` means no material defect was observed in that turn; `WARN` means the result is mechanically valid but a reusable system, narration, pacing, or UX weakness is visible; `FAIL` means false campaign truth, agency/knowledge breakage, blocked intent, major exploit, misleading consequence, chronology error, or persistence risk.

Report at most the strongest reusable finding: observed symptom, player impact, likely owner, and smallest coherent root fix/regression. Valid owners include GM Skill/presentation, runtime interface, runtime/rules mechanics, game data, projection source, explicit state repair, or feature/design. Do not manufacture or repeat a defect to satisfy the footer; `PASS — no material defect observed this turn` is the correct output when nothing material failed. Ordinary play is observational only; actual source/state mutation requires explicit `OOC DEV:` intent.
