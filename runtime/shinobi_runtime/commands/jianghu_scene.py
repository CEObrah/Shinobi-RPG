...
from shinobi_runtime.martial_world.scene_sessions import (
    ATTEMPT_LEDGER_PATH, CLOSE_REASONS, CONTINUITY_NOTE_KINDS, INTERACTION_ACTIONS, RESPONSE_BEARING_ACTIONS, SCENE_FACT_KINDS, SESSION_PATH, SESSION_KINDS, SPEECH_KINDS, _expects_response,
...
def _nonbinding_proposal_terms(payload: Mapping[str, Any], speech_kind: str) -> dict[str, Any] | None:
    """Validate optional machine-readable terms for what an NPC explicitly offered.

    These terms remain authority-false scene history.  They exist so a later
    domain command can validate exactly what was offered without parsing prose;
    they never move money or otherwise create the consequence themselves.
    """
    raw = payload.get("proposal_terms")
    if raw is None:
        return None
    if speech_kind != "nonbinding_proposal" or not isinstance(raw, Mapping):
        raise CommandRejectedError("jianghu_scene_proposal_terms_invalid")
    if set(raw) - {"cash", "recipient_ref"}:
        raise CommandRejectedError("jianghu_scene_proposal_terms_invalid")
    cash = raw.get("cash")
    if isinstance(cash, bool) or not isinstance(cash, int) or cash <= 0 or cash > 1_000_000_000:
        raise CommandRejectedError("jianghu_scene_proposal_terms_invalid")
    terms: dict[str, Any] = {"cash": int(cash)}
    recipient_ref = raw.get("recipient_ref")
    if recipient_ref is not None:
        try:
            terms["recipient_ref"] = safe_ref(recipient_ref, "jianghu_scene_proposal_terms_invalid")
        except ValueError as exc:
            raise CommandRejectedError("jianghu_scene_proposal_terms_invalid") from exc
    return terms
...
class JianghuSceneCommandsMixin:
    ...
    def _jianghu_interaction_resolution(self, command: CommandEnvelope, meta: Mapping[str, Any], current_time: CampaignTime):
        ...
        if action in {"propose", "counteroffer"}:
            action = "offer"
    ...
    def _record_combat_side_speech(self, command: CommandEnvelope, meta: Mapping[str, Any], current_time: CampaignTime, combat_ref: str):
        ...
        if command.payload.get("proposal_terms") is not None:
            # The opaque opposing side is not an exact personal-property owner.
            # Extortion/ransom settlement terms belong to their hard domain.
            raise CommandRejectedError("jianghu_combat_parley_proposal_terms_unsupported")
        try:
    ...
    def _record_combat_ally_speech(self, command: CommandEnvelope, meta: Mapping[str, Any], current_time: CampaignTime, combat_ref: str, combat: Mapping[str, Any]):
        """Persist one reversible line from an exact, arrived ally in active combat.

        The active combat owner is the continuity session, not a mechanical
        speech-act authority. Exact opposing person IDs remain forbidden here;
        hostile speech continues to use the opaque combat ref.
        """
        session_ref = str(command.payload.get("session_ref") or "")
        speaker_ref = str(command.payload.get("speaker_ref") or "")
        if session_ref != combat_ref or not speaker_ref or speaker_ref == combat_ref:
            raise CommandRejectedError("jianghu_combat_scene_speaker_invalid")
        sides = combat.get("sides", {}) if isinstance(combat, Mapping) else {}
        combatants = combat.get("combatants", {}) if isinstance(combat, Mapping) else {}
        ...
        for side_ref, members in sides.items():
            if not isinstance(members, list):
                continue
            if command.actor_id in members:
                actor_side = str(side_ref)
            if speaker_ref in members:
                speaker_side = str(side_ref)
        if actor_side is None or speaker_side != actor_side:
            raise CommandRejectedError("jianghu_combat_scene_speaker_not_allied")
        state = combatants.get(speaker_ref, {})
        ...
        if statuses & {"escaped", "reinforcing"}:
            raise CommandRejectedError("jianghu_combat_scene_speaker_not_present")
        self._require_established_scene_person(speaker_ref, command.actor_id)
        self._colocated_person(command.actor_id, speaker_ref)
        speech_kind = str(command.payload.get("speech_kind") or "")
        if speech_kind not in SPEECH_KINDS:
            raise CommandRejectedError("jianghu_scene_speech_kind_invalid")
        proposal_terms = _nonbinding_proposal_terms(command.payload, speech_kind)
        try:
            statement = bounded_text(command.payload.get("statement"), "jianghu_scene_statement_invalid", 2500)
        except ValueError as exc:
            raise CommandRejectedError(str(exc)) from exc
        basis_refs = _safe_list(command.payload.get("basis_refs", []), "jianghu_scene_basis_invalid", maximum_transport=32)
        allowed_basis = {combat_ref, command.actor_id, speaker_ref}
        for ref in basis_refs:
            if ref in allowed_basis:
                continue
            if not isinstance(scene_history_record(self.repository.read_json, ref, session_ref=combat_ref), Mapping):
                raise CommandRejectedError("jianghu_scene_basis_not_session_visible")
        if command.payload.get("resolves_thread_ref") is not None or command.payload.get("resolves_question_ref") is not None:
            # Allied active-combat thread resolution is intentionally not inferred
            # from presentation sessions. The spoken line can be persisted, but a
            # hard or threaded response needs the normal explicit interaction path.
            raise CommandRejectedError("jianghu_combat_scene_ally_thread_unsupported")
        speech_ref = f"scene_speech_{_digest(command.digest, speaker_ref, str(current_time), str(statement))}"
        speech = {
            "speech_ref": speech_ref, "at": str(current_time), "session_ref": combat_ref,
            "speaker_ref": speaker_ref, "speech_kind": speech_kind, "statement": statement,
            "basis_refs": basis_refs, "resolves_thread_ref": None, "resolves_question_ref": None,
            "truth_status": "attributed_statement", "authority": False,
            "mechanical_consequence_authority": False,
        }
        if proposal_terms is not None:
            speech["proposal_terms"] = proposal_terms
        writes = dict(append_attributed_speech(self.repository.read_json, row=speech))
        return self._simple_plan(
            command, meta, current_time, writes_records=writes,
            code="jianghu_combat_ally_speech_recorded",
            result={
                "command_type": command.command_type, "session_ref": combat_ref,
                "speaker_ref": speaker_ref, "speaker_kind": "allied_combatant",
                "speech_ref": speech_ref, "truth_status": "attributed_statement",
                "mechanical_consequence_authority": False,
            },
        )
    ...
    def _jianghu_scene_session_resolution(self, command: CommandEnvelope, meta: Mapping[str, Any], current_time: CampaignTime):
        ...
        if action == "record_speech" and active_combat is not None:
            ...
            if str(command.payload.get("session_ref") or "") == combat_ref:
                speaker_ref = str(command.payload.get("speaker_ref") or "")
                if speaker_ref == combat_ref:
                    return self._record_combat_side_speech(command, meta, current_time, combat_ref)
                return self._record_combat_ally_speech(command, meta, current_time, combat_ref, active_combat[1])
        ...
        if action == "record_continuity":
            continuity_kind=str(command.payload.get("continuity_kind") or "")
            if continuity_kind not in CONTINUITY_NOTE_KINDS:
                raise CommandRejectedError("jianghu_scene_continuity_kind_invalid")
            try:
                description=bounded_text(command.payload.get("description"),"jianghu_scene_continuity_description_invalid",1500)
            except ValueError as exc:
                raise CommandRejectedError(str(exc)) from exc
            participants=set(str(x) for x in current.get("participant_refs",[]) if isinstance(x,str))
            subject_refs=_safe_list(command.payload.get("subject_refs",[]),"jianghu_scene_continuity_subjects_invalid",maximum_transport=16)
            if any(ref not in participants for ref in subject_refs):
                raise CommandRejectedError("jianghu_scene_continuity_subject_not_present")
            for ref in subject_refs:
                self._colocated_person(command.actor_id,ref)
            basis_refs=_safe_list(command.payload.get("basis_refs",[]),"jianghu_scene_continuity_basis_invalid",maximum_transport=16)
            if not basis_refs:
                raise CommandRejectedError("jianghu_scene_continuity_basis_required")
            primary_basis_present=False
            for ref in basis_refs:
                row=scene_history_record(self.repository.read_json,ref,session_ref=session_ref)
                if not isinstance(row,Mapping):
                    raise CommandRejectedError("jianghu_scene_continuity_basis_not_scene_history")
                if row.get("mechanical_consequence_authority") is not False:
                    raise CommandRejectedError("jianghu_scene_continuity_basis_not_presentation_only")
                if isinstance(row.get("speech_ref"),str) or isinstance(row.get("fact_ref"),str):
                    primary_basis_present=True
            if not primary_basis_present:
                raise CommandRejectedError("jianghu_scene_continuity_requires_primary_scene_evidence")
            continuity_ref=f"scene_continuity_{_digest(command.digest,continuity_kind,str(current_time),str(description))}"
            note={
                "continuity_ref":continuity_ref,"at":str(current_time),"session_ref":session_ref,
                "continuity_kind":continuity_kind,"summary":description,"subject_refs":subject_refs,
                "basis_refs":basis_refs,"location_ref":str(current.get("location_ref") or ""),
                "truth_status":"derived_narrative_continuity","scope":"scene_history_only",
                "derivation_rule":"interpretive_summary_of_cited_authority_false_scene_history_not_objective_world_truth",
                "authority":False,"mechanical_consequence_authority":False,
            }
            writes=dict(append_scene_history_record(self.repository.read_json,row=note))
            session_after=copy.deepcopy(dict(current)); session_after["last_updated_at"]=str(current_time); writes[SESSION_PATH]=session_after
            return self._simple_plan(
                command,meta,current_time,writes_records=writes,code="jianghu_scene_continuity_recorded",
                result={"command_type":command.command_type,"session_ref":session_ref,"continuity_ref":continuity_ref,"truth_status":"derived_narrative_continuity","mechanical_consequence_authority":False},
            )
        ...
        if proposal_terms is not None: speech["proposal_terms"]=proposal_terms
        writes=dict(append_attributed_speech(self.repository.read_json,row=speech))
