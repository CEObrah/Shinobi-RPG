...
def friendly_aid_transfer_envelope(
    ...
    source_monthly = monthly_cash_need(source, source_inventory)
    target_monthly = monthly_cash_need(target, target_inventory)
    source_cash = max(0, int(source.get("treasury_cash", 0)))
    target_cash = max(0, int(target.get("treasury_cash", 0)))
    ...
    maximum = max(0, int(min(need, max(100, spendable // 4), max(250, source_monthly * 2))))
    if maximum <= 0:
        return {"result": "aid_not_affordable"}
    minimum = min(maximum, max(1, min(100, maximum)))
    return {
        "result": "aid_available", "minimum_cash": minimum, "maximum_cash": maximum,
        "source_monthly_cash": source_monthly, "target_monthly_cash": target_monthly,
        "spendable_cash": spendable, "target_need_cash": need,
    }


def resolve_friendly_aid_transfer(
    source: Mapping[str, Any], source_inventory: Mapping[str, Any],
    target: Mapping[str, Any], target_inventory: Mapping[str, Any], *, amount_cash: int,
) -> dict[str, Any]:
    """Apply one exact GM-authored aid amount after current-bound revalidation."""
    envelope = friendly_aid_transfer_envelope(source, source_inventory, target, target_inventory)
    if envelope.get("result") != "aid_available":
        return envelope
    amount = max(0, int(amount_cash))
    minimum = max(1, int(envelope.get("minimum_cash", 1)))
    maximum = max(0, int(envelope.get("maximum_cash", 0)))
    if amount < minimum or amount > maximum:
        return {"result": "aid_amount_outside_current_envelope", "minimum_cash": minimum, "maximum_cash": maximum}
