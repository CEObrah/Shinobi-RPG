...
def make_service(tmp_path: Path):
    ...
    stage_house_change(
        RepositoryStore(campaign), route_records, house_ref="house.test",
        after_members=("pc_wei_tang", "person.test.0"),
    )
    for relative, record in route_records.items():
...
def test_player_house_person_access_handles_hundreds_without_owner_scan(
    ...
    route_records: dict[str, dict] = {}
    stage_house_change(
        RepositoryStore(tmp_path), route_records, house_ref="house.tang",
        after_members=tuple(member_ids),
    )
    for relative, record in route_records.items():
