...
Shinobi is self-contained. Never load or import Sword & Banners runtime, data, state, rules, or Skill resources.
...
- `state/reg/information/knowledge/`: deterministic holder-hash directories containing a bounded holder index plus exact claim-membership hash buckets.
...
- `state/time/causal-scheduler.json`: bounded causal scheduling authority.
...
Use persistent identity-core/cohort representation when the person should continue to exist by name but does not need full exact-person divergence every turn. Resolve via the owner index and person-sheet repository code under:
...
5. reconcile any player-team routing projection after membership changes;
...
`state/time/causal-scheduler.json` is scheduling authority.

Do not globally tick every person, team, faction, or place. Exact identity does not imply an exact periodic host.

Routine offscreen progression belongs to bounded cohort/organization/team/faction/world hosts. Wake an exact person only when individual causality requires it, such as:
...
For exact combat, route through combat mechanics and exact participants. For aggregate war, load the participating formations/sectors, terrain, logistics, intelligence, command, reserves, and named specialists only when individual causality matters.
...
Production uses one source/campaign branch and a Railway persistent checkout. Runtime/game/dependency/deployment/unknown-path changes require Railway deployment synchronization. Runtime-generated `state/**` gameplay commits do not trigger deployment loops. Skill/docs/tests/tools/workflow/README-only commits are runtime-neutral: Railway does not restart for them, and the locked Git remote preflight may fast-forward them only when the remote is a strict descendant and every changed path is on the neutral allowlist. Remote-ahead state is never auto-adopted.
...
Then route only through the deployment authorities that actually matter: `railway.toml`, `railpack.json`, `.python-version`, `pyproject.toml`, `runtime/shinobi_runtime/bootstrap.py`, `runtime/shinobi_runtime/tx/remote.py`, `runtime/shinobi_runtime/api/`, and `.github/workflows/audit.yml`. Relevant runtime-affecting pushes to `main` and pull requests run the cheap smoke gate; full release verification remains deliberate/manual.
