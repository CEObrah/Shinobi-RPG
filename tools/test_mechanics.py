...
el=json.loads((R/'game/data/mechanics/elements.json').read_text());x=by['elemental_advantage_modifier']['inputs'];val=el['opposed_resolution']['advantage_margin_modifier'] if el['primary_cycle'].get(x['attacker'])==x['defender'] else 0;chk('elemental_advantage_modifier',val)
...
gc=json.loads((R/'game/data/mechanics/guardian-current.json').read_text())['ceiling'];chk('guardian_current_fixed_ceiling',{k:gc[k] for k in ('instant_impulse_score','pressure_score','sustained_force_score')})
...
_em=json.loads((R/'game/data/mechanics/technique-effects-manifest.json').read_text())['effect_profiles']; te={}
for _tid,_rel in _em.items(): te[_tid]=json.loads((R/_rel).read_text())['effect_profile']
er=json.loads((R/'game/data/mechanics/effect-resolvers.json').read_text())['resolvers']
_tm=json.loads((R/'game/data/tech/manifest.json').read_text())['techniques']; tech={}
...
ui=json.loads((R/'game/data/mechanics/unique-items.json').read_text())
