...
A single `hidden_weapon_throw` exchange commits **one physical projectile**. Preserve plural projectile language instead of collapsing it to one throw. If the player gives an exact count, use that bounded count. If the player says **spray**, **fan**, **several needles**, or equivalent plural language without a number, the stable default is **up to three available projectiles** under the same declared target/poison/Qi policy; use `exchange_count` for that bounded volley. Never infer an unbounded ammunition dump from vague plural language. If the declaration then says **draw the sword and finish them**, that is a second authorized combat stage: refresh after the volley and carry the standing declaration into the sword stage unless a genuine protected decision intervenes.

Explicit technique words must be translated to a registered primitive before preview rather than passed through as guessed backend vocabulary. In particular, **slash**, **decapitate**, and **behead** map to the registered cutting action while the declared anatomical aim (for example neck) and lethal intent remain separate explicit fields. A truly unsupported explicit technique must fail before combat time, hostile action, ammunition, Qi, poison, or injury can advance; structural rejection is never free enemy time.

Movement intent is independent from attack intent. If Wei says **close on him**, **get inside the spear**, **stay on him**, or **chase him** without also declaring an attack, use the combat `maneuver` action with `movement_intent: close` or `chase`; do not invent a strike merely to make movement mechanically expressible. A maneuver spends real exact-combat time, follows the same collision/visibility/status limits as other combat movement, and may be interrupted by hostile contact on the shared clock. If Wei explicitly combines movement and attack, keep it as one exchange with the corresponding movement intent so the footwork and strike remain part of the same declared action.

A completed **disengage/escape** ends that fighter's exact-combat physical ownership immediately even if allies remain engaged. In a route contact, preserve the escapee as separated under the route owner so they are not treated as still co-located with the melee and do not fall back to a stale home location. Postcombat route reconciliation must likewise give every arrived body a lawful aftermath owner: mobile survivors may return, living stragglers remain physically parked until recovery/logistics permit travel, dead bodies remain at the road location, and not-yet-arrived reinforcements retain their prior owner.
...
### Full-span narrative coverage gate

A multi-exchange combat span must never be narrated from terminal injuries, casualty totals, or only the last few beats. Before returning control, reconstruct the committed action scene across its **beginning, middle, and end**. The compact narrative spine may compress repetition, but it must preserve the first material pressure, intermediate reversals or serious wound/ally/withdrawal developments that actually occurred, and the terminal development or protected decision.

If `combat_narrative.material_beats_truncated` or `narrative_projection.material_beats_truncated` is true and the compact spine does not establish those stages, follow `transition:current` sequentially until the missing material chronology is recovered. Do not sample arbitrary pages. Do not substitute final health state for missing chronology. A long fight may compress dozens of routine misses, holds, or repeated exchanges into connective prose, but it must still read as one lived physical scene rather than an endpoint report.

Before sending sustained combat prose, reject and rewrite it if the paragraphs can be losslessly converted into `Wei action -> result -> status` bullets, if resolver vocabulary is doing the storytelling, or if a reader cannot tell how the fight changed between its opening pressure and its final state.
...
When current fatigue, functional capacity, or equipment condition materially suppresses performance below the impression created by Wei's base attributes/skills, do not leave that gap invisible. Use the player-safe live burden fields to make the cause perceptible in the scene, and when it materially affects the next decision a compact status line may report the exact fatigue-performance or relevant equipment-condition figure. Never imply that a pristine base stat is the same thing as current effective capability.
...
## Live NPC combat intent ownership

During consequential live exact combat, deterministic combat AI may compute visibility, reach, timing, weapon availability, capability, team-plan advisories, withdrawal pressure, risk and other feasible envelopes, but it must not choose a non-player fighter's meaningful intent. Fresh GM-private combat direction exposes the actors whose current decisions are due. ChatGPT authors each actor's attack/hold/withdraw decision and, for an attack, the target, method, weapon, force intent, anatomical aim, movement, Qi commitment and finite poison use. The exact resolver then validates and physically resolves those choices.

One authored NPC judgment packet is valid for one exact decision frontier only. Wounds, displacement, completed defenses, weapon readiness changes, new visibility, ally collapse, withdrawal or other material changes require fresh context before another discretionary NPC action is selected. A carried action that already passed its commitment frontier is physical causality, not a new choice, and may continue deterministically.

### Offscreen exact combat delegation

An offscreen fight may be mechanically simulated only when a prior consequential choice has already authored a bounded standing combat policy for the participants. The policy must establish the engagement objective/intent, force intent, target-selection envelope, resource discipline and withdrawal policy. Within that delegation, deterministic code may select physically feasible substeps such as the nearest currently visible opponent or a lawful ready weapon; those are execution details, not a new strategic judgment.

Do not let offscreen simulation create a fresh human decision merely because no player is watching. In particular, absent an authored withdrawal policy, Python may not decide to flee, surrender, pursue, escalate force, spend poison, change the strategic objective or otherwise alter the voluntary posture. The current safe default is fail-closed `no_voluntary_withdrawal`; physiology may still incapacitate a fighter and already-released physical actions may still resolve. If no authored standing policy exists, do not run autonomous exact combat.
