...
def _staged_new_roster_overlay(path: str, roster: dict):
    ...
    payload = (json.dumps(roster, sort_keys=True, separators=(',', ':')) + '\n').encode('utf-8')
    manifest = TransactionManifest(
        transaction_id='tx.test.staged-roster', campaign_id=meta['campaign_id'],
        request_id='test.staged-roster', command_digest='0' * 64,
        mode='maintenance', base_revision=meta['revision'], target_revision=meta['revision'],
        created_at='2026-08-29T00:00:00Z',
        mutations=(FileMutation(path, repository.digest(path), payload),),
    )
    return StagedOverlay(repository, manifest)


def test_person_route_sees_new_roster_created_only_in_staged_transaction_overlay():
    from shinobi_runtime.martial_world.live_state import person_route

    person_ref = 'person.test.staged.new-faction'
    overlay = _staged_new_roster_overlay(
        'state/martial-world/people/faction.test_staged.json',
        {'faction_ref': 'faction.test_staged', 'people': [{'person_id': person_ref}]},
    )
    assert person_route(overlay, person_ref) == ('faction.test_staged', 0)


def test_person_route_rejects_duplicate_identity_added_only_in_staged_roster():
    import pytest
    from shinobi_runtime.martial_world.live_state import person_route

    overlay = _staged_new_roster_overlay(
        'state/martial-world/people/faction.test_duplicate.json',
        {'faction_ref': 'faction.test_duplicate', 'people': [{'person_id': 'char.zhu'}]},
    )
    with pytest.raises(ValueError, match='duplicate jianghu person identity'):
        person_route(overlay, 'char.zhu')
