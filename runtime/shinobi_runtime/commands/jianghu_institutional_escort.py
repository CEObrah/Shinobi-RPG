...
class JianghuInstitutionalEscortCommandsMixin:
    ...
    def _institutional_escort_operation_specialization(
        ...
        for ref in member_refs:
            ...
            presence = effective_person_presence(self.repository.read_json, ref, person=person)
            effective_ref = str(presence.get("location_ref") or "")
            sites = sites_doc.get("sites", {}) if isinstance(sites_doc, Mapping) else {}
            # Sparse legacy fixtures may carry only a place_ref. Treat that as
            # their exact location only when no more specific site owner exists.
            exact_ref = effective_ref or person_place(
                person,
                local_sites=sites_doc,
                home_place=str(faction.get("headquarters") or ""),
                home_site_ref=str(faction.get("local_site_ref") or ""),
            )
            member_locations.add(str(exact_ref or ""))
            effective_site = sites.get(effective_ref) if isinstance(sites, Mapping) else None
            if isinstance(effective_site, Mapping):
                member_places.add(str(effective_site.get("parent_place_ref") or effective_ref))
            else:
                member_places.add(str(exact_ref or ""))
        if len(member_locations) != 1 or len(member_places) != 1:
        ...
        if player_ref and player_ref in member_refs:
            try:
                scene_transition = close_active_session_writes(
                    self.repository.read_json, at=str(current_time), reason="player_left"
                )
            except ValueError as exc:
                raise CommandRejectedError("jianghu_scene_transition_invalid") from exc
            writes.update({str(path): copy.deepcopy(dict(record)) for path, record in scene_transition.items()})
            try:
