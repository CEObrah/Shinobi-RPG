...
When fresh context exposes an active scene session, treat it as reversible conversational continuity. `jianghu_interaction_action` records Wei's side of an exchange. `jianghu_scene_session_resolution` may open/close an explicit scene or persist an important NPC attributed statement that is already safe to realize from player-visible facts. Persisted speech has no mechanical-consequence authority and may resolve only an open question in that active session.

Do not persist every sentence. Ordinary connective dialogue remains narration. Use the relevant mechanical command when speech itself would create a binding order, acceptance/refusal, secret disclosure, transfer, movement, relationship change, oath, mission disposition, or another hard consequence.
