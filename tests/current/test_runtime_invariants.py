...
def _combat_ready_person(person: dict) -> dict:
    """Detach synthetic combat invariants from the currently committed save."""
...
def test_combat_weapon_readiness_is_explicit_and_switching_consumes_time(monkeypatch):
    ...
    attacker = _combat_ready_person(next(person for person in roster if person["person_id"] == "pc_wei_tang"))
    defender = _combat_ready_person(next(person for person in roster if person["person_id"] == "char.kai"))
