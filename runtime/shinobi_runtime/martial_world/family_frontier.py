    ...
    *, read_json: Callable[[str], Any], writes: dict[str, Any],
    ...
    for event in sorted(due, key=lambda row: (str(row.get("owner_ref") or ""), str(row.get("event_id") or ""))):
        ...
        child = newborn_person(
            ...
            residence_ref=str(residence) if isinstance(residence, str) else str(mother.get("location_ref") or father.get("location_ref") or "") or None,
        )
        ...
        reviews.append({
            ...
            "result": "birth", "faction_ref": destination_fid, "handoff": handoff,
        })
