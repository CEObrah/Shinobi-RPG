...
def test_final_travel_plan_consumes_environment_without_changing_command_contract() -> None:
    ...
    generic, _ = _validated_plan(RepositoryCommandPlanner(REPOSITORY), envelope)
    ...
    assert final.result["travel_seconds"] >= generic.result["travel_seconds"]
    assert final.result["destination_id"] == generic.result["destination_id"] == "place.waves.town"
