...
def test_ranking_review_reads_canonical_publication_snapshot_not_live_fame():
    from shinobi_runtime.api.contracts import CommandRejectedError
    from shinobi_runtime.commands.jianghu_calendar import _published_ranking_review

    occurrence = {"event_id": "jianghu_ranking_publication", "date": "0061-12-15"}
    reputation = {
        "schema": "jianghu-reputation-state-1.0",
        # Live fame now favors p.new, but the publication already printed p.old.
        "audiences": {"p.new": {"public_score": 9999}, "p.old": {"public_score": 1}},
        "rankings": {
            "public": {
                "published_at": "0061-12-15T09:00:00",
                "rows": [{"person_id": "p.old", "public_score": 100, "rank": 1}],
            }
        },
    }
    rows = _published_ranking_review(reputation, occurrence=occurrence, limit=20)
    assert rows == [{"person_id": "p.old", "public_score": 100, "rank": 1}]

    stale = copy.deepcopy(reputation)
    stale["rankings"]["public"]["published_at"] = "0060-12-15T09:00:00"
    try:
        _published_ranking_review(stale, occurrence=occurrence, limit=20)
    except CommandRejectedError as exc:
        assert exc.code == "jianghu_calendar_ranking_publication_unavailable"
    else:
        raise AssertionError("review accepted a stale prior-year publication")
