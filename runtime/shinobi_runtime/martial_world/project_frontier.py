...
def _restaff_project(
    ...
    for refs_key, count_key, score_keys in role_spec:
        ...
        for ref in current:
            ...
            if (
                ref in assigned or ref in blocked or not isinstance(person, Mapping)
        ...
        if missing:
            candidates = [
                ...
                if ref not in assigned and ref not in blocked and _worker_ready(p)
...
def settle_project_frontier(
    *, events: Sequence[Mapping[str, Any]], at: datetime, projects_state: dict[str, Any],
    ...
    for event in events:
        ...
        faction, roster, _training = settle_and_reset_faction_training_cycle(faction, roster, at_iso=at_iso)
        ...
        project, commitments_state, replacements = _restaff_project(
            ...
            location_ref=site_ref, faction=faction,
        )
        if replacements:
