...
class OperationalBattlefieldCommandsMixin:
    ...
    def _battlefield_resolution(
        ...
        if action == "open":
            ...
            by_side: Dict[str, list[tuple[str, str, Mapping[str, Any]]]] = {str(side_refs[0]): [], str(side_refs[1]): []}
            for formation_ref in sorted(str(ref) for ref in front.get("formation_refs", []) if isinstance(ref, str)):
                ...
                if side_ref in by_side:
                    by_side[side_ref].append((formation_ref, force_ref, formation))

            def deployment_readiness(row: tuple[str, str, Mapping[str, Any]]) -> tuple[int, str]:
                formation_ref, _force_ref, formation = row
                readiness = int(formation.get("readiness", 50) or 50)
                cohesion = int(formation.get("cohesion", 50) or 50)
                morale = int(formation.get("morale", 50) or 50)
                memory = formation.get("operational_memory") if isinstance(formation.get("operational_memory"), Mapping) else {}
                recent_loss = int(memory.get("last_loss_milli", 0) or 0)
                inherited_experience = int(memory.get("inherited_experience_milli", 0) or 0)
                score = readiness + cohesion + morale + inherited_experience // 100 - recent_loss // 40
                return score, formation_ref

            for side_ref, rows in sorted(by_side.items()):
                # Healthy, experienced formations receive the first frontline slots;
                # when a reserve is available the most battered eligible formation
                # is held back instead of choosing by lexical ID order.
                rows = sorted(rows, key=deployment_readiness, reverse=True)
                for index, (formation_ref, force_ref, _formation) in enumerate(rows):
