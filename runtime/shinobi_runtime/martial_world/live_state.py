...
def player_view_from_person(person: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        # Base attributes are not the whole current combat condition. Expose the
        # physiological burden that actually scales live output so an elite
        # character at 64% performance does not look mysteriously "nerfed".
        "fatigue_milli": max(0, int(person.get("fatigue_milli", 0))),
        "fatigue_performance_milli": fatigue_performance_milli(int(person.get("fatigue_milli", 0))),
        "functional_capacity": functional_capacity_factors(
            (person.get("health", {}) or {}).get("injuries", [])
            if isinstance(person.get("health"), Mapping) else []
        ),
