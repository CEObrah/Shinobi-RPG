...
class SocialCommandsMixin:
    ...
    def _technique_learning_resolution(
        ...
        research_project_ref = None if research_raw is None else _stable_id(research_raw, "technique_research_project_invalid", prefix="research.")
        summary = command.payload["summary"]
        visibility = command.payload["visibility"]
        if not isinstance(summary, str) or not summary or len(summary) > 1000:
        ...
        if action == "begin":
            ...
            if research_project_ref is not None:
                try:
                    research = self.repository.read_json("state/reg/research.json")
                except (FileNotFoundError, ValueError) as exc:
                    raise CommandRejectedError("technique_research_project_invalid") from exc
                ...
                if (
                    ...
                    or project.get("candidate_kind") != "technique"
                    or project.get("candidate_ref") != technique_ref
                    or project.get("lead_ref") != student_ref
                ):
                    raise CommandRejectedError("technique_research_access_denied")
                research_access = True
            if (teacher_ref is None or teacher_ref == student_ref) and not research_access:
            ...
            outcome_result={"starting_mastery":int(mastery[technique_ref]),"ending_mastery":int(mastery[technique_ref]),"field_usable":False,"access_source_ref":research_project_ref or teacher_ref}
    ...
    def _information_claim_resolution(
        ...
        try:
            if information.claim(claim_id) is not None:
        ...
        for ref in evidence:
            if ref.startswith("claim."):
                try:
                    known_claim = information.claim(ref)
                    holder_knows = information.holder_knows(holder_ref, ref)
                except ValueError as exc:
                    raise CommandRejectedError("information_registry_invalid") from exc
                if known_claim is None or not holder_knows:
        ...
        try:
            ...
            information.grant(holder_ref, claim_id)
        except ValueError as exc:
            raise CommandRejectedError("information_registry_invalid") from exc
        event_id = self._append_semantic_event(
            world_events,
        ...
        def validate(overlay: StagedOverlay, manifest: TransactionManifest) -> None:
            ...
            self._assert_meta(overlay, manifest, meta_path=self.meta_path, command=command, world_time=current_time)
            shard = overlay.read_json(InformationStore.knowledge_shard_path(holder_ref, claim_id))
            if claim_id not in shard.get("claim_refs", []):
                raise ValueError("claim holder knowledge was not persisted")
            claim_shard = overlay.read_json(InformationStore.claim_shard_path(claim_id))
            if claim_id not in claim_shard.get("claims", {}):
                raise ValueError("information claim was not persisted")
    ...
    def _information_delivery(
        ...
        def validate(overlay: StagedOverlay, manifest: TransactionManifest) -> None:
            ...
            knowledge_shard = overlay.read_json(InformationStore.knowledge_shard_path(recipient_ref, claim_id))
            if claim_id not in knowledge_shard.get("claim_refs", []):
                raise ValueError("recipient knowledge was not persisted")
            if "fact_ref" in persisted:
