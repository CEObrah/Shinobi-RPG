...
from .physical_presence import effective_person_presence
...
def _player_message_target(view: _View, player_ref: str, player: Mapping[str, Any], at: datetime) -> tuple[str, datetime]:
    ...
    A person actively traveling is not magically present at the route object. A
    messenger is sent toward that movement's declared destination and cannot be
    considered delivered before the traveler can physically reach it.
    """
    presence = effective_person_presence(view.read_json, player_ref, person=player)
    if str(presence.get("presence_kind") or "") == "route":
        movement_ref = str(presence.get("owner_ref") or "")
        try:
            route_state = view.read_json(_ROUTE_OPERATIONS)
        except FileNotFoundError:
            route_state = {}
        ...
        if isinstance(movement, Mapping):
            target = str(movement.get("destination_place_ref") or "")
            required = max(0, int(movement.get("required_seconds", 0) or 0))
            elapsed = max(0, int(movement.get("elapsed_seconds", 0) or 0))
...
def stage_house_assignment_offers(
    *, read_json: Callable[[str], Any], writes: dict[str, Any], handoffs: list[dict[str, Any]],
    ...
    # Every lawful offer source below is driven by either a faction review or a
    # detected allied-defense call. Other frontiers cannot create an offer, so
    # avoid loading the player/faction/operation owners on those no-op ticks.
    if not any(
    ...
    for review in reviews:
        ...
        for action in actions:
            ...
            ref = _stage_offer(
                read_json=read_json, writes=writes, handoffs=handoffs, pending_one_off_events=pending_one_off_events, player_ref=player_ref,
...
def _stage_player_grade_offer(
    *, view: _View, state: dict[str, Any], row: dict[str, Any], player_ref: str,
    ...
    delivered = _queue_career_offer_delivery(
        view=view, row=row, offer=offer, player_ref=player_ref, faction_ref=faction_ref,
        at=at, pending_one_off_events=pending_one_off_events,
    )
    if delivered:
        notice = {
            "kind": "house_career_offer",
