...
from .family_simulation import (
    advance_npc_relationships, apply_npc_relationship_judgment, apply_recognized_succession, review_conceptions,
)
...
def settle_faction_cycle_frontier(
    *,
    ...
    # Apply exact ChatGPT-authored family judgments at the same timestamp that
    # made them due. Eligibility and co-location are revalidated here; no other
    # relationship choice is substituted if the selected intent is stale.
    for judgment_event in events:
        ...
        if judgment_kind not in {"family_courtship", "family_marriage", "faction_office_appointment", "faction_membership_departure"}:
            continue
        ...
        if judgment_kind == "faction_membership_departure":
            ...
            if intent == "stay":
                applied = {"result":"membership_retained"}
            else:
                ...
                for row in rows:
                    if not isinstance(row, Mapping) or str(row.get("person_id") or "") != person_ref:
                        people_after.append(row); continue
                    person=compact_person_state(row,faction_ref=fid); person.pop("membership_grade",None); person["standing_offices"]=[]
                ...
                roster["people"]=people_after
                equipment_source=writes.get(_EQUIPMENT_PATH,read_json(_EQUIPMENT_PATH))
                equipment_transition=detach_faction_policy_holders(equipment_source,source_faction_ref=fid,holder_refs=[person_ref])
                writes[_EQUIPMENT_PATH]=equipment_transition["equipment_ledger_after"]
                faction=reconcile_faction_population(faction,roster)
                compact=compact_roster_state(roster,faction=faction)
                writes[fpath]=faction; writes[rpath]=compact; writes[_INDEPENDENTS_PATH]=independents
                faction_cache[fid]=(fpath,faction); roster_cache[fid]=(rpath,hydrate_roster_state(compact,faction=faction))
                applied={"result":"membership_departed","person_ref":person_ref}
        elif judgment_kind == "faction_office_appointment":
            intent = str(choice.get("intent") or "")
            if intent not in {"appoint", "leave_vacant"}:
            ...
            if office == "leader" and applied.get("appointment") and fid == "house_tang":
                notice = {"kind":"succession_notice","faction_ref":fid,"successor_ref":applied["appointment"]["person_ref"],"delivered_to_player":True}
                handoffs.append({**notice,"handoff":classify_handoff(notice)})
        else:
            people = [p for p in roster.get("people", []) if isinstance(p, Mapping)]
            applied = apply_npc_relationship_judgment(
                family, social, faction_ref=fid, roster_people=people, at_iso=at_iso,
                judgment_kind=judgment_kind, option=choice, player_ref=player_ref or None,
            ...
            family = applied["family_after"]; social = applied["social_after"]
            writes[_FAMILY_PATH] = family; writes[_SOCIAL_PATH] = social
        reviews.append({
            "kind":"npc_judgment_applied","judgment_kind":judgment_kind,"owner_ref":fid,
            "decision_ref":str(judgment_event.get("decision_ref") or ""),
            "decision_origin":"gm_npc_judgment","result":str(applied.get("result") or "applied"),
        })
    ...
    for event in events:
        ...
        if dead_refs:
            ...
            for judgment in institutional_office_judgment_events(
                faction_ref=fid, at_iso=at_iso, office_result=office_result, succession=succession,
                source_event_id=str(event.get("event_id") or ""),
            ):
                pending_one_off_events.append(judgment); handoffs.append(judgment_handoff(judgment))

        departure_candidates = annual_voluntary_departure_refs(
        ...
        departure_candidates = [ref for ref in departure_candidates if ref not in transition_bound_refs]
        for ref in departure_candidates:
            judgment = build_npc_judgment_event(
                judgment_kind="faction_membership_departure", at_iso=at_iso, actor_ref=ref, owner_ref=fid,
                options=[{"intent":"leave","person_ref":ref},{"intent":"stay","person_ref":ref}],
                decision_context={"person_ref":ref,"institutional_stress_milli":current_stress},
                source_event_id=str(event.get("event_id") or ""),
            )
            pending_one_off_events.append(judgment); handoffs.append(judgment_handoff(judgment))
        departure_refs: list[str] = []
        ...
        # Objective cleanup may change the sparse owners, but courtship and
        # marriage themselves are never selected here. Each meaningful choice
        # becomes a durable same-timestamp GM-private judgment frontier.
        if relationship_review["family_after"] != family:
            family = relationship_review["family_after"]; writes[_FAMILY_PATH] = family
        if relationship_review["social_after"] != social:
            social = relationship_review["social_after"]; writes[_SOCIAL_PATH] = social
        for request in relationship_review.get("judgment_requests", []):
            if not isinstance(request, Mapping):
                continue
            judgment = build_npc_judgment_event(
                judgment_kind=str(request.get("judgment_kind") or ""), at_iso=at_iso,
                actor_ref=str(request.get("actor_ref") or ""), owner_ref=str(request.get("owner_ref") or fid),
                options=request.get("options", []), decision_context=request.get("decision_context", {}),
                source_event_id=str(event.get("event_id") or ""),
            )
            pending_one_off_events.append(judgment); handoffs.append(judgment_handoff(judgment))
        ...
        reviews.append({
            ...
            "training_intensity_milli": desired_intensity, "departures": 0, "departure_judgments": len(departure_candidates),
