...
from shinobi_runtime.martial_world.rankings import apply_personal_fame_evidence
...
def _published_ranking_review(
    reputation: Mapping[str, Any], *, occurrence: Mapping[str, Any], limit: int = 20,
) -> list[dict[str, Any]]:
    """Return the already-published table for this exact calendar occurrence.

    Reviewing a publication is an observation of durable public evidence, not a
    second ranking calculation.  Fame may change after publication; those later
    changes belong to the next publication rather than retroactively rewriting
    what Wei just read.
    """
    rankings = reputation.get("rankings", {}) if isinstance(reputation, Mapping) else {}
    public = rankings.get("public", {}) if isinstance(rankings, Mapping) else {}
    expected_date = str(occurrence.get("date") or "")
    published_at = str(public.get("published_at") or "") if isinstance(public, Mapping) else ""
    if not expected_date or published_at[:10] != expected_date:
        raise CommandRejectedError("jianghu_calendar_ranking_publication_unavailable")
    rows = public.get("rows", []) if isinstance(public, Mapping) else []
    if not isinstance(rows, list):
        raise CommandRejectedError("jianghu_calendar_ranking_publication_unavailable")
    bounded = max(0, min(100, int(limit)))
...
class JianghuCalendarCommandsMixin:
    ...
    def _jianghu_calendar_event_resolution(
        ...
        if action == "review":
            ...
            published=_published_ranking_review(rep, occurrence=occurrence, limit=20)
