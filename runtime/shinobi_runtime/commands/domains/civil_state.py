...
class CivilStateCommandsMixin:
    ...
    def _occupation_basis(self, place_ref: str, sovereign_ref: str) -> Optional[str]:
        """Return evidence only for the current occupation on an active front.

        Occupation history is not perpetual governance authority. Keyed current
        layouts use the exact place slot; legacy lists use the latest ``since`` row.
        Closed fronts and superseded controllers do not establish current control.
        """
        try:
        ...
        if not isinstance(records, Mapping):
            return None
        for conflict in records.values():
            if not isinstance(conflict, Mapping):
                continue
            fronts_raw = conflict.get("fronts", {})
            fronts = fronts_raw.values() if isinstance(fronts_raw, Mapping) else fronts_raw if isinstance(fronts_raw, list) else ()
            for front in fronts:
                if not isinstance(front, Mapping) or front.get("status") != "active":
                    continue
                occupations_raw = front.get("occupations", {})
                current = None
                if isinstance(occupations_raw, Mapping):
                    candidate = occupations_raw.get(place_ref)
                    if isinstance(candidate, Mapping):
                        current = candidate
                elif isinstance(occupations_raw, list):
                    candidates = [row for row in occupations_raw if isinstance(row, Mapping) and row.get("place_ref") == place_ref]
                    if candidates:
                        current = max(candidates, key=lambda row: str(row.get("since") or ""))
                if not isinstance(current, Mapping) or current.get("controller_ref") != sovereign_ref:
                    continue
                evidence_ref = current.get("evidence_event_ref")
