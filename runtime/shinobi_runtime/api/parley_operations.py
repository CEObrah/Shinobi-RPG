...
def _authorized_dialogue_material(
    ...
    *,
    ...
    settlement_terms = contact.get("settlement_terms")
    if isinstance(settlement_terms, Mapping):
        minimum_cash = max(0, int(settlement_terms.get("minimum_cash", 0) or 0))
        opening_cash = max(0, int(settlement_terms.get("opening_demand_cash", 0) or 0))
        if minimum_cash > 0 and opening_cash >= minimum_cash:
            private_causal_context["settlement_terms"] = {
                "minimum_cash": minimum_cash,
                "opening_demand_cash": opening_cash,
            }
    return {
        "source": source,
