...
commitment. When its leader travels, however, the active three-person team moves
...
horses, and all four people are committed and moved together.
"""
...
class JianghuTravelTeamCommandsMixin:
    ...
    def _jianghu_local_travel_resolution(
        ...
        try:
            quote = local_travel_quote(
                ...
                walking_speed_kph=4.8 * walking_milli / 1000.0,
            )
    ...
    def _jianghu_strategic_travel_resolution(
        ...
        profile = self._party_movement_profile(
            ...
            party_people=party_people,
            mode=mode,
            route_error_code="jianghu_route_unavailable",
        )
        party_speed_milli = int(profile["party_speed_milli"])
        party_encumbrance_milli = int(profile["party_encumbrance_milli"])
        try:
            plan = travel_plan(
        ...
        return self._combine_time_plan(
            ...
            result={
                ...
                "party_speed_milli": party_speed_milli,
                "party_encumbrance_milli": party_encumbrance_milli,
                "total_carried_mass_kg": int(profile["total_carried_mass_kg"]),
                "carried_mass_kg": int(profile["mass_by_ref"].get(command.actor_id, 0)),
                "encumbrance": profile["encumbrance_by_ref"].get(command.actor_id, {}),
            },
            scene_override=scene,
        )
