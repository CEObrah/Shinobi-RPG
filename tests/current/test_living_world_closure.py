...
from shinobi_runtime.martial_world.faction_relations import apply_relation_event, settle_positive_obligation
...
def test_faction_obligation_can_drive_aid_and_is_consumed_without_reciprocal_debt():
    ...
    assert any(
        choose_friendly_aid_target([debt_edge],faction_ref='a',year=61,month=m,cash_reserve_months=12)=='b'
        for m in range(1,121)
    )
...
def test_great_tournament_registration_frontier_wires_real_competitor_and_delegate_departures():
    ...
    # A faction may lawfully lack an available delegate in the current save.
    # Reconcile planned departures against explicit failure accounting instead
    # of hard-coding an older campaign snapshot where all 240 could travel.
    delegation_failures = sum(int(v) for v in review["delegation_failure_counts"].values())
    assert review["spectator_delegations_planned"] + delegation_failures == review["faction_attendance_attempts"]
    ...
    assert len(delegation_departures) == review["spectator_delegations_planned"]
