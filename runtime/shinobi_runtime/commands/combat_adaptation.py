...
def _merge_projections(
    ...
    ordinary = [row for row in beats if row.get("salience") != "protected"]
    # A standing/adaptive span may merge many already-bounded exchange
    # projections. Never turn that merge into a second tail-only truncation.
    # Reserve space for every protected beat, then sample the ordinary
    # chronology across beginning, middle, and end deterministically.
    cap=max(18,min(48,18+len(protected)))
    ordinary_cap=max(0,cap-len(protected))
    sampled_ordinary: list[dict[str, Any]]=[]
    if ordinary_cap and ordinary:
        if len(ordinary)<=ordinary_cap:
            ...
            indices=[]
            for slot in range(ordinary_cap):
                idx=slot*(len(ordinary)-1)//(ordinary_cap-1)
                if idx not in indices: indices.append(idx)
            sampled_ordinary=[copy.deepcopy(dict(ordinary[idx])) for idx in indices]
    bounded = sorted(
        [*[copy.deepcopy(dict(row)) for row in protected], *sampled_ordinary],
    ...
    return {
        ...
        "material_beat_count": len(beats),
        "beats_truncated": len(bounded) < len(beats),
...
def _resolve_with_optional_movement(
    ...
    *, attempt: dict[str, Any], player_ref: str,
    ...
    """Route one adaptive geometry change through the canonical span input.

    Historical builds used a process-local ``ContextVar`` consumed by a runtime
    scheduler wrapper.  Once tactical movement became a first-class exact-combat
    input that indirection became both unnecessary and dangerous: the context
    could be set while the canonical resolver never saw it.  Adaptive movement
    is now ordinary explicit reducer input for this one internal exchange.
    """
    movement_intent = str(attempt.pop(_ADAPTIVE_MOVEMENT_SENTINEL, "") or "")
    if movement_intent:
        attempt["movement_intent"] = movement_intent
...
def adaptive_standing_span(
    ...
    **kwargs: Any,
    ...
    while str(combat_cursor.get("status") or "") == "active":
        ...
        resolved_stop_reason = str(resolved.get("scope_stop_reason") or "")

        player_event = _player_attack_event(events, player_ref)
        if isinstance(player_event, Mapping) and str(player_event.get("result") or "") == "action_rejected":
            stop_reason = "player_action_rejected"
            failure_streak += 1
        elif definite_tactical_failure(player_event):
            failure_streak += 1
        else:
            failure_streak = 0
        if (
        ...
        if stop_reason == "player_action_rejected":
            break
        ...
        if str(combat_cursor.get("status") or "") != "active":
            ...
            break
        if resolved_stop_reason == "gm_judgment_refresh":
            # One GM-authored NPC intent packet is valid only for the exact
            # battlefield frontier it described.  Do not let adaptive standing
            # spans silently reuse those human choices after any state change.
