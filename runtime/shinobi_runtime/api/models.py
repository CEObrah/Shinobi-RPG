...
class PlayContextResponse(StrictModel):
    ...
    contract_reads: Dict[str, Any]
    mission_reads: Dict[str, Any]
    world_events: Dict[str, Any]
    commands: Dict[str, Any]
    active_scene_session: Optional[Dict[str, Any]] = None
    recent_scene_history: List[Dict[str, Any]] = Field(default_factory=list)
    active_threads: List[Dict[str, Any]] = Field(default_factory=list)
    active_thread_count: int = Field(default=0, ge=0)
    active_threads_truncated: bool = False
    active_questions: List[Dict[str, Any]] = Field(default_factory=list)
    read_hints: Optional[Dict[str, Any]] = None
...
class PersonSheetResponse(StrictModel):
    ...
    social_titles: Optional[List[str]] = None
    social_commitments: Optional[Dict[str, Any]] = None
    npc_response_envelope: Optional[Dict[str, Any]] = None
    causal_freshness: Optional[Dict[str, Any]] = None

    @field_validator("sheet", "recognition", "social_titles", "social_commitments", "npc_response_envelope", "causal_freshness")
    @classmethod
    def bounded_sheet(cls, value: Any) -> Any:
        return validate_bounded_json(
            value,
            label="person sheet",
            allow_float=True,
...
class GameObjectResponse(StrictModel):
    ...
    view: Literal[
        ...
        "institutional_mission_summary", "attributed_scene_history", "active_scene_threads",
