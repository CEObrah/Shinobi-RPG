...
def transition_handoff_from_result(
    ...
    """Project current-revision combat decision metadata from committed evidence."""
...
def apply_transition_handoff(
    ...
    """Add ephemeral current-transition decision state without mutating campaign truth."""
    out = dict(context)
...
def _combat_narrative_beat(event: Mapping[str, Any]) -> dict[str, Any] | None:
    ...
    material = bool(
        result in {
            ...
            "defended_or_missed", "projectile_intercepted_clean", "counter_intercepted",
            "withdrawal_declared", "withdrawal_in_progress", "withdrew_from_combat",
            "withdrawal_interrupted", "withdrawal_blocked", "maneuver_completed",
...
def _stratified_rows(rows:list[dict[str,Any]],limit:int)->list[dict[str,Any]]:
    """Bound a chronology without deleting its middle or terminal developments."""
    cap=max(1,int(limit))
    if len(rows)<=cap:return [dict(row) for row in rows]
    if cap==1:return [dict(rows[-1])]
    indices=[]
    for slot in range(cap):
        idx=(slot*(len(rows)-1)+((cap-1)//2))//(cap-1)
    ...
    return [dict(rows[idx]) for idx in sorted(indices[:cap])]
...
def _combat_narrative_summary(raw_events: list[Any], opposing_refs: frozenset[str]) -> dict[str, Any]:
    ...
    material_limit = 96
    selected_material=_stratified_rows(material,material_limit)
    safe_beats = _sanitize_opposing_refs(selected_material, opposing_refs)
    ...
    return {
        ...
        "narration_rule": (
            "Use material_beats as the primary chronological scene spine. The bounded spine deliberately preserves beginning, middle and ending developments across the complete receipt. "
            "Routine counts summarize repeated no-change work. Raw event pages remain exact evidence for audit, negative claims, or detail that the compact spine does not establish."
        ),
    }
...
def _trim_optional_combat_narrative(narrative: dict[str, Any]) -> bool:
    ...
    if not isinstance(beats, list) or not beats:
        return False
    if len(beats)==1:
        # The transport shrink loop must be able to terminate even when one rich
        # material beat alone exceeds the public envelope. Exact event pages
        # remain recoverable; the optional duplicate narrative spine may vanish.
        narrative["material_beats"]=[]
        beats=narrative["material_beats"]
    else:
        retained=max(1,len(beats)//2)
        narrative["material_beats"]=_stratified_rows([dict(row) for row in beats if isinstance(row,Mapping)],retained)
        beats=narrative["material_beats"]
...
class TransitionAwareCampaignOperations(ParleyAwareCampaignOperations):
    ...
    def play_context(self) -> Mapping[str, Any]:
        ...
        if campaign_id and revision >= 0:
            ...
            base = apply_transition_handoff(
                base, raw_result if isinstance(raw_result, Mapping) else None, committed_revision=revision
            )
        object_reads = (
