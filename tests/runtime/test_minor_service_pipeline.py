...
def test_minor_village_academies_grow_service_only_by_conserved_graduation_then_train_existing_manpower() -> None:
    ...
    for institution_id in target_ids:
        ...
        academy = result.get("population_pipeline")
        graduates = academy["graduates"] if isinstance(academy, dict) else 0
        assert after["total"] == before["total"] + graduates
        assert after["availability"]["training_or_instruction"] == (
            before["availability"]["training_or_instruction"] + graduates - service["completed"]
        )
        ...
        assert after_training_pool["count"] == before_training_pool["count"] + graduates - service["completed"]
