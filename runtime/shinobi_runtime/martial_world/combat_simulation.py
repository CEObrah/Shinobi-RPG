...
from .exact_combat import automatic_resource_policy, default_action_for, initialize_combat, resolve_exchange
        ...
        if social_state is not None:
            ...
            for event in last_events:
                ...
                if breached.get("broken_obligation_refs"):
                    target_state = combatants_after.get(intended_ref, {}) if isinstance(combatants_after, Mapping) else {}
                    observed = {
                        str(ref) for ref in target_state.get("observed_refs", []) if isinstance(ref, str)
                    } if isinstance(target_state, Mapping) else set()
                    if actor_ref in observed:
