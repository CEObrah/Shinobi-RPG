...
def choose_hostile_action(
    ...
    """Choose one bounded challenge, feud raid, or war strike.
    ...
    Rivalries prefer formal challenges, feuds may raid, and sustained hostility
    at the war threshold can open larger lethal strikes.
    """
    ...
    for edge in edges:
        ...
        if kind == "faction_war_strike":
            threshold = min(520, max(70, 90 + (hostility - 65) * 10 + risk * 2 + political_pressure * 2))
...
def choose_friendly_aid_target(
    ...
    for edge in edges:
        ...
        if trust < 18 or hostility > 12:
            continue
        strength = trust * 3 + max(0, respect)
        roll = stable_permille("friendly-aid", faction_ref, target, year, month)
        threshold = min(90, max(8, strength // 3))
        if roll < threshold:
...
def choose_investment_priority(
    ...
    """
    if active_projects > 0 or int(cash_reserve_months) < 8 or int(stress_milli) > 0:
        return None
