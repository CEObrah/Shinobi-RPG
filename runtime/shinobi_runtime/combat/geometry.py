...
def _lane_contact_metrics(
    *, actor: Mapping[str, Any], aim: Mapping[str, Any] | None, target: Mapping[str, Any], width_mm: int, length_mm: int,
    ...
    # Reach belongs to the swept weapon segment, not to the target's center.
    # A body whose center lies just beyond nominal reach can still intersect the
    # end-cap of that segment. Reject only when the body's *entry* lies beyond
    # the physical segment. The former center-distance check systematically
    # punished short weapons at elevation/range boundaries while long spears
    # almost never encountered the same false miss.
    if entry_mm > max(0, int(length_mm)):
        return None
    return {
        "along_mm": along_mm,
...
def path_clear(
    ...
    *,
    ...
    if not isinstance(actor, Mapping):
        return False
    radius = max(50, int(actor.get("movement_body_radius_mm", actor.get("body_radius_mm", DEFAULT_BODY_RADIUS_MM))))
    ...
    for ref in body_refs:
        ...
        if not isinstance(target, Mapping) or target.get("zone_ref") != actor.get("zone_ref"):
            continue
        movement_target=dict(target)
        movement_target["body_radius_mm"]=max(50,int(target.get("movement_body_radius_mm",target.get("body_radius_mm",DEFAULT_BODY_RADIUS_MM))))
        metrics = _lane_contact_metrics(actor=actor, aim=end, target=movement_target, width_mm=width, length_mm=length)
