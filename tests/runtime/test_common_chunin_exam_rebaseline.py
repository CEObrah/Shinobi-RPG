...
def test_rebaseline_is_revision_one_and_current_exam_is_clean_registration() -> None:
    repo = RepositoryStore(ROOT)
    meta = repo.read_json("state/meta.json")
    assert meta["revision"] == 1
    assert meta["last_checkpoint_revision"] == 1
    assert meta["time"] == "SE-0061-07-22T07:29:58"
    ...
    assert len(phases) == 1 and phases[0]["phase"] == "registration"
    assert not any(r.get("kind") in {"promotion_exam_evaluation", "promotion_exam_bout"} for r in rows)
...
def test_exactification_conserves_population_and_rank_headcount_and_uses_registered_character_shape() -> None:
    ...
    created = exactify_home_village_genin(
        planner, delegation=delegation, cycle_id=CYCLE + ".test", at=CampaignTime.parse("SE-0061-07-22T07:29:58"), record_writes=writes, count=1
    )
...
def test_rebaseline_contains_no_repair_or_discarded_exam_semantic_events() -> None:
    repo = RepositoryStore(ROOT)
    ...
    for event in events:
        ...
        assert CYCLE.lower() not in blob
        assert not kind.startswith("campaign_repair_")
        assert "reconciled" not in kind and "reconciliation" not in kind
        assert "campaign_repair" not in reducer and "reconciliation" not in reducer
