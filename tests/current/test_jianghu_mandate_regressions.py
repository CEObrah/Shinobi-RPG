...
def test_faction_curricula_have_flavored_backups_and_house_tang_stays_sword_unarmed():
    identities = load("game/data/martial-world/faction-identities.json")["identities"]
    tang = identities["house_tang"]["training_curriculum"]
    assert tang["sword"] > 0 and tang["unarmed"] > 0
    assert all(tang.get(k, 0) == 0 for k in ("spear", "bow", "hidden_weapons"))
