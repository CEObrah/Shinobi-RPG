...
def test_blocked_lateral_intent_fails_closed_without_frontal_fallback(monkeypatch) -> None:
    from shinobi_runtime.combat import physical_defense
    ...
    monkeypatch.setattr(physical_defense, "path_clear", lambda *args, **kwargs: False)
    with pytest.raises(ValueError, match="combat tactical movement path blocked"):
