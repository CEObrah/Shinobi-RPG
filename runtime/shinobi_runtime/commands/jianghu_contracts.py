...
class JianghuContractCommandsMixin:
    def _contract_person_location_matches(
        self, ref: str, person: Mapping[str, Any], *, source_place: str, sites: Mapping[str, Any]
    ) -> bool:
    ...
    def _jianghu_contract_resolution(
        ...
        if approved_roster is not None:
            ...
            for ref in standing_party_refs:
                ...
                if health.get("status") in {"dead", "incapacitated"} or int(health.get("consciousness", 100)) <= 0:
                    continue
                if not self._contract_person_location_matches(ref, member, source_place=source_place, sites=sites):
                    continue
                core_escort_refs.append(ref)
            ...
            if needed:
                ...
                for raw_person in people:
                    ...
                    try:
                        ...
                        continue
                    if self._contract_person_location_matches(ref, person, source_place=source_place, sites=sites):
        ...
        for ref in all_people:
            ...
            if health.get("status") in {"dead", "incapacitated"} or int(health.get("consciousness", 100)) <= 0:
                raise CommandRejectedError("jianghu_contract_participant_unavailable")
            if not self._contract_person_location_matches(ref, person, source_place=source_place, sites=sites):
