...
def _history_row(source: object) -> dict[str, Any] | None:
    ...
    proposal_terms = source.get("proposal_terms")
    if isinstance(proposal_terms, Mapping):
        compact_terms: dict[str, Any] = {}
        cash = proposal_terms.get("cash")
        if isinstance(cash, int) and not isinstance(cash, bool) and cash > 0:
            compact_terms["cash"] = cash
        recipient_ref = proposal_terms.get("recipient_ref")
        if isinstance(recipient_ref, str) and recipient_ref:
            compact_terms["recipient_ref"] = recipient_ref
        if compact_terms:
            row["proposal_terms"] = compact_terms
    return row or None
...
def _active_continuity_refs(
    scene: Mapping[str, Any], session: Mapping[str, Any] | None
) -> tuple[str, ...]:
    """Return exact live presentation-process refs that own immediate continuity.

    A formal scene session is one such owner, but it is not the only one. Active
    combat parley deliberately uses the exact ``combat_ref`` as its reversible
    conversation surface so hidden opposing person identities never need to be
    exposed. Treating only formal sessions as live continuity made the primary
    writer workspace forget combat-side exchanges that were still active.
    """
    refs: list[str] = []
    ...
    if isinstance(parley, Mapping):
        combat_ref = parley.get("combat_ref")
        active_combat_ref = scene.get("active_combat_ref")
        if (
            isinstance(combat_ref, str)
            and combat_ref
            and combat_ref == active_combat_ref
            and combat_ref not in refs
        ):
            refs.append(combat_ref)
    return tuple(refs)


def _scene_history_window(
    history: list[dict[str, Any]], continuity_refs: Sequence[str]
) -> list[dict[str, Any]]:
    # Immediate continuity is process-local. Once no live presentation process
    # owns a record, older attributed speech and reversible scene facts remain
    # durable history but must not masquerade as the current conversational beat.
    refs = {str(ref) for ref in continuity_refs if isinstance(ref, str) and ref}
    if not refs:
        return []
    matched = [row for row in history if row.get("session_ref") in refs]
...
def _human_threads(
    ...
    continuity_refs: Sequence[str],
) -> list[dict[str, Any]]:
    """Return only live human threads for the exact active presentation process."""
    refs = {str(ref) for ref in continuity_refs if isinstance(ref, str) and ref}
    if not refs:
        return []
    ...
    if isinstance(raw, list):
        for source in raw[:16]:
            ...
            source_process = source.get("process_ref")
            if refs and source_session not in refs and source_process not in refs:
                continue
            row = _pick(source, (
    ...
    # Combat-side parley is intentionally not a formal session. Its bounded
    # projection is already filtered to the active exact combat and therefore
    # may supply current response-bearing threads directly to the writer.
    parley = scene.get("combat_parley")
    if isinstance(parley, Mapping):
        combat_ref = parley.get("combat_ref")
        open_threads = parley.get("open_threads")
        if isinstance(combat_ref, str) and combat_ref in refs and isinstance(open_threads, list):
            seen = {str(row.get("attempt_ref") or "") for row in rows if isinstance(row, Mapping)}
            for source in open_threads[:8]:
                if not isinstance(source, Mapping):
                    continue
                attempt_ref = str(source.get("attempt_ref") or "")
                if attempt_ref and attempt_ref in seen:
                    continue
                row = _pick(source, ("attempt_ref", "at", "action", "topic", "posture"))
                row["target_ref"] = combat_ref
                row["process_ref"] = combat_ref
                row["thread_status"] = "open"
                statement = _text(source.get("player_statement"), 700)
                if statement:
                    row["player_statement"] = statement
                if row:
                    rows.append(row)
                    if attempt_ref:
                        seen.add(attempt_ref)

    # Do not reconstruct arbitrary old durable refs as live threads. Only the
    # active-thread and active-combat-parley projections above can contribute.
    return rows[:16]


def _recent_player_actions(
    context: Mapping[str, Any], continuity_refs: Sequence[str]
) -> list[dict[str, Any]]:
    """Keep Wei's latest declared words/actions for the exact live process.

    These rows prove only what the player authored or attempted. They never
    import an NPC response or hard outcome. Combat parley normally has no formal
    scene session, so its ``process_ref`` is a first-class continuity key.
    """
    ...
    source_rows = [
        ...
        and (row.get("scene_session_ref") in refs or row.get("process_ref") in refs)
    ]
    rows: list[dict[str, Any]] = []
...
def _compact_observation(key: str, value: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if key == "combat_observation_context":
        return _pick(value, (
            ...
            "npc_judgment_envelopes", "npc_judgment_contract", "privacy",
        ))
    if key == "movement_context":
        return _pick(value, (
            ...
            "journey_started_at", "last_progress_at", "segment_index",
            "segment_elapsed_seconds", "segment_required_seconds",
            "segment_progress_milli", "elapsed_scope", "progress_scope",
...
def build_gm_scene_context(context: Mapping[str, Any]) -> dict[str, Any]:
    ...
    continuity_refs = _active_continuity_refs(scene, session)
    all_history = _compact_history(context.get("recent_scene_history"), 16)
    history = _scene_history_window(all_history, continuity_refs)
    recent_player_actions = _recent_player_actions(context, continuity_refs)
    ...
    player_ref = str(player.get("person_id") or campaign.get("player_id") or "")
    player_faction_ref = str(player.get("faction_ref") or "")
    combat_parley = scene.get("combat_parley") if isinstance(scene.get("combat_parley"), Mapping) else {}
    opposing_identities_hidden = combat_parley.get("identity_policy") == "opposing_person_ids_remain_hidden"
    ...
    friendly_combat_refs = set(_refs(scene.get("combat_present_person_ids"), 128))
    explicitly_known_identity_refs = set(_refs(scene.get("identity_known_person_ids"), 64))
    private_by_ref = {str(row.get("person_ref")): row for row in private_people if row.get("person_ref")}
    present_people: list[dict[str, Any]] = []
    for ref in present_refs:
        physically_visible = ref in visible_refs or not visible_refs
        private_direction = private_by_ref.get(ref)
        truth = private_direction.get("character_truth") if isinstance(private_direction, Mapping) else None
        truth_faction_ref = str(truth.get("faction_ref") or "") if isinstance(truth, Mapping) else ""
        identity_known = True
        if opposing_identities_hidden and ref != player_ref:
            # Hidden-opponent identity must fail closed when the bounded private
            # packet is truncated.  Friendly exact-combat presence is a
            # player-safe side projection; otherwise only same-faction private
            # truth may keep identity known here. A future explicit recognition
            # projection can opt an opponent back in without relying on missing
            # faction metadata.
            same_player_side = ref in friendly_combat_refs
            same_player_faction = bool(player_faction_ref and truth_faction_ref == player_faction_ref)
            identity_known = bool(same_player_side or same_player_faction or ref in explicitly_known_identity_refs)
        row: dict[str, Any] = {
            "person_ref": ref,
            # Compatibility alias. This means physical/sensory visibility only.
            "visible_to_wei": physically_visible,
            "physically_visible_to_wei": physically_visible,
            "identity_known_to_wei": identity_known,
        }
        ...
        if isinstance(private_direction, Mapping):
            if isinstance(truth, Mapping):
                name = truth.get("name")
                if identity_known and isinstance(name, str) and name:
                    row["name"] = name
                offices = truth.get("standing_offices")
                grade = truth.get("membership_grade")
                if identity_known and offices not in (None, [], {}):
                    row["role_hint"] = offices
                elif identity_known and grade not in (None, ""):
                    row["role_hint"] = grade
            # Exact hidden identity may remain available only inside explicitly
            # GM-private direction. Player-facing naming must honor the separate
            # identity_known_to_wei bit above.
    ...
    human_threads = _human_threads(context, scene, session, continuity_refs)
    ...
    return {
        ...
        "writer_contract": {
            ...
            "person_visibility_semantics": (
                "physically_visible_to_wei is sensory presence, not recognition; "
                "identity_known_to_wei separately governs player-facing names and roles"
            ),
        },
    }
