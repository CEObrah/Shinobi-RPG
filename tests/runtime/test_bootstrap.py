...
def source_and_remote(tmp_path: Path) -> tuple[Path, Path]:
    ...
    (source / "state").mkdir()
    (source / "state" / "authority.txt").write_text("one\n", encoding="utf-8")
    git(source, "add", "campaign.txt", "state/authority.txt")
...
def test_bootstrap_preserves_synchronized_dirty_checkout_for_wal_recovery_and_rejects_divergence(
    ...
    git(checkout, "config", "user.email", "runtime@example.invalid")
    git(checkout, "config", "user.name", "Runtime Test")
    (checkout / "state" / "authority.txt").write_text("local\n", encoding="utf-8")
    git(checkout, "add", "state/authority.txt")
    git(checkout, "commit", "-qm", "local")
    (source / "state" / "authority.txt").write_text("remote\n", encoding="utf-8")
    git(source, "add", "state/authority.txt")
