...
def _suppress_reached_player_boundary(root: Path) -> bool:
    ...
    changed = False
    events = []
    cleared_host_ids: set[str] = set()
    for event in scheduler.queue.snapshot():
        if event.requires_player is True and event.due_at <= scheduler.world_time:
            # This disposable autonomous soak treats the reached player boundary
            # as acknowledged. Removing it is valid harness behavior; changing
            # ``requires_player`` would create an invalid non-player event kind.
            cleared_host_ids.add(event.target_host)
            changed = True
            continue
        events.append(event)
    scheduler.queue.replace(events)
    for host_id in cleared_host_ids:
        ...
        wrapper.state.advance_resolved(scheduler.world_time)
        wrapper.state.next_due = future_due
