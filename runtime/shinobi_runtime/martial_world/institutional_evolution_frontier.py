...
def _settle_autonomous_institutional_evolution_candidate(
    *, read_json: Callable[[str], Mapping[str, Any]], writes: dict[str, Any],
    ...
    """Plan/apply the strongest currently eligible institutional transition.

    This private helper preserves the deterministic feasibility/ranking logic. The
    public wrapper below uses it first against a scratch write-set so deterministic
    Python can propose a bounded transition without committing the human choice.
    """
    if not force_boundary and not chunk_contains_final_owner(schedule, events, class_id="faction_annual"):
        return {
    ...
    blocked |= physical_unavailable_person_refs(get)
    ...
    for fid in active:
        ...
        for site_ref in sorted(str(x) for x in controlled):
            ...
            for candidate in sorted(onsite, key=lambda p: str(p.get("person_id") or "")):
                ...
                return {"registry":registry,"relations":relations,"family":family,"independents":independents,"reviews":[{"kind":"autonomous_faction_split","source_faction_ref":fid,"new_faction_ref":new_ref,"leader_ref":candidate_ref,"member_refs":moved_refs,"estate_site_ref":site_ref,"equipment_policy_detached_count":equipment_transition["detached_policy_holder_count"]}]}
    ...
    for site_ref in sorted(by_site):
        ...
        for leader in leaders:
            ...
            writes[REGISTRY_PATH] = registry; writes[_FAMILY] = family
            return {"registry":registry,"relations":relations,"family":family,"independents":independents,"reviews":[{"kind":"autonomous_faction_foundation","faction_ref":new_ref,"leader_ref":leader_ref,"member_refs":sorted(member_refs),"startup_cash":startup,"headquarters_site_ref":site_ref}]}

    return {"registry":registry,"relations":relations,"family":family,"independents":independents,"reviews":[]}


def _evolution_actor_ref(
    review: Mapping[str, Any], *, read_json: Callable[[str], Mapping[str, Any]],
    writes: Mapping[str, Any],
) -> str:
    kind = str(review.get("kind") or "")
    if kind == "autonomous_faction_split":
        return str(review.get("leader_ref") or "")
    if kind == "autonomous_faction_foundation":
        return str(review.get("leader_ref") or "")
    fid = str(review.get("source_faction_ref") or review.get("faction_ref") or "")
    if not fid:
        return ""
    path = roster_path(fid)
    raw = writes.get(path) if isinstance(writes.get(path), Mapping) else read_json(path)
    faction_raw = writes.get(faction_path(fid)) if isinstance(writes.get(faction_path(fid)), Mapping) else read_json(faction_path(fid))
    roster = hydrate_roster_state(raw, faction=hydrate_faction_state(faction_raw))
    return _leader_ref([row for row in roster.get("people", []) if isinstance(row, Mapping)])


def _evolution_proposal(review: Mapping[str, Any]) -> dict[str, Any]:
    kind = str(review.get("kind") or "")
    if kind not in {
        "autonomous_faction_dissolution", "autonomous_faction_merger",
        "autonomous_faction_split", "autonomous_faction_foundation",
    }:
        raise ValueError("institutional evolution review is not a transition proposal")
    payload = {
        str(key): copy.deepcopy(value)
        for key, value in review.items()
        if key not in {"equipment_policy_detached_count"}
    }
    return {"intent": "accept", "proposal": payload}


def _same_evolution_proposal(selected: Mapping[str, Any], review: Mapping[str, Any]) -> bool:
    if str(selected.get("intent") or "") != "accept":
        return False
    proposal = selected.get("proposal")
    return isinstance(proposal, Mapping) and dict(proposal) == _evolution_proposal(review)["proposal"]
...
def settle_autonomous_institutional_evolution(
    *, read_json: Callable[[str], Mapping[str, Any]], writes: dict[str, Any],
    ...
    """Surface meaningful institutional change as GM-authored judgment.

    Deterministic code may score eligibility and construct one bounded proposal at
    the annual frontier. It never commits dissolution, merger, split, or foundation
    until ChatGPT has selected the exact proposal through the durable NPC-judgment
    contract. The accepted proposal is then recomputed against current same-time
    state; any drift fails closed instead of substituting a new human choice.
    """
    committed = next((
        row for row in events
        if isinstance(row, Mapping)
        and row.get("kind") == "npc_judgment_committed"
        and row.get("judgment_kind") == "institutional_evolution_policy"
    ), None)
    at_boundary = chunk_contains_final_owner(schedule, events, class_id="faction_annual")
    if committed is None and not at_boundary:
        return {
            "registry": copy.deepcopy(dict(writes.get(REGISTRY_PATH) or read_json(REGISTRY_PATH))),
            "relations": copy.deepcopy(dict(relations_state)),
            "family": copy.deepcopy(dict(family_state)),
    ...
    if committed is not None:
        choice = committed.get("selected_option")
        if not isinstance(choice, Mapping):
            raise ValueError("institutional evolution judgment choice invalid")
        if str(choice.get("intent") or "") == "defer":
            return {
                "registry": copy.deepcopy(dict(writes.get(REGISTRY_PATH) or read_json(REGISTRY_PATH))),
                "relations": copy.deepcopy(dict(relations_state)),
                "family": copy.deepcopy(dict(family_state)),
                "independents": copy.deepcopy(dict(independent_state)),
                "reviews": [{
                    "kind": "institutional_evolution_judgment_applied",
                    "decision_ref": str(committed.get("decision_ref") or ""),
                    "result": "deferred",
                }],
            }
        probe_writes = copy.deepcopy(writes)
        probe = _settle_autonomous_institutional_evolution_candidate(
            read_json=read_json, writes=probe_writes, schedule=schedule, events=events,
            year=year, at_iso=at_iso, player_ref=player_ref, site_rows=site_rows,
            relations_state=relations_state, family_state=family_state,
            independent_state=independent_state, social_state=social_state, force_boundary=True,
        )
        reviews = [row for row in probe.get("reviews", []) if isinstance(row, Mapping)]
        if len(reviews) != 1 or not _same_evolution_proposal(choice, reviews[0]):
            raise ValueError("institutional evolution proposal changed before commitment")
        applied = _settle_autonomous_institutional_evolution_candidate(
            read_json=read_json, writes=writes, schedule=schedule, events=events,
            year=year, at_iso=at_iso, player_ref=player_ref, site_rows=site_rows,
            relations_state=relations_state, family_state=family_state,
            independent_state=independent_state, social_state=social_state, force_boundary=True,
        )
        applied_reviews = [copy.deepcopy(dict(row)) for row in applied.get("reviews", []) if isinstance(row, Mapping)]
        applied["reviews"] = [{
            "kind": "institutional_evolution_judgment_applied",
            "decision_ref": str(committed.get("decision_ref") or ""),
    ...
    probe = _settle_autonomous_institutional_evolution_candidate(
        ...
        year=year, at_iso=at_iso, player_ref=player_ref, site_rows=site_rows,
        relations_state=relations_state, family_state=family_state,
        independent_state=independent_state, social_state=social_state, force_boundary=False,
    )
    proposals = [row for row in probe.get("reviews", []) if isinstance(row, Mapping)]
    if not proposals:
        return {
            "registry": copy.deepcopy(dict(writes.get(REGISTRY_PATH) or read_json(REGISTRY_PATH))),
            "relations": copy.deepcopy(dict(relations_state)),
    ...
    review = proposals[0]
    actor_ref = _evolution_actor_ref(review, read_json=read_json, writes=writes)
    if not actor_ref or actor_ref == player_ref:
        # Player-owned institutional choices remain protected player agency.
        return {
            "registry": copy.deepcopy(dict(writes.get(REGISTRY_PATH) or read_json(REGISTRY_PATH))),
            "relations": copy.deepcopy(dict(relations_state)),
            "family": copy.deepcopy(dict(family_state)),
            "independents": copy.deepcopy(dict(independent_state)),
            "reviews": [{"kind": "institutional_evolution_player_decision_required", "proposal": copy.deepcopy(dict(review))}],
        }
    event = build_npc_judgment_event(
        judgment_kind="institutional_evolution_policy", at_iso=at_iso,
        actor_ref=actor_ref, owner_ref=str(review.get("source_faction_ref") or review.get("faction_ref") or actor_ref),
        options=[_evolution_proposal(review), {"intent": "defer"}],
        decision_context={"proposal": copy.deepcopy(dict(review)), "annual_transition_limit": 1},
        source_event_id=f"institutional_evolution:{year}",
    )
    if pending_one_off_events is not None:
    ...
    return {
        ...
        "independents": copy.deepcopy(dict(independent_state)),
        "reviews": [{
            "kind": "institutional_evolution_judgment_due",
            "decision_ref": event["decision_ref"], "actor_ref": actor_ref,
            "proposal": copy.deepcopy(dict(review)),
        }],
    }
