...
from shinobi_runtime.martial_world.faction_relations import (
    apply_relation_event, friendly_aid_transfer_envelope, resolve_friendly_aid_transfer,
    settle_positive_obligation,
)
...
def living_faction_members(faction_ref: str) -> int:
    roster = json.loads((ROOT / f"state/martial-world/people/{faction_ref}.json").read_text())
    return sum(
        1 for person in roster.get("people", [])
        if isinstance(person, dict) and (person.get("health") or {}).get("status") != "dead"
    )
...
def test_hostile_autonomy_is_bounded_and_escalates_rivalry_feud_and_war():
    ...
    challenge = choose_hostile_action(moderate, faction_ref="a", year=61, month=1, risk_tolerance=100)
    seed_raid = choose_hostile_action(seed_feud, faction_ref="a", year=61, month=1, risk_tolerance=100)
    war_strike = choose_hostile_action(severe, faction_ref="a", year=61, month=1, risk_tolerance=100)
    assert challenge and challenge["requires_gm_judgment"] is True
    assert seed_raid and seed_raid["requires_gm_judgment"] is True
    assert war_strike and war_strike["requires_gm_judgment"] is True
    assert challenge["options"][0]["action"] == "formal_challenge"
    assert seed_raid["options"][0]["action"] == "faction_raid"
    assert war_strike["options"][0]["action"] == "faction_war_strike"

    # A live war may expose a third-front war-strike option, but the runtime
    # never selects it and never exceeds the authored global front limit.
    third_front = choose_hostile_action(
        severe, faction_ref="a", year=61, month=1,
        risk_tolerance=100, active_strategic_operations=2,
    )
    assert third_front and third_front["options"][0]["action"] == "faction_war_strike"
...
def test_growth_priority_prefers_real_capacity_bottlenecks_and_blocks_parallel_projects():
    ...
    envelope = choose_investment_priority(
        faction,
    ...
    assert envelope and envelope["requires_gm_judgment"] is True
    option = envelope["options"][0]
    assert option["kind"] == "expand_building"
    assert option["building_type"] == "residential_compound"
    assert option["additional_footprint_m2"] > 0
...
def test_friendly_aid_requires_reserves_and_a_real_friendly_edge():
    ...
    envelope = choose_friendly_aid_target(
        edges, faction_ref="a", year=61, month=1, cash_reserve_months=12,
    )
    assert envelope and envelope["requires_gm_judgment"] is True
    assert envelope["options"][0]["target_faction_ref"] == "b"
    assert choose_friendly_aid_target(edges, faction_ref="a", year=61, month=1, cash_reserve_months=7) is None

def test_friendly_aid_amount_is_authored_exactly_and_revalidated_against_current_bounds():
    source = {"treasury_cash":100_000, "population":10, "buildings":{}, "enterprises":{}}
    target = {"treasury_cash":0, "population":10, "buildings":{}, "enterprises":{}}
    ...
    invalid = resolve_friendly_aid_transfer(
        source, inventory, target, inventory, amount_cash=maximum + 1,
    )
    assert invalid == {
        "result":"aid_amount_outside_current_envelope",
        "minimum_cash":minimum, "maximum_cash":maximum,
    }

    authored = minimum + (maximum - minimum) // 2
    result = resolve_friendly_aid_transfer(
        source, inventory, target, inventory, amount_cash=authored,
    )
    assert result["result"] == "aid_transferred"
    assert result["cash"] == authored
    assert result["source_after"]["treasury_cash"] == source["treasury_cash"] - authored
    assert result["target_after"]["treasury_cash"] == target["treasury_cash"] + authored
    assert result["source_after"]["treasury_cash"] + result["target_after"]["treasury_cash"] == source["treasury_cash"] + target["treasury_cash"]


def test_faction_obligation_can_drive_aid_and_is_consumed_without_reciprocal_debt():
    debt_edge={'from_faction':'a','to_faction':'b','trust':12,'respect':8,'hostility':0,'obligation':12}
    envelope=choose_friendly_aid_target(
        [debt_edge],faction_ref='a',year=61,month=1,cash_reserve_months=12,
    )
    assert envelope and envelope['requires_gm_judgment'] is True
    assert envelope['options'][0]['target_faction_ref']=='b'
    assert envelope['options'][0]['obligation']==12
    settled=settle_positive_obligation(debt_edge,from_faction='a',to_faction='b',amount=12)
    assert settled.get('obligation',0)==0
    acknowledged=apply_relation_event(
        {'from_faction':'b','to_faction':'a','trust':10,'obligation':0},
        from_faction='b',to_faction='a',event_kind='silver_obligation_repayment_received',
    )
    assert acknowledged.get('obligation',0)==0
    assert acknowledged['trust']==15
    ordinary_giver=apply_relation_event(
        {'from_faction':'a','to_faction':'b','trust':20},
        from_faction='a',to_faction='b',event_kind='silver_aid_given',
    )
    assert ordinary_giver.get('obligation',0)==0
...
def test_independent_person_roundtrip_preserves_identity_but_removes_faction_rank():
    person = {
...
def test_calendar_generates_annual_regional_and_four_year_great_tournaments():
    ...
    assert (date.fromisoformat(great["registration_closes_on"]) - date.fromisoformat(great["registration_opens_on"])).days == preparation_days
...
def test_great_tournament_preparation_window_covers_every_connected_place_under_worst_registered_travel_factors():
    ...
    # The window follows the current registered geography rather than a stale
    # historical route-length snapshot.
    assert preparation_days == max(180, worst_route_days + 60)
...
def test_great_tournament_major_sects_always_attempt_and_send_serious_delegations():
    ...
    for faction_ref in major_sects:
        ...
        assert tournament_travel_interested(
            ...
            prestige_weight=profile["prestige_weight"],
            faction_type="sect",
            living_members=living_faction_members(faction_ref),
...
def test_great_tournament_reaches_almost_the_entire_canonical_jianghu_and_every_major_sect_attempts():
    ...
    for faction_ref, identity in identities.items():
        ...
        living = living_faction_members(faction_ref)
...
def test_tournament_sportsmanship_can_create_future_aid_eligibility_from_real_interaction():
    ...
    envelope = choose_friendly_aid_target(
        [after], faction_ref="a", year=62, month=1, cash_reserve_months=12,
    )
    assert envelope and envelope["requires_gm_judgment"] is True
    assert envelope["options"][0]["target_faction_ref"] == "b"
...
def test_great_tournament_registration_frontier_requires_faction_judgment_before_participation():
    overlay = {}
    ...
    result = settle_martial_world_frontier(read_json=read_json, schedule=schedule, events=[event], at=at)
    absorb(result)
    review = next(row for row in result["reviews"] if row.get("kind") == "tournament_registration_open")
    assert review["faction_judgments_pending"] > 0
    assert review["local_paid_registrations"] == 0
    assert review["traveling_entrants_planned"] == 0
    assert review["spectator_delegations_planned"] == 0

    tref = "tournament:great_jianghu_tournament:0064-09-01:individual"
    tournament = overlay["state/martial-world/tournaments.json"]["tournaments"][tref]
    assert tournament["registrations"] == []
    assert tournament["prize_escrow_cash"] == 0

    one_off = result["schedule_after"].get("one_off", {})
    assert not any(row.get("kind") == "tournament_trip_departure" for row in one_off.values())
    assert not any(row.get("kind") == "tournament_delegation_departure" for row in one_off.values())
    judgments = [
        row for row in one_off.values()
        if row.get("kind") == "npc_judgment_due"
        and row.get("judgment_kind") == "tournament_participation_policy"
    ]
    assert len(judgments) == review["faction_judgments_pending"]
    assert all(any(opt.get("intent") == "decline" for opt in row.get("options", [])) for row in judgments)

    # Pick one host-city faction with an eligible entrant. Only the GM-authored
    # participation choice may spend its treasury and register that person.
    judgment = next(
        row for row in judgments
        if row.get("decision_context", {}).get("source_place_ref") == "luoyang"
        and any(opt.get("entrant_policy") == "champion" and opt.get("delegation_policy") == "none" for opt in row.get("options", []))
    )
    option = next(
        opt for opt in judgment["options"]
        if opt.get("intent") == "participate"
        and opt.get("entrant_policy") == "champion"
        and opt.get("delegation_policy") == "none"
    )
    committed = {
        "event_id": "npc_judgment_committed:test:tournament-participation",
        "kind": "npc_judgment_committed",
        "due_at": at.isoformat(),
        "owner_ref": judgment["owner_ref"],
    ...
    decided = settle_martial_world_frontier(
        read_json=read_json, schedule=result["schedule_after"], events=[committed], at=at,
    )
    decision_review = next(
        row for row in decided["reviews"]
        if row.get("kind") == "tournament_participation_policy"
        and row.get("faction_ref") == judgment["actor_ref"]
    )
    assert decision_review["local_paid_registrations"] == 1
    assert decision_review["traveling_entrants_planned"] == 0
    assert decision_review["decision_origin"] == "gm_npc_judgment"
    decided_tournament = decided["writes"]["state/martial-world/tournaments.json"]["tournaments"][tref]
    assert len(decided_tournament["registrations"]) == 1
    assert decided_tournament["registrations"][0]["faction_ref"] == judgment["actor_ref"]
    assert decided_tournament["prize_escrow_cash"] == decided_tournament["entry_fee_cash"]


def test_tournament_convergence_presence_is_physical_not_implied_by_registration():
    from shinobi_runtime.martial_world.tournament_frontier import tournament_person_physically_present

    sites = {
        "site.host.venue": {"parent_place_ref": "luoyang"},
        "site.away": {"parent_place_ref": "changan"},
    }
    present = {"person_id": "p.present", "location_ref": "site.host.venue", "health": {"status": "healthy"}}
    away = {"person_id": "p.away", "location_ref": "site.away", "health": {"status": "healthy"}}
    dead = {"person_id": "p.dead", "location_ref": "site.host.venue", "health": {"status": "dead"}}
    assert tournament_person_physically_present(present, host_place_ref="luoyang", local_sites=sites)
    assert not tournament_person_physically_present(away, host_place_ref="luoyang", local_sites=sites)
    assert not tournament_person_physically_present(dead, host_place_ref="luoyang", local_sites=sites)
