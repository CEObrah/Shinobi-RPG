...
from shinobi_runtime.commands import mission_context_index as mission_index
...
def test_current_player_mission_context_filters_terminal_before_context_cap(
    ...
    records = []
    # Historical mission cardinality is intentionally above the retired 256-owner
    # API ceiling.  Only current participant routes may bound the hot handoff.
    for index in range(300):
        records.append(
            {
                "mission_id": f"mission.aaa.terminal.{index:03d}",
                "state": "succeeded",
    ...
    monkeypatch.setattr(mission_index, "MissionOwner", FakeMissionOwner)
