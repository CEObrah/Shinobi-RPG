...
from shinobi_runtime.martial_world.route_activity import ROUTE_PRESENCE_STATUSES
...
def combat_person_arrived(combat: Mapping[str, Any], person_ref: str) -> bool:
    ...
    """
    combatants_raw = combat.get("combatants", {})
    if not isinstance(combatants_raw, Mapping):
        raise ValueError("combat presence owner has invalid combatants")
    state = combatants_raw.get(person_ref)
    if state is None:
        return True
    if not isinstance(state, Mapping):
        raise ValueError(f"combat presence state is invalid for {person_ref}")
    status_families = state.get("status_families", [])
    if not isinstance(status_families, list):
        raise ValueError(f"combat presence status_families is invalid for {person_ref}")
    statuses = {str(value) for value in status_families if isinstance(value, str)}
...
def active_custody_for_person(read_json: Callable[[str], Any], person_ref: str) -> tuple[str, Mapping[str, Any]] | None:
    ...
    if state is None:
        return None
    if not isinstance(state, Mapping):
        raise ValueError("custody owner is malformed")
    rows = state.get("records", [])
    if not isinstance(rows, list):
        raise ValueError("custody records are malformed")
    ...
    for row in reversed(rows):
        if not isinstance(row, Mapping):
            raise ValueError("custody record is malformed")
        if (
            str(row.get("person_ref") or "") == person_ref
...
def active_combat_for_person(read_json: Callable[[str], Any], person_ref: str) -> tuple[str, Mapping[str, Any]] | None:
    ...
    if state is None:
        return None
    if not isinstance(state, Mapping):
        raise ValueError("combat owner is malformed")
    rows = state.get("combats", {})
    if not isinstance(rows, Mapping):
        raise ValueError("combat registry is malformed")
    for combat_ref, combat in rows.items():
        if not isinstance(combat_ref, str) or not combat_ref or not isinstance(combat, Mapping):
            raise ValueError("combat registry entry is malformed")
        if combat.get("status") != "active":
            continue
        combatants = combat.get("combatants", {})
        sides = combat.get("sides", {})
        if not isinstance(combatants, Mapping) or not isinstance(sides, Mapping):
            raise ValueError(f"active combat presence routing is malformed: {combat_ref}")
        registered = person_ref in combatants
        if not registered:
            for members in sides.values():
                if not isinstance(members, list):
                    raise ValueError(f"active combat side membership is malformed: {combat_ref}")
                if person_ref in members:
                    registered = True
                    break
        if registered and combat_person_arrived(combat, person_ref):
            combatant_state = combatants.get(person_ref) if isinstance(combatants, Mapping) else None
            statuses = {
                str(value) for value in combatant_state.get("status_families", []) if isinstance(value, str)
            } if isinstance(combatant_state, Mapping) and isinstance(combatant_state.get("status_families"), list) else set()
            # A completed disengagement removes the body from the combat's
            # physical space even while allies continue fighting. Route/custody
            # or the person's stored location becomes the next presence owner.
...
def active_route_for_person(read_json: Callable[[str], Any], person_ref: str) -> tuple[str, Mapping[str, Any]] | None:
    ...
    if state is None:
        return None
    if not isinstance(state, Mapping):
        raise ValueError("route operations owner is malformed")
    rows = state.get("movements", {})
    if not isinstance(rows, Mapping):
        raise ValueError("route movement registry is malformed")
    for movement_ref, movement in rows.items():
        if not isinstance(movement_ref, str) or not movement_ref or not isinstance(movement, Mapping):
            raise ValueError("route movement entry is malformed")
        if str(movement.get("status") or "") not in ROUTE_PRESENCE_STATUSES:
            continue
        participants_raw = movement.get("participant_refs", [])
        if not isinstance(participants_raw, list):
            raise ValueError(f"route participant list is malformed: {movement_ref}")
        ...
        for key in ("protected_person_refs", "captive_refs", "rescued_refs"):
            ...
            if not isinstance(values, list):
                raise ValueError(f"route carried-person list is malformed: {movement_ref}:{key}")
    ...
    *,
    ...
    if movement is not None:
        ...
        if status in {"lodging_rest", "field_rest"} and isinstance(rest_place, str) and rest_place:
            ...
            # the same exact movement are physically together. A defeated party
            # may preserve escaped/separated members under the same route owner
            # without pretending they have already regrouped around a casualty.
            separated = {
                str(x) for x in row.get("separated_person_refs", []) if isinstance(x, str)
            } if isinstance(row.get("separated_person_refs"), list) else set()
            space_ref = (
                f"movement:{owner_ref}:separated:{ref}"
                if ref in separated
                else f"movement:{owner_ref}"
            )
            available = False
        return {
            "person_ref": ref,
