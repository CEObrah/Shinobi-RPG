...
from shinobi_runtime.tx.git import GitStager
...
class GitRemoteDurability:
    ...
    def from_env(
        ...
        remote = source.get("SHINOBI_GIT_REMOTE")
        branch = source.get("SHINOBI_GIT_BRANCH")
        if remote is None and branch is None:
            if source.get("SHINOBI_GIT_URL") is not None:
                raise RuntimeError(
                    "SHINOBI_GIT_REMOTE and SHINOBI_GIT_BRANCH are required "
                    "when SHINOBI_GIT_URL configures a persistent checkout"
                )
            return None
        if not remote or not branch:
            raise RuntimeError(
                "SHINOBI_GIT_REMOTE and SHINOBI_GIT_BRANCH must be set together"
            )
        raw_timeout = source.get("SHINOBI_GIT_TIMEOUT_SECONDS", "30")
        try:
            timeout = float(raw_timeout)
        except (TypeError, ValueError) as exc:
            raise RuntimeError("invalid SHINOBI_GIT_TIMEOUT_SECONDS") from exc
        try:
    ...
    def _run(
        ...
        if environment.get("SHINOBI_GIT_TOKEN") is not None:
