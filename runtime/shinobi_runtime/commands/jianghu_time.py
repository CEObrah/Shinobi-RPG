...
class JianghuTimeCommandsMixin:
    ...
    def _jianghu_time_frontier_plan(
        ...
        *,
        ...
        boundary_class = str(boundary_summary.get("class") or "internal")
        if isinstance(boundary_event, Mapping) and boundary_class != "gm_judgment":
            ...
            # GM-private judgment frontiers must never leak into the player's
            # presentation/continuity owner. They are returned only in the
            # private command receipt/result for ChatGPT orchestration.
        ...
        if boundary_summary.get("interrupted") and isinstance(boundary_event, Mapping):
            ...
            interrupting = [row for row in handoffs if row.get("handoff", {}).get("interrupts_event_seeking")][:16]
            if boundary_class == "gm_judgment":
                result["gm_boundary_kind"] = "gm_judgment"
                result["gm_handoffs"] = [row for row in interrupting if row.get("handoff", {}).get("class") == "gm_judgment"]
                result["continuation_required"] = boundary < requested_target
                if result["continuation_required"]:
                    result["continuation_reason"] = "gm_judgment_required"
                    result["continuation_target"] = str(target)
            else:
                result["player_boundary_kind"] = boundary_class or "soft_player_facing"
                result["player_handoffs"] = [row for row in interrupting if row.get("handoff", {}).get("class") != "gm_judgment"]
        return _BuiltPlan(
