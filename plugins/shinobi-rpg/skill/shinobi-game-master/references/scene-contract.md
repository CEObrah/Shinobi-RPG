...
Persist an important combat-side line against the exact active combat ref when continuity benefits from it. An established same-side ally may remain the exact `speaker_ref` while the active combat ref is the session owner; never substitute or expose a hidden opposing individual for group-addressed hostile speech. Speech alone cannot establish ceasefire, surrender, custody, injury, movement, transfer, payment, or another hard consequence; those still require their actual mechanics.

For route extortion, GM-private settlement terms may guide coherent bargaining but are not Wei knowledge and are not binding merely because an opponent states a number. If Wei chooses to pay, use the route-contact settlement authority so cash, passage, contact closure, and the attackers' physical return are one conserved consequence. The settlement spends only cash the acting player mechanically owns at that moment. An ally saying **I can cover the difference** is reversible willingness, not pooled purchasing power. When an exact co-located ally explicitly offers a precise amount that may later be accepted, persist that line as `nonbinding_proposal` with authority-false `proposal_terms` carrying the exact `cash` and intended `recipient_ref`. If Wei accepts while that same scene/combat owner is live, route the hard transfer through `jianghu_property_transfer_resolution` with `action: accept_cash_offer`, the donor as `other_ref`, and the exact speech `basis_ref`; that command must revalidate amount, recipient, donor funds, co-presence, and live-session continuity. Only after that transfer may the route settlement spend the now-player-owned cash. Never infer an offer amount from prose or treat authority-false speech itself as a resource transfer.
