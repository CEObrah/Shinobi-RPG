...
loaded. It migrates the legacy yearly named-person wake to true calendar-month
recurrence, annotates conserved Academy graduation flow, and settles aggregate
Genin -> Chunin -> Jonin throughput in chronological order. Exact named rank
changes remain exclusively under the ordinary evidence/authority career reducer.
"""
...
from shinobi_runtime.sim.scheduler import CausalSchedulerRegistry, recurring_event

_CAREER_PATH = "state/reg/shinobi-career-pipeline.json"
_CAREER_RULES_PATH = "game/rules/career/progression.json"
_CONTINUITY_HOST_ID = "host.person_continuity"
_CONTINUITY_EVENT_KIND = "person_continuity.periodic_review"
_PPM = 1_000_000
_MAX_HISTORY = 512
_INSTALLED = False


def _normalize_scheduler_record(record: Mapping[str, Any]) -> Mapping[str, Any]:
    """Replace the one legacy yearly continuity wake with monthly recurrence."""
    if not isinstance(record, Mapping):
        return record
    updated = copy.deepcopy(dict(record))
    hosts = updated.get("hosts")
    events = updated.get("events")
    if not isinstance(hosts, dict) or not isinstance(events, list):
        return updated
    wrapper = hosts.get(_CONTINUITY_HOST_ID)
    ...
    matches = [
        ...
        and event.get("target_host") == _CONTINUITY_HOST_ID
        and event.get("kind") == _CONTINUITY_EVENT_KIND
    ]
    if len(matches) != 1:
        return updated
    ...
    if not isinstance(recurrence, Mapping):
        return updated
    if recurrence.get("kind") == "calendar_month_start":
        return updated
    if recurrence.get("kind") != "calendar_year_start":
        return updated
    clock = recurrence.get("clock", "00:00:00")
    if not isinstance(clock, str) or len(clock.split(":")) != 3:
        return updated
    try:
        hour, minute, second = (int(part) for part in clock.split(":"))
        resolved = CampaignTime.parse(state.get("resolved_through"))
        due = resolved.next_month_start(hour, minute, second)
    except (TypeError, ValueError):
        return updated
    identity = payload.get("identity")
    if not isinstance(identity, str) or not identity:
        return updated
    monthly = dict(recurrence)
    monthly["kind"] = "calendar_month_start"
    successor = recurring_event(
        kind=_CONTINUITY_EVENT_KIND,
        identity=identity,
        host_id=_CONTINUITY_HOST_ID,
        due_at=due,
        recurrence=monthly,
        payload={
            key: value
        ...
        visibility=event.get("visibility", "hidden"),
        requires_player=event.get("requires_player", False),
    )
    replacement = dict(successor.to_record())
    replacement["causation_id"] = event.get("causation_id")
    replacement["correlation_id"] = event.get("correlation_id")
    events[index] = replacement
    state["next_due"] = str(due)
    state["safe_through"] = str(due.add_seconds(-1))
    return updated
...
def _install_monthly_continuity_migration() -> None:
    original = CausalSchedulerRegistry.from_record
    marker_target = getattr(original, "__func__", original)
    if getattr(marker_target, "_monthly_continuity_migration", False):
        return

    def wrapped(
        cls: type[CausalSchedulerRegistry], record: Mapping[str, Any]
    ) -> CausalSchedulerRegistry:
        return original(_normalize_scheduler_record(record))

    wrapped._monthly_continuity_migration = True  # type: ignore[attr-defined]
    CausalSchedulerRegistry.from_record = classmethod(wrapped)
...
def install_shinobi_career_progression() -> None:
    ...
    if _INSTALLED:
        return
    _install_monthly_continuity_migration()
