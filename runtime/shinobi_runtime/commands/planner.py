...
class RepositoryCommandPlanner(JianghuSceneCommandsMixin,JianghuInstitutionalEscortCommandsMixin,JianghuInstitutionalCommandsMixin,JianghuDevelopmentCommandsMixin,JianghuRetinueCommandsMixin,JianghuTravelTeamCommandsMixin,JianghuSocialCommandsMixin,JianghuInformationCommandsMixin,JianghuCalendarCommandsMixin,JianghuFactionLifecycleCommandsMixin,JianghuExtendedCommandsMixin,JianghuContractCommandsMixin,JianghuCommandsMixin,JianghuTimeCommandsMixin):
    ...
    def __init__(self, repository: RepositoryStore, *, meta_path="state/meta.json", scene_path="state/scene.json", **_ignored):
        ...
        # Keep only the most recently validated deterministic build. A normal
        # gameplay write is previewed and then planned at the same campaign
        # revision; reusing that exact build both avoids duplicate long-horizon
    ...
    def _build(self,command:CommandEnvelope)->_BuiltPlan:
        ...
        # from presentation-only scene JSON. Player interaction attempts and one
        # presentation-only opposing-side speech record are the narrow non-combat
        # exceptions. Neither path pauses, resolves, or otherwise alters combat.
        active_combat=active_combat_for_person(self.repository.read_json,command.actor_id)
        combat_allowed={"jianghu_combat_resolution","jianghu_interaction_resolution","jianghu_route_contact_settlement_resolution"}
        combat_scene_speech=False
        if active_combat is not None and command.command_type=="jianghu_scene_session_resolution":
            active_ref=str(active_combat[0])
            # Reversible speech may be persisted against the exact combat owner
            # without pausing the fight. The scene reducer separately proves that
            # an exact person speaker is an arrived, player-established ally; an
            # opposing side still uses the opaque combat ref so hidden identities
            # cannot be probed through this gate.
            combat_scene_speech=(
                command.payload.get("action")=="record_speech"
                and command.payload.get("session_ref")==active_ref
            )
        if active_combat is not None and command.command_type not in combat_allowed and not combat_scene_speech:
    ...
    def _validate_preview_plan(self,command:CommandEnvelope,built:_BuiltPlan)->None:
        ...
        try:
            ...
            validate_active_custody_uniqueness(
                overlay.read_json("state/martial-world/custody.json")
            )
    ...
    def preview(self,command:CommandEnvelope)->CommandPreview:
        ...
        self._validate_preview_plan(command,built)
        self._validated_preview_build=(command.digest,command.expected_revision,built)
        return CommandPreview(status="ready",code=built.code,target_revision=command.expected_revision+1,affected_refs=built.affected_refs)
    def plan(self,command:CommandEnvelope)->CommandPlan:
        cached=self._validated_preview_build
        if cached is not None and cached[0]==command.digest and cached[1]==command.expected_revision:
            # Fail closed if some other writer moved the campaign after preview.
            self.repository.require_revision(command.expected_revision,self.meta_path)
            built=cached[2]
            self._validated_preview_build=None
        else:
            built=self._build(command)
        def validate(overlay: StagedOverlay, manifest: TransactionManifest) -> None:
            ...
            built.validator(overlay, manifest)
        return CommandPlan(transaction_id=("tx.autonomous." if command.mode=="autonomous" else "tx.gameplay.")+command.digest,created_at=command.submitted_at,writes=built.writes,result=built.result,validator=validate)
