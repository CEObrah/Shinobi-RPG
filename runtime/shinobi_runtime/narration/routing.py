...
def select_narration_modules(
    ...
    *,
    ...
    Matching is exact after identifier normalization. An absent scene type uses
    the router's declared default primary; an explicitly supplied malformed
    scene type still fails closed. Unknown pressure prose is ignored; substrings
    such as ``command`` never activate large-war narration. Ambiguous pressure
    overrides fail closed instead of depending on input order.
    """
    ...
    if scene_type is None:
        scene_type_matched = False
        base_primary = default_primary
    else:
