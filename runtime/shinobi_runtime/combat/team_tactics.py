...
def plan_team_exchange(
    *,
    ...
    # Team roles assume complementary actors. A one-person side cannot be an
    # ``anchor`` waiting for a nonexistent pressure/flank/exploit partner. For
    # offensive/capture objectives, give the sole combatant an individually
    # executable active role; protect/extract may still lawfully hold.
    if len(members) == 1 and roles and objective_kind not in {"protect", "extract"}:
        if roles[0] in {"anchor", "screen", "protect", "reserve"}:
            roles[0] = "control" if objective_kind == "capture" else "pressure"
    ...
    # Melee frontage is planning law, not a production-time monkey patch.
    from shinobi_runtime.combat.frontage_targeting import apply_frontage_to_plan
    return apply_frontage_to_plan(plan,known_enemy_refs=enemies,records=records,positions=positions)
...
def plan_player_retinue_exchange(
    *,
    ...
    # Standing retinue professions are capabilities/responsibilities, not combat
    # posture locks. Protective guards still receive first interception duty; all
    # other companions are ordered by actual combat threat/capability so a medic
    # is neither parked out of the fight nor preferentially fed the second-hardest
    # opponent merely because of their medical profession.
    if guards_first:
        ordered_companions = sorted(
            companions,
            key=lambda ref: (
                0 if str(roles.get(ref) or 'companion') == 'protective_guard' else 1,
                -threat_score(records[ref]),
                ref,
            ),
        )
    ...
    for companion_ref, target_ref in zip(ordered_companions, initial_targets):
        ...
        tactical_role = {
            ...
            # Medicine is a support capability, not a standing combat posture.
            # A field medic fights normally until an explicit treatment/extract
            # order or another active support task removes them from offense.
            'field_medic': 'pressure',
