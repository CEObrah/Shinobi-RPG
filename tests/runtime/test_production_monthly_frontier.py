...
assert plan.result.get("processed_causal_events"), plan.result
...
def test_real_campaign_next_causal_frontier_validates_through_production_extensions() -> None:
    env = os.environ.copy()
    ...
    assert completed.returncode == 0, (
        "production causal frontier regression failed\n"
        f"stdout:\n{completed.stdout}\n"
        f"stderr:\n{completed.stderr}"
    )
