...
def _loss_count(actor_active: int, target_active: int, force_margin: int) -> int:
    exposed = min(actor_active, target_active)
...
def _capture_count(actor_active: int, target_active: int, capture_margin: int) -> int:
    exposed = min(actor_active, target_active)
...
def _apply_attack(
    ...
    losses = _loss_count(actor.active, target.active, force_margin)
    ...
    for engagement in ordered_engagements:
        ...
        if actor.active <= 0:
            ...
            if target.active <= 0:
                ...
                captured = _capture_count(actor.active, target.active, capture_margin)
