...
from shinobi_runtime.commands import combat_adaptation as adaptation
from shinobi_runtime.commands import combat_span_safety as safety


def _prepare_static_span(monkeypatch, *, base_span):
    import shinobi_runtime.commands.jianghu_extended as extended

    monkeypatch.setattr(extended, "_resolve_player_combat_span_base", base_span)
    return extended
...
def test_production_adaptation_preserves_improvised_weapon_span(monkeypatch):
    ...
    extended = _prepare_static_span(monkeypatch, base_span=base_span)
    monkeypatch.setattr(safety, "bounded_standing_span", bounded)
    monkeypatch.setattr(adaptation, "adaptive_standing_span", adaptive)
