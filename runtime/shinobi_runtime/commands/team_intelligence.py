...
class TeamIntelligenceMixin:
    ...
    MAX_AUTONOMOUS_CANDIDATES = 128
    MAX_ACTIVE_TEAM_SCAN = 256
    ...
    def _active_exact_team_members(self, record_writes: Mapping[str, Mapping[str, Any]]) -> set[str]:
        members: set[str] = set()
        try:
            registry = self.repository.read_json("state/team/registry.json")
        except (FileNotFoundError, ValueError):
            registry = {}
        refs = registry.get("active_teams") if isinstance(registry, Mapping) else []
        if isinstance(refs, list):
            for team_ref in [x for x in refs if isinstance(x, str)][: self.MAX_ACTIVE_TEAM_SCAN]:
                try:
                    _path, team = self._exact_team(team_ref)
                except CommandRejectedError:
                    continue
                if team.get("status") == "active":
                    members.update(x for x in team.get("member_refs", []) if isinstance(x, str))
        for record in record_writes.values():
            if isinstance(record, Mapping) and record.get("schema") == "exact-team" and record.get("status") == "active":
                members.update(x for x in record.get("member_refs", []) if isinstance(x, str))
        return members
    ...
    def _candidate_refs(self, payload: Mapping[str, Any], faction_record: Mapping[str, Any], spec: Mapping[str, Any]) -> Tuple[str, ...]:
        ...
        force_roster: list[str] = []
        force_ref = payload.get("force_ref")
        if isinstance(force_ref, str):
            try:
                _path, _digest, force = self._resolve_covered_owner_view(force_ref, cache=_OwnerResolutionCache())
            except CommandRejectedError:
                force = {}
            pool_id = force.get("population_pool_id") if isinstance(force, Mapping) else None
            if isinstance(pool_id, str):
                try:
                    population = self.repository.read_json(_POPULATION_REGISTRY_PATH)
                except (FileNotFoundError, ValueError):
                    population = {}
                pools = population.get("pools") if isinstance(population, Mapping) else {}
                pool = pools.get(pool_id) if isinstance(pools, Mapping) else {}
                rep = pool.get("representation") if isinstance(pool, Mapping) else {}
                rostered = rep.get("rostered_person_refs") if isinstance(rep, Mapping) else []
                if isinstance(rostered, list):
                    force_roster = [x for x in rostered if isinstance(x, str)]
        ordered: list[str] = []
        for ref in preferred + explicit + leadership + key_members + sorted(force_roster):
            if ref not in ordered:
                ordered.append(ref)
            if len(ordered) >= self.MAX_AUTONOMOUS_CANDIDATES:
                break
    ...
    def _apply_autonomous_decision(self, *, decision: Any, at: CampaignTime, command: CommandEnvelope, scheduler: CausalSchedulerRegistry, world_events: Dict[str, Any], record_writes: Dict[str, Dict[str, Any]], faction_record: Dict[str, Any]) -> Mapping[str, Any]:
        if decision.kind == "team_form":
            ...
            active_members = set() if spec.get("allow_existing_team_members") is True else self._active_exact_team_members(record_writes)
