...
from .faction_relations import (
    ...
    friendly_aid_transfer_envelope, resolve_friendly_aid_transfer, settle_positive_obligation,
)
from .escort_living_world import principal_ransom_value_cash
from .escort import cargo_unit_mass_grams
from .captivity_lifecycle import close_kin_refs, rescue_force_size
...
from .route_activity import ROUTE_PRESENCE_STATUSES, ROUTE_SERVICE_STATUSES, route_controlling_refs
    ...
    *,
    ...
    def active_route_person_refs() -> set[str]:
        ...
        for movement in movements.values():
            if not isinstance(movement, Mapping) or str(movement.get("status") or "") not in ROUTE_PRESENCE_STATUSES:
                continue
    ...
    def start_strategic_operation(fid: str, intent: Mapping[str, Any]) -> dict[str, Any]:
        ...
        available_by_ref = {
            str(p.get("person_id")): p for p in available
            if isinstance(p.get("person_id"), str) and str(p.get("person_id")) != player_ref
        }
        authored_participants = [
            str(ref) for ref in intent.get("participant_refs", [])
            if isinstance(ref, str) and ref
        ]
        leader_ref = str(intent.get("leader_ref") or "")
        if not authored_participants or len(authored_participants) != len(set(authored_participants)):
            return {"result": "gm_authored_detachment_required"}
        if any(ref == player_ref or ref not in available_by_ref for ref in authored_participants):
            return {"result": "participants_unavailable"}
        if not leader_ref or leader_ref not in authored_participants:
            return {"result": "gm_authored_leader_required"}
        operation_intent = str(intent.get("operation_intent") or "")
        if not operation_intent:
            return {"result": "gm_authored_operation_intent_required"}
        combat_policy = intent.get("combat_policy") if isinstance(intent.get("combat_policy"), Mapping) else None
        if not isinstance(combat_policy, Mapping) or str(combat_policy.get("decision_origin") or "") != "gm_npc_judgment":
            return {"result": "gm_authored_combat_policy_required"}
        targeting_intent = str(combat_policy.get("targeting_intent") or "")
        if targeting_intent not in {"disable", "lethal"}:
            return {"result": "gm_authored_targeting_intent_required"}
        ...
        if kind == "formal_challenge":
            ...
            if kind == "faction_war_strike":
                ...
                maximum = min(deployable, max(3, len(available) * fraction // 100))
            else:
                coordination_scale = max(3, int(len(available) ** 0.5) * max(80, 80 + risk) // 100)
                maximum = min(deployable, coordination_scale)
        if maximum <= 0:
            return {"result": "insufficient_available_fighters"}
        participants = list(authored_participants)
        if len(participants) > maximum:
            return {"result": "gm_authored_detachment_exceeds_current_capacity", "maximum_count": maximum}
        if kind == "formal_challenge" and len(participants) != 1:
            return {"result": "formal_challenge_requires_single_challenger"}
        ...
        try:
            commitments_state = reserve_resources(
                ...
                actor_ref=leader_ref, owner_ref=fid, activity_ref=op_ref,
        ...
        deployments[op_ref] = {
            ...
            "status": "mobilizing",
            "mobilization_basis": "gm_authored_exact_detachment",
            "arrival_event_kind": "faction_operation_arrival",
            "operation_intent": operation_intent,
            "targeting_intent": targeting_intent,
            "combat_policy": copy.deepcopy(dict(combat_policy)),
        }
    ...
    def start_custody_rescue_operation(responder_fid: str, custody_record: Mapping[str, Any], authored_choice: Mapping[str, Any]) -> dict[str, Any]:
        ...
        advisory_size = rescue_force_size(
        ...
        force_scale = str(authored_choice.get("force_scale") or "")
        selection_policy = str(authored_choice.get("selection_policy") or "")
        if force_scale == "limited":
            desired = max(1, min(len(available), (advisory_size + 1) // 2))
        elif force_scale == "measured":
            desired = max(1, min(len(available), advisory_size))
        elif force_scale == "maximum":
            desired = len(available)
        else:
            return {"result": "rescue_commitment_invalid"}
        if selection_policy == "kin_priority":
            available.sort(key=lambda p: (
                0 if str(p.get("person_id") or "") in kin_refs else 1,
                -person_combat_index(p), str(p.get("person_id") or ""),
            ))
        elif selection_policy == "combat_readiness":
            available.sort(key=lambda p: (-person_combat_index(p), str(p.get("person_id") or "")))
        else:
            return {"result": "rescue_selection_policy_invalid"}
        ...
        deployments[op_ref] = {
            ...
            "combat_policy": copy.deepcopy(dict(authored_choice.get("combat_policy") or {
                "decision_origin":"gm_npc_judgment",
                "engagement_intent":"recover_captive",
                "targeting_intent":"disable",
    ...
    def execute_friendly_aid(fid: str, target_fid: str, amount_cash: int) -> dict[str, Any]:
        ...
        transfer = resolve_friendly_aid_transfer(
            faction, inventory, target, target_inventory, amount_cash=max(0, int(amount_cash)),
        )
    ...
    def close_dead_current_authorities(dead_refs: Sequence[str]) -> None:
        ...
        for fid, local_dead in by_faction.items():
            ...
            office_judgments = institutional_office_judgment_events(
                ...
                source_event_id=f"world-death:{fid}:{at_iso}",
            )
            pending_one_off_events.extend(office_judgments)
            handoffs.extend(judgment_handoff(event) for event in office_judgments)
        ...
        # Civic vacancy discovery is immediate, but a voluntary successor
        # appointment is a GM judgment. Materialization/civilian conservation
        # still belongs to the life helper; appointment happens only after the
        # staged exact choice is committed.
        if dead_civic_rows:
            civic_rows = [
            ...
            for judgment in civic_judgments:
                event = build_npc_judgment_event(
                    at_iso=at_iso,
                    judgment_kind="civic_office_appointment",
                    actor_ref=str(judgment.get("affiliation_ref") or "civic_institution"),
                    options=judgment.get("options", []),
                    decision_context={
                        "office": judgment.get("office"),
                        "affiliation_ref": judgment.get("affiliation_ref"),
                        "vacated_person_ref": judgment.get("vacated_person_ref"),
                        "source": "world_death_cleanup",
                    },
                    source_event_id=f"world-civic-death:{judgment.get('vacated_person_ref')}:{at_iso}",
                )
    ...
    settle_captivity_frontier(
        events=sorted_events, at=at, world_seed=world_seed, player_ref=player_ref, family_state=family_state,
    ...
    autonomy_result = settle_faction_autonomy_frontier(
        ...
        execute_friendly_aid=execute_friendly_aid, handoffs=handoffs, pending_one_off_events=pending_one_off_events,
    ...
    route_judgment_events = [
        ...
        if e.get("kind") == "npc_judgment_committed" and e.get("judgment_kind") == "route_interception_policy"
    ]
    ...
    _settle_tournament_phase([
        ...
        if event.get("kind") in {
            ...
            event.get("kind") == "npc_judgment_committed"
            and event.get("judgment_kind") == "tournament_participation_policy"
        )
    ])
