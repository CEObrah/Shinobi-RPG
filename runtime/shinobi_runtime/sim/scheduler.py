...
def settle_scheduler(
    ...
    *,
    ...
    if snapshot is not None and hasattr(snapshot, "root"):
        initial_host_ids = set(getattr(snapshot, "initial_hosts", {}))
        final_host_ids = set(registry.hosts)
        total_host_count = int(snapshot.root.get("host_count", len(initial_host_ids)))
        total_host_count += len(final_host_ids - initial_host_ids) - len(initial_host_ids - final_host_ids)
        initial_event_count = len(getattr(snapshot, "initial_events", {}))
        total_pending_event_count = int(snapshot.root.get("pending_event_count", initial_event_count))
        total_pending_event_count += len(registry.queue) - initial_event_count
    else:
        total_host_count = len(registry.hosts)
        total_pending_event_count = len(registry.queue)
    registry.metrics = {
        **registry.metrics,
        ...
        "pending_event_count": total_pending_event_count,
