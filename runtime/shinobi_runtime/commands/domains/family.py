        ...
        if action == "propose":
            ...
            if result_status == "accepted":
                try:
                    ...
                    player_ref = None
                participants_for_queue = (proposal.get("proposer_id"), proposal.get("target_id"))
                if player_ref not in participants_for_queue and all(isinstance(ref, str) for ref in participants_for_queue):
                    queue = family.get("autonomy_queue_refs")
                    if not isinstance(queue, list):
                        raise CommandRejectedError("family_index_invalid")
                    self._append_unique(queue, proposal_id)
            event_type = {
