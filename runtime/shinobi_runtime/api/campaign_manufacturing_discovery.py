...
class RouteAwareCampaignOperations(_Base):
    ...
    def _manufacturing_catalog(self) -> tuple[list[Mapping[str, Any]], int, int]:
        try:
        ...
        recipe_items = sorted(recipes.items())
        for recipe_ref, recipe in recipe_items[:64]:
        ...
        return rows, weekly_hours, len(recipe_items)

    def _growth_discovery(self, player_id: str) -> Mapping[str, Any]:
        result = dict(super()._growth_discovery(player_id))
        recipes, weekly_hours, recipe_count = self._manufacturing_catalog()
        ...
        for row in houses:
            ...
            updated["manufacturing_recipe_count"] = recipe_count
            updated["manufacturing_recipes_truncated"] = recipe_count > len(recipes)
    ...
    def inspect_game_object(self, object_ref: str) -> Mapping[str, Any]:
        ...
        updated["manufacturing_interface"] = {
            ...
            "candidate_recipes_truncated": recipe_count > len(recipes),
            "standing_weekly_active_hours": weekly_hours,
