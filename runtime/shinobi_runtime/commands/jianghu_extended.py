...
from shinobi_runtime.martial_world.poison import active_qi_purge
...
from shinobi_runtime.martial_world.property import (
    ...
    provenance_claim, set_nonholder_claim, validate_property_evidence,
)
from shinobi_runtime.martial_world.faction_relations import evaluate_proposal, apply_relation_event, proposal_kind_supported
from shinobi_runtime.martial_world.family_life import courtship_eligible, marriage_eligible
from shinobi_runtime.martial_world.crime_custody import create_custody_record, custody_transition, crime_attention
from shinobi_runtime.martial_world.agency import factual_restraint_basis
from shinobi_runtime.martial_world.exact_combat import initialize_combat, resolve_exchange, attempt_disengage
...
from shinobi_runtime.martial_world.faction_state import (
    faction_path as canonical_faction_path, inventory_path as canonical_inventory_path,
...
from shinobi_runtime.martial_world.family_simulation import apply_family_death_status, apply_recognized_succession
from shinobi_runtime.martial_world.field_command import build_deployment_structure, validate_deployment_structure
from shinobi_runtime.martial_world.commitments import release_resources
from shinobi_runtime.martial_world.security import alarm_response_seconds, forced_entry_resolution, infiltration_resolution, select_watch_guards
...
from shinobi_runtime.martial_world.infrastructure import (
    ...
    enterprise_scale_value, enterprise_operating_efficiency_milli,
)
...
_LIBRARY_HOLDINGS='state/martial-world/library-holdings.json'
...
def _present(scene:Mapping[str,Any],ref:str)->bool:
    return ref in set(scene.get('present_person_ids',[]))|set(scene.get('visible_person_ids',[]))
...
class JianghuExtendedCommandsMixin:
    def _person_present_for_command(self, scene: Mapping[str,Any], person_ref: str, current_time: CampaignTime) -> bool:
        if _present(scene,person_ref):
            return True
        ...
        try:
            ...
            return person_attends_site(
                ...
                sites=sites,at=_dt(current_time),unavailable_refs=set(),
            )
    ...
    def _cleanup_command_deaths(self,writes:Mapping[str,Any],dead_refs:set[str],current_time:CampaignTime)->dict[str,Any]:
        ...
        commitments=copy.deepcopy(out.get(_COMMITMENTS) or self.repository.read_json(_COMMITMENTS))
        ...
        # Family, offices and recognized hereditary succession are resolved
        # from the command's roster after-image, not from a stale repository
        # snapshot.
        for fid,refs in faction_dead.items():
            ...
            family=apply_family_death_status(family,dead_refs=sorted(refs),faction_ref=fid,roster_people=[p for p in cleaned if isinstance(p,Mapping)])
            succession=apply_recognized_succession(family,faction_ref=fid,roster_people=[p for p in cleaned if isinstance(p,Mapping)],year=current_time.year)
        ...
        courtships=social.get('courtships',{}) if isinstance(social,Mapping) else {}
        ...
        if isinstance(relationships,dict):
            for key in list(relationships):
                if any(part in dead for part in str(key).split('|',1)):relationships.pop(key,None)
        out[_SOCIAL]=social

        prior_rows=[row for row in custody.get('records',[]) if isinstance(row,Mapping)] if isinstance(custody,Mapping) else []
        released_by_captor_death={str(row.get('person_ref')) for row in prior_rows if str(row.get('captor_ref')) in dead and isinstance(row.get('person_ref'),str) and str(row.get('person_ref')) not in dead}
        custody['records']=[row for row in prior_rows if str(row.get('person_ref')) not in dead and str(row.get('captor_ref')) not in dead]
        ...
        if isinstance(commit_rows,dict) and isinstance(person_index,dict):
            ...
            commitments['commitments']=commit_rows;commitments['person_index']=person_index
            out[_COMMITMENTS]=commitments

        # Temporary deployments restructure immediately after casualties. The
        ...
        if isinstance(dep_rows,dict) and isinstance(commit_rows,dict) and isinstance(person_index,dict):
            ...
            out[_DEPLOYMENTS]=deployments; out[_COMMITMENTS]=commitments
    ...
    def _jianghu_medicine_resolution(self,command:CommandEnvelope,meta:Mapping[str,Any],current_time:CampaignTime):
        ...
        if action=='qi_purge':
            ...
            burden=max(0,int(burdens.get(poison_ref,0)))
            if burden<=0:raise CommandRejectedError('jianghu_qi_purge_no_burden')
            result=active_qi_purge(poison_ref=poison_ref,burden=burden,current_qi=int(subject.get('current_qi',subject.get('qi',0))),qi=int(subject.get('qi',0)),qi_control=int(subject.get('qi_control',0)),elapsed_minutes=minutes)
            time_plan,extra,_target=self._timed_person_activity_plan(
                ...
                owner_ref=subject_ref,location_ref=str(subject.get('location_ref') or ''),
            )
            ...
            final_person=copy.deepcopy(dict(rows[idx])); final_burdens=copy.deepcopy(dict(final_person.get('poison_burdens',{}))) if isinstance(final_person.get('poison_burdens'),Mapping) else {}
            if int(result['burden_after'])>0:final_burdens[poison_ref]=int(result['burden_after'])
            else:final_burdens.pop(poison_ref,None)
            if final_burdens:final_person['poison_burdens']=final_burdens
            else:final_person.pop('poison_burdens',None)
            final_person['current_qi']=int(result['current_qi_after'])
            final_roster=set_roster_person(final_roster,idx,final_person); extra[rpath]=final_roster
            return self._combine_time_plan(command,time_plan,extra_records=extra,code='jianghu_qi_purge_completed',result={'command_type':command.command_type,'subject_ref':subject_ref,'poison_ref':poison_ref,'burden_before':result['burden_before'],'burden_after':result['burden_after'],'qi_spent':result['qi_spent'],'elapsed_minutes':minutes})
        ...
        if facility_site and str(actor.get('location_ref') or '')!=facility_site:raise CommandRejectedError('jianghu_medicine_requires_faction_infirmary')
        scene=self.repository.read_json(self.scene_path)
        if subject_ref!=command.actor_id and not self._person_present_for_command(scene,subject_ref,current_time):raise CommandRejectedError('jianghu_medicine_subject_not_present')
        ...
        if category in {'wound','bone','internal'}:
            ...
            if candidates:
                _cleared,pref,treatment=max(candidates,key=lambda x:(x[0],x[1])); next_b=copy.deepcopy(dict(burdens)); after=int(treatment['burden_after'])
                if after>0:next_b[pref]=after
                else:next_b.pop(pref,None)
                if next_b:subject['poison_burdens']=next_b
        ...
        if int(consequences['organ_stress'])>0:
            injuries=list(health.get('injuries',[])); injuries.append({'zone':'abdomen','created_at':str(current_time).removeprefix('SE-'),'cut':0,'pierce':0,'blunt':0,'penetration':0,'severity':min(200,int(consequences['organ_stress'])),'bleeding_ml_per_min':0,'fracture':0,'tendon_damage':0,'nerve_damage':0,'organ_trauma':min(200,int(consequences['organ_stress'])),'pain':min(200,int(consequences['organ_stress'])//2),'function_loss_pct':min(100,int(consequences['organ_stress'])//3),'treated':False,'healing_progress_milli':0}); health['injuries']=injuries
    ...
    def _jianghu_library_research_resolution(self,command:CommandEnvelope,meta:Mapping[str,Any],current_time:CampaignTime):
        ...
        _fp,faction=read_faction(self.repository,faction_ref)
        if str(actor.get('location_ref') or '')!=str(faction.get('local_site_ref') or ''):raise CommandRejectedError('jianghu_library_research_requires_faction_library')
        level=int(faction.get('buildings',{}).get('library_records',0))
        if level<=0:raise CommandRejectedError('jianghu_library_unavailable')
        holdings=self.repository.read_json(_LIBRARY_HOLDINGS);row=holdings.get('factions',{}).get(faction_ref,{}) if isinstance(holdings.get('factions'),Mapping) else {}
        held={str(x) for x in row.get('record_refs',[]) if isinstance(x,str)} if isinstance(row,Mapping) and isinstance(row.get('record_refs'),list) else set()
        ...
        time_plan,extra,_target=self._timed_person_activity_plan(
            ...
            activity_ref=f'research:{command.request_id}',activity_kind='library_research',owner_ref=faction_ref,location_ref=str(actor.get('location_ref') or ''),
        )
    ...
    def _jianghu_security_resolution(self,command:CommandEnvelope,meta:Mapping[str,Any],current_time:CampaignTime):
        ...
        if action not in {'infiltrate','force_entry'} or not target_faction_ref:
        ...
        sites_data=self.repository.read_json(_LOCAL_SITES);sites=sites_data.get('sites',{}) if isinstance(sites_data,Mapping) else {}
        actor_site=sites.get(str(actor.get('location_ref') or '')) if isinstance(sites,Mapping) else None
        ...
        if str(actor.get('location_ref') or '')==target_site:raise CommandRejectedError('jianghu_security_actor_already_inside')
        ...
        guards=select_watch_guards(
            ...
            buildings=target_faction.get('buildings',{}),infrastructure=target_faction.get('infrastructure',{}),unavailable_refs=self._unavailable_person_refs(),
        ...
        if action=='infiltrate':
            outcome=infiltration_resolution(
                intruder=actor,guards=guards,buildings=target_faction.get('buildings',{}),infrastructure=target_faction.get('infrastructure',{}),
                lighting_milli=550 if current_time.hour<6 or current_time.hour>=20 else 1000,
                weather_visibility_milli=1000,concealment_milli=0,
            )
            ...
            outcome=forced_entry_resolution(intruder=actor,buildings=target_faction.get('buildings',{}),infrastructure=target_faction.get('infrastructure',{}),weapon_impact=impact)
            response=alarm_response_seconds(target_faction.get('buildings',{}),guards,infrastructure=target_faction.get('infrastructure',{}))
        ...
        time_plan,extra,target_time=self._timed_person_activity_plan(
            ...
            owner_ref=command.actor_id,location_ref=str(actor.get('location_ref') or ''),
        )
        ...
        # Detection produces real contact after the guards physically respond.
        # Start one exact combat with the actual on-duty defenders instead of
        # teleporting the whole faction to the wall.
        guard_refs=[str(g.get('person_id')) for g in guards[:3] if isinstance(g.get('person_id'),str)]
        if not guard_refs:
        ...
        scene['location_id']=target_site;scene['active_combat_ref']=combat_ref;scene['present_person_ids']=[command.actor_id,*alive_guards];scene['visible_person_ids']=[command.actor_id,*alive_guards]
        return self._combine_time_plan(command,time_plan,extra_records=extra,code='jianghu_security_detected_contact',result={'command_type':command.command_type,'action':action,'target_faction_ref':target_faction_ref,'outcome':outcome,'combat_ref':combat_ref,'guard_refs':alive_guards},scene_override=scene)
    ...
    def _jianghu_market_trade_resolution(self,command:CommandEnvelope,meta:Mapping[str,Any],current_time:CampaignTime):
        ...
        scene=self.repository.read_json(self.scene_path); site_ref=str(scene.get('location_id') or '')
    ...
    def _jianghu_property_transfer_resolution(self,command:CommandEnvelope,meta:Mapping[str,Any],current_time:CampaignTime):
        ...
        same_physical_place=(
            _present(scene,other_ref)
            or (str(actor.get('location_ref') or '') and str(actor.get('location_ref') or '')==str(target.get('location_ref') or ''))
        )
    ...
    def _jianghu_production_resolution(self,command:CommandEnvelope,meta:Mapping[str,Any],current_time:CampaignTime):
        ...
        if actor.get('faction_ref')!=faction_ref or not _office(actor,'leader','deputy_leader','chief_steward','treasurer','quartermaster','master_weaponsmith','chief_apothecary','field_commander'):raise CommandRejectedError('jianghu_production_not_authorized')
        self._require_person_available_for_activity(command.actor_id)
        ...
        if facility_site and str(actor.get('location_ref') or '')!=facility_site:raise CommandRejectedError('jianghu_production_requires_faction_facility')
        ...
        def qualified_workers(skill_key:str):
            ...
            for raw in people:
                ...
                if facility_site and str(person.get('location_ref') or '')!=facility_site:continue
        ...
        time_plan,extra,_target=self._timed_person_activity_plan(
            ...
            owner_ref=faction_ref,location_ref=str(worker.get('location_ref') or facility_site),
    ...
    def _jianghu_strategic_travel_resolution(self,command:CommandEnvelope,meta:Mapping[str,Any],current_time:CampaignTime):
        destination=str(command.payload.get('destination_site_ref')); mode=str(command.payload.get('mode'))
        if mode not in {'foot','horse','pack'}:raise CommandRejectedError('jianghu_travel_mode_invalid')
        rpath,roster,ordinal,person=self._person(command.actor_id)
        self._require_person_available_for_activity(command.actor_id)
        sites=self.repository.read_json(_LOCAL_SITES).get('sites',{}); start_site=sites.get(person.get('location_ref')); end_site=sites.get(destination)
        if not isinstance(start_site,Mapping) or not isinstance(end_site,Mapping):raise CommandRejectedError('jianghu_travel_site_unresolved')
        start=str(start_site.get('parent_place_ref')); end=str(end_site.get('parent_place_ref'))
        if start==end:raise CommandRejectedError('jianghu_use_local_travel')
        faction_ref=str(person.get('faction_ref') or '')
        if mode in {'horse','pack'}:
            inv=self.repository.read_json(_inventory_path(faction_ref)); key='riding_horses' if mode=='horse' else 'pack_animals'
            ...
            if stock<=0:raise CommandRejectedError('jianghu_transport_asset_unavailable')
        health=person.get('health',{}) if isinstance(person.get('health'),Mapping) else {}
        ...
        if movement_function<=0:raise CommandRejectedError('jianghu_travel_function_unavailable')
        try:
            loadout=effective_person_loadout(self.repository.read_json(_EQUIPMENT),command.actor_id)
            catalog=self.repository.read_json(_EQUIPMENT_DATA)
            ...
            enc=encumbrance_effects(total_mass_kg=mass,strength=int(attrs.get('strength',0)),endurance=int(attrs.get('endurance',0)))
            load_time_milli=max(1000,1_000_000//max(1,int(enc['movement_factor_milli'])))
            if mode in {'horse','pack'}:load_time_milli=1000+max(0,load_time_milli-1000)//2
            # Horse locomotion is supplied by the mount, so rider impairment
            # affects control/endurance less than it affects foot travel. Pack
            # animals carry loads but do not replace the traveler's legs.
            party_speed_milli=max(50,500+mounted_stability//2) if mode=='horse' else max(50,walking)
            plan=travel_plan(world_seed=str(meta.get('world_seed','jianghu')),start_at=_dt(current_time),start=start,end=end,mode=mode,party_speed_milli=party_speed_milli,encumbrance_milli=load_time_milli)
            plan={**plan,'movement_capacity_milli':movement_function,'party_speed_milli':party_speed_milli}
        except (KeyError,ValueError) as exc:raise CommandRejectedError('jianghu_route_unavailable') from exc
        toll=int(plan['toll_cash']); cash=int(person.get('personal_cash',0))
        if cash<toll:raise CommandRejectedError('jianghu_travel_toll_cash_insufficient')
        ...
        for i,segment in enumerate(plan.get('segments',[])):
            ...
            if amount<=0 or i>=len(nodes):continue
        ...
        person['personal_cash']=cash-toll; staged_roster=set_roster_person(copy.deepcopy(roster),ordinal,person); staged_records[rpath]=staged_roster
        ...
        time_plan,extra,_target=self._timed_person_activity_plan(
            ...
            activity_ref=f'strategic-travel:{command.request_id}',activity_kind='travel',
            owner_ref=faction_ref or command.actor_id,location_ref=str(person.get('location_ref') or ''),
            staged_records=staged_records,
        )
        final_roster=copy.deepcopy(dict(extra[rpath])); rows=final_roster.get('people',[])
        if not isinstance(rows,list):raise CommandRejectedError('jianghu_roster_invalid')
        for i,row in enumerate(rows):
            if isinstance(row,Mapping) and row.get('person_id')==command.actor_id:
                updated=copy.deepcopy(dict(row)); updated['location_ref']=destination; rows[i]=updated; break
        else:raise CommandRejectedError('jianghu_person_unresolved')
        extra[rpath]=final_roster
        scene=self._time_after_record(time_plan,self.scene_path,self.repository.read_json(self.scene_path)); scene['location_id']=destination; scene['present_person_ids']=[command.actor_id]; scene['visible_person_ids']=[command.actor_id]
        return self._combine_time_plan(command,time_plan,extra_records=extra,code='jianghu_strategic_travel_completed',result={'command_type':command.command_type,'destination_site_ref':destination,'mode':mode,'distance_km':plan['distance_km'],'toll_cash':toll,'segments':plan['segments'],'carried_mass_kg':mass,'encumbrance':enc},scene_override=scene)
    ...
    def _jianghu_equipment_resolution(self,command:CommandEnvelope,meta:Mapping[str,Any],current_time:CampaignTime):
        ...
        self._require_person_available_for_activity(command.actor_id)
        _fp,faction=read_faction(self.repository,faction_ref); facility_site=str(faction.get('local_site_ref') or '')
        if facility_site and str(actor.get('location_ref') or '')!=facility_site:raise CommandRejectedError('jianghu_equipment_requires_faction_armory')
        if str(actor.get('location_ref') or '')!=str(subject.get('location_ref') or ''):raise CommandRejectedError('jianghu_equipment_subject_not_present')
    ...
    def _jianghu_diplomacy_resolution(self,command:CommandEnvelope,meta:Mapping[str,Any],current_time:CampaignTime):
        ...
        self._require_person_available_for_activity(command.actor_id, allowed_commitment_kinds={'deployment'})
        if not _office(actor,'leader','deputy_leader','chief_steward') and source!='house_tang':raise CommandRejectedError('jianghu_diplomacy_not_authorized')
        ...
        # The active diplomacy surface owns a one-off conserved silver
        # negotiation only. Rejected/countered proposals and already-settled
        # exchanges are ephemeral results, not permanent pseudo-treaties.
        writes={}
        if decision['accept']:
        ...
        return self._simple_plan(command,meta,current_time,writes_records=writes,code='jianghu_diplomacy_resolved',result={'command_type':command.command_type,'proposal':record})
    ...
    def _jianghu_family_resolution(self,command:CommandEnvelope,meta:Mapping[str,Any],current_time:CampaignTime):
        ...
        if str(actor.get('location_ref') or '')!=str(other.get('location_ref') or ''):raise CommandRejectedError('jianghu_family_parties_not_colocated')
        scene=self.repository.read_json(self.scene_path)
        if not self._person_present_for_command(scene,other_ref,current_time):raise CommandRejectedError('jianghu_family_other_party_not_present')
        ...
        existing_marriage=next((m for m in family.get('marriages',{}).values() if isinstance(m,Mapping) and m.get('status')=='married' and set(m.get('spouse_refs',[]))==set((command.actor_id,other_ref))),None) if isinstance(family.get('marriages'),Mapping) else None
        ...
        if action=='courtship':
            ...
            if actor_faction and actor_faction==other_faction:
                ...
                if isinstance(households,dict):
                    ...
                    if actor_hh and not other_hh:
                        ...
                        residence=str(actor.get('location_ref') or other.get('location_ref') or '')
            ...
            # A marriage event affects institutions only when factions differ.
            if actor.get('faction_ref')!=other.get('faction_ref'):
                relations=copy.deepcopy(self.repository.read_json(_RELATIONS)); edges=list(relations.get('edges',[])); changed=[]
                for a,b in ((str(actor.get('faction_ref')),str(other.get('faction_ref'))),(str(other.get('faction_ref')),str(actor.get('faction_ref')))):
    ...
    def _jianghu_custody_resolution(self,command:CommandEnvelope,meta:Mapping[str,Any],current_time:CampaignTime):
        ...
        if action not in {'restrain','release','escape_attempt'}:
        ...
        if action=='restrain':
            ...
            scene=self.repository.read_json(self.scene_path)
            if not self._person_present_for_command(scene,person_ref,current_time):raise CommandRejectedError('jianghu_custody_target_not_present')
            combats=self.repository.read_json(_COMBATS)
            ...
            target_location=str(target.get('location_ref') or scene.get('location_id') or '')
            ...
            if any(isinstance(r,Mapping) and r.get('person_ref')==person_ref and r.get('status') not in {'released','escaped'} for r in records):raise CommandRejectedError('jianghu_custody_already_active')
            ...
            candidates=[r for r in records if isinstance(r,Mapping) and r.get('person_ref')==person_ref and r.get('status') not in {'released','escaped'}]
            if not candidates:raise CommandRejectedError('jianghu_custody_record_not_found')
            record=candidates[-1]
            if action=='escape_attempt':
                # Escape is a physical-state consequence, never a guaranteed
                # button press. A restrained person can leave custody directly
                # only when the current custodian is no longer capable of
                # physically maintaining it or is no longer co-located. If the
                # guard is still present and usable, the opening must first be
                # created through the physical combat/injury system.
                captor_ref=str(record.get('captor_ref') or '')
                opening=False
                try:
                    _cp,_cr,_co,captor=self._person(captor_ref)
                    same_location=str(captor.get('location_ref') or '')==str(target.get('location_ref') or '')
                    opening=(not same_location) or (not _person_is_usable(captor))
                except CommandRejectedError:
                    opening=True
                if not opening:raise CommandRejectedError('jianghu_custody_escape_requires_physical_opening')
            ...
            if not isinstance(index,Mapping) or person_ref not in index:
    ...
    def _jianghu_crime_report_resolution(self,command:CommandEnvelope,meta:Mapping[str,Any],current_time:CampaignTime):
        ...
        attention_rows[subject_ref]={'attention':attention,'bounty_cash':bounty,'prior_offenses':prior+1,'last_evidence_ref':evidence_ref,'last_updated_at':str(current_time).removeprefix('SE-')}
        gov['consumed_evidence_hashes']=sorted(set(str(x) for x in consumed)|{evidence_hash})
        subject={}; place=''; jurisdiction=''
        # Jurisdiction comes from the subject's evidenced location when the
        # report is committed; later government response is not omniscient.
        try:
            _sp,_sr,_so,subject=self._person(subject_ref)
            sites=self.repository.read_json(_LOCAL_SITES).get('sites',{}); site=sites.get(str(subject.get('location_ref'))) if isinstance(sites,Mapping) else None
            place=str(site.get('parent_place_ref') or '') if isinstance(site,Mapping) else str(subject.get('location_ref') or '')
            places=self.repository.read_json(_GEOGRAPHY).get('places',{}); prow=places.get(place) if isinstance(places,Mapping) else None
            jurisdiction=str(prow.get('climate_profile') or '') if isinstance(prow,Mapping) else ''
        ...
        return self._simple_plan(command,meta,current_time,writes_records={_GOVERNMENT:gov,_REPUTATION:rep},code='jianghu_crime_report_committed',result={'command_type':command.command_type,'subject_ref':subject_ref,'attention':attention,'bounty_cash':bounty,'government_audience_ref':audience})

    def _jianghu_combat_resolution(self,command:CommandEnvelope,meta:Mapping[str,Any],current_time:CampaignTime):
        ...
        if action=='start':
            ...
            refs=list(dict.fromkeys(side_a+side_b)); people={}; locations=set()
            ...
            for ref in refs:
                ...
                _p,_r,_o,person=self._person(str(ref)); people[str(ref)]=person; locations.add(person.get('location_ref'))
            ...
            for owner,count in requested_by_faction.items():
                ...
                stock=max(0,int(inv.get('transport_assets',{}).get('riding_horses',0))) if isinstance(inv,Mapping) else 0
                allocated=active_mount_allocations(state,faction_ref=owner)
                if stock-allocated<count:raise CommandRejectedError('jianghu_combat_riding_horses_unavailable')
            equipment_ledger=self.repository.read_json(_EQUIPMENT)
            try:combat=initialize_combat(combat_ref=combat_ref,side_a_refs=side_a,side_b_refs=side_b,people=people,zone_ref=str(next(iter(locations))),started_at=str(current_time).removeprefix('SE-'),objective=objective if isinstance(objective,Mapping) else {'kind':'eliminate','target_refs':side_b},awareness_mode=awareness,initial_range_band=band,equipment_ledger=equipment_ledger,mount_assignments=mount_assignments)
            ...
            return self._simple_plan(command,meta,current_time,writes_records={_COMBATS:state},code='jianghu_combat_started',result={'command_type':command.command_type,'combat_ref':combat_ref,'positions':combat['positions'],'mounted_refs':sorted(str(x) for x in mounted_refs)},scene=scene)
        ...
        if action=='exchange':
            ...
            try:resolved=resolve_exchange(combat=combat,people=people,equipment_ledger=ledger,doctrines=doctrines,player_ref=command.actor_id,player_action_kind=str(command.payload.get('action_kind')),player_target_ref=str(command.payload.get('target_ref')),player_weapon_ref=str(command.payload.get('weapon_ref')),player_hit_zone=str(command.payload.get('hit_zone')),player_target_structure_ref=(str(command.payload.get('target_structure_ref')) if command.payload.get('target_structure_ref') not in (None,'') else None),player_targeting_intent=str(command.payload.get('targeting_intent') or 'disable'),player_poison_ref=(str(command.payload.get('poison_ref')) if command.payload.get('poison_ref') not in (None,'') else None))
            except (KeyError,ValueError,TypeError) as exc:raise CommandRejectedError('jianghu_combat_exchange_invalid') from exc
            ...
            if seen_pairs: final_records[_SOCIAL]=social_after
            ...
            return self._combine_time_plan(command,time_plan,extra_records=final_records,code='jianghu_combat_exchange_resolved',result={'command_type':command.command_type,'combat_ref':combat_ref,'events':resolved['events'],'combat_status':combat_after.get('status'),'mount_service_losses':sum(losses_by_faction.values())},scene_override=scene)
        if action=='disengage':
            ...
            if after.get('status')=='resolved':
                scene=self._time_after_record(time_plan,self.scene_path,self.repository.read_json(self.scene_path));scene.pop('active_combat_ref',None)
            return self._combine_time_plan(command,time_plan,extra_records={_COMBATS:post},code='jianghu_disengage_resolved',result={'command_type':command.command_type,'combat_ref':combat_ref,'escaped':bool(r.get('escaped')),'reason':r.get('reason'),'combat_status':after.get('status')},scene_override=scene)
        if action=='end':
            ...
            scene=copy.deepcopy(self.repository.read_json(self.scene_path));scene.pop('active_combat_ref',None);return self._simple_plan(command,meta,current_time,writes_records={_COMBATS:state},code='jianghu_combat_closed',result={'command_type':command.command_type,'combat_ref':combat_ref},scene=scene)
