...
def point_cost(value: int) -> int:
    """Return the development-unit cost of the next point.

    Fundamentals stay inexpensive, while advanced and elite mastery steepens
    aggressively. The extra post-100 term prevents ordinary recurring training
    from converging the living world toward legendary capability.
    """

    band = max(0, value - 40) // 20
    elite_band = max(0, band - 3)
    return 1 + band * band + 2 * elite_band * elite_band
...
def settle_training(inputs: TrainingInputs) -> TrainingOutcome:
    """Settle ordinary practice without manufacturing legendary breakthroughs.

    ``ROUTINE_TRAINING_CEILING`` is representation-neutral. Exact people,
    rostered cohorts, and aggregates all stop at the same ordinary-training
    boundary. Existing exceptional values are preserved, but ordinary practice
    cannot raise them further. Residual preparation at the boundary is bounded
    below one additional point so decades of blocked practice cannot be banked
    and converted into an instant legendary jump by a later rules change.
    """
    ...
    with localcontext() as context:
        ...
        residual = _nonnegative(inputs.residual_units, "residual_units")
        if inputs.current_value < ROUTINE_TRAINING_CEILING:
            residual += earned
        else:
            earned = _ZERO
        ...
        gained = 0
        while value < ROUTINE_TRAINING_CEILING and residual >= point_cost(value):
            residual -= Decimal(point_cost(value))
            value += 1
            gained += 1

        if value >= ROUTINE_TRAINING_CEILING:
            boundary_cost = Decimal(point_cost(value))
            residual = min(residual, max(_ZERO, boundary_cost - _THREE))
