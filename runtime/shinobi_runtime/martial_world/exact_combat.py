...
def _weapon_for_holder(equipment_ledger: Mapping[str, Any], person_ref: str, weapon_ref: str | None) -> Mapping[str, Any] | None:
...
def _ready_melee_weapon_ref(person_ref: str, person: Mapping[str, Any], equipment_ledger: Mapping[str, Any], combatant_state: Mapping[str, Any]) -> str | None:
    ...
    if isinstance(explicit, str) and _weapon_owned(equipment_ledger, person_ref, explicit):
        row = _weapon_for_holder(equipment_ledger, person_ref, explicit)
...
def _action_profile(action_kind: str, actor: Mapping[str, Any], weapon_ref: str | None, target_position: Mapping[str, Any], actor_position: Mapping[str, Any], *, actor_ref: str | None = None, equipment_ledger: Mapping[str, Any] | None = None) -> tuple[ActionProfile, Mapping[str, Any] | None]:
    weapon = _weapon_for_holder(equipment_ledger, actor_ref, weapon_ref) if isinstance(actor_ref, str) and isinstance(equipment_ledger, Mapping) else _weapon(weapon_ref)
...
def _projectile_interception(
    *, defender_ref: str, defender: Mapping[str, Any], defender_state: Mapping[str, Any],
    ...
    ready=_weapon_for_holder(equipment_ledger, defender_ref, ready_ref)
...
def _settle_guard_exertion(*, combat: dict[str,Any], people: dict[str,dict[str,Any]], equipment_ledger: Mapping[str,Any], elapsed_ms: int) -> None:
    ...
    for ref,person in people.items():
        ...
        ready=_weapon_for_holder(equipment_ledger, ref, ready_ref) if ready_ref else None
...
def _schedule_action(*, combat: Mapping[str, Any], actor_ref: str, target_ref: str, action_kind: str, weapon_ref: str, poison_ref: str | None, hit_zone: str, target_structure_ref: str | None, decision_origin: str, people: Mapping[str, Mapping[str, Any]], equipment_ledger: Mapping[str, Any]) -> _ScheduledAction:
    ...
    profile, weapon = _action_profile(action_kind, people[actor_ref], weapon_ref, target_position, actor_position, actor_ref=actor_ref, equipment_ledger=equipment_ledger)
...
def _interception_damage(*, defender_ref: str, attacker_ref: str, people: dict[str, dict[str, Any]], equipment_ledger: dict[str, Any], combat: dict[str, Any], at_ms: int) -> dict[str, Any] | None:
    defender_state=combat["combatants"][defender_ref]; weapon_ref=_ready_melee_weapon_ref(defender_ref,people[defender_ref],equipment_ledger,defender_state); weapon=_weapon_for_holder(equipment_ledger,defender_ref,weapon_ref)
...
def _resolve_scheduled_action(*, combat: dict[str, Any], action: _ScheduledAction, people: dict[str, dict[str, Any]], equipment_ledger: dict[str, Any]) -> dict[str, Any]:
    ...
    if weapon_clash:
        _apply_weapon_wear(equipment_ledger,person_ref=actor_ref,weapon_ref=action.weapon_ref,event_kind="weapon_contact_heavy")
        _apply_weapon_wear(equipment_ledger,person_ref=actual_ref,weapon_ref=defender_weapon_ref,event_kind="weapon_contact_heavy")
    ...
    if channel=="projectile":
        ...
        if interception.get("outcome")=="clean":
            _apply_weapon_wear(equipment_ledger,person_ref=actual_ref,weapon_ref=defender_weapon_ref,event_kind="weapon_contact_light")
    ...
    if channel=="melee" and not weapon_clash:
        _apply_weapon_wear(equipment_ledger,person_ref=actor_ref,weapon_ref=action.weapon_ref,event_kind="weapon_contact_light")
...
def resolve_exchange(*, combat: Mapping[str, Any], people: Mapping[str, Mapping[str, Any]], equipment_ledger: Mapping[str, Any], doctrines: Mapping[str, Mapping[str, Any]], player_ref: str, player_action_kind: str, player_target_ref: str, player_weapon_ref: str, player_hit_zone: str = "chest", player_target_structure_ref: str | None = None, player_targeting_intent: str = "disable", player_poison_ref: str | None = None, player_qi_allocation_milli: Mapping[str, int] | None = None, player_qi_reserve_milli: int | None = None, player_auto_qi: bool = False, player_auto_poison: bool = False, npc_targeting_intent: str | None = None, martial_familiarity: Mapping[str, Any] | None = None, player_retinue_context: Mapping[str, Any] | None = None, equipment_ledger_hydrated: bool = False, compact_equipment_result: bool = True, mutate_equipment_ledger: bool = False) -> dict[str, Any]:
    ...
    for actor_ref in active_at_declaration:
        ...
        if actor_ref==player_ref:
            ...
            if not _weapon_owned(ledger,actor_ref,weapon_ref):
            ...
            # Team assignments remain the default.  A severe explicit personal
            # conflict can redirect the actor to another lawfully known enemy,
            # while vengeance can make a particular enemy more compelling.  No
            # social pressure grants contact or damage; the physical resolver
            # still owns every outcome.
            if assigned_target is not None and pressure_by_target.get(assigned_target,0)>-60:
                ...
                target=min(
                    ...
                    key=lambda ref:(
                        ...
                        planar_distance_mm(out["positions"][actor_ref],out["positions"][ref]),
                        ref,
                    ),
                )
...
def default_target_for(*, combat: Mapping[str, Any], people: Mapping[str, Mapping[str, Any]], actor_ref: str, martial_familiarity: Mapping[str, Any] | None = None) -> str:
    ...
    if assigned in known and pressure.get(str(assigned), 0) > -60:
        return str(assigned)
...
def default_action_for(*, combat: Mapping[str, Any], people: Mapping[str, Mapping[str, Any]], equipment_ledger: Mapping[str, Any], actor_ref: str, target_ref: str, role: str | None = None, martial_familiarity: Mapping[str, Any] | None = None, preferred_weapon_ref: str | None = None) -> tuple[str, str]:
    ...
    if isinstance(preferred_weapon_ref,str) and preferred_weapon_ref not in {'','auto'}:
        if preferred_weapon_ref=='body_unarmed':
            return 'unarmed_strike','body_unarmed'
        items=_loadout_items(equipment_ledger,actor_ref)
        if max(0,int(items.get(preferred_weapon_ref,0)))<=0:
            raise ValueError("preferred combat weapon not carried")
        weapon=_weapon_for_holder(equipment_ledger,actor_ref,preferred_weapon_ref)
