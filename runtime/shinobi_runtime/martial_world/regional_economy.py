...
def market_path(region_id: str) -> str:
    return f'state/martial-world/markets/{str(region_id)}.json'
...
def initial_market_state(region_id:str)->dict[str,Any]:
    ...
    return {'schema':'jianghu-market-state-1.0','region_id':region_id,'stock':copy.deepcopy(row['initial_market_stock']),'cash_pool':int(row['initial_market_cash_pool'])}
...
def current_cargo_market_value_cash(movement:Mapping[str,Any],*,read_json)->int:
    """Project the shipment's current local market value from conserved cargo.

    Cargo value is deliberately derived rather than persisted on movement rows.
    Destination value is preferred because it is the realizable value at the
    shipment goal; origin value is the deterministic fallback.
    """
    item_ref=str(movement.get('item_ref') or '')
    quantity=max(0,int(movement.get('quantity',0)))
    if not item_ref or quantity<=0:
        return 0
    for place_ref in (movement.get('destination_place_ref'),movement.get('origin_place_ref')):
        if not isinstance(place_ref,str) or not place_ref:
            continue
        try:
            region=region_for_place(place_ref)
            market=read_json(market_path(region))
            stock=market.get('stock',{}) if isinstance(market,Mapping) else {}
            return max(0,unit_market_price_cash(region,item_ref,stock if isinstance(stock,Mapping) else {})*quantity)
