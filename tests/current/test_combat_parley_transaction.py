    ...
    if isinstance(people, list):
        for person in people:
            if isinstance(person, dict) and person.get("person_id") in {"pc_wei_tang", "char.zhu"}:
                ...
                # These transaction tests need an isolated playable fixture. The
                # repository's live campaign may legitimately contain an injured or
                # incapacitated Wei, which must not decide whether synthetic parley
                # tests can start a new combat inside their temporary copy.
                person["health"] = {
                    "status": "ready",
                    "injuries": [],
                    "blood_lost_ml": 0,
