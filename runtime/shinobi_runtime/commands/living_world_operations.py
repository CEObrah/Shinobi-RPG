...
from shinobi_runtime.commands.paths import DOMAIN_REGISTRY_PATH, ECONOMY_WORLD_PATH
...
class LivingWorldOperationsMixin:
    ...
    def _owned_mission_market(self, faction_id: str, *, signal_candidates: Sequence[str]) -> Optional[tuple[str, str, str]]:
        """Return the acting faction's market and first compatible demand signal.

        Market ownership and geography remain in the world economy registry; the
        runtime mission-market registry only owns mutable evidence-driven scores.
        """
        try:
            world = self.repository.read_json(ECONOMY_WORLD_PATH)
        except (FileNotFoundError, ValueError) as exc:
            raise CommandRejectedError("mission_market_world_invalid") from exc
        rows = (
            world.get("payload", {}).get("economies_and_mission_markets", {}).get("markets", [])
            if isinstance(world, Mapping) else []
        )
        owned = [row for row in rows if isinstance(row, Mapping) and row.get("owner_id") == faction_id]
        for row in sorted(owned, key=lambda item: str(item.get("id") or "")):
            market_ref = row.get("id")
            place_ref = row.get("place_ref")
            demand = row.get("demand")
            if not isinstance(market_ref, str) or not isinstance(place_ref, str) or not isinstance(demand, list):
                raise CommandRejectedError("mission_market_world_invalid")
            supported = set(value for value in demand if isinstance(value, str))
            signal_ref = next((value for value in signal_candidates if value in supported), None)
            if signal_ref is not None:
                return market_ref, place_ref, signal_ref
        return None
        ...
        *,
        ...
        if kind == "institutional_assessment":
            ...
            if str(effect.get("assessment_kind") or "") == "mission":
                mechanics = self._operational_world_mechanics().get("mission_market")
                objective_map = mechanics.get("objective_signal_candidates") if isinstance(mechanics, Mapping) else None
                if not isinstance(objective_map, Mapping):
                    raise CommandRejectedError("operational_world_mechanics_invalid")
                selected_market: Optional[tuple[str, str, str]] = None
                for mission_ref in sorted(ref for ref in refs if isinstance(ref, str) and ref.startswith("mission.")):
                    ...
                    for objective in objectives:
                        if not isinstance(objective, Mapping) or objective.get("status") not in ("pending", "in_progress"):
                            continue
                        candidates = objective_map.get(str(objective.get("kind") or ""))
                        if not isinstance(candidates, list) or any(not isinstance(value, str) for value in candidates):
                            continue
                        selected_market = self._owned_mission_market(faction_id, signal_candidates=candidates)
                        if selected_market is not None:
                            break
                    if selected_market is not None:
                        break
                if selected_market is not None:
                    market_ref, market_place_ref, signal_ref = selected_market
                    ...
                    if isinstance(scores, dict) and signal_ref in scores:
                        step = int(mechanics.get("signal_delta_milli", 0))
                        if step <= 0:
                            raise CommandRejectedError("operational_world_mechanics_invalid")
                        ...
                        refs = [*refs, f"{market_ref}:place:{market_place_ref}:demand:{signal_ref}:{scores[signal_ref]}"]
