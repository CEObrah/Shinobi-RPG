...
def _data(): return json.loads((_MW/'anatomy.json').read_text(encoding='utf-8'))
...
def settle_physiology(*,body_mass_kg:float,wounds:Sequence[Mapping[str,Any]],blood_lost_ml:int,elapsed_seconds:int,endurance:int,willpower:int,medicine_modifiers:Mapping[str,int]|None=None)->dict[str,Any]:
    if elapsed_seconds<0:raise ValueError('elapsed invalid')
    total_bleed=sum(max(0,int(w.get('bleeding_ml_per_min',0))) for w in wounds)
    added=total_bleed*elapsed_seconds//60;blood=max(0,int(blood_lost_ml)+added);volume=blood_volume_ml(body_mass_kg)
    loss_frac=min(1000,blood*1000//volume);pain=sum(max(0,int(w.get('pain',0))) for w in wounds);organ=sum(max(0,int(w.get('organ_trauma',0))) for w in wounds)
    shock=max(0,pain//3+organ//2+loss_frac//4-max(0,endurance)//4);consciousness=max(0,200+max(0,willpower)//2+max(0,endurance)//3-shock)
    lethal=lethal_state(wounds=wounds,blood_loss_fraction_milli=loss_frac,shock=shock,consciousness=consciousness)
    return {'blood_volume_ml':volume,'blood_lost_ml':blood,'blood_loss_fraction_milli':loss_frac,'bleeding_ml_per_min':total_bleed,'pain_total':pain,'shock':shock,'consciousness':consciousness,'lethal_state':lethal,'vision':vision_state(wounds),'functional_penalties':functional_penalties(wounds)}
...
def lethal_state(*,wounds:Sequence[Mapping[str,Any]],blood_loss_fraction_milli:int,shock:int,consciousness:int)->str:
    ...
    for w in wounds:
        ...
        if structure=='heart' and sd>=int(_data()['structures']['heart']['destruction_threshold']):return 'dying'
        if zone=='head' and max(organ,severity)>=int(l['catastrophic_brain_trauma']):return 'dead'
        if zone=='neck' and max(organ,severity)>=int(l['catastrophic_neck_trauma']):return 'dead'
        if zone=='chest' and organ>=int(l['catastrophic_chest_organ_trauma']):return 'dying'
...
__all__=['blood_volume_ml','structure_definition','target_zone','structure_family_members','wound_from_contact','vision_state','combat_status_families','functional_penalties','functional_capacity_factors','settle_physiology','lethal_state','recovery_advance','wound_requires_persistence']
