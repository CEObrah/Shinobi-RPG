...
This package establishes the repaired current world snapshot as a **new revision-1 campaign baseline**. The old revision-158 state is retained only as forensic evidence in `docs/forensics/campaign-state-revision-158-pre-rebaseline.tar.gz`.
...
Do not point revision 1 at an old revision-158 checkout or old receipt store. The runtime intentionally refuses unsafe rollback/divergence states.
