...
def test_readme_delegates_mutable_campaign_revision_to_state_authority():
    revision = load("state/meta.json")["revision"]
    text = (ROOT / "README.md").read_text(encoding="utf-8").lower()
    assert "state/meta.json" in text
    assert "documentation never hard-codes the mutable live revision" in text
    assert f"campaign revision: **{revision}**" not in text
