...
def test_revalidated_specialist_team_state_contains_no_orphan_rosters_or_stale_training() -> None:
    ...
    for team_ref in current_specialist_refs:
        ...
        assert history["training_sessions"] == 0
        assert history["notable_event_refs"] == []
...
def test_legacy_vague_autonomy_missions_are_not_left_live_after_rich_operation_upgrade() -> None:
    ...
    assert mission["terminal_reason_ref"] == "repair.rich_autonomy_upgrade.v1"
...
def test_existing_institutional_reports_are_routed_to_their_faction_holder() -> None:
    repo = RepositoryStore(ROOT)
    ...
    for claim_id, claim in claims.items():
        ...
        if claim["epistemic_kind"] != "report" or not subject_ref.startswith("faction.") or source_ref == subject_ref:
            continue
