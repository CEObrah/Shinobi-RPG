...
class JianghuRetinueCommandsMixin:
    def _jianghu_retinue_resolution(
        ...
        if action == "request":
            ...
            # New standing travel teams are always three permanent companions.
            # Zero means the chooser owns the identities, not the headcount.
            if requested_count not in {0, 3}:
            ...
            rows[retinue_ref] = {
                ...
                # chooser_refs is authoritative. chooser_ref remains a temporary
                # compatibility projection for older player-facing readers.
            ...
            def validate(overlay: StagedOverlay, manifest: TransactionManifest) -> None:
                ...
                if int(row.get("requested_count", 0)) != 3:
                    raise ValueError("retinue permanent team size changed after planning")
            ...
            return _BuiltPlan(
                ...
                result={
                    ...
                    "assignment_member_count": 3,
                    "chooser_selects_identities": requested_count == 0,
