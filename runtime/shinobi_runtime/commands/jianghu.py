        ...
        *,
        ...
        while True:
            ...
            if plan.result.get("interrupted"):
                gm_boundary = str(plan.result.get("gm_boundary_kind") or "") == "gm_judgment"
                if gm_boundary:
                    handoffs = [
                        dict(row) for row in plan.result.get("gm_handoffs", [])
                        if isinstance(row, Mapping)
                    ]
                    stopped_for_handoff = True
                    stop_boundary_kind = "gm_judgment"
                    for row in handoffs:
                        key = (str(row.get("event_id") or ""), str(row.get("kind") or ""))
                        if key in seen_handoff_keys:
                            continue
                        seen_handoff_keys.add(key)
                        if len(gm_handoffs) < 32:
                            gm_handoffs.append(row)
                else:
        ...
        if stopped_for_handoff:
            if stop_boundary_kind == "gm_judgment":
        ...
        if gm_handoffs:
            result["gm_handoffs"] = gm_handoffs
        return _BuiltPlan(
