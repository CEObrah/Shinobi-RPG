...
from shinobi_runtime.martial_world.faction_state import (
    compact_faction_state, hydrate_faction_state, inventory_path, roster_path, faction_profile, living_roster_population,
)
...
from shinobi_runtime.martial_world.live_state import _derived_person_routes, person_route
...
def main() -> int:
    ...
    faction_registry = load("state/martial-world/faction-registry.json")
    registered_factions = faction_registry.get("faction_refs", []) if isinstance(faction_registry, dict) else []
    active_owner_refs = []
    for path in faction_files:
        raw = json.loads(path.read_text(encoding="utf-8"))
        if str(raw.get("status") or "active") != "extinct":
            active_owner_refs.append(path.stem)
    active_owner_refs.sort()
    if not isinstance(registered_factions, list) or sorted(registered_factions) != active_owner_refs:
        fail(errors, "current faction registry does not exactly match active canonical faction owners")
    ...
    for path in faction_files:
        ...
        if "population" in raw_faction:
            fail(errors, f"{fid}: derived living population must not be persisted")
        if living != living_roster_population(roster_raw):
            fail(errors, f"{fid}: derived living-population projection is inconsistent")
    ...
    civic_owner = load("state/martial-world/civic-people.json")
    civic_people = civic_owner.get("people", []) if isinstance(civic_owner, dict) else []
    civic_ids: set[str] = set()
    if not isinstance(civic_people, list):
        fail(errors, "civic people owner has invalid people array")
        civic_people = []
    for ordinal, person in enumerate(civic_people):
        if not isinstance(person, dict):
            fail(errors, f"civic person ordinal {ordinal} is not an object")
            continue
        pid = person.get("person_id")
        if not isinstance(pid, str) or not pid:
            fail(errors, f"civic person ordinal {ordinal} has invalid ID")
            continue
        if pid in people_by_id or pid in independent_ids or pid in civic_ids:
            fail(errors, f"duplicate exact person identity {pid}")
        civic_ids.add(pid)
        cash = person.get("personal_cash", 0)
        if isinstance(cash, bool) or not isinstance(cash, int) or cash < 0:
            fail(errors, f"{pid}: invalid civic personal_cash")

    total_persistent_martial = total_people + len(independent_ids)
    total_exact_people = total_persistent_martial + len(civic_ids)
    ...
    # Person routing is a runtime projection of authoritative faction rosters.
    # Canonical save state must never contain a second mutable route index.
    if (ROOT / "state/martial-world/person-routes.json").exists() or (ROOT / "state/martial-world/person-routes").exists():
        fail(errors, "persistent person-route index/shards survived derived-state cleanup")
    try:
        derived_routes = _derived_person_routes(repo)
        if set(derived_routes) != set(people_by_id):
            fail(errors, f"derived person route coverage mismatch missing={len(set(people_by_id)-set(derived_routes))} extra={len(set(derived_routes)-set(people_by_id))}")
        for pid in sorted(people_by_id)[:8] + sorted(people_by_id)[-8:]:
            fid, _ordinal = person_route(repo, pid)
            if fid != people_by_id[pid][3]:
                fail(errors, f"{pid}: derived person route resolves to wrong faction {fid}")
    except Exception as exc:
        fail(errors, f"derived person routing failed: {exc}")
    ...
    ht_roster = load("state/martial-world/people/house_tang.json")
    ht_population = living_roster_population(ht_roster)
    if ht_population < 100:
        fail(errors, f"House Tang population fell below bootstrap floor: {ht_population}")
    ...
    for place_ref, pool in places.items():
        ...
        if int(pool.get("identity_ordinal_cursor", 0)) == 0 and "identity_ordinal_cursor" in pool:
            fail(errors, f"{place_ref}: zero identity cursor should be sparse")
    ...
    print(
        ...
        f"{total_people} faction people, {len(independent_ids)} independent people, {len(civic_ids)} civic people, {total_exact_people} exact people, "
