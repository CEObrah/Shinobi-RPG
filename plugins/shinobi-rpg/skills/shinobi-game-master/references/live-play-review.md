...
Otherwise preserve scene flow and mention only the strongest useful finding at a natural stopping point. Batch related observations when several symptoms share one cause. When a concrete reusable defect, thin system, or improvement is worth carrying forward, make it visible as one concise `OOC IMPROVEMENT:` note with the observed symptom, why it matters, likely owner, and smallest coherent fix. Do not emit a note just to prove review happened, repeat an unchanged finding, or turn normal play into a changelog.
Classify the note when useful as **BUG**, **DEPTH GAP**, **CAUSAL FLOW**, **UX**, **BALANCE**, **PERFORMANCE**, or **DESIGN**. Prefer observed evidence -> player impact -> authoritative owner -> smallest reusable fix -> focused regression to add. A note is diagnosis, not permission to mutate code or campaign state.
...
- completing one procedural subtask does not create a dead-end handoff when an established next stage should continue without a new decision;
- `unresolved_decision: null` is not mistaken for scene completion, and standing waits/obvious continuations remain alive until a genuine boundary;
