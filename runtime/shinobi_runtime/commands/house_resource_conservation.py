...
class HouseResourceConservationMixin:
    ...
    def _house_resource_snapshot(self, house: Mapping[str, Any]) -> Mapping[str, Any]:
        ...
        for cohort in cohorts:
            ...
            if "instructor" in role or "instruction" in role or "command" in role:
        ...
        sheet_resolver = RepositoryPersonSheetResolver(self.repository)
        for ref in dict.fromkeys(instructor_candidates):
            ...
            if isinstance(record, Mapping):
                ...
                continue
            try:
                sheet = sheet_resolver(ref)
            except (FileNotFoundError, TypeError, ValueError):
                sheet = None
            core = sheet.get("core") if isinstance(sheet, Mapping) else None
            if isinstance(core, Mapping) and core.get("life_status") == "alive" and core.get("current_host_ref") == home:
                available_instructors.append(ref)
