...
def test_short_weapon_endcap_contact_counts_body_intersection_not_target_center_distance():
    positions={
        'attacker':_pos(0,0,500),
        # Horizontal separation is inside jian reach, while center-to-center 3D
        # distance exceeds it. The target body still physically overlaps the
        # 1.15 m swept endpoint and therefore must be contactable.
        'target':_pos(1149,0,0),
    }
    trace=trace_attack_geometry(
        positions,actor_ref='attacker',aim_ref='target',body_refs=['attacker','target'],
        geometry={'shape':'direct','width_m':0.35,'length_m':1.15},obstacles=[],
        target_limit=1,maximum_range_m=1.15,channel='melee',
        trajectory={
            'launch_x_mm':0,'launch_y_mm':0,'launch_elevation_mm':500,
            'aim_x_mm':1149,'aim_y_mm':0,'aim_elevation_mm':0,
        },
    )
    assert trace['contacts']
    assert trace['contacts'][0]['participant_ref']=='target'
    assert int(trace['contacts'][0]['entry_mm']) <= 1150

    positions['target']=_pos(1800,0,0)
    miss=trace_attack_geometry(
        positions,actor_ref='attacker',aim_ref='target',body_refs=['attacker','target'],
        geometry={'shape':'direct','width_m':0.35,'length_m':1.15},obstacles=[],
        target_limit=1,maximum_range_m=1.15,channel='melee',
        trajectory={
            'launch_x_mm':0,'launch_y_mm':0,'launch_elevation_mm':500,
            'aim_x_mm':1800,'aim_y_mm':0,'aim_elevation_mm':0,
        },
    )
    assert not miss['contacts']
...
def test_bilateral_blindness_blocks_visually_aimed_bow(monkeypatch):
    ...
    monkeypatch.setattr(exact,'_currently_visible_enemies',lambda combat,actor_ref,enemy_refs,people: [] if actor_ref==target['person_id'] else original_visible(combat,actor_ref=actor_ref,enemy_refs=enemy_refs,people=people))
...
def test_defended_or_missed_melee_exchange_advances_combat_clock(monkeypatch):
    ...
    original_observe=exact._observe_visible_enemies
    original_visible=exact._currently_visible_enemies
    # This test isolates elapsed-time accounting for the player's attacked
    # action.  The current-contact model is a separate authority from cumulative
    # observation, so suppress both for the defender rather than relying on the
    # old single-helper implementation detail to keep the defender passive.
    ...
    monkeypatch.setattr(exact,'_currently_visible_enemies',lambda combat,actor_ref,enemy_refs,people: [] if actor_ref==defender['person_id'] else original_visible(combat,actor_ref=actor_ref,enemy_refs=enemy_refs,people=people))
...
def test_idle_exchange_without_observed_targets_still_advances_clock(monkeypatch):
    ...
    monkeypatch.setattr(exact,'_currently_visible_enemies',lambda *args,**kwargs:[])
    ...
    assert all(event['result']=='no_currently_visible_target' for event in result['events'])
...
def test_melee_schedule_persists_original_committed_approach_envelope():
    ...
    # If an earlier simultaneous action moves the defender farther away, the
    # attack may spend its finite startup-follow envelope. It may not teleport or
    # expand beyond the movement budget authored when the action was scheduled.
    ...
    assert trace['reason'] in {'partial_committed_approach','closed_into_melee_reach'}
    assert int(trace['distance_mm'])>=int(params['approach_distance_mm'])
    assert int(trace['distance_mm'])<=int(params['melee_movement_budget_mm'])
    if trace['reason']=='partial_committed_approach':
...
def test_three_vs_three_teammates_create_real_defensive_interruptions(monkeypatch):
    ...
    first_events=[event for event in result['events'] if event.get('actor_ref') in people and 'declared_at_ms' in event]
    # The player decision horizon ends at A0's resolved action. B1/B2 have
    # already declared on the same physical clock, but their later contacts stay
    # queued instead of forcing every actor to wait for the slowest action.
    queued={row['actor_ref']:row for row in result['combat_after'].get('scheduled_actions',[])}
    assert {'b1','b2'} <= set(queued)
    assert all(int(queued[ref]['declared_at_ms'])==0 for ref in ('b1','b2'))
    assert all(int(queued[ref]['contact_at_ms'])>int(result['combat_after']['elapsed_ms']) for ref in ('b1','b2'))
    assert '_pending_actions' not in result['combat_after'] and '_defense_interruptions' not in result['combat_after']

    second=resolve_exchange(combat=result['combat_after'],people=result['people_after'],equipment_ledger=result['equipment_ledger_after'],doctrines={},player_ref='a0',player_action_kind='unarmed_strike',player_target_ref='b0',player_weapon_ref='body_unarmed',player_hit_zone='chest',player_targeting_intent='disable')
    by_actor={event.get('actor_ref'):event for event in second['events'] if event.get('actor_ref') in people}
    interrupted=[by_actor[ref] for ref in ('b1','b2') if ref in by_actor and (str(by_actor[ref].get('result','')).startswith('action_interrupted_by_defense') or str(by_actor[ref].get('result','')).startswith('action_disrupted_by_defense'))]
    assert interrupted, {ref:by_actor.get(ref,{}).get('result') for ref in ('b0','b1','b2')}
    assert all(int(event['declared_at_ms'])==0 for event in interrupted)
    ...
    assert '_pending_actions' not in second['combat_after'] and '_defense_interruptions' not in second['combat_after']


def test_recovery_commitments_preserve_independent_actor_clocks_between_horizons(monkeypatch):
    import shinobi_runtime.martial_world.exact_combat as exact
    base=load('state/martial-world/people/house_tang.json')['people'][0]
    people={}
    for side in ('a','b'):
    ...
    queued_before={row['actor_ref']:row for row in first['combat_after'].get('scheduled_actions',[])}
    assert {'a1','b0','b1'} <= set(queued_before)
    assert all(int(queued_before[ref]['declared_at_ms'])==0 for ref in ('a1','b0','b1'))
    ...
    second_events=[event for event in second['events'] if event.get('actor_ref') in people and 'start_at_ms' in event]
    # Carried actions keep their original declarations instead of being snapped
    # to the next player horizon. A0's new glaive action begins only after its
    # own recovery, while faster actors may already have acted and redeclared.
    carried=[event for event in second_events if event.get('actor_ref') in {'a1','b0','b1'} and int(event.get('declared_at_ms',-1))==0]
    assert carried
    a0_events=[event for event in second_events if event.get('actor_ref')=='a0']
    assert a0_events
    assert all(int(event['start_at_ms'])>=recovery_until['a0'] for event in a0_events)
    assert any(int(event['declared_at_ms'])>first_clock for event in second_events if event.get('actor_ref')!='a0')
    earliest_nonplayer=min(int(event['start_at_ms']) for event in second_events if event.get('actor_ref')!='a0')
    assert earliest_nonplayer < min(int(event['start_at_ms']) for event in a0_events)
...
def test_scene_improvised_prop_resolves_as_transient_weapon_without_inventory_minting():
    from shinobi_runtime.martial_world.exact_combat import improvised_weapon_state_from_scene

    roster=load('state/martial-world/people/house_tang.json')['people']
    attacker=copy.deepcopy(roster[0]); defender=copy.deepcopy(roster[3])
    people={attacker['person_id']:attacker,defender['person_id']:defender}
    ledger=load('state/martial-world/equipment-ledger.json')
    fact_ref='scene_fact_test_ceramic_bowl'
    before_items=copy.deepcopy(effective_person_loadout(ledger,attacker['person_id']).get('items',{}))
    transient=improvised_weapon_state_from_scene(
        fact_ref=fact_ref,holder_ref=attacker['person_id'],source_object_fact_ref='scene_fact_bowl_exists',
        source_session_ref='scene_session_bowl',source_location_ref='site.house_tang',
        summary='Wei holds the established ceramic bowl.',form='small_rigid',material='ceramic',condition='intact',
    )
    combat=initialize_combat(
        combat_ref='improvised-bowl',side_a_refs=[attacker['person_id']],side_b_refs=[defender['person_id']],people=people,
        zone_ref='site.house_tang',started_at='x',objective={'kind':'eliminate','target_refs':[defender['person_id']]},
        initial_range_band=0,equipment_ledger=ledger,
    )
    result=resolve_exchange(
        combat=combat,people=people,equipment_ledger=ledger,doctrines={},
        player_ref=attacker['person_id'],player_action_kind='improvised_strike',player_target_ref=defender['person_id'],
        player_weapon_ref=fact_ref,player_hit_zone='chest',player_targeting_intent='disable',
        player_improvised_weapon_state=transient,
    )
    event=next(e for e in result['events'] if e.get('actor_ref')==attacker['person_id'])
    assert event['weapon_ref']==fact_ref
    assert event['result'] not in {'weapon_not_owned','action_rejected'}
    saved=result['combat_after']['combatants'][attacker['person_id']]['improvised_weapon_state']
    assert saved['fact_ref']==fact_ref
    assert saved['durable_item_created'] is False
    assert saved['status'] in {'held','broken'}
    after_items=effective_person_loadout(result['equipment_ledger_after'],attacker['person_id']).get('items',{})
    assert after_items==before_items
    assert fact_ref not in after_items
