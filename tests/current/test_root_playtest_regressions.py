...
def test_field_medic_profession_does_not_remove_an_unassigned_fighter_from_combat():
    ...
    assert any(row.get('action_kind') in {'thrust','cut','hidden_weapon_throw','bow_shot','unarmed_strike'} for row in medic_events)
    assert not any(row.get('result') == 'holding_medical_support_position' for row in medic_events)
