...
COMMAND_SPECS={
 ...
 "jianghu_scene_session_resolution":CommandSpec(("action","session_ref","kind","participant_refs","process_ref","purpose","agenda","close_reason","speaker_ref","statement","speech_kind","basis_refs","resolves_thread_ref","resolves_question_ref","actor_ref","fact_kind","description","improvised_prop"),(),"Open/close a reversible live scene, persist important attributed speech, or preserve a salient reversible scene-local fact. This command has no mechanical-consequence authority. For a mundane prop that may later cross into combat, the first object_state may carry the bounded form/material/condition descriptor; a later handling object_state must cite that exact earlier fact and repeat the exact descriptor before combat may derive transient physics.",{},variants={
 ...
 "jianghu_combat_resolution":CommandSpec(("action","combat_ref","side_a_refs","side_b_refs","objective","awareness_mode","initial_range_band","mounted_refs","action_kind","target_ref","weapon_ref","hit_zone","target_structure_ref","targeting_intent","poison_ref","qi_allocation_milli","exchange_count","duration_seconds","until_resolution","improvised_prop_fact_ref"),(),"Start or resolve anatomy-first exact Jianghu combat on authoritative local geometry and named anatomy. A bare exchange means attack under standing doctrine: omitted target, action, weapon, anatomical aim, Qi, and poison are resolved from lawful perception, current geometry, Wei's personal doctrine, and active team doctrine. Combat Qi allocation uses only body, movement, and sensing flow; Qi temporarily improves the fighter's ordinary capabilities and never adds a separate weapon-damage channel. Every concrete player detail overrides only that detail. Optional exchange_count, duration_seconds, or until_resolution extends the same high-level attack intent across a bounded combat span; doctrine is re-evaluated each exchange. An exact improvised_prop_fact_ref may promote one already-established mundane scene object into a transient server-derived combat profile without minting inventory or value.",{},variants={
    "start":CommandVariant(("action","combat_ref","side_a_refs","side_b_refs","objective","awareness_mode","initial_range_band"),("mounted_refs",)),
    "exchange":CommandVariant(("action","combat_ref"),("action_kind","target_ref","weapon_ref","hit_zone","target_structure_ref","targeting_intent","poison_ref","qi_allocation_milli","exchange_count","duration_seconds","until_resolution","improvised_prop_fact_ref")),
