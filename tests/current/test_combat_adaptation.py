...
def test_high_intelligence_replans_geometry_after_one_clear_failure_and_preserves_resources(monkeypatch):
    ...
    def base(**kwargs):
        ...
        movement_contexts.append(kwargs.get("movement_intent"))
        if len(calls) == 1:
    ...
    assert movement_contexts == [None, "lateral"]
...
def test_player_safe_pressure_target_is_preserved_after_geometry_and_same_discipline_candidates(monkeypatch):
    ...
    candidates = adapt.adaptive_override_candidates(
        original_kwargs=kwargs,
        combat=kwargs["combat"],
        people=kwargs["people"],
        player_ref="wei",
        previous_event={"intended_ref": "enemy1", "action_kind": "thrust", "hit_zone": "neck"},
        previous_projection=projection,
    )

    assert candidates[0] == {"_adaptive_movement_intent": "lateral"}
    assert candidates[1] == {"raw_action_kind": "cut"}
    pressure_index = candidates.index({"raw_target_ref": "enemy2"})
    assert pressure_index > 1


def test_lower_intelligence_repeats_before_geometry_adaptation(monkeypatch):
    _install_unit_stubs(monkeypatch, visible=("enemy1",))
    calls = []
    movement_contexts = []
    ...
    assert [call["raw_action_kind"] for call in calls[:4]] == ["attack", "attack", "attack", "attack"]
    assert movement_contexts[:3] == [None, None, None]
    assert movement_contexts[3] == "lateral"
