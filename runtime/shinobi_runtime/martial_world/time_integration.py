...
def _is_extracted_event(row: Mapping[str, Any]) -> bool:
    kind = str(row.get("kind") or "")
    if kind in _EXTRACTED_EVENT_KINDS:
        return True
    return (
        kind == "npc_judgment_committed"
        and str(row.get("judgment_kind") or "") == "strategic_raid_aftermath_policy"
    )
...
def settle_core_frontier(
    *,
    ...
    extracted = [row for row in normalized if _is_extracted_event(row)]
    bridge_events = [
        _bridge_placeholder(row) if _is_extracted_event(row) else row
