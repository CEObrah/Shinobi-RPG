...
def team_fujin_slice(source_root: Path, work_root: Path) -> ScenarioResult:
    ...
    scheduler = ex.repository.read_json("state/time/causal-scheduler.json")
    ...
    if not any(
        e.get("kind") == "commitment.due"
        and e.get("payload", {}).get("commitment_id") == "commitment.acceptance.fujin_after_action_drill"
        for e in scheduler.get("events", [])
    ):
    ...
    return _result(
        ...
        metrics={
            ...
            "scheduler_person_scans": scheduler.get("metrics", {}).get("global_person_scans"),
            "scheduler_faction_scans": scheduler.get("metrics", {}).get("global_faction_directory_scans"),
        },
        assertions=(
...
def black_hound_slice(source_root: Path, work_root: Path) -> ScenarioResult:
    ...
    info = ex.repository.read_json("state/reg/information-deliveries.json")
...
def information_slice(source_root: Path, work_root: Path) -> ScenarioResult:
    ...
    registry = ex.repository.read_json("state/reg/information-deliveries.json")
    claim_id = "claim.acceptance.restricted_observation"
    if claim_id not in registry["knowledge"].get(PLAYER, []):
        raise AssertionError("claim creator did not retain knowledge")
    if claim_id not in registry["knowledge"].get(HIRUZEN, []):
        raise AssertionError("lawful delivery did not create recipient knowledge")
    if claim_id in registry["knowledge"].get("char.kai", []):
        raise AssertionError("restricted claim leaked to unrelated team member")
    return _result(
        "information",
        root,
        receipts,
        metrics={"transactions": len(receipts), "claims": len(registry["claims"]), "deliveries": len(registry["deliveries"])},
        assertions=(
...
def cross_system_slice(source_root: Path, work_root: Path) -> ScenarioResult:
    ...
    if "claim.acceptance.cross.black_hound" in info.get("knowledge", {}).get("char.kai", []):
        raise AssertionError("Black Hound compartmented information leaked across affiliation")
    scheduler = ex.repository.read_json("state/time/causal-scheduler.json")
    if not any(e.get("payload", {}).get("commitment_id") == "commitment.acceptance.cross.fujin_report" for e in scheduler.get("events", [])):
