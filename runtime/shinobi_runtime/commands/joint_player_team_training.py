...
def _joint_config(repository: Any, team_ref: str) -> Mapping[str, Any] | None:
    ...
    shared = row.get("player_joint_shared_core_active_hours_per_week")
    supplemental = row.get("player_supplemental_active_hours_per_week")
    if (
        ...
        or shared <= 0
        or isinstance(supplemental, bool)
        or not isinstance(supplemental, int)
        or supplemental <= 0
        or shared + supplemental != hours
    ):
...
def _select_supplemental_target(
    self: Any,
    ...
    *,
    ...
    for target in cycle:
        try:
            _container, _leaf, value = self._training_target(dict(player), target)
    ...
    if exclude is not None and len(assessed) > 1:
        for _value, target in assessed:
            if target != exclude:
                return target
    return assessed[0][1]
...
def install_joint_player_team_training() -> None:
    ...
    def wrapped(self: Any, *, team: dict[str, Any], owner_ref: str, at: Any, compacted: int, command: Any, scheduler: Any, policy_book: Any, world_events: dict[str, Any], record_writes: dict[str, dict[str, Any]]) -> Mapping[str, Any]:
        ...
        shared_target = _select_joint_target(self, player, cycle, team_ref=team_ref, at=at, team=team)
        supplemental_target = _select_supplemental_target(
            self, player, cycle, exclude=shared_target
        )
        shared_hours = Decimal(int(config["player_joint_shared_core_active_hours_per_week"]))
        supplemental_hours = Decimal(int(config["player_supplemental_active_hours_per_week"]))
        if shared_hours + supplemental_hours != active_hours:
            raise CommandRejectedError("team_training_participation_policy_invalid")
        ...
        required_categories = (
            "team_drill",
            self._training_category_for_target(shared_target),
            self._training_category_for_target(supplemental_target),
        )
        try:
        ...
        for target, target_hours in (
            (shared_target, shared_hours),
            (supplemental_target, supplemental_hours),
        ):
            ...
            try:
                outcome = settle_training(
                    TrainingInputs(
                        scheduled_hours=str(target_hours),
                        attendance="1",
                        available_instructor_hours=str(target_hours),
                        required_instructor_hours=str(target_hours),
            ...
            target_results[target] = {
                "hours": format(target_hours.normalize(), "f"),
        ...
        self._record_team_training_session(
            ...
            member_targets={player_ref: shared_target},
        ...
        event_id = self._append_internal_event(
            ...
            material_consequence_refs=(
                *tuple(
                    f"training:{player_ref}:{target}:{row['starting_value']}->{row['ending_value']}"
                    for target, row in sorted(target_results.items())
                ),
                f"joint_team_hours:{active_hours}",
                f"joint_shared_hours:{shared_hours}",
                f"joint_supplemental_hours:{supplemental_hours}",
            ),
        ...
        return {
            **dict(base),
            "player_joint_training": {
                ...
                "supplemental_target": supplemental_target,
                "shared_hours": format(shared_hours.normalize(), "f"),
                "supplemental_hours": format(supplemental_hours.normalize(), "f"),
