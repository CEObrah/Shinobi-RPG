...
from shinobi_runtime.membership_routes import stage_house_change
...
class CampaignCommandPlanner(_Base):
    ...
    def _institution_intake_resolution(
        ...
        route_before_members = tuple(ref for ref in house_base.get("member_ids", []) if isinstance(ref, str))
        ...
        try:
            stage_house_change(
                self.repository, route_records, house_ref=institution_ref,
                before_members=route_before_members,
                after_members=tuple(ref for ref in house.get("member_ids", []) if isinstance(ref, str)),
            )
        ...
        writes.update({path: _json_bytes(record) for path, record in route_records.items()})
