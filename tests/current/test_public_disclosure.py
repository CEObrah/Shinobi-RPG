...
def _seed_public_escort_movement(root: Path) -> tuple[str, str]:
    """Create one disposable Changan->Huashan escort party from a currently free exact person."""
    route_path = root / "state/martial-world/route-operations.json"
    route_state = json.loads(route_path.read_text(encoding="utf-8"))
    combats = json.loads((root / "state/martial-world/combats.json").read_text(encoding="utf-8"))
    blocked: set[str] = set()
    for row in combats.get("combats", {}).values():
        if not isinstance(row, dict) or row.get("status") != "active":
            continue
        for members in (row.get("sides") or {}).values():
            if isinstance(members, list):
                blocked.update(str(ref) for ref in members if isinstance(ref, str))
    for row in route_state.get("movements", {}).values():
        if not isinstance(row, dict) or row.get("status") in {"completed", "cancelled", "failed"}:
            continue
        for key in ("participant_refs", "protected_person_refs", "captive_refs", "rescued_refs"):
            values = row.get(key)
            if isinstance(values, list):
                blocked.update(str(ref) for ref in values if isinstance(ref, str))
    roster = json.loads((root / "state/martial-world/people/house_tang.json").read_text(encoding="utf-8"))
    actor_ref = next(
        str(row["person_id"]) for row in roster.get("people", [])
        if isinstance(row, dict)
        and isinstance(row.get("person_id"), str)
        and row["person_id"] not in blocked
        and (row.get("health") or {}).get("status") != "dead"
    )
    movement_ref = "escort_contract:test:public-disclosure"
    route_state.setdefault("movements", {})[movement_ref] = {
        ...
        "participant_refs": [actor_ref],
        "escort_refs": [actor_ref],
        "protected_person_refs": [],
        "status": "lodging_rest",
        "rest_place_ref": "site.changan.inn",
        "mode": "foot",
        "started_at": "0061-09-27T18:00:00",
        "last_progress_at": "0061-09-27T18:00:00",
        "required_seconds": 3600,
        "elapsed_seconds": 0,
        "transport_reservation": None,
    }
    route_path.write_text(json.dumps(route_state, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    return movement_ref, actor_ref
...
def test_public_escort_boast_can_create_hidden_real_pursuit_without_persisting_a_rumor(tmp_path):
    ...
    route_path = root / "state/martial-world/route-operations.json"
    movement_ref, actor_ref = _seed_public_escort_movement(root)
    ...
    # Public speech may create a consequential opportunity, but it must not
    # author the hostile response. The command stages a same-timestamp NPC
    # judgment containing stand-down/observe/attack choices and leaves route
    # ownership unchanged until a choice is committed and applied.
    assert "state/martial-world/route-operations.json" not in built.writes
    schedule_write = json.loads(built.writes["state/martial-world/scheduler.json"])
    judgments = [
        row for row in schedule_write.get("one_off", {}).values()
        if isinstance(row, dict)
        and row.get("kind") == "npc_judgment_due"
        and row.get("judgment_kind") == "route_interception_policy"
        and row.get("owner_ref") == movement_ref
    ]
    assert judgments
    intents = {str(option.get("intent")) for option in judgments[0].get("options", []) if isinstance(option, dict)}
    assert {"ignore", "observe", "attack"}.issubset(intents)
    assert any(option.get("intent") == "attack" and option.get("attacker_refs") for option in judgments[0]["options"])
    assert all(
        "pc_wei_tang" not in option.get("attacker_refs", [])
        for option in judgments[0]["options"] if isinstance(option, dict)
    )
    ...
    # Speech itself is not a save-game journal. A hostile opportunity is only a
    # durable decision frontier here; no pursuit exists until the GM-authored
    # NPC judgment is committed and the route authority applies it.
...
def test_aggregate_civilian_becomes_exact_only_when_tip_causes_real_pursuit(tmp_path, monkeypatch):
    ...
    # listener who hears is the transient aggregate-civilian candidate. The
    # promotion boundary is a real durable institutional judgment caused by the
    # tip, not a Python-selected pursuit.
    monkeypatch.setattr(
    ...
    # outlaw organization and a bounded force envelope so the disclosure
    # deterministically reaches the judgment frontier.
    monkeypatch.setattr(info, "current_faction_refs_at_place", lambda *_args, **_kwargs: ["black_lance_company"])
    ...
    assert "state/martial-world/route-operations.json" not in built.writes
    schedule_after = json.loads(built.writes["state/martial-world/scheduler.json"])
    judgments = [
    ...
    assert judgments
    assert any(row.get("decision_context", {}).get("observer_ref") == new_ref for row in judgments)
