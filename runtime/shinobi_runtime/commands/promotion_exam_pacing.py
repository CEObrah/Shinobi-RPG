...
def _effective_phase_offsets(
    repository: Any, profile: Mapping[str, Any]
) -> tuple[tuple[str, int], ...]:
    """Apply a deterministic common-exam travel shift to authored phase offsets.

    Authored offsets remain minimum pacing.  When an invited foreign delegation
    needs more registration-to-qualification time, qualification and every
    later phase shift by the same delta so later stage spacing is preserved.
    """
    schedule = _phase_offsets(profile)
    names = [phase for phase, _ in schedule]
    if "registration" not in names or "qualification" not in names:
        return schedule
    try:
    ...
    if config is None:
        return schedule
    offsets = dict(schedule)
    authored_lead = offsets["qualification"] - offsets["registration"]
    required_lead = required_arrival_lead_days(repository, profile)
    shift = max(0, required_lead - authored_lead)
    if not shift:
        return schedule
    qualification_index = names.index("qualification")
    return tuple(
        (phase, offset + (shift if index >= qualification_index else 0))
        for index, (phase, offset) in enumerate(schedule)
    )
...
def _next_phase_due(
    ...
    if not isinstance(cycle_id, str) or not isinstance(current_phase, str):
        raise CommandRejectedError("promotion_exam_cycle_state_invalid")
    schedule = _effective_phase_offsets(repository, profile)
...
def next_promotion_exam_boundary(
    ...
    for cycle in active:
        ...
        if not isinstance(profile, Mapping):
            raise CommandRejectedError("promotion_exam_cycle_state_invalid")
        next_due = _next_phase_due(repository, profile, pipeline, cycle)
        if next_due is None:
            continue
        _phase, due = next_due
        due_points.append(
    ...
    def wrapped(
        ...
        while True:
            ...
            for cycle in active:
                ...
                if not isinstance(profile, Mapping):
                    raise CommandRejectedError("promotion_exam_cycle_state_invalid")
                next_due = _next_phase_due(self.repository, profile, pipeline, cycle)
                if next_due is None:
                    continue
