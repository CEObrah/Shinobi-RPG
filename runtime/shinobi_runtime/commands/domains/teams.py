...
class TeamCommandsMixin:
    ...
    def _record_team_training_session(
        ...
        *,
        ...
        member_target_hours: Optional[Mapping[str, Mapping[str, Decimal]]] = None,
    ) -> None:
        ...
        if member_target_hours is not None:
            if set(member_target_hours) != set(members):
                raise CommandRejectedError("team_training_target_hours_invalid")
            normalized_target_hours: Dict[str, Dict[str, str]] = {}
            for member_ref in members:
                row = member_target_hours.get(member_ref)
                if not isinstance(row, Mapping) or not row:
                    raise CommandRejectedError("team_training_target_hours_invalid")
                normalized: Dict[str, str] = {}
                allocated = Decimal("0")
                for target, hours in row.items():
                    if not isinstance(target, str) or not target:
                        raise CommandRejectedError("team_training_target_hours_invalid")
                    try:
                        amount = Decimal(str(hours))
                    except Exception as exc:
                        raise CommandRejectedError("team_training_target_hours_invalid") from exc
                    if not amount.is_finite() or amount <= 0:
                        raise CommandRejectedError("team_training_target_hours_invalid")
                    normalized[target] = format(amount.normalize(), "f")
                    allocated += amount
                if allocated != active_hours:
                    raise CommandRejectedError("team_training_target_hours_invalid")
                normalized_target_hours[member_ref] = normalized
            session["target_hours"] = normalized_target_hours
