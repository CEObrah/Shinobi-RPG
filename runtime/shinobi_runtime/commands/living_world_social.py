...
from shinobi_runtime.commands.paths import FAMILY_INDEX_PATH, KINSHIP_INDEX_PATH
...
class LivingWorldSocialMixin:
    ...
    def _settle_one_autonomous_npc_family_process(
        self, *, at: CampaignTime, command: CommandEnvelope, world_events: Dict[str, Any],
        record_writes: Dict[str, Dict[str, Any]],
    ) -> Optional[Mapping[str, Any]]:
        """Continue one already-consented NPC-only family process.

        This never invents attraction, proposals, marriage intent, or parenthood. It only
        converts an already accepted proposal into the next procedural family authority
        record. A player participant makes the item ineligible for autonomous settlement.
        The queue is unbounded fictional work; one item is settled per world-registry
        review so transaction cost stays bounded.
        """
        family = record_writes.get(FAMILY_INDEX_PATH)
        if family is None:
            try:
                family = copy.deepcopy(self.repository.read_json(FAMILY_INDEX_PATH))
            except (FileNotFoundError, ValueError) as exc:
                raise CommandRejectedError("family_index_invalid") from exc
            record_writes[FAMILY_INDEX_PATH] = family
        kinship = record_writes.get(KINSHIP_INDEX_PATH)
        if kinship is None:
            try:
                kinship = copy.deepcopy(self.repository.read_json(KINSHIP_INDEX_PATH))
            except (FileNotFoundError, ValueError) as exc:
                raise CommandRejectedError("family_index_invalid") from exc
            record_writes[KINSHIP_INDEX_PATH] = kinship
        queue = family.get("autonomy_queue_refs") if isinstance(family, dict) else None
        proposals = family.get("proposals") if isinstance(family, dict) else None
        counts = family.get("counts") if isinstance(family, dict) else None
        person_index = family.get("person_index") if isinstance(family, dict) else None
        person_links = kinship.get("person_links") if isinstance(kinship, dict) else None
        if not isinstance(queue, list) or not isinstance(proposals, dict) or not isinstance(counts, dict) or not isinstance(person_index, dict) or not isinstance(person_links, dict):
            raise CommandRejectedError("family_index_invalid")
        if not queue:
            return None
        proposal_id = sorted(ref for ref in queue if isinstance(ref, str))[0]
        proposal_path = proposals.get(proposal_id)
        if not isinstance(proposal_path, str):
            queue[:] = [ref for ref in queue if ref != proposal_id]
            return {"kind":"family_autonomy","proposal_ref":proposal_id,"status":"dropped_unresolved"}
        proposal = record_writes.get(proposal_path)
        if proposal is None:
            try:
        ...
        proposer = proposal.get("proposer_id"); target = proposal.get("target_id"); kind = proposal.get("kind")
        try:
            player_ref = self.repository.read_json("state/player.json").get("owner_id")
        except (FileNotFoundError, ValueError):
            player_ref = None
        if proposal.get("status") != "accepted" or not isinstance(proposer, str) or not isinstance(target, str) or player_ref in (proposer, target):
            queue[:] = [ref for ref in queue if ref != proposal_id]
            return {"kind":"family_autonomy","proposal_ref":proposal_id,"status":"protected_or_ineligible"}

        def person_entry(ref: str) -> Dict[str, list[str]]:
            return person_index.setdefault(ref, {
                "courtships":[],"proposals":[],"unions":[],"households":[],"kinships":[],
                "parenthoods":[],"parentage":[],"successions":[],"events":[],
            })
        def link_entry(ref: str) -> Dict[str, list[str]]:
            return person_links.setdefault(ref, {
                "spouses":[],"former_spouses":[],"parents":[],"children":[],"guardians":[],
                "wards":[],"households":[],"succession_claims":[],"kinships":[],
            })
        def add_unique(values: list[str], value: str) -> None:
            if value not in values:
                values.append(value); values.sort()

        digest = hashlib.sha256(proposal_id.encode()).hexdigest()[:20]
        participants = sorted((proposer, target))
        if kind == "courtship_offer":
            result_id = f"family.courtship.autonomy.{digest}"
            result_path = f"state/family/courtships/{result_id}.json"
            existing = record_writes.get(result_path)
            if existing is None and self.repository.read_optional_bytes(result_path) is None:
                record_writes[result_path] = {
                    "schema":"family-courtship","courtship_id":result_id,"participants":participants,
                    "status":"active","authority":True,"started_at":str(at),
                    "intent_basis":f"accepted proposal {proposal_id}",
                }
                family["courtships"][result_id] = result_path
                counts["courtships"] = len(family["courtships"])
                for ref in participants: add_unique(person_entry(ref)["courtships"], result_id)
            event_type = "courtship_started"
        elif kind in ("marriage_proposal", "betrothal_proposal", "political_match_offer"):
            result_id = f"family.union.autonomy.{digest}"
            result_path = f"state/family/unions/{result_id}.json"
            status = "married" if kind == "marriage_proposal" else "betrothed"
            existing = record_writes.get(result_path)
            if existing is None and self.repository.read_optional_bytes(result_path) is None:
                record_writes[result_path] = {
                    "schema":"family-union","union_id":result_id,"participants":participants,
                    "status":status,"authority":True,"formed_at":str(at),"date_precision":"exact",
                    "recognition":{"recognized":True,"basis":f"accepted proposal {proposal_id}"},
                    "relationship_refs":[],
                }
                family["unions"][result_id] = result_path
                counts["unions"] = len(family["unions"])
                for ref in participants: add_unique(person_entry(ref)["unions"], result_id)
                if status == "married":
                    add_unique(link_entry(proposer)["spouses"], target); add_unique(link_entry(target)["spouses"], proposer)
            event_type = "marriage_formed" if status == "married" else "betrothal_formed"
        else:
            queue[:] = [ref for ref in queue if ref != proposal_id]
            return {"kind":"family_autonomy","proposal_ref":proposal_id,"status":"unsupported_proposal_kind"}

        event_id = f"family.event.autonomy.{digest}"
        event_path = f"state/family/events/{event_id}.json"
        if self.repository.read_optional_bytes(event_path) is None and event_path not in record_writes:
            record_writes[event_path] = {
                "schema":"family-event","event_id":event_id,"event_type":event_type,"occurred_at":str(at),
                "authority":True,"subject_refs":participants,"source_refs":[proposal_id],
            }
            family["events"][event_id] = event_path
            counts["events"] = len(family["events"])
            for ref in participants: add_unique(person_entry(ref)["events"], event_id)
        queue[:] = [ref for ref in queue if ref != proposal_id]
        semantic_event_id = self._append_internal_event(
            world_events, command=command, identity=f"{proposal_id}:{at}:autonomous-family",
            kind="family_lifecycle_autonomous", at=at, host_refs=(proposal_id,), actor_refs=tuple(participants),
            affected_owner_refs=(FAMILY_INDEX_PATH, KINSHIP_INDEX_PATH, result_path, event_path),
            material_consequence_refs=(result_id, event_id), classification="restricted",
            audience_refs=tuple(participants), source_refs=(proposal_id,),
        )
        return {
            "kind":"family_autonomy","proposal_ref":proposal_id,"status":"settled",
            "record_ref":result_id,"family_event_ref":event_id,"semantic_event_id":semantic_event_id,
        }
    ...
    def _apply_team_relationship_event(self, participant_refs: Sequence[str], *, event_ref: str, interaction_kind: str, summary: str, at: CampaignTime, record_writes: Dict[str, Dict[str, Any]], player_id: str) -> None:
        refs=list(dict.fromkeys(ref for ref in participant_refs if isinstance(ref,str))); applied=0
        for source in refs:
