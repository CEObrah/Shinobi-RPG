#!/usr/bin/env python3
"""Validate current registered JSON owner/static structures against release templates."""
...
def load_schema_templates(repo):
    idx=json.load(open(os.path.join(repo,'runtime/contracts/template-index.json'))); out={}
...
def schema_object_contracts(schema,path='',out=None):
    ...
    (allOf/anyOf/oneOf). The runtime schema and domain regressions still own semantic
...
def main(repo):
    ...
    # Every mutable-state template must also have one fact-free blank-owner skeleton.
    blank_index_path=os.path.join(repo,'runtime/contracts/blank-owner-index.json')
    if not os.path.exists(blank_index_path):
        errors.append('blank-owner-index missing')
    else:
        blank_index=json.load(open(blank_index_path))
        blank_owners=blank_index.get('owners',{})
        if not isinstance(blank_owners,dict):
            errors.append('blank-owner-index owners must be object')
            blank_owners={}
        mutable_templates=set()
        for sid,ent in entries.items():
            tp=os.path.join(repo,ent['path'])
            if os.path.exists(tp) and json.load(open(tp)).get('scope')=='mutable_state':
                mutable_templates.add(sid)
        missing=sorted(mutable_templates-set(blank_owners))
        extra=sorted(set(blank_owners)-mutable_templates)
        if missing: errors.append(f'blank-owner-index missing mutable templates {missing}')
        if extra: errors.append(f'blank-owner-index has non-mutable templates {extra}')
        for sid,rel in blank_owners.items():
            full=os.path.join(repo,rel) if isinstance(rel,str) else ''
            if not rel or not os.path.isfile(full):
                errors.append(f'blank owner {sid}: missing {rel}')
                continue
            blank=json.load(open(full))
            if not isinstance(blank,dict): errors.append(f'blank owner {sid}: root must be object')
    ...
    static_checked=0
    skip_prefixes=('runtime/contracts/templates/','runtime/contracts/template-index-shards/','runtime/contracts/path-templates/')
    static_paths = glob.glob(os.path.join(repo,'game','data','**','*.json'),recursive=True) + glob.glob(os.path.join(repo,'runtime','contracts','**','*.json'),recursive=True)
    ...
    pidx=os.path.join(repo,'runtime/contracts/path-template-index.json');path_checked=0
    ...
    scidx=os.path.join(repo,'runtime/contracts/system-contract-index.json')
    if not os.path.exists(scidx): errors.append('system-contract-index missing')
        ...
        for system_id,rel in si.get('systems',{}).items():
            ...
            c=json.load(open(cp))
            if c.get('schema')!='system-contract' or c.get('system_id')!=system_id: errors.append(f'{rel}: malformed system contract')
    ...
    nrp=os.path.join(repo,'runtime/contracts/narration-router.json')
    ...
    for sid,ent in entries.items():
        ...
        c=json.load(open(tp))
        if c.get('schema')!='file-template' or c.get('target_schema')!=sid or c.get('unknown_key_policy')!='reject': errors.append(f'{ent["path"]}: malformed structural template')
