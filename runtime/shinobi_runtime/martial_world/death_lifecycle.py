...
def settle_exact_death_estates(
    *, read_json: Callable[[str], Any], writes: dict[str, Any], faction_refs: Sequence[str],
    ...
    person_routes: Mapping[str, Mapping[str, Any]] | None = None,
) -> dict[str, Any]:
    ...
    """
    index = (
        {str(ref): copy.deepcopy(dict(route)) for ref, route in person_routes.items() if isinstance(ref, str) and isinstance(route, Mapping)}
        if isinstance(person_routes, Mapping)
        else exact_person_index(read_json=read_json, writes=writes, faction_refs=faction_refs)
    )
    living_people = {
...
def prune_dead_from_durable_activities(
    *, read_json: Callable[[str], Any], writes: dict[str, Any], dead_refs: Sequence[str],
    faction_refs: Sequence[str], person_routes: Mapping[str, Mapping[str, Any]] | None = None,
) -> dict[str, Any]:
    ...
    def current_person(ref: str) -> Mapping[str, Any] | None:
        route = people_index.get(ref)
        if not isinstance(route, Mapping):
            return None
        ...
        if not (
            0 <= ordinal < len(rows)
            and isinstance(rows[ordinal], Mapping)
            and rows[ordinal].get("person_id") == ref
        ):
            ordinal = next((
        ...
        person = copy.deepcopy(dict(rows[ordinal]))
        if route.get("owner_kind") == "faction":
            person["faction_ref"] = str(route.get("owner_ref") or "")
        else:
            person.pop("faction_ref", None)
        people_index[ref] = {**dict(route), "ordinal": ordinal, "person": person}
        return person
    try:
    ...
    if isinstance(deployments, dict) and isinstance(deployments.get("deployments"), dict):
        ...
        for dref, raw in list(rows.items()):
            ...
            if structure is not None:
                ...
                survivor_people: dict[str, Mapping[str, Any]] = {}
                for ref in member_refs:
                    if ref in dead:
                        continue
                    person = current_person(ref)
                    if isinstance(person, Mapping) and is_living(person):
                        survivor_people[ref] = person
                survivors = list(survivor_people)
                if survivors != member_refs:
                    if not survivors:
                        rows.pop(dref, None); removed_deployments.append(str(dref)); touched = True
                        continue
                    records = {ref: copy.deepcopy(dict(survivor_people[ref])) for ref in survivors}
