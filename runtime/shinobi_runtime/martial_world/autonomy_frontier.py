...
from .faction_relations import friendly_aid_transfer_envelope
...
from .strategic_autonomy import friendly_aid_options, hostile_action_options, investment_priority_options
...
def settle_faction_autonomy_frontier(
    *,
    ...
    execute_friendly_aid: Callable[[str, str, int], dict[str, Any]],
    ...
    def strategic_detachment_variants(
        *, faction: Mapping[str, Any], roster: Mapping[str, Any], action: str,
        hostility: int, risk_tolerance: int,
    ) -> list[dict[str, Any]]:
        """Enumerate bounded exact detachments without choosing one for the faction."""
        home = str(faction.get("headquarters") or "")
        if not home:
            return []
        candidates: list[Mapping[str, Any]] = []
        for person in usable_martial_people(roster, exclude_committed=unavailable_person_refs()):
            if not isinstance(person, Mapping):
                continue
            ref = str(person.get("person_id") or "")
            if not ref or ref == player_ref or bool(person.get("retired_from_field", False)):
                continue
            if at.year - int(person.get("birth_year", at.year)) < 16:
                continue
            location = str(person.get("location_ref") or "")
            site = site_rows.get(location) if isinstance(site_rows, Mapping) else None
            place = str(site.get("parent_place_ref") or "") if isinstance(site, Mapping) else location
            if not place:
                place = home
            if place != home:
                continue
            candidates.append(person)
        candidates.sort(key=lambda p: (-person_combat_index(p), str(p.get("person_id") or "")))
        refs = [str(person.get("person_id")) for person in candidates if person.get("person_id")]
        if not refs:
            return []
        if action == "formal_challenge":
            return [{"participant_refs":[ref], "leader_ref":ref, "detachment_posture":"single_challenger"} for ref in refs[:8]]

        risk = max(0, min(100, int(risk_tolerance)))
        hostility = max(0, min(100, int(hostility)))
        home_reserve = max(1, len(refs) * max(15, 45 - risk // 3) // 100)
        deployable = max(0, len(refs) - home_reserve)
        if deployable <= 0:
            return []
        if action == "faction_war_strike":
            fraction = max(25, min(80, 25 + risk // 2 + max(0, hostility - 60) // 2))
            advisory = min(deployable, max(3, len(refs) * fraction // 100))
            minimum = min(deployable, 3)
        else:
            advisory = min(deployable, max(2, int(len(refs) ** 0.5) * max(80, 80 + risk) // 100))
            minimum = min(deployable, 2)
        sizes = sorted({size for size in (minimum, max(minimum, advisory // 2), advisory, deployable) if 0 < size <= deployable})
        variants: list[dict[str, Any]] = []
        # Enumerate varied complete parties rather than fixing the non-leader
        # slots to a combat-ranked prefix before the GM sees the judgment.
        pool = refs[: min(len(refs), 10)]
        for size in sizes:
            if size <= 0 or size > len(pool):
                continue
            for combo in combinations(pool, size):
                for leader in combo[: min(3, len(combo))]:
                    variants.append({
                        "participant_refs":list(combo), "leader_ref":leader,
                        "detachment_posture":"lean" if size == sizes[0] else ("heavy" if size == sizes[-1] else "balanced"),
                    })
                    if len(variants) >= 24:
                        return variants
        return variants

    def execute_autonomous_escort_contract(fid: str, choice: Mapping[str, Any]) -> dict[str, Any]:
        """Apply one exact GM-authored NPC escort acceptance after revalidation."""
        if str(choice.get("intent") or "") != "accept":
            return {"result": "declined"}
        cid = str(choice.get("contract_ref") or "")
        participants = [str(ref) for ref in choice.get("participant_refs", []) if isinstance(ref, str)]
        leader_ref = str(choice.get("leader_ref") or "")
        muster_location = str(choice.get("muster_location_ref") or "")
        if not cid or not participants or len(set(participants)) != len(participants) or not muster_location or leader_ref not in participants:
            raise ValueError("npc contract acceptance choice invalid")
        ...
        if not isinstance(contract, Mapping) or contract.get("status") != "offered" or contract.get("beneficiary_ref") not in {None, ""}:
            raise ValueError("npc contract offer no longer available")
        ...
        if objective.get("kind") != "escort_shipment":
            raise ValueError("npc contract objective unsupported")
        route_ref = str(objective.get("route_ref") or "")
        route = route_index.get(route_ref)
        if not isinstance(route, Mapping):
            raise ValueError("npc contract route unavailable")
        ...
        if not origin or not destination or origin == destination or {origin, destination} != ends or str(faction.get("headquarters", "")) != origin:
            raise ValueError("npc contract endpoints changed")
        ...
        if not source_region or not destination_region:
            raise ValueError("npc contract regions unavailable")
        minimum = max(1, int(objective.get("minimum_escort_count", 1)))
        if len(participants) < minimum:
            raise ValueError("npc contract escort below minimum")
        usable = {
        ...
        for ref in participants:
            if ref == player_ref or ref not in usable:
                raise ValueError("npc contract participant unavailable")
            exact_location = str(usable[ref].get("location_ref") or "")
            site = sites.get(exact_location) if isinstance(sites, Mapping) else None
            at_origin = exact_location == origin or (isinstance(site, Mapping) and str(site.get("parent_place_ref") or "") == origin)
            if not at_origin or exact_location != muster_location:
                raise ValueError("npc contract party no longer co-located")
        ...
        if not isinstance(stock, dict) or quantity <= 0 or int(stock.get(item_ref, 0)) < quantity:
            raise ValueError("npc contract cargo unavailable")
        toll_cash = max(0, int(route.get("toll_cash", 0)))
        if max(0, int(source_market.get("cash_pool", 0))) < toll_cash:
            raise ValueError("npc contract toll cash unavailable")
        ...
        if freight_required > int(available_logistics.get("freight_capacity_kg", 0)) or crew_required > int(available_logistics.get("crew_capacity", 0)):
            raise ValueError("npc contract logistics unavailable")
        route_plan = travel_plan(world_seed=world_seed, start_at=at, start=origin, end=destination, mode="convoy")
        inventory_after, provision_reservation = reserve_faction_rations(
            copy.deepcopy(inventory), faction_ref=fid, participant_count=len(participants),
            travel_seconds=provisioning_journey_seconds(route_plan),
        )
        next_commitments = reserve_resources(
            refresh_commitments(), resources=[("person", ref, fid) for ref in participants],
            actor_ref=leader_ref, owner_ref=fid, activity_ref=cid,
            activity_kind="contract_escort", started_at=at_iso, location_ref=muster_location,
        )
        accepted = contract_transition(contract, at=at_iso, to_status="accepted", actor_ref=leader_ref, participants=participants)
        accepted["beneficiary_ref"] = fid
        started = contract_transition(accepted, at=at_iso, to_status="in_progress", actor_ref=leader_ref)
        ...
        movement = build_route_journey(
            ...
            plan=route_plan, participants=participants, leader_ref=leader_ref,
        ...
        inventory_cache[fid] = (ipath, inventory_after)
        writes[mpath] = source_market
        market_cache[source_region] = (mpath, source_market)
        return {"result": "accepted_and_started", "contract_ref": cid, "participant_refs": participants, "route_ref": route_ref}

    # A consequential NPC intent authored by the GM is applied only when its
    # durable same-timestamp judgment event returns through the scheduler.
    # The callbacks below remain hard-authority validators: they may reject an
    # intent whose people/resources/route/treasury are no longer feasible, but
    # they never substitute a different human choice.
    for judgment_event in sorted_events:
        if str(judgment_event.get("kind") or "") != "npc_judgment_committed":
            continue
        judgment_kind = str(judgment_event.get("judgment_kind") or "")
        if judgment_kind not in {"faction_hostile_strategy", "faction_investment", "faction_aid_or_diplomacy", "faction_contract_acceptance"}:
            continue
        fid = str(judgment_event.get("actor_ref") or "")
        choice = judgment_event.get("selected_option")
        if not fid or not isinstance(choice, Mapping):
            raise ValueError("committed npc judgment invalid")
        if judgment_kind == "faction_hostile_strategy":
            if str(choice.get("action") or choice.get("intent") or "") == "stand_down":
                outcome = {"result":"stood_down"}
            else:
                outcome = start_strategic_operation(fid, choice)
                refresh_commitments()
        elif judgment_kind == "faction_investment":
            if str(choice.get("intent") or "") == "defer" or str(choice.get("kind") or "") == "defer_investment":
                outcome = {"result":"deferred"}
            else:
                outcome = start_autonomous_investment(fid, choice)
                refresh_commitments()
        elif judgment_kind == "faction_contract_acceptance":
            outcome = execute_autonomous_escort_contract(fid, choice)
            refresh_commitments()
        else:
            target_fid = str(choice.get("target_faction_ref") or "")
            if str(choice.get("intent") or "") in {"defer", "no_action"}:
                outcome = {"result": "deferred"}
            else:
                outcome = execute_friendly_aid(fid, target_fid, max(0, int(choice.get("amount_cash", 0))))
            obligation_ref = str(choice.get("personal_obligation_ref") or "")
            if outcome.get("result") == "aid_transferred" and obligation_ref:
                settled = resolve_personal_obligation(social_after, obligation_ref_value=obligation_ref)
                if settled.get("resolved"):
                    social_after = settled["state_after"]
                    outcome = {**outcome, "personal_obligation_repaid_ref": obligation_ref}
        reviews.append({
            "kind": "npc_judgment_applied", "judgment_kind": judgment_kind,
            "actor_ref": fid, "decision_ref": str(judgment_event.get("decision_ref") or ""),
            "decision_origin": "gm_npc_judgment", **dict(outcome),
        })
    ...
    for event in sorted_events:
        ...
        for action in review["ordered_actions"]:
            ...
            if action == "address_hostile_relation":
                ...
                if faction_type == "outlaw_faction":
                    ...
                    for route_ref in [str(x) for x in faction.get("operating_routes", []) if isinstance(x, str) and x]:
                        ...
                        if isinstance(route, Mapping):
                            local_places.update(str(route[key]) for key in ("from", "to") if route.get(key))
                    ...
                    for edge in relation_index.get(fid, []):
                        ...
                        if not target_ref or target_ref == fid: continue
                        try: _target_path, target_faction = load_faction(target_ref)
                ...
                options = hostile_action_options(
                    relation_index.get(fid, []), faction_ref=fid, year=at.year, month=at.month,
                    risk_tolerance=int(policy.get("risk_tolerance", 50)),
                    active_strategic_operations=int(active_strategic_operations(fid)),
                    faction_type=faction_type, outlaw_subtype=str(faction.get("outlaw_subtype") or ""),
                    eligible_raid_target_refs=eligible_raid_target_refs,
                    coalition_target_refs=set(coalition_targets_by_faction.get(fid, set())),
                )
                if options:
                    authored_options: list[dict[str, Any]] = []
                    risk = int(policy.get("risk_tolerance", 50))
                    for raw_option in options[:8]:
                        base = copy.deepcopy(dict(raw_option))
                        intent_variants = [
                            str(value) for value in base.pop("operation_intent_options", [])
                            if isinstance(value, str) and value
                        ]
                        advisory_intent = str(base.get("advisory_operation_intent") or "")
                        if not intent_variants:
                            intent_variants = [advisory_intent or ("honor_challenge" if str(base.get("action") or "") == "formal_challenge" else "punitive_expedition")]
                        detachments = strategic_detachment_variants(
                            faction=faction, roster=review_roster, action=str(base.get("action") or ""),
                            hostility=int(base.get("hostility", 0)), risk_tolerance=risk,
                        )
                        for operation_intent in intent_variants:
                            targeting_variants = ["disable"] if (
                                str(base.get("action") or "") == "formal_challenge"
                                or operation_intent in {"robbery","kidnapping","cargo_seizure","extortion","cargo_diversion"}
                            ) else ["disable", "lethal"]
                            for detachment in detachments:
                                for targeting_intent in targeting_variants:
                                    option = {**copy.deepcopy(base), **copy.deepcopy(detachment), "operation_intent": operation_intent}
                                    option["combat_policy"] = {
                                        "decision_origin":"gm_npc_judgment",
                                        "engagement_intent":str(option.get("action") or "strategic_contact"),
                                        "targeting_intent":targeting_intent,
                                        "target_policy":"pressure_then_nearest",
                                        "resource_policy":"conservative_auto",
                                        "withdrawal_policy":"no_voluntary_withdrawal",
                                    }
                                    authored_options.append(option)
                                    if len(authored_options) >= 48:
                                        break
                                if len(authored_options) >= 48:
                                    break
                            if len(authored_options) >= 48:
                        ...
                        if len(authored_options) >= 48:
                            break
                    authored_options.append({"action":"stand_down","intent":"stand_down"})
                    judgment = build_npc_judgment_event(
                        judgment_kind="faction_hostile_strategy", at_iso=at_iso, actor_ref=fid, owner_ref=fid,
                        options=authored_options, decision_context={"senior_decision_refs": senior_decision_refs},
                        source_event_id=str(event.get("event_id") or ""),
                    )
                    pending_one_off_events.append(judgment)
                    handoffs.append(judgment_handoff(judgment))
                    executed_actions.append({"action":action,"result":"awaiting_gm_judgment","decision_ref":judgment["decision_ref"],"option_count":len(options)})
                else:
                    executed_actions.append({"action": action, "result": "no_bounded_hostile_action"})
                continue

            if action == "invest_growth":
                options = investment_priority_options(
                    faction, living_population=living_population, residential_capacity=housing_capacity,
                ...
                if options:
                    options = [copy.deepcopy(dict(row)) for row in options[:23]] + [{"kind":"defer_investment","intent":"defer"}]
                    judgment = build_npc_judgment_event(
                        judgment_kind="faction_investment", at_iso=at_iso, actor_ref=fid, owner_ref=fid,
                        options=options, source_event_id=str(event.get("event_id") or ""),
                    )
                    pending_one_off_events.append(judgment)
                    handoffs.append(judgment_handoff(judgment))
                    executed_actions.append({"action":action,"result":"awaiting_gm_judgment","decision_ref":judgment["decision_ref"],"option_count":len(options)})
            ...
            if action == "consider_diplomacy":
                ...
                target_options = friendly_aid_options(
                    relation_index.get(fid, []), faction_ref=fid, year=at.year, month=at.month,
                    cash_reserve_months=cash_reserve_months,
                )
                if personal_aid and personal_aid[0]:
                    target_fid, obligation_ref = personal_aid
                    if not any(str(row.get("target_faction_ref") or "") == target_fid for row in target_options):
                        target_options.insert(0,{"target_faction_ref":target_fid,"support_pressure":1000,"personal_obligation_ref":obligation_ref})
                options: list[dict[str, Any]] = []
                for target_option in target_options[:12]:
                    target_fid = str(target_option.get("target_faction_ref") or "")
                    if not target_fid:
                        continue
                    try:
                        _tfpath, target_faction = load_faction(target_fid)
                        _tipath, target_inventory = load_inventory(target_fid)
                    except (FileNotFoundError, KeyError, TypeError, ValueError):
                        continue
                    envelope = friendly_aid_transfer_envelope(faction, inventory, target_faction, target_inventory)
                    if envelope.get("result") != "aid_available":
                        continue
                    minimum = max(1, int(envelope.get("minimum_cash", 1)))
                    maximum = max(minimum, int(envelope.get("maximum_cash", minimum)))
                    amounts = sorted({minimum, max(minimum, maximum // 2), maximum})
                    for amount in amounts:
                        options.append({
                            **copy.deepcopy(dict(target_option)), "intent":"aid", "amount_cash":int(amount),
                            "maximum_current_aid_cash":maximum,
                        })
                if options:
                    options.append({"intent":"defer"})
                    judgment = build_npc_judgment_event(
                        judgment_kind="faction_aid_or_diplomacy", at_iso=at_iso, actor_ref=fid, owner_ref=fid,
                        options=options[:24], source_event_id=str(event.get("event_id") or ""),
                    )
                    pending_one_off_events.append(judgment)
                    handoffs.append(judgment_handoff(judgment))
                    executed_actions.append({"action":action,"result":"awaiting_gm_judgment","decision_ref":judgment["decision_ref"],"option_count":len(options)})
        ...
        # A funded escort offer is a meaningful institutional choice. Python may
        # identify a physically feasible exact party and logistics envelope, but
        # acceptance itself belongs to the GM-authored NPC judgment frontier.
        ...
        if "evaluate_contracts" in review["ordered_actions"]:
            ...
            for cid in ([] if escort_capacity <= current_escort_jobs else sorted(str(x) for x in active_contracts)):
                ...
                for person in usable_martial_people(review_roster, exclude_committed=unavailable_person_refs()):
                    ...
                    site = site_rows.get(exact_location) if isinstance(site_rows, Mapping) else None
                    at_origin = exact_location == origin or (isinstance(site, Mapping) and str(site.get("parent_place_ref") or "") == origin)
                    if at_origin:
                        available_by_location.setdefault(exact_location, []).append(person)
                viable_groups: list[tuple[str, list[dict[str, Any]]]] = []
                for exact_location, rows in available_by_location.items():
                    rows.sort(key=lambda p: (-person_combat_index(p), str(p.get("person_id", ""))))
                    if len(rows) >= minimum:
                        viable_groups.append((exact_location, rows))
                if not viable_groups:
                    continue
                item_ref = str(objective.get("item_ref") or "")
                quantity = max(0, int(objective.get("quantity", 0)))
                try:
                    _mpath, source_market = load_market(source_region)
                except FileNotFoundError:
                    continue
                stock = source_market.get("stock", {}) if isinstance(source_market, Mapping) else {}
                toll_cash = max(0, int(route.get("toll_cash", 0)))
                if not isinstance(stock, Mapping) or quantity <= 0 or int(stock.get(item_ref, 0)) < quantity or max(0, int(source_market.get("cash_pool", 0))) < toll_cash:
                    continue
                freight_required = max(0, int(objective.get("freight_capacity_kg", 0)))
                crew_required = max(0, int(objective.get("civilian_crew_count", freight_crew_required(freight_required))))
                pools = civilian_state.get("places", {}) if isinstance(civilian_state.get("places"), Mapping) else {}
                origin_pool = pools.get(origin, {}) if isinstance(pools, Mapping) else {}
                population = max(0, int(origin_pool.get("current_population", 0))) if isinstance(origin_pool, Mapping) else 0
                available_logistics = civilian_available_capacity(place_ref=origin, place_population=population, route_operations=route_ops_state)
                if freight_required > int(available_logistics.get("freight_capacity_kg", 0)) or crew_required > int(available_logistics.get("crew_capacity", 0)):
                    continue
                try:
                    route_plan = travel_plan(world_seed=world_seed, start_at=at, start=origin, end=destination, mode="convoy")
                    ...
                    continue
                accept_options: list[dict[str, Any]] = []
                candidate_refs: list[str] = []
                for muster_location, available in sorted(viable_groups, key=lambda item:item[0]):
                    refs = [str(row.get("person_id")) for row in available if isinstance(row.get("person_id"), str)]
                    candidate_refs.extend(refs)
                    # Enumerate multiple complete exact parties.  Python proves
                    # feasibility; it does not choose which people or leader the
                    # institution commits.
                    for party_tuple in combinations(refs, minimum):
                        party = list(party_tuple)
                        try:
                            reserve_faction_rations(
                                copy.deepcopy(inventory), faction_ref=fid, participant_count=len(party),
                                travel_seconds=provisioning_journey_seconds(route_plan),
                            )
                        except (KeyError, TypeError, ValueError):
                            continue
                        for leader_ref in party:
                            accept_options.append({
                                "intent":"accept","contract_ref":cid,"participant_refs":party,
                                "leader_ref":leader_ref,"muster_location_ref":muster_location,"route_ref":route_ref,
                            })
                            if len(accept_options) >= 23:
                                break
                        if len(accept_options) >= 23:
                            break
                    if len(accept_options) >= 23:
                ...
                if not accept_options:
                    continue
                decision = build_npc_judgment_event(
                    judgment_kind="faction_contract_acceptance", at_iso=at_iso,
                    actor_ref=fid, owner_ref=cid,
                    options=accept_options + [{"intent":"decline","contract_ref":cid}],
                    decision_context={
                        "faction_ref":fid,"contract_ref":cid,"reward_cash":max(0,int(contract.get("reward_cash",0))),
                        "minimum_escort_count":minimum,"candidate_participant_refs":sorted(set(candidate_refs)),
                        "route_ref":route_ref,
                    },
                    source_event_id=str(next((e.get("event_id") for e in sorted_events if e.get("kind")=="faction_review" and e.get("owner_ref")==fid), "")),
                )
                pending_one_off_events.append(decision)
                reviews.append({"kind":"faction_contract_judgment_due","faction_ref":fid,"contract_ref":cid,"decision_ref":decision["decision_ref"]})
                executed_actions.append({"action":"evaluate_contracts","result":"gm_judgment_required","contract_ref":cid})
                break
