...
def _legacy_detection_channels(sector: Mapping[str, Any]) -> list[str]:
    """Infer only capabilities that legacy sector construction actually paid for.

    Historical sectors predate explicit channel metadata, but establishment consumed
    barrier anchors and sensor relays.  Treat those as barrier/chakra capability; do
    not invent patrol, scent, or physical tracking channels merely for compatibility.
    """
    channels: list[str] = []
    stock_ref = sector.get("stock_ref")
    if isinstance(stock_ref, str) and stock_ref:
        channels.extend(("barrier", "chakra"))
    return sorted(set(channels or ("visual",)))


def _environment_channel_factor(channels: Sequence[str], factors: Mapping[str, int]) -> int:
    """Return a monotonic environmental-retention factor for alternative sensors.

    ``detection_milli`` already represents the network's combined detection quality.
    Weather therefore modifies that baseline through the best *available* independent
    sensing channel. Adding a new channel can never make an existing network worse.
    """
    values = [max(0, min(1000, int(factors[channel]))) for channel in channels if channel in factors]
    if not values:
        return 0
    return max(values)
...
def apply_route_security_detection(
    ...
    *,
    ...
    for sector_ref, sector in sorted(sectors.items()):
        ...
        channels = sector.get("detection_channels")
        if not isinstance(channels, list) or not channels:
            channels = _legacy_detection_channels(sector)
        allowed_channels = {"visual", "auditory", "tracking", "scent", "chakra", "barrier"}
        if any(not isinstance(value, str) or value not in allowed_channels for value in channels):
            raise CommandRejectedError("security-network-registry_invalid")
        channels = sorted(set(channels))
        environment = environment_snapshot(owner.repository, world_time=str(at), location_ref=place_ref)
        effects = environment.get("mechanical_effects")
        if not isinstance(effects, Mapping):
            raise CommandRejectedError("derived_environment_invalid")
        channel_factors = {
            "visual": int(effects.get("visibility_milli", 1000)),
            "auditory": max(400, 1000 - int(effects.get("sound_masking_milli", 0)) * 3 // 5),
            "tracking": int(effects.get("track_evidence_milli", 1000)),
            "scent": int(effects.get("scent_tracking_milli", 1000)),
            "chakra": int(effects.get("chakra_sensing_milli", 1000)),
            "barrier": 1000,
        }
        channel_factor = _environment_channel_factor(channels, channel_factors)
        effective = (coverage * detection) // 1000
        effective = effective * channel_factor // 1000
        ...
        detections[detection_ref] = {
            ...
            "detection_channel_factor_milli": channel_factor,
            "effective_detection_channels": channels,
            "environment_ref": environment.get("weather_block_ref"),
            "environment_condition": environment.get("condition"),
            "roll_milli": roll,
