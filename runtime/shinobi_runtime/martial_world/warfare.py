...
from .npc_judgment import build_npc_judgment_event, institutional_office_judgment_events, judgment_handoff
...
def _strategic_raid_cargo_options(
    view: Callable[[str], Any], inventory: Mapping[str, Any], *, carrier_count: int,
) -> list[dict[str, Any]]:
    """Enumerate bounded exact cargo choices from real current target stock.

    Python establishes physical capacity and current inventory only. It does not
    rank one stack as the humans' preferred loot. Quantity variants keep the
    judgment surface bounded while preserving a meaningful take-less/take-more
    choice for each available stack.
    """
    ...
    if food_qty > 0:
        ...
        if take > 0:
            candidates.append(("food", "food_ration_day", take, unit_value))

    raw = inventory.get("raw_materials", {}) if isinstance(inventory.get("raw_materials"), Mapping) else {}
    for item_ref, raw_qty in sorted(raw.items()):
        ...
        if take > 0:
            candidates.append(("raw_materials", str(item_ref), take, unit_value))

    equipment = inventory.get("equipment", {}) if isinstance(inventory.get("equipment"), Mapping) else {}
    for item_ref, raw_qty in sorted(equipment.items()):
        ...
        if take > 0:
            candidates.append(("equipment", str(item_ref), take, unit_value))

    options: list[dict[str, Any]] = []
    for bucket, item_ref, maximum, unit_value in candidates:
        quantities = sorted({1, max(1, maximum // 2), maximum})
        for qty in quantities:
            if qty <= maximum:
                options.append({
                    "intent": "seize_cargo", "bucket": bucket, "item_ref": item_ref,
                    "quantity": qty, "estimated_value_cash": qty * unit_value,
                })
    return options[:36]


def _strategic_raid_aftermath_options(
    *, view: Callable[[str], Any], operation: Mapping[str, Any], target_faction: Mapping[str, Any],
    target_fid: str, attacker_refs: Sequence[str], defender_refs: Sequence[str],
    people_after: Mapping[str, Mapping[str, Any]], at: datetime,
) -> list[dict[str, Any]]:
    """Return feasible post-victory choices without selecting one."""
    intent = str(operation.get("operation_intent") or "")
    active_carriers = [
        ref for ref in attacker_refs
        if isinstance(people_after.get(ref), Mapping) and _combat_active(people_after[ref], year=at.year)
    ]
    options: list[dict[str, Any]] = [{"intent": "withdraw_without_seizure"}]
    if not active_carriers:
        return options

    if intent in {"robbery", "extortion"}:
        treasury = max(0, int(target_faction.get("treasury_cash", 0)))
        per_carrier = 2500 if intent == "robbery" else 1800
        maximum = min(treasury, len(active_carriers) * per_carrier)
        if maximum > 0:
            for amount in sorted({max(1, maximum // 4), max(1, maximum // 2), maximum}):
                if amount <= maximum:
                    options.append({"intent": "seize_cash", "amount_cash": amount})
    elif intent in {"cargo_seizure", "cargo_diversion"}:
        try:
            target_inventory = _load_inventory(view, target_fid)
        except (FileNotFoundError, KeyError, TypeError, ValueError):
            target_inventory = {}
        options.extend(_strategic_raid_cargo_options(view, target_inventory, carrier_count=len(active_carriers)))
    elif intent == "kidnapping":
        try:
            custody = view(_CUSTODY)
        except FileNotFoundError:
            custody = {"records": []}
        rows = custody.get("records", []) if isinstance(custody, Mapping) else []
        active_records = {
        ...
        captor_refs = active_carriers[:6]
        for captive_ref in sorted(set(str(ref) for ref in defender_refs if isinstance(ref, str) and ref)):
            person = people_after.get(captive_ref)
            if not isinstance(person, Mapping) or not _living(person) or _combat_active(person, year=at.year) or captive_ref in active_records:
                continue
            ransom = principal_ransom_value_cash(person)
            for captor_ref in captor_refs:
                options.append({
                    "intent": "take_captive", "captive_ref": captive_ref,
                    "captor_ref": captor_ref, "ransom_value_cash": ransom,
                })
                if len(options) >= 48:
                    return options
    return options
...
def _apply_strategic_raid_objective(
    *, view: Callable[[str], Any], writes: dict[str, Any], operation_ref: str, operation: dict[str, Any],
    ...
    authored_choice: Mapping[str, Any],
) -> tuple[dict[str, Any], dict[str, Any], Mapping[str, Any]]:
    """Apply one exact GM-authored post-raid choice after fresh revalidation."""
    operation_intent = str(operation.get("operation_intent") or "")
    choice_intent = str(authored_choice.get("intent") or "")
    active_carriers = [
        ref for ref in attacker_refs
        if isinstance(people_after.get(ref), Mapping) and _combat_active(people_after[ref], year=at.year)
    ]
    if not active_carriers:
        return operation, {"result": "no_combat_active_carriers", "operation_intent": operation_intent}, commitments
    operation["return_escort_refs"] = list(active_carriers)
    if choice_intent == "withdraw_without_seizure":
        return operation, {"result": "withdrew_without_seizure", "operation_intent": operation_intent}, commitments

    fresh_options = _strategic_raid_aftermath_options(
        view=view, operation=operation, target_faction=target_faction, target_fid=target_fid,
        attacker_refs=attacker_refs, defender_refs=defender_refs, people_after=people_after, at=at,
    )
    comparable = {k: v for k, v in authored_choice.items() if k not in {"option_ref", "ransom_value_cash", "estimated_value_cash"}}
    valid = False
    for option in fresh_options:
        check = {k: v for k, v in option.items() if k not in {"option_ref", "ransom_value_cash", "estimated_value_cash"}}
        if check == comparable:
            valid = True
            break
    if not valid:
        raise ValueError("jianghu strategic raid aftermath choice no longer feasible")

    if choice_intent == "seize_cash":
        seized = max(0, int(authored_choice.get("amount_cash", 0)))
        treasury = max(0, int(target_faction.get("treasury_cash", 0)))
        if seized <= 0 or seized > treasury:
            raise ValueError("jianghu strategic raid cash choice invalid")
        target_faction["treasury_cash"] = treasury - seized
        operation["seized_cash"] = max(0, int(operation.get("seized_cash", 0))) + seized
        return operation, {"result": "cash_seized_in_transit", "operation_intent": operation_intent, "cash": seized}, commitments

    if choice_intent == "seize_cargo":
        target_inventory = _load_inventory(view, target_fid)
        cargo = {
            "bucket": str(authored_choice.get("bucket") or ""),
            "item_ref": str(authored_choice.get("item_ref") or ""),
            "quantity": max(0, int(authored_choice.get("quantity", 0))),
            "estimated_value_cash": max(0, int(authored_choice.get("estimated_value_cash", 0))),
        }
        ...
        operation["seized_item_ref"] = cargo["item_ref"]
        operation["seized_quantity"] = cargo["quantity"]
        operation["seized_cargo_bucket"] = cargo["bucket"]
        return operation, {"result": "cargo_seized_in_transit", "operation_intent": operation_intent, **cargo}, commitments

    if choice_intent == "take_captive":
        captive_ref = str(authored_choice.get("captive_ref") or "")
        captor_ref = str(authored_choice.get("captor_ref") or "")
        if captor_ref not in active_carriers:
            raise ValueError("jianghu strategic raid captor unavailable")
        try:
        ...
        rows.append(create_custody_record(
            person_ref=captive_ref, captor_ref=captor_ref, at=at.isoformat(), location_ref=target_site,
        ...
        try:
            ...
            rows.pop()
            raise ValueError("jianghu strategic raid captive no longer available")
        ...
        return operation, {"result": "captive_seized_in_transit", "operation_intent": operation_intent, "captive_ref": captive_ref, "captor_ref": captor_ref}, commitments

    raise ValueError("jianghu strategic raid aftermath intent invalid")
    ...
    *, read_json: Callable[[str], Any], writes: dict[str, Any], at: datetime,
    ...
    for op_ref in sorted(str(x) for x in rows if isinstance(x, str)):
        ...
        if not isinstance(raw, Mapping) or raw.get("status") not in {"mobilizing", "traveling_outbound"}:
            continue
        if str(raw.get("mobilization_basis") or "") == "gm_authored_exact_detachment":
            continue
        ...
        paused_before = institutional_training_pause_refs(
            ...
            unavailable_refs=sorted(paused_before_mobilization | physically_unavailable),
        )
...
def _settle_released_training(
    *, view: Callable[[str], Any], writes: dict[str, Any], released_refs: Sequence[str], at: datetime,
    ...
    busy |= physical_unavailable_person_refs(current)
...
def _clear_dead_current_state(
    *, view: Callable[[str], Any], writes: dict[str, Any], dead: set[str],
    ...
    if not dead:
        return []
    judgment_events: list[dict[str, Any]] = []
    ...
    for fid in sorted(by_faction):
        ...
        judgment_events.extend(institutional_office_judgment_events(
            faction_ref=fid, at_iso=at.isoformat(), office_result=office_result, succession=succession,
            source_event_id=f"warfare-death:{fid}:{at.isoformat()}",
        ))
    ...
    if extinction.get("extinct_refs"):
        extinct_refs = {str(ref) for ref in extinction.get("extinct_refs", []) if isinstance(ref, str)}
        judgment_events = [event for event in judgment_events if str(event.get("actor_ref") or "") not in extinct_refs]
...
def settle_faction_operation_arrivals(
    *, read_json: Callable[[str], Any], writes: dict[str, Any],
    ...
    aftermath_due = [
        ...
        if isinstance(row, Mapping)
        and row.get("kind") == "npc_judgment_committed"
        and row.get("judgment_kind") == "strategic_raid_aftermath_policy"
    ]
    if not due and not aftermath_due:
        return {"writes":{},"reviews":[],"handoffs":[],"schedule_after":copy.deepcopy(dict(schedule_after))}
    view = _View(read_json, writes)
    ...
    # A victorious raid reaches the post-contact human decision at the exact
    # battle frontier.  The operation is already preparing to return, but the
    # +1 second departure cannot overtake this same-timestamp committed choice.
    # Apply only the exact GM-authored seizure/captive/withdraw option and
    # revalidate it against the post-battle owners before changing hard state.
    for event in sorted(aftermath_due, key=lambda row: (str(row.get("owner_ref") or ""), str(row.get("event_id") or ""))):
        op_ref = str(event.get("owner_ref") or "")
        raw_op = rows.get(op_ref) if isinstance(rows, Mapping) else None
        choice = event.get("selected_option")
        if not isinstance(raw_op, Mapping) or str(raw_op.get("operation_kind") or "") != "faction_raid" or str(raw_op.get("status") or "") != "return_preparing" or not isinstance(choice, Mapping):
            reviews.append({"kind":"strategic_raid_aftermath","event_id":event.get("event_id"),"operation_ref":op_ref,"result":"operation_not_awaiting_aftermath"})
            continue
        op = copy.deepcopy(dict(raw_op))
        expected_ref = str(op.get("raid_aftermath_decision_ref") or "")
        if expected_ref and expected_ref != str(event.get("decision_ref") or ""):
            raise ValueError("jianghu strategic raid aftermath decision mismatch")
        fid = str(op.get("faction_ref") or "")
        target_fid = str(op.get("target_faction_ref") or "")
        ...
        source_people = {
            str(p.get("person_id")): copy.deepcopy(dict(p))
            for p in source_roster.get("people", []) if isinstance(p, Mapping) and p.get("person_id")
        }
        target_people = {
            str(p.get("person_id")): copy.deepcopy(dict(p))
            for p in target_roster.get("people", []) if isinstance(p, Mapping) and p.get("person_id")
        }
        people_after = {**target_people, **source_people}
        attacker_refs = [
            str(ref) for ref in op.get("participant_refs", [])
            if isinstance(ref, str) and ref in source_people
        ]
        context = event.get("decision_context") if isinstance(event.get("decision_context"), Mapping) else {}
        defender_refs = [
            str(ref) for ref in context.get("defender_refs", [])
            if isinstance(ref, str) and ref in target_people
        ]
        target_site = str(context.get("target_site_ref") or op.get("target_site_ref") or target_faction.get("local_site_ref") or target_faction.get("headquarters") or "")
        op, objective_result, commitments = _apply_strategic_raid_objective(
            ...
            defender_refs=defender_refs, people_after=people_after, commitments=commitments,
            target_site=target_site, at=at, authored_choice=choice,
        )
        op["raid_aftermath_resolved_at"] = at.isoformat()
        op.pop("raid_aftermath_decision_ref", None)
        rows[op_ref] = op
        writes[faction_path(target_fid)] = compact_faction_state(target_faction)
        reviews.append({
            "kind":"strategic_raid_aftermath","event_id":event.get("event_id"),
            "operation_ref":op_ref,"faction_ref":fid,"target_faction_ref":target_fid,
            "decision_origin":"gm_npc_judgment",**dict(objective_result),
        })
    ...
    for event in sorted(due, key=lambda row: (str(row.get("owner_ref") or ""), str(row.get("event_id") or ""))):
        ...
        while True:
            ...
            result=simulate_exact_combat(
                ...
                authored_combat_policy=(
                    copy.deepcopy(dict(op.get("combat_policy", {})))
                    if isinstance(op.get("combat_policy"), Mapping) and op.get("combat_policy") else {
                        "decision_origin":"legacy_committed_operation",
                        "engagement_intent":str(op.get("operation_intent") or kind),
                        "targeting_intent":strategic_operation_targeting_intent(op),
        ...
        if kind == "faction_raid" and aggregate_winner == "side_a":
            aftermath_options = _strategic_raid_aftermath_options(
                view=_View(read_json, writes), operation=op, target_faction=target_faction,
                target_fid=target_fid, attacker_refs=attacker_refs, defender_refs=defender_refs,
                people_after=master, at=at,
            )
            if len(aftermath_options) > 1:
                judgment = build_npc_judgment_event(
                    judgment_kind="strategic_raid_aftermath_policy", at_iso=at.isoformat(),
                    actor_ref=fid, owner_ref=op_ref, options=aftermath_options,
                    decision_context={
                        "operation_ref": op_ref, "target_faction_ref": target_fid,
                        "target_site_ref": target_site, "operation_intent": str(op.get("operation_intent") or ""),
                        "defender_refs": sorted(set(defender_refs)),
                    },
                    source_event_id=str(event.get("event_id") or f"faction_operation_arrival:{op_ref}"),
                )
                schedule = upsert_one_off_event(schedule, judgment)
                handoffs.append(judgment_handoff(judgment))
                op["raid_aftermath_decision_ref"] = str(judgment.get("decision_ref") or "")
                raid_objective = {
                    "result":"gm_judgment_required","operation_intent":str(op.get("operation_intent") or ""),
                    "decision_ref":str(judgment.get("decision_ref") or ""),"option_count":len(aftermath_options),
                }
            else:
                raid_objective = {"result":"no_seizable_objective","operation_intent":str(op.get("operation_intent") or "")}
        ...
        if newly_dead:
            ...
            office_judgments = _clear_dead_current_state(
                view=post_view,writes=writes,dead=newly_dead,
                involved_factions=tuple([fid,target_fid]+sorted(allied_support)),at=at,
            )
