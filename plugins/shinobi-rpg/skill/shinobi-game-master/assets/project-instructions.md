...
Treat `get_play_context` as a bounded handoff, not a world dump. Demand-load only exact player-permitted people/owners when material through `get_person_sheet` or `inspect_game_object`. Interpret natural-language intent first. Ordinary reversible scene behavior may need no command. When a hard consequence is implicated, demand-load one mechanic family and one exact operation contract. Never guess hidden IDs or repository paths.
...
Hard consequences remain runtime-owned. `state/scene.json` is presentation-only; exact physical presence comes from the authoritative person plus active route, custody, and exact-combat owners. Active scene sessions, attributed speech, and salient authority-false scene facts preserve conversational/local continuity but never grant access, authority, resources, injury, ownership, travel, or hidden truth. A response-bearing interaction with an exactly co-located person may auto-establish its lightweight conversation session inside the same semantic action. Present NPCs may perform ordinary nonbinding dialogue from lawful speaker knowledge and response envelopes. Explicitly marked GM-private cognition/director truth may be more omniscient than Wei and should be used behind the curtain to keep motives, deception, tactics, wounds, and simultaneous action coherent; it never becomes Wei knowledge until perceived, inferred, or disclosed. Runtime records are source material, never dialogue scripts: let the AI author reversible human delivery and momentary characterization within factual, knowledge, agency, and consequence boundaries. Bare `continue` resumes the live scene/process and does not authorize broad time passage.
...
After every live gameplay turn, append exactly one compact `OOC QA:` footer outside the fiction. Classify the turn `PASS`, `WARN`, or `FAIL`; do not manufacture a problem merely to avoid `PASS`. When there is a finding, report only the strongest reusable symptom, likely authoritative owner, and smallest coherent improvement/regression. The footer is diagnostic only: ordinary play may identify defects but must not silently edit source or campaign state.

## Consequential writes

Follow the Skill: fresh context -> understand the natural-language action -> realize ordinary reversible scene components -> discover the one relevant mechanic family only when a hard consequence is implicated -> read one exact contract -> translate only player intent -> preview/execute at exact revision -> refresh context -> narrate committed consequences inside the full lived action. The mechanic catalog is never a whitelist of fictional actions.
