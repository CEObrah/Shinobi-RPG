...
def test_shinobi_skill_uses_canonical_folder_and_interaction_first_contract():
    plugin_root=ROOT/'plugins/shinobi-rpg'
    skill_root=plugin_root/'skill'/'shinobi-game-master'
    assert skill_root.is_dir()
    assert not (plugin_root/'shinobi-skill').exists()
    ...
    contract=(skill_root/'references/scene-contract.md').read_text(encoding='utf-8')
    review=(skill_root/'references/live-play-review.md').read_text(encoding='utf-8')
    assert 'interaction-first presentation as a general invariant' in skill
    assert 'Compact play context is a handoff' in craft
    assert 'Do not mistake mechanical strictness for presentation strictness' in contract
    assert 'guidance, not a permission token' in contract
    assert 'substantive interaction flattened into a status digest' in review
