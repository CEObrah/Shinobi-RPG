...
This file defines the universal stat scale. Body dimensions are authoritative in character body state and are resolved through `game/data/mechanics/body.json`; they are not attributes.
...
Attribute Support is calculated by `game/data/mechanics/stats.json`.
...
Use the registered bands in `game/data/mechanics/stats.json`.
...
- Grappling escape: grappling + strength or coordination support plus body mass geometry from `game/data/mechanics/body.json`.
...
Character attributes are resolved only from the canonical attribute fields and the registered formulas for the active subsystem.
