...
for path in sorted((ROOT / "state/team").glob("*.json")):
    ...
    for member_id in members:
        ...
        if "unassigned" in status or "pending_assignment" in status:
            warnings.append(f"stale_team_status_mirror:{team_id}:{owner_path.relative_to(ROOT)}:{member.get('team_status')}")
        current = member.get("current_unit_or_office")
        if member_id != data.get("jonin_instructor") and current is None:
            warnings.append(f"stale_team_unit_mirror:{team_id}:{owner_path.relative_to(ROOT)}:null")
        career = member.get("career_state")
        if isinstance(career, dict) and "current_unit_or_office" in career and career.get("current_unit_or_office") is None:
            warnings.append(f"stale_team_career_mirror:{team_id}:{owner_path.relative_to(ROOT)}:null")
        if member_id == data.get("jonin_instructor"):
            ...
            if "no permanent team" in command:
                warnings.append(f"stale_team_command_mirror:{team_id}:{owner_path.relative_to(ROOT)}:{command}")
