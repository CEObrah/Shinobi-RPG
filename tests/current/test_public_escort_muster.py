...
class _Fallback:
    def _jianghu_institutional_operation_resolution(self, command, meta, current_time):
        return {"delegated": True, "action": command.payload.get("action")}
...
def test_non_escort_institutional_actions_delegate_to_existing_owner():
    ...
    assert result == {"delegated": True, "action": "dispatch"}
