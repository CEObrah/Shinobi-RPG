...
> **Historical lineage record:** This document records the 2026-09-02 revision-1 rebaseline event. Revision 1 is the lineage origin, not the permanent current campaign revision; later lawful gameplay advances `state/meta.json`.


This package establishes the repaired current world snapshot as a **new revision-1 campaign baseline**. The pre-rebaseline state is retained only as forensic evidence in `docs/forensics/campaign-state-pre-rebaseline.tar.gz`.
...
5. Deploy from `main`. The shipped start command pins `SHINOBI_GIT_BRANCH=main` and runs the ordinary fail-closed `shinobi_runtime.bootstrap`.
6. Confirm the live runtime reports revision **1** before consequential play.

Do not point the rebaseline at a pre-rebaseline checkout or receipt store. The runtime intentionally refuses unsafe rollback/divergence states.
