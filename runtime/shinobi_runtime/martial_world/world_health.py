...
def retirement_due(person: Mapping[str, Any], *, year: int) -> bool:
    """Return whether one exact living person has reached field-retirement age.

    Retirement is biological/current-capability state, not faction membership.
    Institution-specific consequences are applied by the owning frontier.
    """
...
def annual_voluntary_departure_refs(
    ...
    period_key: str | None = None, allow_voluntary_departure: bool = True,
) -> list[str]:
    """Bounded normal churn plus crisis-driven departures from exact current members.

    Factions with an authored life-service tenure pass ``allow_voluntary_departure=False``.
    The rule is faction-level doctrine; no redundant oath flag is stored on every person.
    """
    if not allow_voluntary_departure:
        return []
