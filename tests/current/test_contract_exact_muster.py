...
def test_contract_start_rejects_escort_members_in_different_sites_of_same_city(tmp_path):
    ...
    player = next(row for row in roster["people"] if row.get("person_id") == "pc_wei_tang")
    second = next(row for row in roster["people"] if row.get("person_id") == second_ref)
    make_person_activity_ready_fixture(player)
    make_person_activity_ready_fixture(second)
    assert player["location_ref"] == "site.changan.inn"
