...
def expand_new_strategic_mobilizations(
    *, read_json: Callable[[str], Any], writes: dict[str, Any], at: datetime,
    ...
    for op_ref in sorted(str(x) for x in rows if isinstance(x, str)):
        ...
        blocked = paused_before_mobilization - set(existing)
...
def settle_faction_operation_arrivals(
    *, read_json: Callable[[str], Any], writes: dict[str, Any],
    ...
    for event in sorted(due, key=lambda row: (str(row.get("owner_ref") or ""), str(row.get("event_id") or ""))):
        ...
        target_busy = sorted(str(ref) for ref in commitments.get("person_index", {}) if isinstance(ref, str))
        ...
        if kind == "faction_reconnaissance":
            ...
            defender_count = len([p for p in target_people if _combat_active(p, year=at.year) and _person_place(p, target_faction, sites) == target_place])
            scout_values = []
        ...
        for support_ref, support_raw in sorted(rows.items()):
            ...
            refs = [ref for ref in refs if ref in ally_map and _combat_active(ally_map[ref], year=at.year) and _person_place(ally_map[ref], ally_faction, sites) == target_place]
            if not refs:
                continue
