...
def projectile_impact_speed_mps(
    projectile: Mapping[str, Any], *, launch_speed_mps: float, distance_m: float,
    maximum_range_m: float, interception_speed_factor_milli: int = 1000,
) -> float:
    """Return deterministic range-retained projectile speed at physical contact.

    Projectile damage previously used launch speed at every distance, making a
    needle at the edge of its range retain the same impact as one released at
    arm's length. Static projectile data owns the retained-speed endpoint so
    tuning remains explicit rather than hidden in combat code.
    """
    launch=max(0.0,float(launch_speed_mps))
    max_range=max(0.001,float(maximum_range_m))
    fraction=max(0.0,min(1.0,float(distance_m)/max_range))
    endpoint=_clamp(250,1000,_num(projectile.get("speed_retention_milli_at_max_range"),700))
    retained=1000.0-(1000.0-endpoint)*fraction
    intercept=max(0,min(1000,int(interception_speed_factor_milli)))
    return max(0.0,launch*retained/1000.0*intercept/1000.0)
...
def transition_seconds(item: Mapping[str, Any], *, action: str) -> float:
    ...
    if action == "draw": return max(0.0, _num(item.get("draw_seconds"), _num(item.get("ready_seconds"))))
    if action == "stow": return max(0.0, _num(item.get("stow_seconds")))
    raise ValueError("action must be ready, draw, or stow")
...
FIELD_CONSUMERS = {
    ...
    "draw_seconds": ("transition_seconds",),
    ...
    "projectile_width_mm": ("exact_combat.projectile_detection", "exact_combat.projectile_interception"),
    "projectile_length_mm": ("exact_combat.projectile_detection", "exact_combat.projectile_interception"),
    "speed_retention_milli_at_max_range": ("projectile_impact_speed_mps",),
