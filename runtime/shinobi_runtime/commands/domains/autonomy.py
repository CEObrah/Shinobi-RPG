...
class AutonomyCommandsMixin:
    ...
    def _apply_institution_autonomy_review(
        ...
        *,
        ...
        suffix = hashlib.sha256(f"{institution_id}\x00{at}\x00institution".encode()).hexdigest()[:18]
        ...
        if assignment.get("kind") == "academy_pipeline":
            ...
            replenishment_rate = assignment.get("youth_replenishment_per_review", 0)
            candidate_source_id = assignment.get("candidate_source_pool_id")
            if any(
                isinstance(v, bool) or not isinstance(v, int) or v < 0
                for v in (intake_rate, graduation_rate, replenishment_rate)
            ):
                raise CommandRejectedError("institution_autonomy_policy_invalid")
            if replenishment_rate and (not isinstance(candidate_source_id, str) or not candidate_source_id):
                raise CommandRejectedError("institution_autonomy_policy_invalid")
            periods = max(1, min(240, compacted))
            replenishment = 0
            replenishment_id = None
            candidate_source_record = pools.get(candidate_source_id) if isinstance(candidate_source_id, str) else None
            if replenishment_rate:
                if not isinstance(candidate_source_record, dict):
                    raise CommandRejectedError("institution_autonomy_population_invalid")
                candidate_pool = self._pool_reducer_view(candidate_source_id, candidate_source_record)
                candidate_representation = candidate_source_record.get("representation")
                if not isinstance(candidate_representation, Mapping):
                    raise CommandRejectedError("population_representation_invalid")
                candidate_anonymous = candidate_representation.get("anonymous_count")
                if isinstance(candidate_anonymous, bool) or not isinstance(candidate_anonymous, int) or candidate_anonymous < 0:
                    raise CommandRejectedError("population_representation_invalid")
                source_pool = self._pool_reducer_view(source_id, source_record)
                age_band = candidate_pool.dimensions.get("age_band")
                adolescent_available = age_band.get("adolescent", 0) if isinstance(age_band, Mapping) else 0
                if isinstance(adolescent_available, bool) or not isinstance(adolescent_available, int) or adolescent_available < 0:
                    raise CommandRejectedError("institution_autonomy_population_invalid")
                replenishment = min(candidate_anonymous, adolescent_available, replenishment_rate * periods)
                if replenishment:
                    selected = dict(neutral_proportional_selection(candidate_pool, replenishment))
                    selected["age_band"] = {
                        category: (replenishment if category == "adolescent" else 0)
                        for category in candidate_pool.dimensions["age_band"]
                    }
                    transfer = PopulationTransfer(
                        transfer_id=f"autonomy.candidate-replenishment.{suffix}",
                        source_pool_id=candidate_source_id,
                        destination_pool_id=source_id,
                        count=replenishment,
                        selected_dimensions=selected,
                        selection_mode="explicit_selection",
                    )
                    candidate_pool, source_pool = apply_transfer(candidate_pool, source_pool, transfer)
                    self._transfer_population_representation(candidate_source_record, source_record, replenishment)
                    replenishment_id = transfer.transfer_id
                    transfers.append({
                        "id": transfer.transfer_id, "at": str(at), "source_pool_id": candidate_source_id,
                        "destination_ref": source_id, "requested_count": replenishment, "accepted": replenishment, "rejected": 0,
                        "authority_ref": institution_id, "authority_basis": "autonomous_academy_candidate_replenishment",
                        "policy_ref": "autonomy.academy_pipeline", "method": "explicit_adolescent_selection",
                        "accepted_profile": {"numeric_distributions": {}, "category_counts": {"youth_candidate": replenishment}, "dimension_counts": {k: dict(v) for k, v in selected.items()}, "tags": ["academy_candidate_replenishment"]},
                        "materialized_person_ids": [], "source_removed": replenishment, "destination_added": replenishment,
                        "selection_note": "Already-existing adolescent civilians moved into the village Academy candidate pool; no adult is reclassified as youth and total population is conserved.",
                    })
                    self._trim_population_transfer_history(transfers)
                    self._persist_population_pool_record(candidate_source_record, candidate_pool, at=at)
            else:
                source_pool = self._pool_reducer_view(source_id, source_record)
            ...
            if graduation:
                ...
                reserve = force.get("reserve_capability")
                training_reserve = reserve.get("training_or_instruction") if isinstance(reserve, dict) else None
                if not isinstance(training_reserve, Mapping):
                    raise CommandRejectedError("force_reserve_capability_invalid")
                self._reserve_add(
                    force,
                    "training_or_instruction",
                    self._reserve_capability_state(training_reserve),
                    graduation,
                )
            ...
            pipeline_result = {
                "candidate_replenishment": replenishment,
                "intake": intake,
                "graduates": graduation,
                "candidate_replenishment_transfer_id": replenishment_id,
        ...
        service_training = (
            ...
            else assignment.get("service_training")
            if assignment.get("kind") == "academy_pipeline"
            else None
        )
        if isinstance(service_training, Mapping):
            force_ref = assignment.get("force_ref")
            fraction = service_training.get("completion_fraction_milli", 50)
            training_hours = service_training.get("training_hours_per_review", 24)
            if (
