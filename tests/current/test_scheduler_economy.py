...
def test_government_warrant_monthly_frontier_can_physically_close_for_npc():
    ...
    warrant_ref=f'warrant:{subject}'
    assert after['warrants'][warrant_ref]['status']=='mobilizing'
    assert 'state/martial-world/custody.json' not in result['writes']
    assert any(r.get('kind')=='government_response_mobilized' and r.get('mobilizations',0)>=1 for r in result['reviews'])
    contact=result['schedule_after']['one_off'][f'government_contact_due:{warrant_ref}']
    due=datetime.fromisoformat(contact['due_at'])
    assert due==at+timedelta(hours=24)

    first_writes=result['writes']
    def read_json_after(rel):
        return first_writes[rel] if rel in first_writes else read_json(rel)
    # The production scheduler may surface intervening calendar boundaries
    # before the contact frontier; this integration test feeds the exact
    # registered one-off after confirming its due time.
    contact_result=settle_martial_world_frontier(
        read_json=read_json_after,schedule=result['schedule_after'],events=[contact],at=due,
    )
    contact_gov=contact_result['writes']['state/martial-world/government.json']
    assert contact_gov['warrants'][warrant_ref]['status']=='contact_pending'
    assert 'state/martial-world/custody.json' not in contact_result['writes']
    judgment=next(
        row for row in contact_result['schedule_after']['one_off'].values()
        if row.get('kind')=='npc_judgment_due'
        and row.get('judgment_kind')=='government_contact_response_policy'
        and row.get('owner_ref')==warrant_ref
    )
    surrender=next(row for row in judgment['options'] if row.get('intent')=='surrender')

    second_overlay=dict(first_writes)
    second_overlay.update({str(k):copy.deepcopy(v) for k,v in contact_result['writes'].items()})
    def read_json_after_contact(rel):
        return second_overlay[rel] if rel in second_overlay else read_json(rel)
    committed={
        'event_id':'npc_judgment_committed:test-government-surrender',
        'kind':'npc_judgment_committed','due_at':due.isoformat(),
        'owner_ref':warrant_ref,'actor_ref':subject,
        'judgment_kind':'government_contact_response_policy',
        'decision_ref':judgment['decision_ref'],'selected_option':copy.deepcopy(surrender),
        'decision_context':copy.deepcopy(judgment['decision_context']),
    }
    final=settle_martial_world_frontier(
        read_json=read_json_after_contact,schedule=contact_result['schedule_after'],events=[committed],at=due,
    )
    final_gov=final['writes']['state/martial-world/government.json']
    assert warrant_ref not in final_gov['warrants']
    custody=final['writes']['state/martial-world/custody.json']
    ...
    assert any(r.get('kind')=='government_contact_response_policy' and r.get('result')=='surrendered_into_custody' for r in final['reviews'])
...
def test_subsecond_one_off_deadlines_round_up_to_campaign_second_without_becoming_scheduler_future():
    start=datetime(61,9,27,21,16,52)
    first=start+timedelta(seconds=14,microseconds=270000)
    second=start+timedelta(seconds=14,microseconds=730000)
    schedule={
        "schema":"jianghu-scheduler-1.0",
        "settled_through":start.isoformat(),
        "recurring":{},
        "one_off":{
            "person_physiology_due:a":{
                "event_id":"person_physiology_due:a","kind":"person_physiology_due",
                "owner_ref":"a","due_at":first.isoformat(),"last_settled_at":start.isoformat(),
                "requires_player_decision":False,
            },
            "person_physiology_due:b":{
                "event_id":"person_physiology_due:b","kind":"person_physiology_due",
                "owner_ref":"b","due_at":second.isoformat(),"last_settled_at":start.isoformat(),
                "requires_player_decision":False,
            },
        },
    }
    frontier=start+timedelta(seconds=15)
    events=due_events(schedule,after=start,through=frontier)
    assert {row["event_id"] for row in events}=={"person_physiology_due:a","person_physiology_due:b"}
    assert {row["due_at"] for row in events}=={frontier.isoformat()}
    after=settle_schedule(schedule,through=frontier,processed_events=events)
    assert after["settled_through"]==frontier.isoformat()
    assert after["one_off"]=={}


def test_subsecond_one_off_at_same_campaign_second_remains_due_after_other_frontier_class():
    start=datetime(61,9,27,21,16,52)
    frontier=start+timedelta(seconds=15)
    schedule={
        "schema":"jianghu-scheduler-1.0",
        ...
        "recurring":{
            "region_monthly":{
                "interval_days":30,"next_due_at":frontier.isoformat(),"owner_cursor":0,
                "owner_refs":["region.a"],"event_kinds":["regional_market_cycle"],
            }
        },
        "one_off":{
            "notice":{
                "event_id":"notice","kind":"test_notice",
                "due_at":(frontier-timedelta(microseconds=500000)).isoformat(),
                "requires_player_decision":False,
            }
        },
    }
    # Recurring regional work wins the class priority at this campaign second.
    first=due_events(schedule,after=start,through=frontier)
    assert first and first[0].get("schedule_class")=="region_monthly"
    after=settle_schedule(schedule,through=frontier,processed_events=first)
    # The sub-second one-off is quantized to the same representable second and
    # therefore remains discoverable instead of falling behind settled_through.
    second=due_events(after,after=frontier,through=frontier)
    assert [row["event_id"] for row in second]==["notice"]
    assert second[0]["due_at"]==frontier.isoformat()
