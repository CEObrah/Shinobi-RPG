...
_SPARSE_MAP_KEYS = ("obligations", "beliefs", "vows", "martial_familiarity")
...
def compact_social_state(state: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for key in _SPARSE_MAP_KEYS:
        rows = state.get(key, {})
        if rows not in (None, {}) and not isinstance(rows, Mapping):
            raise ValueError(f"jianghu social {key} invalid")
        if isinstance(rows, Mapping) and rows:
            out[key] = copy.deepcopy(dict(rows))
    return out
