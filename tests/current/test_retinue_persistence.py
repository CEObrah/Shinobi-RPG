...
def test_assignment_review_cannot_reuse_member_from_another_active_retinue(monkeypatch):
    ...
    pending = out["writes"][_DEPLOYMENTS]["deployments"]["retinue.two"]
    assert pending["status"] == "assignment_pending"
    assert pending["member_refs"] == []
    schedule = out["writes"]["state/martial-world/scheduler.json"]
    judgments = [row for row in schedule.get("one_off", {}).values() if row.get("kind") == "npc_judgment_due"]
    assert len(judgments) == 1
    event = judgments[0]
    assert event["judgment_kind"] == "retinue_assignment_policy"
    assign_options = [row for row in event["options"] if row.get("intent") == "assign"]
    assert assign_options
    assert all("reserved.guard" not in row["member_refs"] for row in assign_options)
    assert all(len(row["member_refs"]) == 3 for row in assign_options)
