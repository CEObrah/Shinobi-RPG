...
def transport_capacity_check(buildings: Mapping[str,Any], infrastructure:Mapping[str,Any]|None, inventory: Mapping[str,Any]) -> dict[str,Any]:
    ...
    assets=inventory.get('transport_assets',{}) if isinstance(inventory.get('transport_assets'),Mapping) else {}
    mounts=max(0,int(assets.get('riding_horses',0)))+max(0,int(assets.get('pack_animals',0))); wagons=max(0,int(assets.get('wagons',0)))
    return {'capacity':capacity,'mount_or_pack_used':mounts,'wagon_used':wagons,'within_capacity':bool(mounts<=capacity['mount_or_pack_slots'] and wagons<=capacity['wagon_slots'])}
