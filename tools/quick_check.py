...
def check_live_rule_consumers() -> None:
    result = subprocess.run(
        [sys.executable, str(ROOT / "tools/check_live_rule_consumers.py")],
        cwd=ROOT,
        env={**__import__("os").environ, "PYTHONDONTWRITEBYTECODE": "1"},
        check=False,
    )
    if result.returncode != 0:
        fail("live_rule_consumer_validation_failed")


def check_force_population_conservation() -> None:
    result = subprocess.run(
        [sys.executable, str(ROOT / "tools/check_force_population_conservation.py")],
        cwd=ROOT,
        env={**__import__("os").environ, "PYTHONDONTWRITEBYTECODE": "1"},
        check=False,
    )
    if result.returncode != 0:
        fail("force_population_conservation_failed")


def check_world_geography() -> None:
    result = subprocess.run(
        [sys.executable, str(ROOT / "tools/check_world_geography.py")],
        cwd=ROOT,
        env={**__import__("os").environ, "PYTHONDONTWRITEBYTECODE": "1"},
        check=False,
    )
    if result.returncode != 0:
        fail("world_geography_validation_failed")
...
def main() -> int:
    ...
    check_live_rule_consumers()
    check_force_population_conservation()
    check_world_geography()
