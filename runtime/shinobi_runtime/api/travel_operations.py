...
def movement_scene_projection(
    *,
    ...
    for key in (
        ...
        "segment_origin_place_ref",
        "segment_destination_place_ref",
        "started_at",
        "last_progress_at",
        "rest_place_ref",
    ):
    ...
    started_at = movement.get("started_at")
    if isinstance(started_at, str) and started_at:
        # ``started_at`` is journey-scoped while the exact movement clocks below
        # are deliberately current-segment scoped. Keep the compatibility field
        # and expose the scope explicitly so a long multi-edge journey cannot be
        # mistaken for a chronology desynchronization.
        context["journey_started_at"] = started_at

    route_index = movement.get("route_index")
    if isinstance(route_index, int) and not isinstance(route_index, bool) and route_index >= 0:
        context["segment_index"] = route_index
    ...
    if isinstance(elapsed, int) and not isinstance(elapsed, bool) and elapsed >= 0:
        ...
        context["segment_elapsed_seconds"] = elapsed
        context["elapsed_scope"] = "current_segment"
    if isinstance(required, int) and not isinstance(required, bool) and required > 0:
        context["required_seconds"] = required
        context["segment_required_seconds"] = required
        context["progress_scope"] = "current_segment"
        if isinstance(elapsed, int) and not isinstance(elapsed, bool) and elapsed >= 0:
            progress = min(1000, elapsed * 1000 // required)
            context["progress_milli"] = progress
            context["segment_progress_milli"] = progress
