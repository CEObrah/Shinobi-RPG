...
Review every live gameplay turn internally, but keep normal IC free of mandatory QA footers. Surface `OOC QA:` only when the player explicitly requests playtest/developer QA or when a serious defect risks false campaign truth, agency/knowledge, declared intent, a major exploit, a consequential decision, chronology, or persistence.

In explicit QA mode, report only the strongest reusable finding: observed symptom, player impact, likely owner, and smallest coherent fix/regression. Valid owners include GM Skill/presentation, runtime interface, runtime/rules mechanics, game data, projection source, explicit state repair, or feature/design. Do not manufacture or repeat a finding merely to satisfy a format. Ordinary play is observational only; actual source/state mutation requires explicit `OOC DEV:` intent.
