...
def deterministic_sex(*,stable:str,faction_id:str)->str:
    identity=_identity_rules().get(faction_id,{})
    policy=identity.get('admission_policy',{}) if isinstance(identity,Mapping) else {}
    allowed=policy.get('allowed_sexes') if isinstance(policy,Mapping) else None
    if not isinstance(allowed,list) or not allowed: allowed=['male','female']
    allowed=[str(x) for x in allowed if str(x) in {'male','female'}]
...
def person_lite(*,world_seed:str,faction_id:str,headquarters:str,ordinal:int,training:Mapping[str,int],recruitment_policy:Mapping[str,Any],current_year:int=61)->dict[str,Any]:
    ...
    identity=_identity_rules().get(faction_id,{})
    policy=identity.get('admission_policy',{}) if isinstance(identity,Mapping) else {}
    minimum=max(8,int(policy.get('minimum_entry_age',8)))
    ...
    sex=deterministic_sex(stable=stable,faction_id=faction_id)
    ...
    return {
      ...
      'membership_grade':grade,**({'standing_duty_ref':duty_ref} if duty_ref else {})}
