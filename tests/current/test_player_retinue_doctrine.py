...
from shinobi_runtime.commands.jianghu_extended import _legacy_route_temporary_combat_refs
...
def test_retinue_uses_four_sector_back_to_back_when_outnumbered():
    ...
    assert plan['assignments']['han']['preferred_action'] == 'hold'
...
def test_retinue_template_values_are_behavior_authority_not_trace_only():
    ...
    assert plan['assignments']['han']['preferred_action'] == 'attack'
    assert plan['assignments']['han']['role'] == 'pressure'
...
def test_legacy_route_participants_recover_only_real_allied_temporary_companions():
    movement={
        'participant_refs':['wei','guard','ally.a','ally.b','protected.civilian','enemy.accident'],
    }
    combat={'sides':{'side_a':['wei','guard','ally.a','ally.b'],'side_b':['enemy.accident']}}
    assert _legacy_route_temporary_combat_refs(
        movement=movement, combat=combat, actor_ref='wei', standing_refs=['guard'],
    ) == ['ally.a','ally.b']


def test_explicit_legacy_escort_refs_outrank_broader_participant_membership():
    movement={
        'participant_refs':['wei','guard','ally.a','protected.civilian'],
        'escort_refs':['wei','guard','ally.a'],
    }
    combat={'sides':{'side_a':['wei','guard','ally.a'],'side_b':['enemy']}}
    assert _legacy_route_temporary_combat_refs(
        movement=movement, combat=combat, actor_ref='wei', standing_refs=['guard'],
    ) == ['ally.a']
