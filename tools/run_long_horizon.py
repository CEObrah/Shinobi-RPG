...
from shinobi_runtime.martial_world.scheduler import due_events
...
_CHECKPOINT_SCHEMA = "jianghu-long-horizon-checkpoint-1.1"
...
def _person_metrics(overlay: Mapping[str, Mapping[str, Any]]) -> dict[str, int]:
    ...
    for path in sorted((ROOT / "state/martial-world/people").glob("*.json")):
        ...
        for person in doc.get("people", []):
            ...
            if health.get("status") == "dead":
                ...
                living_faction += 1
            if "training_state" in person or "training_carry_milli" in person:
                training_records += 1
    ...
    return {
        ...
        "personal_training_records": training_records,
...
def _tracked_cash_metrics(overlay: Mapping[str, Mapping[str, Any]]) -> dict[str, int]:
    ...
    Currency is conserved across faction treasuries, personal purses, regional
    market cash, contract escrow, tournament prize escrow, faction-sponsored
    ...
    for path in sorted((ROOT / "state/martial-world/people").glob("*.json")):
        ...
        for person in row.get("people", []) if isinstance(row, Mapping) else []:
            if isinstance(person, Mapping):
                personal_cash += max(0, int(person.get("personal_cash", 0)))
    ...
    total = (
        faction_cash + personal_cash + market_cash + contract_escrow
...
def _world_activity_metrics(overlay: Mapping[str, Mapping[str, Any]]) -> dict[str, Any]:
    def doc(rel: str) -> Mapping[str, Any]:
        row = overlay.get(rel)
        if isinstance(row, Mapping):
            return row
    ...
    history = doc("state/martial-world/world-history.json")
    counters = history.get("counters", {}) if isinstance(history.get("counters"), Mapping) else {}
    totals = history.get("totals", {}) if isinstance(history.get("totals"), Mapping) else {}
    ...
    return {
        "world_history_recent_count": len(recent),
        "world_history_counters": dict(sorted((str(k), int(v)) for k, v in counters.items() if isinstance(v, int))),
        "world_history_totals": totals,
...
def _conservation_checks(overlay: Mapping[str, Mapping[str, Any]], before_parentage: int, people_before: int, before_tracked_cash: int) -> dict[str, Any]:
    def doc(rel: str) -> Mapping[str, Any]:
    ...
    counters = history.get("counters", {}) if isinstance(history, Mapping) and isinstance(history.get("counters"), Mapping) else {}
    totals = history.get("totals", {}) if isinstance(history, Mapping) and isinstance(history.get("totals"), Mapping) else {}
    recruitment_totals = totals.get("faction_recruitment", {}) if isinstance(totals.get("faction_recruitment"), Mapping) else {}
    civilian_recruits = max(0, int(recruitment_totals.get("civilian_count", 0)))
    births = max(0, int(counters.get("birth", 0)))
    ...
    if actual_people_delta != expected_people_delta:
        errors.append(f"people delta {actual_people_delta} != parentage growth {expected_people_delta}")
...
def _final_result(cp: Mapping[str, Any]) -> dict[str, Any]:
    ...
    commitments = read_json("state/martial-world/commitments.json")
    ...
    world_activity = _world_activity_metrics(overlay)
    ...
    return {
        ...
        "personal_training_records": people["personal_training_records"],
        ...
        "semantic_history_events": world_activity["world_history_recent_count"],
...
def main() -> int:
    ...
    while int(cp["frontiers"]) < int(args.max_frontiers):
        ...
        review_kinds.update(str(r.get("kind")) for r in result.get("reviews", []) if isinstance(r, Mapping))
