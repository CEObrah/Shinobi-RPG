...
def test_hostile_choice_filters_impossible_outlaw_raid_targets_before_ranking(monkeypatch):
    ...
    assert options
    assert {row["action"] for row in options} == {"faction_raid"}
    assert {row["target_faction_ref"] for row in options} == {"faction.local"}
    ...
    assert len(war["options"]) == 1
    assert war["options"][0]["action"] == "faction_war_strike"
    assert war["options"][0]["target_faction_ref"] == "faction.nonlocal"
