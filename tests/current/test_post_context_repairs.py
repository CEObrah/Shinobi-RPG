...
def test_canonical_transport_is_pooled_reserved_and_not_individual_animals():
    ...
    for row in movements.values():
        mobilizing_pursuit = (
            str(row.get("movement_kind") or "") == "route_pursuit"
            and str(row.get("status") or "") == "pursuing"
            and not int(row.get("required_seconds", 0))
        )
        if mobilizing_pursuit:
            assert isinstance(row.get("ready_at"), str) and row["ready_at"]
        else:
        ...
        if mobilizing_pursuit:
            assert row.get("mode") is None and reservation is None
        elif str(row.get("mode") or "") == "foot":
...
def test_calendar_event_attendance_and_socializing_are_real_timed_commands(tmp_path):
    ...
    remove_people_from_route_fixture(root, meta["player_id"])
    remove_people_from_active_combat_fixture(root, meta["player_id"])
