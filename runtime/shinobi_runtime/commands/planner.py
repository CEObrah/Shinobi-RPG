...
class RepositoryCommandPlanner(TimeCommandsMixin, AutonomyCommandsMixin, InternalEventCommandsMixin, MissionCommandsMixin, TeamCommandsMixin, TrainingCommandsMixin, PopulationTravelCommandsMixin, SocialCommandsMixin, CommitmentForceCommandsMixin, FamilyCommandsMixin, EconomyCommandsMixin, MedicalCommandsMixin, SpecialCombatCommandsMixin, CombatCommandsMixin, StrategicCommandsMixin):
    ...
    def _load_scheduler(
        ...
        *,
        ...
        """Load the single production temporal authority.

        Gameplay requires the persisted causal scheduler and never reconstructs
        temporal authority from broad coverage scans.
        """
        raw = self.repository.read_optional_bytes(self.scheduler_path)
        if raw is None:
            raise CommandRejectedError("temporal_authority_missing")
        try:
            scheduler = CausalSchedulerRegistry.from_record(
    ...
    def _formation_by_id(
        ...
        """Resolve one explicit formation through the bounded formation index."""
        ...
        try:
            ...
            raise CommandRejectedError("formation_registry_index_invalid") from exc
        registries = index.get("registries") if isinstance(index, Mapping) else None
        if not isinstance(registries, Mapping) or len(registries) > 64:
        ...
        for force_ref, path in sorted(registries.items()):
            ...
            if not isinstance(formations, list) or len(formations) > 512:
            ...
            for row in formations:
                if isinstance(row, Mapping) and row.get("id") == formation_ref:
                    if row.get("force_ref") != force_ref:
                        raise CommandRejectedError("formation_registry_invalid")
    ...
    def _resolve_actor_for_write(
        ...
        if view is not record and record.get("schema") != "shinobi_character":
            # Rostered person-core registries require a dedicated materialized
            # component writer; do not silently mutate nested roster records.
    ...
    def _build(self, command: CommandEnvelope) -> _BuiltPlan:
        ...
        return handler(command, meta, current_time)
