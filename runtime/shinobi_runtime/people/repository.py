    ...
    def _standing_retinues_for_player(self, person_id: str) -> list[dict[str, Any]]:
        ...
        for retinue_ref in sorted(str(ref) for ref in rows if isinstance(ref, str)):
            ...
            out.append({
                ...
                "status": status,
                "chooser_refs": [str(ref) for ref in row.get("chooser_refs", []) if isinstance(ref, str)] if isinstance(row.get("chooser_refs"), list) else [],
                "member_count": len(member_refs) if status == "active" else max(0, int(row.get("requested_count", 0))),
