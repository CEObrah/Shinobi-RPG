"""Release-baseline identity and state-tree verification for production bootstrap.

The packaged release contract is source-owned. It can therefore authorize an
intentional campaign reset when a persistent Railway checkout still contains an
older lineage. This module verifies only the generic campaign baseline
(campaign/player/revision/time plus the complete state-tree digest); subsystem-
specific acceptance checks remain in ``tools/verify_release_state.py``.
"""
