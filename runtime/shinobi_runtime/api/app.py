...
def create_app(
    *,
    ...
    def preview_command(request: CommandEnvelopeRequest) -> Mapping[str, Any]:
        ...
        return _player_safe_transport(operations.preview_command(command))
    ...
    def execute_command(request: CommandEnvelopeRequest) -> Mapping[str, Any]:
        command = _domain_command(request)
        return _player_safe_transport(operations.execute_command(command))
...
def create_app_from_env() -> FastAPI:
    ...
    # Fail production startup before recovery or API/MCP assembly if the
    # immutable image cannot prove it covers all non-state source in the live
    # campaign checkout. A stale service must not remain readable-but-unwritable
    # and appear playable to ChatGPT.
