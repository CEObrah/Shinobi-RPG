...
def _progression_backlog_resolution(
    ...
    scheduler_record = _decode_write(base.writes, self.scheduler_path)
    if scheduler_record is None:
        raise CommandRejectedError("progression_backlog_scheduler_invalid")
    scheduler_record["world_time"] = str(current_time)
    try:
        repaired_scheduler = CausalSchedulerRegistry.from_record(scheduler_record)
    except (TypeError, ValueError) as exc:
    ...
    writes[self.scheduler_path] = _json_bytes(repaired_scheduler.to_record())
