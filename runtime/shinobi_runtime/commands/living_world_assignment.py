...
class LivingWorldAssignmentMixin:
    ...
    def _mission_capacity(self, payload: Mapping[str, Any]) -> int:
        raw = payload.get("max_active_missions")
        faction_id = payload.get("faction_id")
        if (isinstance(raw, bool) or not isinstance(raw, int)) and isinstance(faction_id, str):
            try:
                _profile, assignment = self._autonomy_policy_book().faction_context(faction_id)
            except (CommandRejectedError, TypeError, ValueError):
                assignment = {}
            raw = assignment.get("max_active_missions") if isinstance(assignment, Mapping) else None
        if isinstance(raw, bool) or not isinstance(raw, int):
            raw = 2
        return max(1, min(_MAX_ACTIVE_MISSIONS, raw))
