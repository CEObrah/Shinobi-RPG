...
COMMAND_SPECS = {
    ...
    "recruitment_resolution": _spec(
        ...
        "Derive accepted recruitment from an eligible conserved source population; optional background selection consumes only that conserved specialist bucket.",
        optional=("background_ref", "scout_ref"),
    ),
    ...
    "person_exactification": _spec(
        ("person_ref", "authority_ref"),
        "Add exact combat/health/training components to one already-rostered persistent person without rerolling identity or capability.",
        optional=("reason",),
    ),
    ...
    "ocular_stockpile_resolution": _spec(
        ("action", "stockpile_ref", "eye_ref"),
        "Materialize one deterministic conserved exact eye from an ocular stockpile batch before transfer or implantation.",
        variants={
            "materialize": _variant(("action", "stockpile_ref"), optional=("eye_ref",)),
        },
    ),
    ...
    "technique_learning_resolution": _spec(
        ("action", "student_ref", "technique_ref", "teacher_ref", "research_project_ref", "target_time", "active_hours", "summary", "visibility"),
        "Begin, train, and validate field usability for a technique from authoritative prerequisites, teaching, or an approved research prototype.",
        variants={
            "begin": _variant(("action","student_ref","technique_ref","teacher_ref","summary","visibility"), optional=("research_project_ref",)),
    ...
    "battlefield_resolution": _spec(
        ("action", "conflict_ref", "front_ref", "battlefield_ref", "name", "place_ref", "side_refs", "layout_ref", "formation_ref", "sector_ref", "target_sector_ref", "pace", "order"),
        "Operate one persistent multi-sector battlefield inside an active strategic conflict front. The battlefield tracks sector pressure, timed redeployment, interception and delayed command reports; exact combat still owns casualties and combat outcomes.",
        variants={
            "open": _variant(("action","conflict_ref","front_ref","battlefield_ref","name","place_ref","side_refs","layout_ref")),
            "assign": _variant(("action","conflict_ref","front_ref","battlefield_ref","formation_ref","sector_ref"), optional=("order",)),
            "redeploy": _variant(("action","conflict_ref","front_ref","battlefield_ref","formation_ref","target_sector_ref"), optional=("pace","order")),
            "set_order": _variant(("action","conflict_ref","front_ref","battlefield_ref","formation_ref","order")),
            "close": _variant(("action","conflict_ref","front_ref","battlefield_ref")),
        },
        hints={
            "layout_ref": "battlefield.layout.line_three|battlefield.layout.deep_five",
            "pace": "forced|standard|cautious",
            "order": "hold|attack|breakthrough|delay|reserve|withdraw",
        },
    ),
    ...
    "legal_case_resolution": _spec(
        ("action","case_ref","issuer_ref","subject_ref","case_kind","offense_refs","summary","visibility","evidence_ref","authority_ref","payer_ref","amount_ryo","hunter_ref","custody_ref","disposition"),
        "Create and resolve evidence-backed legal/warrant/bounty cases with conserved escrow.",
        variants={
            "open": _variant(("action","case_ref","issuer_ref","subject_ref","case_kind","offense_refs","summary","visibility")),
            "add_evidence": _variant(("action","case_ref","evidence_ref")),
            "issue_warrant": _variant(("action","case_ref","authority_ref")),
            "post_bounty": _variant(("action","case_ref","payer_ref","amount_ryo")),
            "assign_hunter": _variant(("action","case_ref","hunter_ref")),
            "verify_bounty": _variant(("action","case_ref","evidence_ref")),
            "settle_bounty": _variant(("action","case_ref","hunter_ref")),
            "cancel_bounty": _variant(("action","case_ref")),
            "record_custody": _variant(("action","case_ref","custody_ref")),
            "resolve": _variant(("action","case_ref","disposition")),
            "dismiss": _variant(("action","case_ref","disposition")),
        },
    ),
    "diplomacy_resolution": _spec(
        ("action","agreement_ref","representing_ref","agreement_type","party_refs","terms","visibility","provisions","evidence_ref","incident_kind","summary"),
        "Propose, consent to, reject, terminate, evidence, or settle a typed multi-party diplomatic agreement.",
        variants={
            "propose": _variant(("action","agreement_ref","representing_ref","agreement_type","party_refs","terms","visibility"), optional=("provisions",)),
            "accept": _variant(("action","agreement_ref","representing_ref")),
            "reject": _variant(("action","agreement_ref","representing_ref")),
            "terminate": _variant(("action","agreement_ref","representing_ref")),
            "record_incident": _variant(("action","agreement_ref","representing_ref","evidence_ref","incident_kind","summary")),
            "settle_tribute": _variant(("action","agreement_ref","representing_ref")),
        },
    ),
    "commerce_resolution": _spec(
        ("action","contract_ref","client_ref","carrier_ref","source_holder_ref","destination_holder_ref","item_ref","quantity","unit_price_ryo","route_ref","classification","contraband","shipment_ref","authority_ref","crossing_ref","evidence_event_ref","seizure_holder_ref"),
        "Run an escrow-backed conserved trade/shipping lifecycle, including border inspection and seizure of contraband.",
        variants={
            "offer_contract": _variant(("action","contract_ref","client_ref","carrier_ref","source_holder_ref","destination_holder_ref","item_ref","quantity","unit_price_ryo","route_ref","classification","contraband")),
            "accept_contract": _variant(("action","contract_ref")),
            "fund_contract": _variant(("action","contract_ref")),
            "dispatch": _variant(("action","contract_ref")),
            "inspect": _variant(("action","contract_ref","shipment_ref","authority_ref","crossing_ref"), optional=("evidence_event_ref",)),
            "seize": _variant(("action","contract_ref","crossing_ref","seizure_holder_ref")),
            "deliver": _variant(("action","contract_ref")),
            "cancel": _variant(("action","contract_ref")),
        },
    ),
    "research_resolution": _spec(
        ("action","project_ref","institution_ref","lead_ref","place_ref","project_kind","subject_ref","custody_ref","stock_ref","hypothesis","classification","evidence_ref","candidate_kind","candidate_ref"),
        "Run evidence-backed research, then prototype, test, and approve registered deployment candidates without granting free capabilities.",
        variants={
            "start": _variant(("action","project_ref","institution_ref","lead_ref","place_ref","project_kind","stock_ref","hypothesis","classification"), optional=("subject_ref","custody_ref")),
            "advance": _variant(("action","project_ref","evidence_ref")),
            "cancel": _variant(("action","project_ref")),
            "prototype": _variant(("action","project_ref","candidate_kind","candidate_ref")),
            "test_prototype": _variant(("action","project_ref")),
            "approve": _variant(("action","project_ref")),
        },
    ),
    "security_network_resolution": _spec(
        ("action","sector_ref","owner_ref","place_ref","route_refs","stock_ref","classification","detection_channels","axis","subject_ref","evidence_ref","recipient_refs","alarm_ref"),
        "Establish resource-backed security sectors, improve them from evidence, and route evidence-backed alarms to explicit recipients. Establishment currently materializes barrier/chakra sensing from consumed anchor/relay stock; physical patrol, scent, auditory, or tracking channels require their own future resource-backed owner rather than caller declaration.",
        variants={
            "establish_sector": _variant(("action","sector_ref","owner_ref","place_ref","route_refs","stock_ref","classification"), optional=("detection_channels",)),
            "improve_sector": _variant(("action","sector_ref","axis","evidence_ref")),
            "raise_alarm": _variant(("action","alarm_ref","sector_ref","subject_ref","evidence_ref","recipient_refs")),
            "acknowledge_alarm": _variant(("action","alarm_ref")),
            "resolve_alarm": _variant(("action","alarm_ref","evidence_ref")),
            "mark_false_alarm": _variant(("action","alarm_ref","evidence_ref")),
        },
    ),
    "mission_market_resolution": _spec(
        ("action","market_ref","evidence_ref","axis","signal_ref","direction"),
        "Change an existing mission-market demand or competition signal only from persisted causal evidence.",
        variants={
            "record_signal": _variant(("action","market_ref","evidence_ref","axis","signal_ref","direction")),
        },
    ),
    "governance_resolution": _spec(
        ("action","jurisdiction_ref","place_ref","base_place_ref","settlement_name","sovereign_ref","administration_ref","source_population_pool_id","resident_pool_id","initial_population","treasury_holder_ref","garrison_force_ref","parent_jurisdiction_ref","migration_agreement_ref","recruitment_rights","tax_milli","evidence_ref","delta_milli","target_status","recognition_agreement_ref","summary","visibility"),
        "Establish administration, found conserved settlements, set recruitment/tax policy, integrate territory, and grow settlements into villages.",
        variants={
            "establish_administration": _variant(("action","jurisdiction_ref","place_ref","sovereign_ref","administration_ref","summary","visibility"), optional=("garrison_force_ref",)),
            "found_settlement": _variant(("action","jurisdiction_ref","place_ref","base_place_ref","settlement_name","sovereign_ref","administration_ref","source_population_pool_id","resident_pool_id","initial_population","treasury_holder_ref","summary","visibility"), optional=("garrison_force_ref","parent_jurisdiction_ref","migration_agreement_ref")),
            "set_policy": _variant(("action","jurisdiction_ref","recruitment_rights","tax_milli")),
            "set_garrison": _variant(("action","jurisdiction_ref","garrison_force_ref")),
            "integrate": _variant(("action","jurisdiction_ref","evidence_ref","delta_milli")),
            "upgrade": _variant(("action","jurisdiction_ref","target_status"), optional=("recognition_agreement_ref",)),
        },
    ),
    ...
    "combat_resolution": _spec(
        ...
        optional=("mission_ref", "location_ref", "parent_combat_ref", "range_band", "line_of_sight"),
        hints={
            "scale": "duel|skirmish|formation|battle",
            "participants": "exact: [{actor_ref,side_ref,action,target_refs,objective_ref,lethal,...}] | aggregate: [{participant_ref:'formation:<id>',committed_count,side_ref,action,target_refs,objective_ref,lethal,command_authority_ref,named_actor_refs?}]; embedded exact actors are discovered authoritatively even if omitted",
            ...
            "range_band": "0 engaged/melee | 1 short | 2 medium | 3 long",
            "line_of_sight": "shared current engagement line-of-sight state",
        },
    ),
