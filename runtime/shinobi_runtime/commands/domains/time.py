    ...
    def _advance_time(
        ...
        for fact in catchup.public_facts:
            ...
            if kind == "person.recovery.periodic_review":
                ...
                representation = record.get("schema")
                identity_ref = record.get("id") if representation == "person" else record.get("owner_id")
                if identity_ref != actor_ref:
                    raise CommandRejectedError("person_recovery_owner_invalid")
                try:
                ...
                if representation == "person":
                    before_condition = copy.deepcopy(record.get("health"))
                    stats = record.get("stats")
                    before_resources = copy.deepcopy(
                        stats.get("resources") if isinstance(stats, Mapping) else None
                    )
                else:
                ...
                if representation == "person":
                    health_state = record.get("health")
                    ready = (
                        isinstance(health_state, Mapping)
                        and str(health_state.get("status", "")).lower()
                        in {"healthy", "ready", "fit", "active"}
                    )
                else:
                    ready = (
                        record.get("life_status") in ("active", "alive")
                        and isinstance(record.get("condition"), Mapping)
                        and record["condition"].get("readiness") == "ready"
                    )
                        ...
                        if return_class == "deployed":
                            ...
                            if can_reembed:
                                ...
                                current_location = (
                                    record.get("loc")
                                    if representation == "person"
                                    else record.get("current_location_id")
                                )
                                if (
                ...
                if representation == "person":
                    stats = record.get("stats")
                    after_resources = stats.get("resources") if isinstance(stats, Mapping) else None
                    changed = (
                        before_condition != record.get("health")
                        or before_resources != after_resources
                    )
                else:
                    changed = (
                        before_condition != record.get("condition")
                        or before_resources != record.get("resources")
                    )
                event_id = None
                if changed:
