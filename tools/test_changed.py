...
CORE = {
    ...
    "tests/current/test_combat_gameplay_certification.py",
    "tests/current/test_pytest_shard_runner.py",
}
COMBAT = {
    "tests/current/test_production_combat_composition.py",
    ...
    "tests/current/test_combat_resolution_integrity.py",
    "tests/current/test_combat_continuous_pursuit.py",
...
RETINUE = {
    ...
    "tests/current/test_compound_combat_medic_regression.py",
}
INSTITUTIONAL = {
...
RELEASE = {
    ...
    "tests/current/test_release_campaign_state_contract.py",
}
SCENE_FLOW = {
    "tests/current/test_scene_session_contract.py",
...
DIRECTOR_CONTEXT_TESTS = {
    "tests/current/test_open_world_gm_architecture.py",
    "tests/current/test_gm_scene_process_continuity.py",
}
PARLEY_INTEGRATION_TESTS = {
    "tests/current/test_gm_scene_process_continuity.py",
    "tests/current/test_parley_operations.py",
    "tests/current/test_combat_parley.py",
}
TRAVEL_PROJECTION_TESTS = {
    "tests/current/test_travel_party_projection.py",
    "tests/current/test_open_world_gm_architecture.py",
}
SKILL_BEHAVIOR_TESTS = {
    "tests/current/test_combat_narration_authority.py",
    "tests/current/test_narrative_quality_hard_gate.py",
...
ADAPTATION_TESTS = {
    "tests/current/test_combat_adaptation.py",
    "tests/current/test_combat_adaptation_installation.py",
    "tests/current/test_combat_adaptation_liveness_v2.py",
}
PLAYABILITY_DELIVERY_TESTS = {
    "tests/playability/test_delivery_chain_contract.py",
}
PLAYABILITY_COMBAT_TESTS = {
    "tests/playability/test_combat_physics_torture.py",
}
PLAYABILITY_PACKAGING_TESTS = {
    "tests/playability/test_release_packaging_contract.py",
}
PLAYABILITY_LAB_SCOPE_TESTS = {
    "tests/playability/test_playability_lab_scope.py",
}
...
def _test_dependents_for_helper(path: str) -> set[str]:
    """Map a non-test helper module under tests/ to the tests that import it."""
    module = Path(path).stem
    needles = (f"from {module} import", f"import {module}")
    selected: set[str] = set()
    for base in (ROOT / "tests/current", ROOT / "tests/playability"):
        if not base.is_dir():
            continue
        for candidate in base.glob("test_*.py"):
            try:
    ...
    for raw in paths:
        ...
        if path.startswith("tests/current/") and path.endswith(".py"):
            if Path(path).name.startswith("test_"):
                ...
                selected.update(_test_dependents_for_helper(path))
        ...
        if path == "runtime/shinobi_runtime/api/gm_scene_context.py":
            ...
            selected.update(PARLEY_INTEGRATION_TESTS)
        if path == "runtime/shinobi_runtime/api/parley_operations.py":
            selected.update(PARLEY_INTEGRATION_TESTS)
        if path == "runtime/shinobi_runtime/api/travel_operations.py":
            selected.update(TRAVEL_PROJECTION_TESTS)
        ...
        if path.endswith("/combat_adaptation.py") or path.endswith("/combat_adaptation_installation.py"):
            selected.update(ADAPTATION_TESTS)
        ...
        if (
            ...
            selected.update(PLAYABILITY_COMBAT_TESTS)
        ...
        if path.startswith("plugins/shinobi-rpg/skill/") or path in {"pyproject.toml", "requirements.txt"} or path.startswith("runtime/contracts/"):
            ...
            if path.startswith("plugins/shinobi-rpg/skill/"):
                selected.update(SKILL_BEHAVIOR_TESTS)
        if path in {
            ...
            "tools/sync_gm_skill_contract.py",
...
def main(argv: list[str]) -> int:
    ...
    from run_pytest_shards import run
    rc = run(tests, timeout_seconds=60.0, quiet_passes=True)
