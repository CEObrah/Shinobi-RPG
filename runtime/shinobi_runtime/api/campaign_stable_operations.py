...
class RouteAwareCampaignOperations(_BaseRouteAwareCampaignOperations):
    ...
    def _current_player_mission_context(
        ...
        try:
            index = self._mission_context_index()
            current = participant_current_refs(index, player_id)
            briefing = set(participant_briefing_refs(index, player_id))
        except (TypeError, ValueError) as exc:
            raise OperationError(503, "mission_context_invalid") from exc

        selected = current[:_MAX_CONTEXT_MISSION_IDS]
        briefing_ids = tuple(ref for ref in selected if ref in briefing)
        return tuple(selected), briefing_ids, len(current) > _MAX_CONTEXT_MISSION_IDS

    def command_contract(self, command_type: str) -> Mapping[str, Any]:
        """Return one dynamic command descriptor without constructing live scene context."""
        supported = set(getattr(self.command_planner, "COMMAND_TYPES", ()))
        ...
        if spec is None:
            raise OperationError(503, "command_contract_unavailable")
        descriptor = dict(spec.public_descriptor())
        try:
            ...
            if command_type in _MISSION_COMMANDS:
                mission_ids, _briefing_ids, _truncated = self._current_player_mission_context(player_id)
                descriptor["availability"] = "available" if mission_ids else "no_mission_owner"
                if command_type == "mission_objective_update" and mission_ids:
                    descriptor["availability"] = "requires_persisted_terminal_world_event_evidence"
            elif command_type == "advance_time":
                scene = self.repository.read_json("state/scene.json")
                time_allowed = scene.get("time_passage_allowed") is True if isinstance(scene, Mapping) else False
                pending_zoom_ref = None
                try:
                    zoom_registry = self.repository.read_json("state/reg/combat-zoom.json")
                    pending = zoom_registry.get("pending_by_actor") if isinstance(zoom_registry, Mapping) else None
                    candidate = pending.get(player_id) if isinstance(pending, Mapping) else None
                    if isinstance(candidate, str) and candidate:
                        pending_zoom_ref = candidate
                except (FileNotFoundError, ValueError):
                    pending_zoom_ref = None
                descriptor["availability"] = (
                    "blocked_by_pending_combat_zoom" if pending_zoom_ref is not None
                    else ("available" if time_allowed else "blocked_by_scene_decision")
                )
        except OperationError:
            raise
        except (FileNotFoundError, ValueError, TypeError) as exc:
            raise OperationError(503, "command_contract_unavailable") from exc
        return {"command_type": command_type, **descriptor}
    ...
    def _player_scene_cast(
        ...
        *,
        ...
        # Scene cast is a bounded projection. Exact person access remains
        # available through the direct lookup policy even for very large Houses.
