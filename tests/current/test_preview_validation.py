...
class _FakeRepository:
    def __init__(self):
        self.files = {
            ...
            # Preview validation now enforces the campaign-wide exact-custody
            # uniqueness invariant even when the staged write does not touch
            # custody. Keep the isolated repository fixture structurally
            # complete rather than weakening the production validator.
            "state/martial-world/custody.json": _json_bytes({
...
def _active_player_retinue_ref(repository, player_id):
    deployments = repository.read_json('state/martial-world/deployments.json')
    for ref, row in deployments.get('deployments', {}).items():
        if isinstance(row, dict) and row.get('operation_kind') == 'standing_retinue' and row.get('leader_ref') == player_id and row.get('status') == 'active':
            return ref
    raise AssertionError('active player retinue missing')


def test_retinue_can_add_an_eligible_permanent_member_without_fixed_cap_and_preview_is_read_only():
    repository = RepositoryStore(ROOT)
    planner = RepositoryCommandPlanner(repository)
    meta_before = repository.read_bytes('state/meta.json')
    deployments_before = repository.read_bytes('state/martial-world/deployments.json')
    meta = json.loads(meta_before.decode('utf-8'))
    retinue_ref = _active_player_retinue_ref(repository, meta['player_id'])
    command = CommandEnvelope(
        campaign_id=meta['campaign_id'], request_id='request.retinue-add-preview-regression',
        actor_id=meta['player_id'], command_type='jianghu_retinue_resolution',
    ...
    assert plan.result['member_count_after'] == 4
    assert plan.result['permanent_cap'] is None
    assert repository.read_bytes('state/meta.json') == meta_before
    assert repository.read_bytes('state/martial-world/deployments.json') == deployments_before


def test_retinue_can_remove_one_member_without_replacement_or_vacancy_slot():
    repository = RepositoryStore(ROOT)
    planner = RepositoryCommandPlanner(repository)
    meta = repository.read_json('state/meta.json')
    retinue_ref = _active_player_retinue_ref(repository, meta['player_id'])
    deployments_before = repository.read_bytes('state/martial-world/deployments.json')
    current = repository.read_json('state/martial-world/deployments.json')['deployments'][retinue_ref]
    member_ref = current['member_refs'][0]
    command = CommandEnvelope(
        campaign_id=meta['campaign_id'], request_id='request.retinue-remove-preview-regression',
        actor_id=meta['player_id'], command_type='jianghu_retinue_resolution',
        expected_revision=meta['revision'], submitted_at='2026-08-26T00:00:00Z',
    ...
    assert preview.status == 'ready'
    plan = planner.plan(command)
    assert plan.result['member_count_after'] == len(current['member_refs']) - 1
    assert plan.result['permanent_cap'] is None
    assert repository.read_bytes('state/martial-world/deployments.json') == deployments_before
