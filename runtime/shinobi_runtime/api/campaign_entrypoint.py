...
causal reaction-timing integrity, player tactical-movement integrity, combat
liveness integrity, and resolved-route-contact reconciliation. Historical repair
implementations remain available for forensic tests, but one-time campaign repair
anchors are not installed into normal production composition after their repair is
complete.
"""
...
def create_app_from_env():
    ...
    # consumption. Tactical movement extends the existing player exchange seam
    # and melee approach budget without replacing exact combat authority.
    # Liveness wraps the final hardened span/disengage surfaces that commit.
    install_combat_simulation_hardening()
