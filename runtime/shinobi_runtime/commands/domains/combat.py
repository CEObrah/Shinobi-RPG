...
class CombatCommandsMixin:
    ...
    def _combat_environment_effects(self, location_ref: str) -> Mapping[str, Any]:
        try:
            ...
            if not isinstance(world_time, str) or not world_time:
                raise ValueError("world time missing")
            environment = environment_snapshot(self.repository, world_time=world_time, location_ref=location_ref)
            effects = environment.get("mechanical_effects") if isinstance(environment, Mapping) else None
            if not isinstance(effects, Mapping):
                raise ValueError("mechanical effects missing")
            return effects
    ...
    @staticmethod
    def _technique_chakra_cost(
        record: Mapping[str, Any],
        technique: Mapping[str, Any],
        *,
        environment_effects: Optional[Mapping[str, Any]] = None,
    ) -> int:
        chakra = record.get("chakra_dimensions") if isinstance(record.get("chakra_dimensions"), Mapping) else {}
        ...
        if raw == 0:
            return 0
        cost = max(1, (raw * factor_milli + 999) // 1000)
        # Existing technique data explicitly distinguishes water methods that can
        # draw from an available medium versus paying to generate that material.
        # Surface water reduces only that generation burden; it never grants free
        # chakra or a generic elemental damage bonus.
        required_medium = technique.get("required_medium")
        domain = str(technique.get("domain") or "").lower()
        if domain == "water" and isinstance(required_medium, Sequence) and not isinstance(required_medium, (str, bytes, bytearray)) and environment_effects is not None:
            requires_available_water = any(isinstance(value, str) and ("water" in value or "moisture" in value) for value in required_medium)
            surface_water = environment_effects.get("surface_water_milli", 0)
            if requires_available_water and isinstance(surface_water, int) and not isinstance(surface_water, bool) and surface_water > 0:
                reduction_milli = min(200, max(0, surface_water) // 5)
                cost = max(1, (cost * (1000 - reduction_milli) + 999) // 1000)
        return cost
    ...
    def _prepare_exact_elite_overlay(
        ...
        *,
        ...
        if technique_ref is not None:
            ...
            chakra_cost = self._technique_chakra_cost(record, technique, environment_effects=self._combat_environment_effects(location_id))
            if chakra_cost:
                chakra_resource = record.get("resources", {}).get("chakra") if isinstance(record.get("resources"), Mapping) else None
    ...
    def _aggregate_combat_resolution(
        ...
        for ref, item in parsed.items():
            ...
            loss_milli = 0 if aggregate_resolved_count <= 0 else min(1000, roster_losses * 1000 // aggregate_resolved_count)
            if aggregate_resolved_count > 0:
                memory = formation.get("operational_memory")
                if not isinstance(memory, dict):
                    ...
                    formation["operational_memory"] = memory
                side_ref = item["side_ref"]
                victorious = set(effect_plan.victorious_side_refs)
                outcome = "victory" if side_ref in victorious else "defeat" if victorious else "contested"
                memory["engagements_total"] = int(memory.get("engagements_total", 0)) + 1
                memory["aggregate_personnel_committed_total"] = int(memory.get("aggregate_personnel_committed_total", 0)) + aggregate_resolved_count
                memory["killed_total"] = int(memory.get("killed_total", 0)) + after.killed
                memory["wounded_total"] = int(memory.get("wounded_total", 0)) + after.wounded
                memory["incapacitated_total"] = int(memory.get("incapacitated_total", 0)) + after.incapacitated
                memory["captured_total"] = int(memory.get("captured_total", 0)) + after.captured
                counter_key = {"victory": "victories_total", "defeat": "defeats_total", "contested": "contested_total"}[outcome]
                memory[counter_key] = int(memory.get(counter_key, 0)) + 1
                memory["last_combat_ref"] = combat_id
                memory["last_result_at"] = str(current_time)
                memory["last_outcome"] = outcome
                memory["last_loss_milli"] = loss_milli
    ...
    def _combat_resolution(
        ...
        for actor_ref, spec in parsed_specs.items():
            ...
            if technique_ref is not None:
                ...
                chakra_cost = self._technique_chakra_cost(record, technique, environment_effects=self._combat_environment_effects(encounter_location))
                if chakra_cost:
                    chakra_resource = record.get("resources", {}).get("chakra") if isinstance(record.get("resources"), Mapping) else None
                    current_chakra = chakra_resource.get("current") if isinstance(chakra_resource, Mapping) else None
                    if isinstance(current_chakra, bool) or not isinstance(current_chakra, int) or current_chakra < chakra_cost:
