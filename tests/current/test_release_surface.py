...
def test_combat_state_registers_future_clock_commitments_without_trace_history():
    t=json.loads((ROOT/'runtime/contracts/templates/jianghu-combat-state-1.0.template.json').read_text())
    keys=set(t['object_contracts']['/combats/*']['allowed_keys'])
    assert {'scheduled_actions','scheduled_defense_interruptions'} <= keys
    position_keys=set(t['object_contracts']['/combats/*/positions/*']['allowed_keys'])
    assert 'motion_expires_at_ms' in position_keys
    assert t['array_contracts']['/combats/*/scheduled_actions']['item_types']==['object']
    assert t['object_contracts']['/combats/*/scheduled_actions/*']['mode']=='closed'
    assert t['object_contracts']['/combats/*/scheduled_actions/*/profile/effect_parameters']['mode']=='open'
...
def test_active_scene_projection_fails_closed_on_unreadable_exact_participant():
    from shinobi_runtime.api.operations import _project_active_session_presence

    people = {
        'pc_wei_tang': {'person_id': 'pc_wei_tang', 'location_ref': 'site.house_tang.hall'},
    }
    session = {
        'session_ref': 'scene.corrupt', 'status': 'active', 'kind': 'conversation',
        'location_ref': 'site.house_tang.hall',
        'participant_refs': ['pc_wei_tang', 'npc.missing'],
    }
    ...
    with pytest.raises(KeyError):
        _project_active_session_presence(
            _ScenePresenceReader().read_json, resolve, session,
            player_id='pc_wei_tang', player_sheet=people['pc_wei_tang'],
            player_location='site.house_tang.hall', live_combat=None,
        )
