...
def test_escort_muster_rejects_same_city_but_different_exact_sites(monkeypatch):
    refs, people, records = _fixture()
    records[SITES]["sites"] = {
        "site.test.manor": {"parent_place_ref": "luoyang"},
        "site.test.inn": {"parent_place_ref": "luoyang"},
    }
    for index, ref in enumerate(refs):
        people[ref]["location_ref"] = "site.test.manor" if index < len(refs) - 1 else "site.test.inn"
    captured = {}
    _patch_physics(monkeypatch, captured)
    harness = _Harness(records, people)
