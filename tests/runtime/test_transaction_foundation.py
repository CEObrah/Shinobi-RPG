...
def test_receipt_index_rebuilds_beyond_legacy_4096_lifetime_count_without_ceiling(tmp_path: Path, monkeypatch) -> None:
    store = ReceiptStore(tmp_path / "receipts")
    for revision in range(1, 4106):
        request_id = f"request-long-campaign-{revision}"
        receipt = IdempotencyReceipt(
            request_id=request_id,
            request_digest=(f"{revision:064x}"[-64:]),
        ...
        store._path(request_id).write_bytes(canonical_json_bytes(receipt.to_record()))

    assert store.rebuild_index()["test-campaign"] == 4105
    assert store.campaign_max_revision("test-campaign") == 4105

    def lifetime_scan_forbidden():
        raise AssertionError("normal high-water lookup scanned lifetime receipts")

    monkeypatch.setattr(store, "_receipt_paths", lifetime_scan_forbidden)
    assert store.campaign_max_revision("test-campaign") == 4105
