...
def test_konoha_player_offer_cycle_contains_only_executable_brief_templates() -> None:
    ...
    assert [mapping[key] for key in cycle[:4]] == ["escort", "investigate", "observe", "protect"]
    assert all(key in templates or mapping[key] in templates or key in dynamic for key in cycle)
