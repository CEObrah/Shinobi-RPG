...
def test_canonical_route_rows_are_seconds_native_only():
    ...
    assert rows
    assert all(int(row.get('required_seconds',0))>0 for row in rows.values())
    assert all('required_hours' not in row and 'elapsed_hours' not in row for row in rows.values())
