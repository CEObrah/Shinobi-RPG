...
from .escort_living_world import (
    ...
    interception_attack_options, interception_force_size, observed_escort_strength, principal_ransom_value_cash, private_interception_decision_context,
    route_extortion_settlement_terms, route_interception_opportunity_permille,
)
from .exact_combat import initialize_combat
from .physical_presence import combat_person_arrived
...
def bounded_raid_recovery_ref(parent_movement_ref: str, attacker_fid: str, at_iso: str) -> str:
    """Return a bounded identity for attackers unable to join the immediate return.

    Exact combat can leave arrived attackers escaped, unconscious, incapacitated,
    ringed out, or dead. Those bodies must not fall through to a stale stored
    headquarters merely because the main return party departs. Living stragglers
    keep a finite road owner until they can travel again; dead bodies keep their
    exact road location.
    """
    at = datetime.fromisoformat(str(at_iso))
    digest = hashlib.sha256(
        f"recovery|{parent_movement_ref}|{attacker_fid}|{at.date().isoformat()}".encode("utf-8")
    ).hexdigest()[:20]
    return f"raid_recovery:{at.date().isoformat()}:{digest}"

_RETURN_EXTRA_KEYS = (
    "trade_leg", "trade_outcome", "item_ref", "quantity", "cash_quantity", "contract_ref",
    "protected_person_refs", "captive_refs", "rescued_refs", "escort_refs",
)


def route_return_extra(movement: Mapping[str, Any]) -> dict[str, Any]:
    """Copy every conserved payload that must survive a parked-return retry."""
    return {
...
def route_combatant_available_for_postcombat_movement(
    combat_after: Mapping[str, Any] | None, person_ref: str,
) -> bool:
    """Return whether one combatant can lawfully join an immediate route return.

    A resolved fight does not teleport escaped, not-yet-arrived, unconscious,
    incapacitated, dead, or ringed-out bodies back into the victorious/retreating
    party. Missing legacy combat detail remains compatible; current exact combat
    state is authoritative whenever it exists.
    """
    if not isinstance(combat_after, Mapping):
        return True
    combatants = combat_after.get("combatants", {})
    if not isinstance(combatants, Mapping):
        raise ValueError("route postcombat combatants invalid")
    state = combatants.get(str(person_ref))
    if state is None:
        return True
    if not isinstance(state, Mapping):
        raise ValueError("route postcombat combatant state invalid")
    raw = state.get("status_families", [])
    if not isinstance(raw, list):
        raise ValueError("route postcombat combatant statuses invalid")
    statuses = {str(value) for value in raw if isinstance(value, str)}
    unavailable = {
        "dead", "unconscious", "incapacitated", "escaped", "reinforcing", "ring_out",
    }
    return not bool(statuses & unavailable)


def route_combatant_physically_controlled_for_seizure(
    combat_after: Mapping[str, Any] | None, person_ref: str,
) -> bool:
    """Return whether a defeated route participant can still be physically seized.

    Side-level victory is not custody. A person who completed an escape, or who
    has not yet arrived as a reinforcement, is outside the victors' immediate
    physical control and therefore keeps personal property. Missing legacy
    combat detail remains conservative-compatible and does not invent escape.
    """
    if not isinstance(combat_after, Mapping):
        return True
    combatants = combat_after.get("combatants", {})
    if not isinstance(combatants, Mapping):
        raise ValueError("route seizure combatants invalid")
    state = combatants.get(str(person_ref))
    if state is None:
        return True
    if not isinstance(state, Mapping):
        raise ValueError("route seizure combatant state invalid")
    raw = state.get("status_families", [])
    if not isinstance(raw, list):
        raise ValueError("route seizure combatant statuses invalid")
    statuses = {str(value) for value in raw if isinstance(value, str)}
    return not bool(statuses & {"escaped", "reinforcing"})


def _route_interception_objective(
    movement_ref: str, movement: Mapping[str, Any], *, cargo_value_cash: int = 0,
) -> dict[str, Any]:
    """Derive interception stakes from physical holdings at the current route phase.

    Planned future procurement is not cargo. Only a positive quantity/value that
    exists on the movement now can create a cargo-protection objective.
    """
    movement_kind = str(movement.get("movement_kind") or "")
    if movement_kind == "raid_return":
        return {"kind": "retain_seized_people_and_cargo", "movement_ref": movement_ref}
    protected = [
        str(ref) for key in ("protected_person_refs", "rescued_refs", "captive_refs")
        for ref in (movement.get(key, []) if isinstance(movement.get(key), list) else [])
        if isinstance(ref, str) and ref
    ]
    quantity = max(0, int(movement.get("quantity", 0) or 0), int(movement.get("cargo_quantity", 0) or 0))
    if quantity > 0 or max(0, int(cargo_value_cash)) > 0:
        return {"kind": "protect_cargo", "movement_ref": movement_ref}
    if protected:
        return {"kind": "protect_party", "movement_ref": movement_ref, "protected_person_refs": list(dict.fromkeys(protected))}
    if movement_kind in {"player_strategic_travel", "faction_operation_travel", "route_pursuit"}:
...
def settle_route_frontier(
    *,
    ...
    route_judgment_events = [
        e for e in sorted_events
        if isinstance(e, Mapping) and e.get("kind") == "npc_judgment_committed"
        and e.get("judgment_kind") == "route_interception_policy"
    ]
    if route_events or route_judgment_events:
        ...
        def _route_combat_objective(movement_ref: str, movement: Mapping[str, Any]) -> dict[str, Any]:
            return _route_interception_objective(
                movement_ref, movement, cargo_value_cash=_movement_cargo_value_cash(movement),
            )
        ...
        def _movement_environment(movement_ref: str, movement: Mapping[str, Any]) -> dict[str, Any]:
            ...
            # Route progress remains authoritative on the movement owner.  The exact
            # combat environment contract is deliberately closed and stores only
            # physical combat conditions, so do not duplicate route-progress
            # bookkeeping into combat state.  Progress still selects terrain and
            # participates in the deterministic environment seed above.
        ...
        def _apply_shared_danger_social(participant_refs: Sequence[str]) -> None:
            ...
            for ref in participant_refs:
                ...
                if health.get("status") != "dead":
                    living.append(ref)
            # The relationship helper is already sparse O(n), so there is no
            # reason to mechanically ignore member 13+ of a real traveling
            # party.  Transport projections may be bounded; shared danger is a
            # world consequence and must cover the whole actual party.
            party = living
            if len(party) > 1:
        ...
        def _pending_route_judgment(attacker_fid: str, movement_ref: str) -> Mapping[str, Any] | None:
            pools: list[Mapping[str, Any]] = []
            one_off = schedule.get("one_off", {}) if isinstance(schedule, Mapping) else {}
            if isinstance(one_off, Mapping):
                pools.extend(row for row in one_off.values() if isinstance(row, Mapping))
            pools.extend(row for row in pending_one_off_events if isinstance(row, Mapping))
            for row in pools:
                if row.get("kind") != "npc_judgment_due" or row.get("judgment_kind") != "route_interception_policy":
                    continue
                ctx = row.get("decision_context", {}) if isinstance(row.get("decision_context"), Mapping) else {}
                if str(ctx.get("attacker_faction_ref") or "") == attacker_fid and str(ctx.get("target_movement_ref") or "") == movement_ref:
                    return row
            return None

        def _stage_route_interception_judgment(
            *, attacker_fid: str, movement_ref: str, route_ref: str, attacker_refs: Sequence[str],
            decision: Mapping[str, Any], observed: Mapping[str, Any], source_event_id: str,
            opportunity_kind: str, stop_place: str = "",
        ) -> Mapping[str, Any]:
            existing = _pending_route_judgment(attacker_fid, movement_ref)
            if isinstance(existing, Mapping):
                return existing
            private = private_interception_decision_context(decision)
            candidates = [str(ref) for ref in attacker_refs if isinstance(ref, str) and ref and str(ref) != player_ref]
            candidates = list(dict.fromkeys(candidates))
            attack_options = interception_attack_options(
                attacker_faction_ref=attacker_fid, target_movement_ref=movement_ref, route_ref=route_ref,
                candidate_attacker_refs=candidates, decision=decision, player_ref=player_ref,
            )
            event = build_npc_judgment_event(
                judgment_kind="route_interception_policy", at_iso=at_iso, actor_ref=attacker_fid,
                owner_ref=movement_ref,
                options=[
                    {"intent": "ignore", "attacker_faction_ref": attacker_fid, "target_movement_ref": movement_ref, "route_ref": route_ref},
                    {"intent": "observe", "attacker_faction_ref": attacker_fid, "target_movement_ref": movement_ref, "route_ref": route_ref},
                    *attack_options,
                ],
                decision_context={
                    "attacker_faction_ref": attacker_fid, "target_movement_ref": movement_ref,
                    "route_ref": route_ref, "opportunity_kind": opportunity_kind, "stop_place_ref": stop_place,
                    "observer_ref": str(observed.get("observer_ref") or decision.get("observer_ref") or ""),
                    "visible_escort_count": max(0, int(observed.get("visible_escort_count", decision.get("observed_escort_count", 0)))),
                    "estimated_escort_combat_index": max(0, int(observed.get("estimated_combat_index", decision.get("observed_escort_combat_index", 0)))),
                    "candidate_attacker_refs":candidates,
                    "advisory": private,
                },
            ...
            reviews.append({
                "kind": "route_interception_judgment", "attacker_faction_ref": attacker_fid,
                "movement_ref": movement_ref, "route_ref": route_ref, "decision_ref": event["decision_ref"],
                "result": "gm_judgment_required",
            })
            return event

        # Apply already-authored interception choices first. Attack creates a real
        # pursuit owner and an immediate route-service wake; observe/ignore are
        # complete human choices with no hard effect beyond consuming the decision.
        for judgment_event in route_judgment_events:
            choice = judgment_event.get("selected_option")
            if not isinstance(choice, Mapping):
                raise ValueError("committed route interception judgment invalid")
            intent = str(choice.get("intent") or "")
            movement_ref = str(choice.get("target_movement_ref") or judgment_event.get("owner_ref") or "")
            route_ref = str(choice.get("route_ref") or "")
            attacker_fid = str(choice.get("attacker_faction_ref") or judgment_event.get("actor_ref") or "")
            movement = movements.get(movement_ref)
            if intent in {"ignore", "observe"}:
                reviews.append({
                    "kind": "route_interception_policy", "result": intent, "attacker_faction_ref": attacker_fid,
                    "movement_ref": movement_ref, "decision_ref": str(judgment_event.get("decision_ref") or ""),
                    "decision_origin": "gm_npc_judgment",
                })
                continue
            if intent != "attack":
                raise ValueError("route interception judgment intent invalid")
            if not isinstance(movement, Mapping) or str(movement.get("route_ref") or "") != route_ref:
                reviews.append({"kind": "route_interception_policy", "result": "target_movement_unavailable", "movement_ref": movement_ref})
                continue
            attacker_refs = [str(ref) for ref in choice.get("attacker_refs", []) if isinstance(ref, str) and ref]
            leader_ref = str(choice.get("leader_ref") or "")
            if not attacker_fid or not attacker_refs or leader_ref not in attacker_refs:
                raise ValueError("route interception attack judgment incomplete")
            blocked = unavailable_person_refs()
            validated: list[str] = []
            for ref in attacker_refs:
                try:
                    owner_fid, _path, _owner, _ordinal, person = load_person_ref(ref)
                except (FileNotFoundError, KeyError, TypeError, ValueError):
                    continue
                health = person.get("health", {}) if isinstance(person.get("health"), Mapping) else {}
                if owner_fid == attacker_fid and ref not in blocked and health.get("status") != "dead" and int(health.get("consciousness", 100)) > 0:
                    validated.append(ref)
            if validated != attacker_refs:
                reviews.append({
                    "kind": "route_interception_policy", "result": "attackers_unavailable",
                    "attacker_faction_ref": attacker_fid, "movement_ref": movement_ref,
                })
                continue
            afpath, attacker = load_faction(attacker_fid)
            arpath, roster = load_roster(attacker_fid)
            attacker, roster, _boundary = settle_and_reset_faction_training_cycle(
                attacker, roster, at_iso=at_iso, paused_refs=sorted(unavailable_person_refs()),
            )
            writes[afpath] = attacker; writes[arpath] = roster
            faction_cache[attacker_fid] = (afpath, attacker); roster_cache[attacker_fid] = (arpath, roster)
            pursuit_ref = f"pursuit:{movement_ref}:{attacker_fid}:{at.strftime('%Y%m%d%H%M%S')}"
            if pursuit_ref not in movements:
                try:
                    commitments_state = reserve_resources(
                        commitments_state, resources=[("person", ref, attacker_fid) for ref in attacker_refs],
                        actor_ref=leader_ref, owner_ref=attacker_fid, activity_ref=pursuit_ref,
                        activity_kind="route_pursuit", started_at=at_iso,
                        location_ref=str(movement.get("segment_origin_place_ref") or movement.get("origin_place_ref") or route_ref),
                    )
                except ValueError:
                    reviews.append({"kind": "route_interception_policy", "result": "attackers_unavailable", "movement_ref": movement_ref})
                    continue
                pause_people_for_commitment(attacker_fid, attacker_refs)
                movements[pursuit_ref] = {
                    "movement_kind": "route_pursuit", "target_movement_ref": movement_ref, "route_ref": route_ref,
                    "origin_place_ref": str(movement.get("segment_origin_place_ref") or movement.get("origin_place_ref") or ""),
                    "destination_place_ref": str(movement.get("segment_destination_place_ref") or movement.get("destination_place_ref") or ""),
                    "beneficiary_ref": attacker_fid, "participant_refs": attacker_refs, "escort_refs": attacker_refs,
                    "leader_ref": leader_ref,
                    "started_at": at_iso, "ready_at": at_iso, "status": "pursuing",
                    "contact_intent": str(choice.get("contact_intent") or "hostile_interception"),
                    "motive_kind": str(choice.get("motive_kind") or ""),
                    "gm_private_decision_context": copy.deepcopy(dict(judgment_event.get("decision_context") or {})),
                    "combat_policy": copy.deepcopy(dict(choice.get("combat_policy") or {})),
                }
                writes[_ROUTE_OPERATIONS_PATH] = route_ops_state
            pending_one_off_events.append({
                "event_id": f"route_activity_after_judgment:{hashlib.sha256((str(judgment_event.get('decision_ref') or '')+'|'+route_ref).encode()).hexdigest()[:20]}",
                "kind": "route_activity_cycle", "due_at": at_iso, "owner_ref": route_ref,
                "requires_player_decision": False, "decision_origin": "gm_npc_judgment",
            })
            reviews.append({
                "kind": "route_interception_policy", "result": "pursuit_started",
                "attacker_faction_ref": attacker_fid, "movement_ref": movement_ref, "pursuit_ref": pursuit_ref,
                "decision_ref": str(judgment_event.get("decision_ref") or ""), "decision_origin": "gm_npc_judgment",
            })
        ...
        def _start_settlement_exposure_pursuits(
            ...
            for candidate in list(local_factions_by_place.get(stop_place,[])):
                ...
                refs=[str(row.get("person_id")) for row in local_available if isinstance(row.get("person_id"),str)]
                decision=dict(decision)
                decision["advisory_force_count"]=force_size
                ...
                _stage_route_interception_judgment(
                    attacker_fid=attacker_fid, movement_ref=movement_ref,
                    route_ref=str(movement.get("route_ref") or ""), attacker_refs=refs,
                    decision=decision, observed=observed, source_event_id=f"settlement-stop:{movement_ref}:{site_ref}",
                    opportunity_kind="settlement_stop", stop_place=stop_place,
                )
            return started
        ...
        def _park_postcombat_nonreturning_attackers(
            movement_ref: str, movement: Mapping[str, Any], *, attacker_fid: str,
            attacker_refs: Sequence[str], returning_refs: Sequence[str],
            people_after: Mapping[str, Any] | None = None, combat_after: Mapping[str, Any] | None = None,
        ) -> dict[str, Any]:
            """Preserve exact road presence for arrived attackers outside the main return.

            An exact combat overlay is temporary. When it closes, any arrived
            attacker not placed into the immediate return needs another physical
            owner or their stale stored headquarters becomes authoritative again.
            Living stragglers are parked in a finite return owner that retries
            after recovery/logistics permit travel. Dead bodies remain at the
            exact road location rather than teleporting home. Future reinforcements
            that never arrived keep their pre-combat owner untouched.
            """
            if not attacker_fid or not isinstance(combat_after, Mapping):
                return {"parked_refs": [], "dead_refs": []}
            route_ref = str(movement.get("route_ref") or "")
            returning = {str(x) for x in returning_refs if isinstance(x, str) and x}
            combatants = combat_after.get("combatants", {})
            if not isinstance(combatants, Mapping):
                raise ValueError("route postcombat combatants invalid")
            living_stragglers: list[str] = []
            escaped_stragglers: list[str] = []
            dead_refs: list[str] = []
            for ref in list(dict.fromkeys(str(x) for x in attacker_refs if isinstance(x, str) and x)):
                if ref in returning:
                    continue
                if not combat_person_arrived(combat_after, ref):
                    continue
                cstate = combatants.get(ref)
                if cstate is not None and not isinstance(cstate, Mapping):
                    raise ValueError("route postcombat combatant state invalid")
                raw_statuses = cstate.get("status_families", []) if isinstance(cstate, Mapping) else []
                if not isinstance(raw_statuses, list):
                    raise ValueError("route postcombat combatant statuses invalid")
                statuses = {str(x) for x in raw_statuses if isinstance(x, str)}
                person = people_after.get(ref) if isinstance(people_after, Mapping) else None
                if isinstance(person, Mapping):
                    person = copy.deepcopy(dict(person))
                else:
                    try:
                        _fid, _path, _owner, _ordinal, loaded = load_person_ref(ref)
                        person = copy.deepcopy(dict(loaded))
                    except (FileNotFoundError, KeyError, TypeError, ValueError):
                        continue
                health = person.get("health", {}) if isinstance(person.get("health"), Mapping) else {}
                dead = health.get("status") == "dead" or "dead" in statuses
                if dead:
                    if route_ref:
                        person["location_ref"] = route_ref
                        save_exact_person(ref, person)
                    dead_refs.append(ref)
                    continue
                living_stragglers.append(ref)
                if "escaped" in statuses:
                    escaped_stragglers.append(ref)
            if not living_stragglers:
                return {"parked_refs": [], "dead_refs": dead_refs}
            _afpath, attacker_faction = load_faction(attacker_fid)
            headquarters = str(attacker_faction.get("headquarters") or "")
            recovery_ref = bounded_raid_recovery_ref(movement_ref, attacker_fid, at_iso)
            existing = movements.get(recovery_ref)
            if isinstance(existing, Mapping):
                parked = copy.deepcopy(dict(existing))
                merged = list(dict.fromkeys([
                    *[str(x) for x in parked.get("participant_refs", []) if isinstance(x, str)],
                    *living_stragglers,
                ]))
                parked["participant_refs"] = merged
                separated = list(dict.fromkeys([
                    *[str(x) for x in parked.get("separated_person_refs", []) if isinstance(x, str)],
                    *escaped_stragglers,
                ]))
                if separated:
                    parked["separated_person_refs"] = separated
                movements[recovery_ref] = compact_route_movement_roles(parked)
            else:
                extra = {"postcombat_recovery": True}
                if escaped_stragglers:
                    extra["separated_person_refs"] = list(escaped_stragglers)
                movements[recovery_ref] = _park_waiting_return(
                    recovery_ref, movement, beneficiary=attacker_fid,
                    participants=living_stragglers, destination_place=headquarters,
                    movement_kind="raid_return", mode="foot", from_current_position=True,
                    extra=extra,
                )
            writes[_ROUTE_OPERATIONS_PATH] = route_ops_state
            return {
                "movement_ref": recovery_ref,
                "parked_refs": list(living_stragglers),
                "dead_refs": dead_refs,
            }
        ...
        def _start_seizure_return(
            ...
            combat_after: Mapping[str, Any] | None = None, allow_empty_return: bool = False,
        ) -> dict[str, Any]:
            ...
            for ref in [str(x) for x in attacker_refs if isinstance(x, str)]:
                if not route_combatant_available_for_postcombat_movement(combat_after, ref):
                    continue
                person = people_after.get(ref) if isinstance(people_after, Mapping) else None
            ...
            if not living_attackers:
                recovery = _park_postcombat_nonreturning_attackers(
                    movement_ref, movement, attacker_fid=attacker_fid,
                    attacker_refs=attacker_refs, returning_refs=[],
                    people_after=people_after, combat_after=combat_after,
                )
                return {"started": False, "postcombat_recovery": recovery}
            ...
            if intent in {"robbery", "rob_cargo", "extortion", "cargo_seizure", "hostile_interception"}:
                for ref in [str(x) for x in movement.get("participant_refs", []) if isinstance(x, str) and x]:
                    # Winning the side-level fight is not physical custody of a
                    # defender who already completed a withdrawal. Escaped or
                    # not-yet-arrived bodies keep their own property.
                    if not route_combatant_physically_controlled_for_seizure(combat_after, ref):
                        continue
                    person = people_after.get(ref) if isinstance(people_after, Mapping) else None
                    if isinstance(person, Mapping):
                        person = copy.deepcopy(dict(person))
            ...
            if quantity <= 0 and cash_quantity <= 0 and not carried_refs and not allow_empty_return:
                ...
                return {
                    "started": False, "captured_refs": captured_refs, "rescued_refs": rescued_refs,
                    "postcombat_recovery": recovery,
                }
            ...
            if return_ref in movements:
                recovery = _park_postcombat_nonreturning_attackers(
                    movement_ref, movement, attacker_fid=attacker_fid,
                    attacker_refs=attacker_refs, returning_refs=living_attackers,
                    people_after=people_after, combat_after=combat_after,
                )
                return {
        ...
        def _start_attacker_retreat_return(
            ...
            combat_after: Mapping[str, Any] | None = None,
        ) -> dict[str, Any]:
            ...
            if return_ref in movements:
                ...
                recovery = _park_postcombat_nonreturning_attackers(
                    movement_ref, movement, attacker_fid=attacker_fid,
                    attacker_refs=attacker_refs, returning_refs=living,
                    people_after=people_after, combat_after=combat_after,
                )
                return {
            ...
            try:
                ...
                return {
                    "started": False, "reason": "attackers_unavailable_for_return",
                    "postcombat_recovery": recovery,
                }
            returning = _fresh_faction_return_from_current_position(
                ...
                extra={
                    ...
                    "item_ref": "", "quantity": 0,
                    "cash_quantity": max(0, int(movement.get("contact_settlement_cash", 0))),
                },
        ...
        def _close_escort(
            movement_ref: str, movement: Mapping[str, Any], *, success: bool,
            attacker_fid: str | None = None, defer_seized_cargo: bool = False,
            captured_refs: Sequence[str] = (), combat_after: Mapping[str, Any] | None = None,
        ) -> dict[str, Any]:
            ...
            if movement.get("movement_kind") == "player_strategic_travel":
                ...
                if movement_contact:
                    contacts.pop(movement_contact, None)
                _refund_movement_provisions(movement)
                stopped = copy.deepcopy(dict(movement))
                # The unused part of the old reservation has already been
                # refunded above.  A stranded owner preserves physical
                # presence only; it must never retain a spent/refunded
                # reservation that a later recovery could consume or refund a
                # second time.
                stopped.pop("provision_reservation", None)
                for key in (
                ...
                stopped_participants = [ref for ref in participants if ref not in captured]
                combatants = (
                    combat_after.get("combatants", {})
                    if isinstance(combat_after, Mapping) and isinstance(combat_after.get("combatants"), Mapping)
                    else {}
                )
                separated = []
                for ref in stopped_participants:
                    state = combatants.get(ref) if isinstance(combatants, Mapping) else None
                    statuses = {
                        str(x) for x in state.get("status_families", []) if isinstance(x, str)
                    } if isinstance(state, Mapping) and isinstance(state.get("status_families"), list) else set()
                    if "escaped" in statuses:
                        separated.append(ref)
                stopped["participant_refs"] = stopped_participants
                stopped["status"] = "stranded"
                stopped["stranded_at"] = at_iso
                stopped["stranded_reason"] = "combat_defeat"
                stopped["last_progress_at"] = at_iso
                if separated:
                    stopped["separated_person_refs"] = separated
                else:
                    stopped.pop("separated_person_refs", None)
                movements[movement_ref] = stopped
                writes[_ROUTE_OPERATIONS_PATH] = route_ops_state
                return {
                    "closed": False, "success": False, "journey_stopped": True,
                    "movement_ref": movement_ref, "participant_count": len(stopped_participants),
                    "separated_person_count": len(separated),
                }
            ...
            if success:
                ...
                # Shared travel is a real consequence for the full physical
                # escort party.  apply_sparse_group_relationship_event keeps
                # autonomous NPC-NPC edges O(n), while retaining exact direct
                # player relationships, so no arbitrary party cap is needed.
                social_party = participants
            ...
            for movement_ref, raw_movement in route_movements:
                    ...
                    if isinstance(combat, Mapping) and combat.get("status") == "resolved":
                        ...
                        if attacker_control and attacker_fid:
                            seizure = _start_seizure_return(
                                ...
                                combat_after=combat, allow_empty_return=True,
                            )
                            ...
                            _start_attacker_retreat_return(
                                movement_ref, movement, attacker_fid=attacker_fid,
                                attacker_refs=contact_attacker_refs, combat_after=combat,
                            )
                        ...
                        if movement.get("movement_kind") == "player_strategic_travel" and escort_success:
                            ...
                            for key in (
                                "contact_ref", "combat_ref", "contact_attacker_faction_ref",
                                "contact_outlaw_faction_ref", "contact_attacker_refs", "contact_intent",
                                "contact_settlement_cash", "contact_settlement_payer_ref",
                            ):
                                movement.pop(key, None)
                            ...
                            outcome = _close_escort(
                                ...
                                captured_refs=seizure.get("captured_refs", []), combat_after=combat,
                            )
                ...
                for pursuit_ref, pursuit in sorted(movements.items(), key=lambda item: str(item[0])):
                    ...
                    if not attacker_fid or not attackers:
                        continue
                    chosen=(attacker_fid,attackers,{
                        "attack":True,
                        "intent":str(pursuit.get("contact_intent") or "hostile_interception"),
                        "motive_kind":str(pursuit.get("motive_kind") or ""),
                        "gm_private_decision_context":dict(pursuit.get("gm_private_decision_context",{})) if isinstance(pursuit.get("gm_private_decision_context"),Mapping) else {},
                        "combat_policy":dict(pursuit.get("combat_policy",{})) if isinstance(pursuit.get("combat_policy"),Mapping) else {
                            "decision_origin":"legacy_committed_operation","engagement_intent":"legacy_route_interception",
                            "targeting_intent":"disable","target_policy":"nearest_visible","resource_policy":"none",
                ...
                for candidate in route_interception_candidates(str(rid), route):
                    ...
                    attacker_current, attacker_roster, _training_boundary = settle_and_reset_faction_training_cycle(
                        attacker_current, attacker_roster, at_iso=at_iso, paused_refs=sorted(unavailable_person_refs()),
                    )
                    ...
                    # The faction's total roster is capacity, not proof that every
                    # member is physically available on this road. Explicitly
                    # located people away from the route cannot materialize into
                    # a spontaneous interception. People without an exact current
                    # location may represent the faction's ordinary local/route
                    # presence only when the faction itself has lawful presence
                    # on this edge (endpoint institution or registered outlaw
                    # operating route). A previously mobilized route_pursuit is
                    # handled above and already owns its exact participants.
                    route_row = route if isinstance(route, Mapping) else {}
                    route_places = {
                        str(route_row.get("from") or ""),
                        str(route_row.get("to") or ""),
                        str(movement.get("segment_origin_place_ref") or ""),
                        str(movement.get("segment_destination_place_ref") or ""),
                    } - {""}
                    hq = str(attacker_current.get("headquarters") or "")
                    operating_routes = {
                        str(x) for x in attacker_current.get("operating_routes", [])
                        if isinstance(x, str)
                    } if isinstance(attacker_current.get("operating_routes"), Sequence) and not isinstance(attacker_current.get("operating_routes"), (str, bytes, bytearray)) else set()
                    faction_has_route_presence = (
                        hq in route_places
                        or str(rid) in operating_routes
                    )
                    available = []
                    for person in usable_martial_people(attacker_roster, exclude_committed=unavailable_person_refs()):
                        explicit_location = str(person.get("location_ref") or "")
                        if explicit_location:
                            settlement = person_settlement(person, faction_headquarters=hq, sites=site_rows)
                            if explicit_location != str(rid) and settlement not in route_places:
                                continue
                        elif not faction_has_route_presence:
                            continue
                    ...
                    decision = dict(decision)
                    decision["advisory_force_count"] = attack_count
                    decision["observer_ref"] = str(observer.get("person_id") or "")
                    decision["observed_escort_count"] = int(observed.get("visible_escort_count", 0))
                    decision["observed_escort_combat_index"] = int(observed.get("estimated_combat_index", 0))
                    decision["known_ransom_value_cash"] = known_ransom_value
                    _stage_route_interception_judgment(
                        attacker_fid=attacker_fid, movement_ref=movement_ref, route_ref=str(rid),
                        attacker_refs=[str(p.get("person_id")) for p in available if isinstance(p.get("person_id"), str)],
                        decision=decision, observed=observed, source_event_id=str(event.get("event_id") or ""),
                        opportunity_kind="moving_route",
                    )
                    judgment_staged = True
                    break

                if judgment_staged:
                    movements[movement_ref] = movement
                    writes[_ROUTE_OPERATIONS_PATH] = route_ops_state
                    route_review["npc_judgment_required"] = True
                    continue
                ...
                if chosen is not None:
                    ...
                    if isinstance(source_region, str) and int(exposure.get("witness_milli",0)) >= 350:
                        ...
                        for attacker_ref in attacker_refs:
                            ...
                            if total >= 40:
                                ...
                                government_judgment = build_npc_judgment_event(
                                    judgment_kind="government_enforcement_policy", at_iso=at_iso,
                                    actor_ref=f"government:{source_region}", owner_ref=attacker_ref,
                                    options=[
                                        {"intent":"maintain" if existing else "no_action", "subject_ref":attacker_ref, "jurisdiction_ref":source_region},
                                        {
                                            "intent":"issue_warrant", "subject_ref":attacker_ref, "jurisdiction_ref":source_region,
                                            "offense":public_offense, "bounty_cash":desired, "evidence_ref":contact_ref,
                                        },
                                    ],
                                    decision_context={
                                        "subject_ref":attacker_ref, "jurisdiction_ref":source_region, "attention":total,
                                        "prior_offenses":prior_offenses + 1, "public_offense":public_offense,
                                        "evidence_ref":contact_ref, "witness_confidence":confidence,
                                    ...
                                    source_event_id=str(event.get("event_id") or contact_ref),
                                )
                                pending_one_off_events.append(government_judgment)
                                handoffs.append(judgment_handoff(government_judgment))
                    ...
                    if player_ref and player_ref in participants:
                        ...
                        combat = initialize_combat(
                            ...
                            objective=_route_combat_objective(movement_ref, movement),
                        ...
                        settlement_terms = route_extortion_settlement_terms(
                            attacker_faction_type=attacker_type,
                            attacker_intent=str(decision.get("intent") or "hostile_interception"),
                            motive_kind=str(decision.get("motive_kind") or ""),
                            observed_escort_count=max(1, int(decision.get("observed_escort_count", len(escort_refs)))),
                            cargo_value_cash=cargo_value,
                        )
                        contacts[contact_ref] = {
                            ...
                            # Private causal provenance for AI direction. This is never
                            # player knowledge merely because the GM can read it. Keep a
                            # bounded decision snapshot so later combat narration and NPC
                            # dialogue can remain causally faithful even after the original
                            # autonomous review has left the hot scheduler context.
                            "attacker_intent": str(decision.get("intent") or "hostile_interception"),
                            "motive_kind": str(decision.get("motive_kind") or ""),
                            ...
                            **({"settlement_terms": settlement_terms} if settlement_terms else {}),
                        }
                    ...
                    result = simulate_exact_combat(
                        ...
                        authored_combat_policy=(
                            copy.deepcopy(dict(decision.get("combat_policy", {})))
                            if isinstance(decision.get("combat_policy"), Mapping) else {
                                "decision_origin":"legacy_committed_operation",
                                "engagement_intent":"legacy_route_interception",
                                "targeting_intent":"disable","target_policy":"nearest_visible",
                                "resource_policy":"none","withdrawal_policy":"no_voluntary_withdrawal",
                            }
                        ),
                    )
                    ...
                    if protected_dead or attacker_control:
                        ...
                        if attacker_control:
                            seizure = _start_seizure_return(
                                ...
                                people_after=people_after, combat_after=result.get("combat_after"),
                            ...
                            _start_attacker_retreat_return(
                                ...
                                combat_after=result.get("combat_after"),
                            )
                        outcome = _close_escort(
                            movement_ref, movement, success=False,
                            attacker_fid=attacker_fid if attacker_control else None,
                            defer_seized_cargo=bool(seizure.get("started")),
                            captured_refs=seizure.get("captured_refs", []), combat_after=result.get("combat_after"),
                        )
                        route_review["completed_movements"] += int(bool(outcome.get("closed")))
                        if outcome.get("closed"):
                            route_review["closed_outcomes"][movement_ref] = bool(outcome.get("success"))
