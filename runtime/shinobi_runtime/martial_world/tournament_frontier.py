...
def settle_tournament_frontier(
    *,
    ...
    # Tournament participation is a meaningful institutional/personnel choice.
    # Registration eligibility, route burden, reserve floors and candidate
    # capability are mechanical evidence; they never sponsor a person or spend
    # faction resources by themselves.  A same-time GM-authored participation
    # policy is revalidated here before any registration/travel/delegation write.
    for event in sorted_events:
        if event.get("kind") != "npc_judgment_committed" or event.get("judgment_kind") != "tournament_participation_policy":
            continue
        ...
        if not isinstance(choice, Mapping) or not isinstance(context, Mapping):
            raise ValueError("tournament participation judgment invalid")
        fid = str(context.get("faction_ref") or event.get("actor_ref") or "")
        tref = str(context.get("tournament_ref") or event.get("owner_ref") or "")
        registry = tournament_state.get("tournaments", {}) if isinstance(tournament_state, Mapping) else {}
        tournament = registry.get(tref) if isinstance(registry, Mapping) else None
        if not fid or not isinstance(tournament, dict) or tournament.get("status") != "registration_open":
            reviews.append({"kind":"tournament_participation_policy","decision_ref":event.get("decision_ref"),"faction_ref":fid,"tournament_ref":tref,"result":"tournament_unavailable"})
            continue
        intent = str(choice.get("intent") or "")
        if intent == "decline":
            reviews.append({"kind":"tournament_participation_policy","decision_ref":event.get("decision_ref"),"faction_ref":fid,"tournament_ref":tref,"result":"declined","decision_origin":"gm_npc_judgment"})
            continue
        if intent != "participate":
            raise ValueError("tournament participation intent invalid")
        tournament_kind = str(context.get("tournament_kind") or tournament.get("tournament_kind") or "")
        host = str(context.get("host_place_ref") or tournament.get("host_place_ref") or "")
        competition_date = str(context.get("competition_date") or tournament.get("competition_date") or "")
        registration_closes_on = str(context.get("registration_closes_on") or tournament.get("registration_closes_on") or "")
        try:
            profile = tournament_event_profile(tournament_kind)
            fpath, faction = load_faction(fid)
            rpath, roster = load_roster(fid)
        except (KeyError, FileNotFoundError, ValueError):
            reviews.append({"kind":"tournament_participation_policy","decision_ref":event.get("decision_ref"),"faction_ref":fid,"tournament_ref":tref,"result":"owner_unresolved"})
            continue
        source_place = str(faction.get("headquarters") or "")
        source_site = str(faction.get("local_site_ref") or "")
        blocked = unavailable_person_refs()
        rows = roster.get("people", []) if isinstance(roster, Mapping) else []
        by_ref = {str(row.get("person_id")): row for row in rows if isinstance(row, Mapping) and isinstance(row.get("person_id"), str)} if isinstance(rows, list) else {}

        entrant_refs: list[str] = []
        for ref in [str(x) for x in context.get("entrant_candidate_refs", []) if isinstance(x, str)]:
            person = by_ref.get(ref)
            if not isinstance(person, Mapping) or ref == player_ref or ref in blocked:
                continue
            if person_place(person, home_place=source_place, home_site_ref=source_site) != source_place:
                continue
            if not tournament_person_eligible(tournament_kind, person, year=at.year):
                continue
            health = person.get("health", {}) if isinstance(person.get("health"), Mapping) else {}
            if health.get("status") in {"dead", "incapacitated"} or bool(person.get("retired_from_field", False)):
                continue
            entrant_refs.append(ref)
        entrant_refs.sort(key=lambda ref: (-person_combat_index(by_ref[ref]), ref))
        entrant_policy = str(choice.get("entrant_policy") or "none")
        if entrant_policy == "none":
            selected_entrants: list[str] = []
        elif entrant_policy == "champion":
            selected_entrants = entrant_refs[:1]
        elif entrant_policy == "top_three":
            selected_entrants = entrant_refs[:3]
        elif entrant_policy == "broad":
            selected_entrants = entrant_refs
        else:
            raise ValueError("tournament entrant policy invalid")

        fee = max(0, int(tournament.get("entry_fee_cash", 0)))
        local_registered = 0
        travel_planned = 0
        if source_place == host:
            try:
                ipath, inventory = load_inventory(fid)
            except (KeyError, FileNotFoundError, ValueError):
                inventory = {}; ipath = ""
            transport = inventory.get("transport_capacity", {}) if isinstance(inventory, Mapping) and isinstance(inventory.get("transport_capacity"), Mapping) else {}
            quote = monthly_upkeep_quote(faction, rider_capacity_slots=max(0, int(transport.get("rider_slots", 0))), freight_capacity_kg=max(0, int(transport.get("freight_capacity_kg", 0))))
            policy = faction.get("autonomy_policy", {}) if isinstance(faction.get("autonomy_policy"), Mapping) else {}
            reserve_floor = max(0, int(quote.get("total_cash", 0))) * max(2, int(policy.get("reserve_cash_months", 6)))
            for ref in selected_entrants:
                treasury = max(0, int(faction.get("treasury_cash", 0)))
                ...
                try:
                    reg = tournament_register(tournament, entrant_ref=ref, qualifying_score=qualifying, payer_cash=treasury, alive=True, medically_eligible=True)
                except ValueError:
                    continue
                tournament = dict(reg["tournament_after"]); tournament["registrations"][-1]["faction_ref"] = fid
                registry[tref] = tournament
                faction["treasury_cash"] = int(reg["payer_cash_after"])
                is_leader, is_senior = _tournament_delegate_roles(person)
                _add_tournament_delegation_presence(tref, fid, entrant_refs=[ref], leader_refs=[ref] if is_leader else (), senior_refs=[ref] if is_senior else ())
                tournament = registry[tref]
                local_registered += 1
            if local_registered:
                writes[fpath] = faction; faction_cache[fid] = (fpath, faction)
        else:
            minimum_host_days = max(1, int(context.get("minimum_host_days", 1)))
            for ref in selected_entrants:
                outcome = plan_tournament_trip(
                    fid, person_ref=ref, tournament_ref=tref, host_place=host,
                    registration_closes_on=registration_closes_on, competition_date=competition_date,
                    entry_fee_cash=fee, arrival_lead_hours_min=int(profile.get("arrival_lead_hours_min", 12)),
                    ...
                    minimum_host_days=minimum_host_days,
                )
                if outcome.get("result") == "departure_planned":
                    travel_planned += 1

        delegation_planned = 0
        delegation_policy = str(choice.get("delegation_policy") or "none")
        if delegation_policy not in {"none", "senior_representatives"}:
            raise ValueError("tournament delegation policy invalid")
        if delegation_policy == "senior_representatives":
            delegate_refs: list[str] = []
            selected_set = set(selected_entrants)
            for ref in [str(x) for x in context.get("delegate_candidate_refs", []) if isinstance(x, str)]:
                person = by_ref.get(ref)
                if not isinstance(person, Mapping) or ref == player_ref or ref in blocked or ref in selected_set:
                    continue
                if person_place(person, home_place=source_place, home_site_ref=source_site) != source_place:
                    continue
                health = person.get("health", {}) if isinstance(person.get("health"), Mapping) else {}
                if health.get("status") in {"dead", "incapacitated"}:
                    continue
                delegate_refs.append(ref)
            if delegate_refs:
                outcome = plan_tournament_delegation_trip(
                    fid, candidate_refs=delegate_refs, tournament_ref=tref, host_place=host,
                ...
                delegation_planned = 1 if outcome.get("result") == "delegation_departure_planned" else 0
        writes[_TOURNAMENTS_PATH] = tournament_state
        reviews.append({
            "kind":"tournament_participation_policy","decision_ref":event.get("decision_ref"),
            "faction_ref":fid,"tournament_ref":tref,"local_paid_registrations":local_registered,
            "traveling_entrants_planned":travel_planned,"spectator_delegations_planned":delegation_planned,
            "result":"participation_committed","decision_origin":"gm_npc_judgment",
        })
    ...
    if tournament_events:
        ...
        for event in tournament_events:
            ...
            if kind == "tournament_registration_open":
                ...
                # Registration opening now creates bounded factual envelopes for
                # faction leadership instead of turning deterministic interest
                # rolls into sponsorship, travel, attendance or treasury writes.
                # Capability ranking is advisory and bounds transport size; the
                # actual institutional participation policy is authored by the GM.
                office_priority = {
                    "leader": 1000, "deputy_leader": 900, "chief_martial_instructor": 850,
                    "chief_physician": 800, "chief_steward": 760, "treasurer": 720, "quartermaster": 680,
                }
                ...
                for fid in all_faction_ids:
                    try:
                        _fpath, faction = load_faction(fid); _rpath, roster = load_roster(fid)
                    except (KeyError, FileNotFoundError, ValueError):
                        continue
                    ...
                    blocked = unavailable_person_refs()
                    available = [
                        person for person in usable_martial_people(roster, exclude_committed=blocked)
                        if str(person.get("person_id") or "") != player_ref
                        and person_place(person, home_place=source_place, home_site_ref=source_site) == source_place
                    ]
                    entrant_people = [person for person in available if tournament_person_eligible(tournament_kind, person, year=at.year)]
                    entrant_people.sort(key=lambda person: (-person_combat_index(person), str(person.get("person_id", ""))))
                    entrant_refs = [str(person.get("person_id")) for person in entrant_people[:8] if person.get("person_id")]
                    def _delegate_priority(person: Mapping[str, Any]) -> tuple[int, int, int, str]:
                    ...
                    delegates = [person for person in available if at.year - int(person.get("birth_year", at.year)) >= 14]
                    delegates.sort(key=_delegate_priority)
                    delegate_refs = [str(person.get("person_id")) for person in delegates[:6] if person.get("person_id")]
                    if not entrant_refs and not delegate_refs:
                        continue
                    travel_days_hint = 0
                    if source_place and source_place != host:
                        try:
                            route_hint = shortest_route(start=source_place, end=host, mode="foot")
                    ...
                    candidate_packets.append({
                        "faction_ref":fid,"faction_type":ftype,"source_place_ref":source_place,
                        "entrant_candidate_refs":entrant_refs,"delegate_candidate_refs":delegate_refs,
                        "travel_days_hint":travel_days_hint,"treasury_cash":max(0,int(faction.get("treasury_cash",0))),
                    })
                estimated_field_size = sum(len(row["entrant_candidate_refs"]) for row in candidate_packets)
                budgeted_host_days = tournament_estimated_host_days(tournament_kind, max(2, estimated_field_size))
                judgment_count = 0
                for packet in candidate_packets:
                    options: list[dict[str, Any]] = [{"intent":"decline"}]
                    if packet["entrant_candidate_refs"]:
                        options.extend([
                            {"intent":"participate","entrant_policy":"champion","delegation_policy":"none"},
                            {"intent":"participate","entrant_policy":"top_three","delegation_policy":"none"},
                            {"intent":"participate","entrant_policy":"broad","delegation_policy":"none"},
                        ])
                    if packet["delegate_candidate_refs"]:
                        options.append({"intent":"participate","entrant_policy":"none","delegation_policy":"senior_representatives"})
                        if packet["entrant_candidate_refs"]:
                            options.append({"intent":"participate","entrant_policy":"champion","delegation_policy":"senior_representatives"})
                    judgment = build_npc_judgment_event(
                        judgment_kind="tournament_participation_policy", at_iso=at_iso,
                        actor_ref=str(packet["faction_ref"]), owner_ref=tref, options=options,
                        decision_context={
                            **packet,"tournament_ref":tref,"tournament_kind":tournament_kind,
                            "host_place_ref":host,"competition_date":competition_date,
                            "registration_closes_on":str(event.get("registration_closes_on") or ""),
                            "entry_fee_cash":fee,"minimum_host_days":budgeted_host_days,
                            "prestige_weight":prestige,"prize_cash":int(tournament.get("prize_escrow_cash",0)),
                        },
                        source_event_id=str(event.get("event_id") or ""),
                    )
                    pending_one_off_events.append(judgment); handoffs.append(judgment_handoff(judgment)); judgment_count += 1

                tournaments[tref] = tournament
                writes[_TOURNAMENTS_PATH] = tournament_state
                writes[mpath] = organizer_market; market_cache[host_region] = (mpath, organizer_market)
                notice = {
                    "kind":"tournament_registration","tournament_ref":tref,"tournament_kind":tournament_kind,
                    "host_place_id":host,"organizer_ref":tournament.get("organizer_ref"),
                    "competition_date":competition_date,"entry_fee_cash":fee,
                    "prize_cash":int(tournament.get("prize_escrow_cash",0)),
                    "faction_judgments_pending":judgment_count,"estimated_candidate_field_size":estimated_field_size,
                    "budgeted_host_days":budgeted_host_days,"delivered_to_player":True,"requires_player_decision":False,
                }
                handoff = classify_handoff(notice)
                reviews.append({
                    "kind":"tournament_registration_open","event_id":event.get("event_id"),
                    "tournament_ref":tref,"faction_judgments_pending":judgment_count,
                    "estimated_candidate_field_size":estimated_field_size,"budgeted_host_days":budgeted_host_days,
                    "local_paid_registrations":0,"traveling_entrants_planned":0,"spectator_delegations_planned":0,
