...
def _settle(*, final=True, civic=None, independent=None, civilians=None, family=None):
    ...
    result = settle_annual_life_frontier(
        ...
        load_market=load_market, market_cache={}, writes=writes, reviews=reviews, handoffs=handoffs,
        pending_one_off_events=pending, pending_training_resume_refs=set(),
    ...
    return result, writes, reviews, markets, pending, handoffs
...
def test_global_nonfaction_life_runs_only_on_final_faction_annual_chunk():
    ...
    result, writes, reviews, markets, pending, handoffs = _settle(final=False, civic=[old])
    ...
    result, writes, reviews, markets, pending, handoffs = _settle(final=True, civic=[old])
...
def test_emperor_vacancy_requires_gm_appointment_without_minting_person():
    ...
    result, writes, reviews, _markets, pending, handoffs = _settle(final=True, civic=[emperor, prince], civilians=civilians)
    rows = result["civic_state"]["people"]
    successor = next(row for row in rows if row["person_id"] == "civic.prince")
    assert "emperor" not in successor.get("standing_offices", [])
    assert "prince" in successor.get("standing_offices", [])
    assert len(rows) == 2
    assert result["civilian_state"]["places"]["luoyang"]["current_population"] == 1000
    judgment = next(row for row in pending if row.get("judgment_kind") == "civic_office_appointment")
    assert any(opt.get("intent") == "appoint_existing" and opt.get("person_ref") == "civic.prince" for opt in judgment["options"])
    assert any(row.get("decision_ref") == judgment.get("decision_ref") for row in handoffs)


def test_local_office_vacancy_does_not_consume_civilian_before_gm_choice():
    magistrate = _person("civic.magistrate", -200, office="magistrate", affiliation="government.luoyang")
    civilians = {"schema":"jianghu-civilian-populations-1.0","places":{"luoyang":{"current_population":1000,"identity_ordinal_cursor":7}}}
    result, writes, reviews, _markets, pending, handoffs = _settle(final=True, civic=[magistrate], civilians=civilians)
    rows = result["civic_state"]["people"]
    living_magistrates = [row for row in rows if "magistrate" in row.get("standing_offices", []) and row.get("health",{}).get("status") != "dead"]
    assert living_magistrates == []
    assert len(rows) == 1
    assert result["civilian_state"]["places"]["luoyang"]["current_population"] == 1000
    assert result["civilian_state"]["places"]["luoyang"]["identity_ordinal_cursor"] == 7
    assert "state/martial-world/civilian-populations.json" not in writes
    judgment = next(row for row in pending if row.get("judgment_kind") == "civic_office_appointment")
    assert any(opt.get("intent") == "appoint_local_civilian" for opt in judgment["options"])
...
def test_nonfaction_estate_prefers_exact_living_spouse():
    ...
    result, _writes, _reviews, markets, _pending, _handoffs = _settle(final=True, independent=[dead, spouse], family=family)
