    ...
    *,
    ...
    for event in events:
        ...
        try:
            ...
            recipient_place = ""
        message_due = None
        message_route_status = "endpoints_unresolved"
        if holder_place and recipient_place:
            if holder_place == recipient_place:
                # Same-place delivery still needs local handling rather than
                # instantaneous knowledge/decision at the captor-review tick.
                message_due = at + timedelta(hours=4)
                message_route_status = "local_delivery_pending"
            else:
                try:
                    ...
                    message_route_status = "in_transit"
                except (KeyError, ValueError, TypeError):
                    # Missing geography is not permission to fabricate a three-day
                    # courier. The ransom demand remains real captor-side truth,
                    # but no recipient response exists until a physical route does.
                    message_route_status = "route_unavailable"
        if message_due is not None:
            pending_one_off_events.append({
                ...
                "information_route_status": message_route_status,
                "message_origin_location_ref": holder_place,
                "message_target_location_ref": recipient_place,
        ...
        reviews.append({
            ...
            "message_route_status": message_route_status,
            "result": "ransom_demanded" if message_due is not None else "ransom_demanded_message_route_unavailable",
        })
