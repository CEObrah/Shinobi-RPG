...
def _offscreen_policy(*, target_policy: str = "nearest_visible", targeting_intent: str = "disable") -> dict:
    return {
        "decision_origin": "gm_npc_judgment",
        "engagement_intent": "test_bounded_combat",
...
def test_war_coalitions_are_sparse_current_facts_and_feed_existing_war_choice():
    ...
    # Coalition membership is decision evidence, never a deterministic attack
    # selector. It may raise advisory urgency for an already-lawful hostile
    # option, but the returned envelope remains unselected for the GM.
    plain = choose_hostile_action(
        relations["edges"], faction_ref="a", year=61, month=9, risk_tolerance=10,
    ...
    assert plain and joined
    assert plain["requires_gm_judgment"] is True
    assert joined["requires_gm_judgment"] is True
    plain_enemy = next(row for row in plain["options"] if row["target_faction_ref"] == "enemy")
    joined_enemy = next(row for row in joined["options"] if row["target_faction_ref"] == "enemy")
    assert plain_enemy["coalition_pressure"] is False
    assert joined_enemy["coalition_pressure"] is True
    assert joined_enemy["urgency_milli"] > plain_enemy["urgency_milli"]
...
def test_autonomous_mass_combat_does_not_materialize_new_pairwise_familiarity(monkeypatch):
    ...
    result = sim.simulate_exact_combat(
        ...
        authored_combat_policy=_offscreen_policy(),
    )
...
def test_personal_obligation_can_redirect_npc_target_without_granting_combat_success(monkeypatch):
    ...
    for row in (player, ally, enemy):
        make_person_activity_ready_fixture(row)
    ...
    for _ in range(3):
        ...
        enemy_event = next(
            (row for row in result["events"] if row.get("actor_ref") == enemy["person_id"] and "intended_ref" in row),
            None,
        )
    ...
    assert enemy_event is not None
...
def test_autonomous_combat_driver_uses_personal_target_pressure_and_obeys_nonlethal_vow(monkeypatch):
    ...
    result = sim.simulate_exact_combat(
        ...
        authored_combat_policy=_offscreen_policy(target_policy="pressure_then_nearest", targeting_intent="lethal"),
    )
...
def test_autonomous_combat_reuses_one_hydrated_equipment_view_and_compacts_once(monkeypatch):
    ...
    make_person_activity_ready_fixture(a); make_person_activity_ready_fixture(b)
