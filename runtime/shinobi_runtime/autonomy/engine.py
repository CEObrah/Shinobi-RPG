...
class AutonomousPolicyBook:
    ...
    def from_record(cls, record: Mapping[str, Any]) -> "AutonomousPolicyBook":
        ...
        if not all(isinstance(value, Mapping) for value in (profiles, assignments, team_profiles, institution_assignments)):
    ...
    def faction_context(self, faction_id: str) -> Tuple[Mapping[str, Any], Mapping[str, Any]]:
        ...
        if not isinstance(profile_ref, str) or not isinstance(profile, Mapping):
            raise ValueError("faction autonomy assignment references an unknown profile")
...
def _stable_index(identity: str, at: CampaignTime, modulo: int, salt: str = "") -> int:
    if modulo <= 0:
        return 0
...
def _leader(faction: Mapping[str, Any]) -> Optional[str]:
    ...
    if isinstance(leadership, Sequence) and not isinstance(leadership, (str, bytes, bytearray)):
        for value in leadership:
            if isinstance(value, str) and value:
                return value
    key_members = faction.get("key_member_ids")
    if isinstance(key_members, Sequence) and not isinstance(key_members, (str, bytes, bytearray)):
...
def review_faction(
    *,
    ...
    """Return at most two bounded semantic intents for one faction review.

    Routine repeated reviews are analytically compacted.  A ten-year skip does
    not create hundreds of missions; it creates one material action plus a
    compacted routine-activity summary whose count is explicit.
    """
    ...
    if not actions:
        return ()
    action = actions[_stable_index(faction_id, at, len(actions), salt=str(compacted_reviews))]
    reason = str(profile.get("reason", faction.get("current_plan") or faction.get("goal") or "scheduled autonomous review"))
    plan_state = faction.get("plan_state") if isinstance(faction.get("plan_state"), Mapping) else {}
    open_missions = [
        value for value in plan_state.get("autonomous_mission_refs", [])
        if isinstance(value, str) and value
    ] if isinstance(plan_state, Mapping) and isinstance(plan_state.get("autonomous_mission_refs", []), list) else []
    formed_teams = [
        value for value in plan_state.get("autonomous_team_refs", [])
        if isinstance(value, str) and value
    ] if isinstance(plan_state, Mapping) and isinstance(plan_state.get("autonomous_team_refs", []), list) else []
    team_creation = dict(assignment.get("team_creation", {})) if isinstance(assignment.get("team_creation"), Mapping) else None
    ...
    common = {
        ...
        "team_refs": list(assignment.get("team_refs", [])) if isinstance(assignment.get("team_refs"), list) else [],
        ...
        "mission_participant_refs": list(assignment.get("mission_participant_refs", [])) if isinstance(assignment.get("mission_participant_refs"), list) else [],
        "classification": profile.get("classification", "restricted"),
        "mission_objective_cycle": list(profile.get("mission_objective_cycle", [])) if isinstance(profile.get("mission_objective_cycle"), list) else [],
        "mission_difficulty": int(profile.get("mission_difficulty", 60)) if isinstance(profile.get("mission_difficulty", 60), int) and not isinstance(profile.get("mission_difficulty", 60), bool) else 60,
        "team_creation": team_creation,
        "open_autonomous_mission_refs": open_missions,
        "formed_autonomous_team_refs": formed_teams,
        "allow_formation_expand": bool(assignment.get("allow_formation_expand", False)),
    }
    decisions = []
    if open_missions:
        decisions.append(
            AutonomousDecision(
                ...
                actor_ref=actor,
                reason="Advance one already-active autonomous mission before creating additional tasking.",
                payload=common,
                material=True,
            )
        )
    if common["team_creation"]:
        decisions.append(
            AutonomousDecision(
                kind="team_form",
                actor_ref=actor,
                reason="A lawful named cell is available to be organized from already-existing personnel.",
                payload=common,
                material=True,
            )
        )
    ...
    if (
        ...
        and isinstance(formation_size, int) and not isinstance(formation_size, bool) and formation_size > 0
        ...
        decisions.append(
            AutonomousDecision(
                ...
                actor_ref=actor,
                reason="Maintain a bounded operational formation topology from already-conserved force manpower.",
                payload=common,
                material=True,
            )
        )
...
def review_team(
    *,
    ...
    if team_record.get("schema") != "exact-team" or team_record.get("status") != "active":
        return ()
    team_id = team_record.get("id")
    team_type = team_record.get("team_type")
    leader = team_record.get("leader_ref")
    if not all(isinstance(value, str) and value for value in (team_id, team_type, leader)):
        return ()
    ...
    payload = {
        ...
        "training_focus": list(profile.get("training_focus", [])) if isinstance(profile.get("training_focus"), list) else [],
        "doctrine_identity": str(profile.get("doctrine_identity", "adaptive mission-team doctrine")),
        "motto": str(profile.get("motto", "Observe. Coordinate. Complete the mission. Return together.")),
        "compacted_reviews": compacted_reviews,
    }
    return (
        AutonomousDecision(
            ...
            actor_ref=leader,
            reason="The exact team reviews doctrine, training priorities, and practiced coordination.",
            payload=payload,
            material=True,
        ),
    )
