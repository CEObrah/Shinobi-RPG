...
from .infrastructure import building_upgrade_requirements, facility_physical_effects
...
def forced_entry_resolution(*, intruder:Mapping[str,Any], buildings:Mapping[str,Any], infrastructure:Mapping[str,Any]|None=None, weapon_impact:int=0, structure_condition:Mapping[str,Any]|None=None) -> dict[str,Any]:
    attrs=intruder.get('attributes',{}) if isinstance(intruder.get('attributes'),Mapping) else {}
    ...
    base_barrier=max(1,int(facility_physical_effects(buildings,infrastructure,'walls_gate').get('barrier_integrity',1)))
    condition=structure_condition if isinstance(structure_condition,Mapping) else {}
    integrity=max(0,min(1000,int(condition.get('walls_gate_integrity_milli',1000))))
    if bool(condition.get('walls_gate_breached')): integrity=min(integrity,100)
    barrier=max(1,base_barrier*max(50,integrity)//1000)
    ...
    return {'barrier_integrity':barrier,'base_barrier_integrity':base_barrier,'integrity_before_milli':integrity,'work_rate':work_rate,'breach_seconds':breach_seconds}


def breach_repair_quote(buildings:Mapping[str,Any], structure_condition:Mapping[str,Any]|None=None) -> dict[str,Any]:
    """Quote structural repair from the authoritative current wall recipe.

    Damage is a fraction of rebuilding the faction's existing wall/gate level.
    This keeps construction and repair on one physical-material/labor authority:
    1,000 milli damage requires the full registered level recipe; partial damage
    requires the same fraction, rounded up so a non-zero repair never receives
    free materials or labor. Cash overhead is deliberately excluded because this
    is repair of owned infrastructure rather than purchase of a new project.
    """
    level=max(0,int(buildings.get('walls_gate',0)))
    condition=structure_condition if isinstance(structure_condition,Mapping) else {}
    integrity=max(0,min(1000,int(condition.get('walls_gate_integrity_milli',1000))))
    damage=max(0,1000-integrity)
    breached=bool(condition.get('walls_gate_breached'))
    if level<=0 or (damage<=0 and not breached):
        return {'materials':{},'general_labor_hours':0,'skilled_labor_hours':0,'labor_hours':0,'damage_milli':damage}
    # A recorded breach is never cosmetically tiny: at least 35% of the
    # structural fabric must be repaired even if an older save only persisted
    # the breached flag and omitted a matching integrity loss.
    damage=max(damage,350 if breached else damage)
    recipe=building_upgrade_requirements('walls_gate',level)
    base_materials=recipe.get('materials',{}) if isinstance(recipe.get('materials'),Mapping) else {}
    materials={
        str(ref): max(1, math.ceil(max(0,int(qty))*damage/1000))
        for ref,qty in base_materials.items() if int(qty)>0
    }
    general=max(1,math.ceil(max(0,int(recipe.get('general_labor_hours',0)))*damage/1000))
    skilled=max(1,math.ceil(max(0,int(recipe.get('skilled_labor_hours',0)))*damage/1000))
    return {
        'materials':materials,
        'general_labor_hours':general,
        'skilled_labor_hours':skilled,
        'labor_hours':general+skilled,
        'damage_milli':damage,
        'required_crafting_or_administration':max(0,int(recipe.get('required_crafting_or_administration',0))),
    }
...
__all__=['alarm_response_seconds','breach_repair_quote','forced_entry_resolution','infiltration_resolution','institutional_guard_duty_milli','select_watch_guards','watch_requirement']
