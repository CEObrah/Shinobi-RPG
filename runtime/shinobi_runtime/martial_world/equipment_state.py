...
def effective_person_loadout(ledger: Mapping[str, Any], person_ref: str) -> dict[str, Any]:
    ...
    # ``hydrate_equipment_ledger`` has already composed policy + sparse
    # deviations into complete person rows. Exact combat calls this accessor
    # thousands of times; recomputing the global assignment map for an already
    # hydrated private ledger is pure overhead. The transient marker is dropped
    # again by compaction and is never persisted.
    if ledger.get("_hydrated") is True and isinstance(raw, Mapping):
