...
def factual_restraint_basis(*,target:Mapping[str,Any],target_ref:str,actor_ref:str,combats:Mapping[str,Any],existing_custody:Sequence[Mapping[str,Any]])->str|None:
    ...
    for row in existing_custody:
        if isinstance(row,Mapping) and row.get('person_ref')==target_ref and row.get('status') not in {'released','escaped','executed'} and row.get('captor_ref')==actor_ref: return 'existing_lawful_custody'
    return None
