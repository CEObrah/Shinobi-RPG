...
def _coarse_trauma_function_loss(wound:Mapping[str,Any])->int:
    """Functional floor for coarse wounds lacking an exact structure label."""
    cut=max(0,int(wound.get('cut',0) or 0)); pierce=max(0,int(wound.get('pierce',0) or 0))
    blunt=max(0,int(wound.get('blunt',0) or 0)); penetration=max(0,int(wound.get('penetration',0) or 0))
    fracture=max(0,int(wound.get('fracture',0) or 0)); tendon=max(0,int(wound.get('tendon_damage',0) or 0)); nerve=max(0,int(wound.get('nerve_damage',0) or 0))
    tissue=cut+pierce+blunt//2+penetration
    return min(100,max(fracture,tendon,nerve)//2+min(50,tissue//5))


def _saturating_trauma(current:int,incoming:int,*,cap:int=200)->int:
    """Accumulate uncertain coarse trauma with diminishing returns.

    Exact named structures retain linear accumulation because repeated contact
    is known to strike the same physical structure. A coarse zone is an
    intentionally lossy current-state bucket, so treating every later contact
    as if it landed on the identical tendon/nerve/bleeding source overstates
    injury. This bounded curve preserves worsening trauma without manufacturing
    certainty from missing anatomical resolution.
    """
    limit=max(1,int(cap)); cur=max(0,min(limit,int(current))); inc=max(0,min(limit,int(incoming)))
    if inc<=0 or cur>=limit:return cur
    if cur<=0:return inc
    gain=(inc*(limit-cur)+limit-1)//limit
    return min(limit,cur+max(1,gain))


def _coarse_bleeding_merge(current:int,incoming:int)->int:
    """Compact multiple uncertain-zone bleeds without summing them as one vessel."""
    cur=max(0,int(current)); inc=max(0,int(incoming))
    if cur<=0:return inc
    if inc<=0:return cur
    high=max(cur,inc); low=min(cur,inc); cap=240
    if high>=cap:return high
    # Add only one third of the lesser source's remaining headroom contribution.
    gain=(low*(cap-high)+cap*3-1)//(cap*3)
    return min(cap,high+max(1,gain))
...
def _merge_current_wound(existing:Mapping[str,Any],incoming:Mapping[str,Any])->dict[str,Any]:
    ...
    Named structures accumulate exact repeated damage linearly. Coarse zone
    buckets accumulate with diminishing returns because the runtime does not
    know that two separate contacts struck the same tendon, nerve, bone or
    bleeding source. Permanent effects are floors and can never be erased by
    later recovery or by merging a new acute contact.
    """
    locus=_wound_locus(existing)
    if locus!=_wound_locus(incoming):
        raise ValueError('cannot merge wounds at different loci')
    exact_structure=locus[0]=='structure'
    ...
    channels=('cut','pierce','blunt','penetration','fracture','tendon_damage','nerve_damage','organ_trauma','structure_damage','pain')
    for key in channels:
        cur=max(0,int(out.get(key,0))); inc=max(0,int(new.get(key,0)))
        out[key]=min(200,cur+inc) if exact_structure else _saturating_trauma(cur,inc)
    ...
    old_bleed=max(0,int(existing.get('bleeding_ml_per_min',0))); new_bleed=max(0,int(new.get('bleeding_ml_per_min',0)))
    out['bleeding_ml_per_min']=old_bleed+new_bleed if exact_structure else _coarse_bleeding_merge(old_bleed,new_bleed)
    ...
    if structure is not None:
        ...
        functional={str(k):_saturating_trauma(int(old_fx.get(k,0)),int(new_fx.get(k,0)),cap=100) for k in set(old_fx)|set(new_fx)}
    ...
    out['function_loss_pct']=max(
        max([0]+[max(0,min(100,int(v))) for v in functional.values()]),
        _coarse_trauma_function_loss(out),
    )
...
def functional_penalties(wounds:Sequence[Mapping[str,Any]])->dict[str,int]:
    ...
    zones=_data()['zones']
    for w in wounds:
        z=zones.get(w.get('zone'),{}); fn=z.get('function'); loss=max(
            max(0,min(100,int(w.get('function_loss_pct',0)))),
            _coarse_trauma_function_loss(w),
        )
...
def functional_capacity_factors(wounds:Sequence[Mapping[str,Any]])->dict[str,int]:
    ...
    # Side-unknown wounds are a separate source of established impairment.
    # Aggregate penalty fields also mirror known left/right wounds, so deriving
    # the generic fallback from the aggregate can either double-count a known
    # unilateral injury or, conversely, make a catastrophic unknown-side wound
    # disappear as soon as any minor side-specific wound is present.  Recompute
    # only the truly unlocalized wounds for the generic channel.
    unknown_side_wounds=[
        wound for wound in wounds
        if isinstance(wound,Mapping) and wound.get('side') in (None,'')
    ]
    generic_p=functional_penalties(unknown_side_wounds) if unknown_side_wounds else {}
    generic_leg=max(0,min(100,int(generic_p.get('leg',0))))
    generic_foot=max(0,min(100,int(generic_p.get('footwork',0))))
    generic_upper=max(0,min(100,max(
        int(generic_p.get('arm',0)),int(generic_p.get('grip',0)),int(generic_p.get('weapon_control',0))
    )))
    ...
    generic_walk=(generic_leg*60+generic_foot*40)//100
    generic_run=(generic_leg*45+generic_foot*55)//100
    generic_stand=(generic_leg*80+generic_foot*20)//100
    generic_climb=(generic_leg*60+generic_foot*40)//100
    generic_ride=(generic_leg*70+generic_foot*30)//100
    generic_combat=(generic_leg*50+generic_foot*50)//100
    walking=max(0,1000-max(_paired_function_loss(walk_left,walk_right,unilateral_weight=82),generic_walk)*10)
    running=max(0,1000-max(_paired_function_loss(run_left,run_right,unilateral_weight=94),generic_run)*10)
    standing=max(0,1000-max(_paired_function_loss(stand_left,stand_right,unilateral_weight=76),generic_stand)*10)
    climbing_lower=max(0,1000-max(_paired_function_loss(climb_left,climb_right,unilateral_weight=88),generic_climb)*10)
    mounted_stability_lower=max(0,1000-max(_paired_function_loss(ride_left,ride_right,unilateral_weight=78),generic_ride)*10)
    combat_lower=max(0,1000-max(_paired_function_loss(combat_left,combat_right,unilateral_weight=92),generic_combat)*10)
    ...
    hand_loss=max(_paired_function_loss(lh,rh,unilateral_weight=72),generic_upper)
