...
from shinobi_runtime.martial_world.exact_combat import combat_default_targeting_intent, default_action_for, default_target_for, default_weapon_for_action, initialize_combat, resolve_exchange, attempt_disengage
...
def _resolve_player_combat_span(
    *, combat: Mapping[str, Any], people: Mapping[str, Mapping[str, Any]],
    ...
    while str(combat_cursor.get('status') or '')=='active' and not requested_scope_complete():
        ...
        if raw_action_kind in {'attack','auto'}:
...
class JianghuExtendedCommandsMixin:
    ...
    def _jianghu_security_resolution(self,command:CommandEnvelope,meta:Mapping[str,Any],current_time:CampaignTime):
        ...
        if action=='repair_breach':
            ...
            target_site=str(actor.get('location_ref') or '')
    ...
    def _jianghu_property_transfer_resolution(self,command:CommandEnvelope,meta:Mapping[str,Any],current_time:CampaignTime):
        ...
        if action=='claim_extinct_estate':
            ...
            scene=self.repository.read_json(self.scene_path)
            actor_location=str(actor.get('location_ref') or scene.get('location_id') or '')
    ...
    def _jianghu_custody_resolution(self,command:CommandEnvelope,meta:Mapping[str,Any],current_time:CampaignTime):
        ...
        if action=='restrain':
            ...
            if any(isinstance(r,Mapping) and r.get('person_ref')==person_ref and r.get('status') not in {'released','escaped','rescued'} for r in records):raise CommandRejectedError('jianghu_custody_already_active')
            ...
            candidates=[r for r in records if isinstance(r,Mapping) and r.get('person_ref')==person_ref and r.get('status') not in {'released','escaped','rescued'}]
            ...
            if action=='deliver_to_government':
                ...
                scene=self.repository.read_json(self.scene_path)
                if not self._person_present_for_command(scene,person_ref,current_time):
            ...
            if action in {'escape_attempt','rescue'}:
                ...
                if holder_faction_ref:
                    ...
                    for ref in set(scene.get('present_person_ids',[]))|set(scene.get('visible_person_ids',[])):
                        if not isinstance(ref,str) or ref in {person_ref,command.actor_id}:continue
                        try:
                            _gp,_gr,_go,guard=self._person(ref)
                        except CommandRejectedError:
                            continue
                        if str(guard.get('faction_ref') or '')==holder_faction_ref and is_living_and_conscious(guard):
    ...
    def _jianghu_combat_core_resolution(self,command:CommandEnvelope,meta:Mapping[str,Any],current_time:CampaignTime):
        ...
        if action=='start':
            ...
            for ref in refs:
                ...
                _p,_r,_o,person=self._person(str(ref)); people[str(ref)]=person; participant_routes[str(ref)]=(_p,_r,_o); locations.add(person.get('location_ref'))
