...
Deterministic runtime code is consequence authority, not a substitute mind. It may score capability, constraints, urgency, doctrine, risk, incentives and feasible envelopes, but it must not select a consequential NPC judgment that depends on human motive, loyalty, values, relationships, fear, office, strategy or personality. ChatGPT authors that intent from fresh lawful GM context; the owning Runtime mechanic validates feasibility, conservation, authority and consequences before persistence.

A practical classification test is whether two believable people with the same immediate physical capabilities and circumstances could reasonably choose differently for human reasons. If yes, the choice belongs to the LLM. Deterministic selectors may remain as explicitly non-authoritative benchmark/scoring helpers, never as the live consequential chooser. Routine already-authorized work, physiology, travel continuation, accounting, projectile motion, cooldowns and other non-discretionary causality remain deterministic.

For offscreen exact combat, an authored standing combat policy is the transaction-level proof that the meaningful engagement judgment already occurred. The resolver may execute bounded tactical substeps inside that delegation, but it must fail closed if the policy is absent and must not synthesize a new voluntary withdrawal, escalation, surrender, pursuit or objective change from deterministic scores.
