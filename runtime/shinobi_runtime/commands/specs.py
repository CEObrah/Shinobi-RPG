...
COMMAND_SPECS={
 ...
 "jianghu_interaction_resolution":CommandSpec(("action","target_ref"),("process_ref","player_statement","posture","topic","scopes"),"Record one player-authored social attempt without inventing the target's response; typed topic/scopes route ordinary conversation without prose keyword matching.",{}),
 "jianghu_scene_session_resolution":CommandSpec(("action","session_ref","kind","participant_refs","process_ref","purpose","agenda","close_reason","speaker_ref","statement","speech_kind","basis_refs","resolves_question_ref"),(),"Open/close a reversible live scene or persist attributed nonbinding speech. This command has no mechanical-consequence authority.",{},variants={
    ...
    "record_speech":CommandVariant(("action","session_ref","speaker_ref","statement","speech_kind"),("basis_refs","resolves_question_ref")),
 }),
 ...
 "jianghu_combat_resolution":CommandSpec(("action","combat_ref","side_a_refs","side_b_refs","objective","awareness_mode","initial_range_band","mounted_refs","action_kind","target_ref","weapon_ref","hit_zone","target_structure_ref","targeting_intent","poison_ref","qi_allocation_milli"),(),"Start or resolve anatomy-first exact Jianghu combat on authoritative local geometry and named anatomy. Each public gameplay exchange resolves exactly one exact-combat exchange. A standing player instruction is continued through separate committed exchanges so the caller must refresh current state between them and cannot skip a death, incapacitation, severe injury, surrender, or other material combat frontier inside one opaque transaction. Omitted target, action, weapon, anatomical aim, Qi, and poison are resolved from lawful perception, current geometry, Wei's personal doctrine, and active team doctrine. Combat Qi allocation uses only body, movement, and sensing flow; Qi temporarily improves the fighter's ordinary capabilities and never adds a separate weapon-damage channel. Every concrete player detail overrides only that detail.",{},variants={
    "start":CommandVariant(("action","combat_ref","side_a_refs","side_b_refs","objective","awareness_mode","initial_range_band"),("mounted_refs",)),
    "exchange":CommandVariant(("action","combat_ref"),("action_kind","target_ref","weapon_ref","hit_zone","target_structure_ref","targeting_intent","poison_ref","qi_allocation_milli")),
