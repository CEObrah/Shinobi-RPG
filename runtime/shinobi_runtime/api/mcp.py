...
def _preview_attestation(
    ...
    *,
    ...
    signature = hmac.new(
        oauth.preview_secret.encode("ascii"),
        payload,
        hashlib.sha256,
    ).digest()
    ...
    *,
    ...
    server = MCPServer(
        ...
        instructions=(
            ...
            "context after writes or stale revisions. OOC audit is read-only. Unsupported hard-state mutation fails closed; absence of a bespoke command never makes ordinary conversation or reversible fiction impossible. Never substitute chat memory, model canon recall, or prose for hard state."
        ),
        version="0.2.0",
    ...
    @server.tool(
        ...
        description=(
            ...
            "player summary, exact revision/time, permitted IDs, and compact mechanic families. The family catalog describes consequence resolvers, not the limits of fictional action."
        ),
