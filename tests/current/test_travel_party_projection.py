...
def test_player_route_movement_projects_exact_cotravelers_and_progress():
        ...
        "state/martial-world/route-operations.json": {
            "movements": {
                "escort_muster:test": {
                    ...
                    "route_index": 1,
                    "started_at": "0061-09-14T10:15:00",
                    "last_progress_at": "0061-09-27T21:15:00",
    ...
    assert result["segment_progress_milli"] == 500
    assert result["segment_elapsed_seconds"] == 500
    assert result["segment_required_seconds"] == 1000
    assert result["elapsed_scope"] == "current_segment"
    assert result["progress_scope"] == "current_segment"
    assert result["journey_started_at"] == "0061-09-14T10:15:00"
    assert result["segment_index"] == 1
    assert result["segment_origin_place_ref"] == "luoyang"
    assert result["segment_destination_place_ref"] == "changan"
