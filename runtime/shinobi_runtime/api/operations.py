...
class CampaignOperations:
        ...
        try:
            ...
            try:
                ...
                for row in c.get('records',[]) if isinstance(c,Mapping) else []:
                    if isinstance(row,Mapping) and row.get('status') not in {'released','escaped','rescued','executed'} and isinstance(row.get('person_ref'),str):unavailable.add(str(row['person_ref']))
            except FileNotFoundError:pass
            try:
    ...
    def _active_calendar_context(self, meta: Mapping[str,Any], scene: Mapping[str,Any], player_sheet: Mapping[str,Any]) -> list[dict[str,Any]]:
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                custody=self.repository.read_json('state/martial-world/custody.json')
                for row in custody.get('records',[]) if isinstance(custody,Mapping) else []:
                    if isinstance(row,Mapping) and row.get('status') not in {'released','escaped','rescued','executed'} and isinstance(row.get('person_ref'),str):
                        unavailable.add(str(row['person_ref']))
            except FileNotFoundError:
                pass
            try:
                combats=self.repository.read_json('state/martial-world/combats.json')
                for row in combats.get('combats',{}).values() if isinstance(combats,Mapping) and isinstance(combats.get('combats'),Mapping) else []:
                    if not isinstance(row,Mapping) or row.get('status')!='active': continue
                    sides=row.get('sides',{}) if isinstance(row.get('sides'),Mapping) else {}
                    for side in ('side_a','side_b'):
                        members=sides.get(side,[]) if isinstance(sides,Mapping) else []
    ...
    def play_context(self)->Mapping[str,Any]:
        ...
        result={
          ...
          'object_reads':{'supported_ref_prefixes':['faction:','inventory:','contract:','mission:','deployment:','project:','tournament:','market:','site:','relations','government'],'use':'Inspect one exact Jianghu owner when its current state matters.'},
