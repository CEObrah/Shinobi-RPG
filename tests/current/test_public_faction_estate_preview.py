...
def _clear_founder_obligations(root, *person_refs):
    refs=set(person_refs)
    deployments=json.loads((root/'state/martial-world/deployments.json').read_text())
    rows=deployments.get('deployments',{})
    if isinstance(rows,dict):
        for dep_ref,row in list(rows.items()):
            if not isinstance(row,dict): continue
            named=set()
            for key in ('participant_refs','member_refs','chooser_refs'):
                values=row.get(key,[])
                if isinstance(values,list): named.update(str(x) for x in values if isinstance(x,str))
            for key in ('leader_ref','commander_ref','deputy_ref'):
                value=row.get(key)
                if isinstance(value,str): named.add(value)
            if refs & named: rows.pop(dep_ref,None)
    _write(root,'state/martial-world/deployments.json',deployments)
    contracts=json.loads((root/'state/martial-world/contracts/index.json').read_text())
    active=contracts.get('active',{})
    if isinstance(active,dict):
        for contract_ref,row in list(active.items()):
            participants=set(row.get('participants',[])) if isinstance(row,dict) and isinstance(row.get('participants'),list) else set()
            if refs & participants: active.pop(contract_ref,None)
    _write(root,'state/martial-world/contracts/index.json',contracts)
...
def test_public_found_dynamic_faction_preview_validates_new_closed_owners(tmp_path):
    ...
    remove_people_from_route_fixture(root, player, 'char.kai')
    remove_people_from_active_combat_fixture(root, player, 'char.kai')
    _clear_founder_obligations(root, player, 'char.kai')
    ...
    # Trust is not teleportation or presence. A willing cofounder at another
    # exact site in the same settlement cannot be silently moved into the
    # founding ceremony.
    other_site=next(ref for ref,row in sorted(sites.items()) if ref!=site_ref and row.get('parent_place_ref')==place_ref and row.get('public_access','public')=='public')
    independent_state=json.loads((root/'state/martial-world/independent-people.json').read_text())
    kai=next(row for row in independent_state['people'] if row.get('person_id')=='char.kai')
    kai['location_ref']=other_site
    _write(root,'state/martial-world/independent-people.json',independent_state)
    with pytest.raises(CommandRejectedError) as exc:
        RepositoryCommandPlanner(RepositoryStore(root)).preview(command)
    assert exc.value.code=='jianghu_faction_founders_not_colocated'
    kai['location_ref']=site_ref
    _write(root,'state/martial-world/independent-people.json',independent_state)
    repo=RepositoryStore(root); planner=RepositoryCommandPlanner(repo)
...
def test_public_extinct_estate_claim_preview_validates_controlled_facilities(tmp_path):
    ...
    meta=json.loads((root/'state/meta.json').read_text()); player=meta['player_id']
    remove_people_from_route_fixture(root, player)
    remove_people_from_active_combat_fixture(root, player)
    ...
    current_at=meta['time'].removeprefix('SE-')
    assert adopted['last_progress_at']==current_at
    ...
    expected_due=(datetime.fromisoformat(current_at)+timedelta(days=1)).isoformat()
    assert schedule_after['one_off'][f'autonomous_project_due:{physical_ref}']['due_at']==expected_due
