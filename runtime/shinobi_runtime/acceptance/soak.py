...
def _scheduler_metrics(root: Path) -> Dict[str, int]:
    ...
    return {
        "host_count": int(data.get("host_count", 0)) if isinstance(data, Mapping) else 0,
        "pending_event_count": int(data.get("pending_event_count", 0)) if isinstance(data, Mapping) else 0,
