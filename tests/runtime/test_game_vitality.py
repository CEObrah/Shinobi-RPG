...
class _TeamHarness(team_vitality.LivingWorldTeamVitalityMixin):
    ...
    def _append_internal_event(self, *args, **kwargs):
        ...
        refs = kwargs["material_consequence_refs"]
        assert refs[0] == "player_led_team_checkin:team.blackhound:person.hayama"
        assert "team_checkin_label:Black%20Hound" in refs
        assert "team_checkin_topic:containment" in refs
        assert "team_checkin_topic:extraction" in refs
        assert any(ref.startswith("team_checkin_topic:readiness%2C%20equipment") for ref in refs)
