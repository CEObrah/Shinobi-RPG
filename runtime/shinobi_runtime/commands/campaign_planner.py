...
class CampaignCommandPlanner(RepositoryCommandPlanner):
    ...
    def _combat_build_with_staged_records(
        self, command, staged_records: Mapping[str, Mapping[str, Any]],
    ):
        """Build through the canonical combat reducer over a read overlay.

        Route-contact compatibility records are staged *before* normal command
        validation/resolution, but they never become a second combat resolver.
        The base repository planner still owns the semantic command and its
        development post-processing.
        """
        if not staged_records:
            return super()._build(command)
        ...
        return base_planner._build(command)

    def _build(self, command):
        if str(getattr(command, "command_type", "")) != "jianghu_combat_resolution":
            return super()._build(command)
        ...
        plan = self._combat_build_with_staged_records(command, staged)
        if staged:
            untouched = {
                path: record
