...
class RepositoryOocAudit:
    ...
    def _audit_scheduler(
        ...
        raw = self._read_repository_json(
            report, "state/time/causal-scheduler.json", "causal_scheduler"
        )
        if raw is None:
            return {}
        try:
            scheduler = CausalSchedulerRegistry.from_record(raw)
        ...
        if world_time is not None:
            relation = (
                "current" if scheduler.world_time == world_time
                else "stale" if scheduler.world_time < world_time
                else "future"
            )
        events = scheduler.queue.snapshot()
        overdue = sum(1 for event in events if event.due_at <= scheduler.world_time)
        legacy_hosts = sum(
            1
            for host in scheduler.hosts.values()
            if host.authority_kind == "scheduled_process"
            or host.state.host_id.startswith("host.process.")
        )
        earliest = min((event.due_at for event in events), default=None)
        over_budget = len(scheduler.hosts) > self.max_scheduler_hosts
        report.diagnostic(
            "causal_scheduler:summary "
            f"world_time={scheduler.world_time} relation={relation} "
            f"hosts={len(scheduler.hosts)} events={len(events)} overdue={overdue} "
            f"unexpected_process_hosts={legacy_hosts} "
            f"earliest_due={earliest if earliest is not None else 'none'} "
            f"budget_exceeded={str(over_budget).lower()}"
        )
        if relation != "current" or overdue or legacy_hosts:
            report.suggestion("reconcile_causal_scheduler_before_gameplay")
        if over_budget:
            report.suggestion("review_causal_scheduler_host_locality_before_raising_host_budget")
        return scheduler.hosts

    def _audit_pressures(
        self,
        report: _Report,
        world_time: Optional[CampaignTime],
        scheduler_hosts: Mapping[str, Any],
    ) -> None:
        ...
        for pressure_id, raw_pressure in pressures.items():
            ...
            host = scheduler_hosts.get(host_ref) if isinstance(host_ref, str) else None
            if host is None:
    ...
    def __call__(
        ...
        scheduler_hosts = self._audit_scheduler(report, meta_time)
        self._audit_pressures(report, meta_time, scheduler_hosts)
