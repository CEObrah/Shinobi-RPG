...
training_base = json.loads((ROOT / "data/organization/training-records/train.house_tang.core.json").read_text())
...
for name in TRAINING:
    data = json.loads((ROOT / "data/organization/training-records" / name).read_text())
    if data["profile"].get("inherits") != "train.house_tang.core":
        errors.append(f"{name}: must inherit train.house_tang.core")

doctrine_base = json.loads((ROOT / "data/organization/doctrine-records/doc.house_tang.invisible_court.json").read_text())
...
if base.get("institution_overlay_ref") != "data/organization/institution-doctrine-overlays.json#house_tang":
...
for name in DOCTRINES:
    data = json.loads((ROOT / "data/organization/doctrine-records" / name).read_text())
    ...
    if doctrine.get("institution_overlay_ref") != "data/organization/institution-doctrine-overlays.json#house_tang":
