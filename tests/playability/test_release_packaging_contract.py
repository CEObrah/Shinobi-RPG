...
def test_release_verifier_requires_failure_discovery_gates():
    verifier = (ROOT / "tools/verify_release.sh").read_text(encoding="utf-8")
    playability_command = "python tools/playability_lab.py --no-mutations"
    playability = verifier.find(playability_command)
    mutation = verifier.find("python tools/mutation_audit.py")
    changed = verifier.find("python tools/test_changed.py")
    assert playability >= 0
    assert verifier.count(playability_command) == 1
    assert mutation > playability
