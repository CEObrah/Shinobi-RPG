...
from shinobi_runtime.martial_world.exact_combat import capability_from_person, combat_npc_judgment_envelopes
...
from shinobi_runtime.martial_world.scene_sessions import (
    active_scene_session, active_scene_thread_page, inspect_history_object, interaction_ledger, recent_scene_history, relevant_scene_continuity, session_projection,
)
...
def _player_equipment_projection(equipment_ledger: Mapping[str, Any], player_id: str) -> dict[str, Any]:
    """Bounded own-loadout projection for player-facing combat context."""
    try:
        loadout = effective_person_loadout(equipment_ledger, player_id)
    except (KeyError, TypeError, ValueError):
        return {"items": {}, "condition_milli": {}}
    if not isinstance(loadout, Mapping):
        return {"items": {}, "condition_milli": {}}
    return {
        "items": copy.deepcopy(loadout.get("items", {})) if isinstance(loadout.get("items"), Mapping) else {},
        "condition_milli": copy.deepcopy(loadout.get("condition_milli", {})) if isinstance(loadout.get("condition_milli"), Mapping) else {},
    }
...
def _project_active_session_presence(
    ...
    *,
    ...
    still co-located with Wei as live participants.  If everyone else has left,
    a continuity-only lifecycle projection remains so the LLM can close or
    transition the formal session without treating absent people as present.
    The durable session remains untouched; exact speech/fact commands
    independently revalidate co-location before writes.
    """
    ...
    for ref in original:
        ...
        if not isinstance(other, Mapping):
            raise ValueError(f"active scene participant does not resolve to an exact person: {ref}")
        colocated=same_effective_location(
    ...
    if len(present)<2:
        ...
        projected['lifecycle_reconciliation_reason']='formal_session_has_no_other_physically_present_participant'
    return projected
...
def _player_visible_identity_card(
    sheet: Mapping[str, Any], *, person_id: str, knows_identity: bool, knows_faction: bool,
    knows_office: bool, visible: bool, exact_presence: Mapping[str, Any], appearance: Mapping[str, Any],
) -> dict[str, Any]:
    """Project a non-player person as observation/identity, never a stat sheet."""
    card: dict[str, Any] = {"person_id": person_id, "appearance_profile": dict(appearance)}
    if visible:
        card["location_ref"] = exact_presence.get("location_ref")
        card["physical_presence"] = {
            key: exact_presence.get(key)
            for key in ("location_ref", "presence_kind", "owner_ref", "available_for_site_activity")
        }
    if knows_identity:
        for key in ("name", "sex", "birth_year"):
            value=sheet.get(key)
            if value not in (None, "", [], {}): card[key]=value
    if knows_faction and sheet.get("faction_ref") not in (None, ""):
        card["faction_ref"]=sheet.get("faction_ref")
    if knows_office:
        for key in ("membership_grade", "standing_offices", "social_rank"):
            value=sheet.get(key)
            if value not in (None, "", [], {}): card[key]=copy.deepcopy(value)
    return card
...
class CampaignOperations:
    ...
    def _gm_private_scene_director_context(
        ...
        return {
            ...
            "performance_cues_rule": (
                "Use exact role, lawful social/relationship evidence, observed scene history, current goals/condition, open human threads, practical responsibility and audience as qualitative directing evidence. "
                "A missing secret personality profile is not a reason to make an established person mute or generic. Infer only reversible moment-to-moment performance; never invent a hidden durable personality fact or quote numeric relationship/mechanical state as characterization."
            ),
            "relationship_edges": edges,
            "mechanical_consequence_authority": False,
            "director_rule": (
                "Use this backstage truth to actively direct the people already in the scene, including before a formal conversation session exists. Present NPCs do not wait for Wei to activate them: on continue or after Wei acts, the LLM may select whoever has the strongest grounded reason to speak, react, interrupt, joke, disagree, lie, omit, coordinate, leave, or address another NPC. "
                ...
                "Do not expose hidden motives, numeric private state, or undisclosed knowledge as Wei's knowledge; render only perception, lawful inference, or what an NPC actually discloses. Hard consequences still require their mechanical authority."
            ),
        }
    ...
    def play_context(self)->Mapping[str,Any]:
        try:
            with self._locked():
                ...
                meta=self.repository.read_json('state/meta.json'); scene=self.repository.read_json('state/scene.json'); player_sheet=self.sheet_resolver(str(meta.get('player_id') or '')); contract_index=self.repository.read_json('state/martial-world/contracts/index.json'); institutional_operations=self.repository.read_json('state/martial-world/institutional-operations.json'); equipment_ledger=self.repository.read_json('state/martial-world/equipment-ledger.json')
        ...
        # Physical visibility and identity recognition are separate player-safe
        # facts.  Publish an explicit bounded recognition set so the GM writer
        # can fail closed for hidden opponents without also erasing an identity
        # Wei has genuinely recognized from familiarity/appearance/equipment.
        identity_known_refs=[player_id]
        try:
            social=self.repository.read_json('state/martial-world/social.json')
            relationships=social.get('relationships',{}) if isinstance(social,Mapping) else {}
            equipment_catalog=self.repository.read_json('game/data/martial-world/equipment.json')
        except (FileNotFoundError,KeyError,TypeError,ValueError):
            relationships={}; equipment_catalog={}
        for ref in list(scene_view.get('visible_person_ids',[]))[:24]:
            if not isinstance(ref,str) or ref==player_id:
                continue
            try:
                target=self.sheet_resolver(ref)
            except (FileNotFoundError,KeyError,TypeError,ValueError):
                target=None
            if not isinstance(target,Mapping):
                continue
            same_faction=bool(target.get('faction_ref') and target.get('faction_ref')==player_sheet.get('faction_ref'))
            edge=relationships.get(f'{player_id}|{ref}',{}) if isinstance(relationships,Mapping) else {}
            familiarity=max(0,int(edge.get('familiarity',0))) if isinstance(edge,Mapping) else 0
            recognized=same_faction or familiarity>=35
            if not recognized and isinstance(equipment_catalog,Mapping):
                try:
                    loadout=effective_person_loadout(equipment_ledger,ref)
                    assessment=recognition_assessment(
                        observer=player_sheet,target=target,target_items=loadout.get('items',{}),
                        equipment_catalog=equipment_catalog,familiarity=familiarity,
                    )
                    recognized=bool(isinstance(assessment,Mapping) and assessment.get('recognized'))
                except (KeyError,TypeError,ValueError):
                    recognized=False
            if recognized:
                identity_known_refs.append(ref)
        scene_view['identity_known_person_ids']=identity_known_refs
        if isinstance(active_session,Mapping):
            # _project_active_session_presence has already resolved and physically
            # revalidated every surviving participant.  Do not perform a second
            # lossy read that can turn owner/presence corruption into absence.
            session_present=[
                ref for ref in active_session.get('participant_refs',[])
                if isinstance(ref,str) and ref
            ]
        ...
        if live_combat is not None:
            combat_ref, combat_owner = live_combat
            try:
                combat_people: dict[str, Mapping[str, Any]] = {}
                refs = [
                    str(ref) for members in combat_owner.get('sides',{}).values() if isinstance(members,list)
                    for ref in members if isinstance(ref,str)
                ] if isinstance(combat_owner,Mapping) and isinstance(combat_owner.get('sides'),Mapping) else []
                combat_doctrines: dict[str, Mapping[str, Any]] = {}
                for ref in refs:
                    person = self.sheet_resolver(ref)
                    if not isinstance(person,Mapping):
                        continue
                    combat_people[ref] = person
                    faction_ref = str(person.get('faction_ref') or '')
                    if faction_ref and faction_ref not in combat_doctrines:
                        try:
                            _fpath, faction = read_faction(self.repository, faction_ref)
                            combat_doctrines[faction_ref] = faction.get('doctrine',{}) if isinstance(faction,Mapping) else {}
                        except (FileNotFoundError,KeyError,TypeError,ValueError):
                            combat_doctrines[faction_ref] = {}
                judgments = combat_npc_judgment_envelopes(
                    combat=combat_owner,people=combat_people,equipment_ledger=equipment_ledger,
                    doctrines=combat_doctrines,player_ref=player_id,martial_familiarity=social,
                )
                if not isinstance(director_context,dict):
                    director_context = {}
                director_context['combat'] = {
                    'privacy':'gm_private_exact_combat_direction_not_player_knowledge',
                    'combat_ref':str(combat_ref),
                    'status':str(combat_owner.get('status') or ''),
                    'elapsed_ms':max(0,int(combat_owner.get('elapsed_ms',0) or 0)),
                    'npc_judgment_envelopes':judgments,
                    'npc_judgment_contract':{
                        'meaningful_choice_owner':'chatgpt',
                        'runtime_role':'facts_constraints_feasibility_validation_and_consequence_only',
                        'freshness':'one_exact_decision_frontier',
                        'rule':'Author one intent for every envelope before the next consequential combat resolution. Do not expose hidden actor identity or intent as Wei knowledge.',
                    },
                }
            except (FileNotFoundError,KeyError,TypeError,ValueError):
                # A malformed private direction packet must not fabricate NPC
                # intent. The live combat command itself fails closed without
                # a complete authored judgment mapping.
                pass
        if director_context:
        ...
        # Equipment condition materially changes combat but used to be hidden
        # behind the base-stat projection. Keep this bounded to the player's own
        # logical loadout so the writer/player can explain degraded performance
        # without exposing unrelated ledger state.
        player['equipment_loadout']=_player_equipment_projection(equipment_ledger,player_id)
        ...
        recent_interaction_rows = [
            row for row in attempt_state.get('attempts', [])
            if isinstance(row, Mapping) and (row.get('player_statement') or row.get('action'))
        ]
        recent_interaction_rows = sorted(
            recent_interaction_rows, key=lambda row: str(row.get('at') or '')
        )[-8:]
        recent_interaction_attempts = [
            {
                k: row.get(k)
                for k in (
                    'attempt_ref','at','action','target_ref','process_ref','player_statement','posture','topic',
                    'scopes','scene_session_ref','thread_status','resolved_at','response_ref'
                )
                if row.get(k) not in (None,'',[])
            }
            for row in recent_interaction_rows
        ]
        continuity_subjects = [player_id]
        for ref in scene_view.get('present_person_ids', []):
            if isinstance(ref, str) and ref not in continuity_subjects:
                continuity_subjects.append(ref)
        scene_literary_continuity = relevant_scene_continuity(
            self.repository.read_json, subject_refs=continuity_subjects,
            location_ref=str(player_presence.get('location_ref') or ''), limit=16,
        )
        result={
          ...
          'literary_continuity':scene_literary_continuity,
          'recent_interaction_attempts':recent_interaction_attempts,
    ...
    def person_sheet(self,person_id:str)->Mapping[str,Any]:
        ...
        try:
            ...
            social_titles=[]
            knows_identity=bool(person_id==player_id or same_faction)
            knows_faction=bool(same_faction)
            knows_office=bool(same_faction)
        if person_id!=player_id:
            safe=_player_visible_identity_card(
                sheet,person_id=person_id,knows_identity=knows_identity,knows_faction=knows_faction,
                knows_office=knows_office,visible=visible,exact_presence=exact_presence,
                appearance=safe.get('appearance_profile',{}),
            )
