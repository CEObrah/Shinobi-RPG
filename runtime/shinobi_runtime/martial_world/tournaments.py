...
def event_profile(tournament_kind:str)->dict[str,Any]:
    ...
    return {
        ...
        'ring_radius_mm': max(2000,int(raw.get('ring_radius_mm',6000))),
    }




def _resolved_tournament_kind(tournament: Mapping[str,Any]) -> str:
    """Return the registered event profile owned by the current tournament row."""
    profiles=_data().get('event_profiles',{})
    explicit=str(tournament.get('tournament_kind') or '')
    if isinstance(profiles,Mapping) and explicit in profiles:
        return explicit
    raise ValueError('tournament_kind missing or invalid')
...
def tournament_person_eligible(tournament_kind:str, person:Mapping[str,Any], *, year:int)->bool:
    """Hard entrant eligibility shared by local and traveling registration."""
    health=person.get('health',{}) if isinstance(person.get('health'),Mapping) else {}
    if health.get('status') in {'dead','incapacitated'} or int(health.get('consciousness',100))<=0 or bool(person.get('retired_from_field',False)):
        return False
    age=max(0,int(year)-int(person.get('birth_year',year)))
    if tournament_kind=='midyear_junior_tournament':
        return age>=14 and str(person.get('membership_grade') or '')=='junior'
    return age>=16

def open_tournament(*,event_id:str,format_ref:str,organizer_ref:str,great:bool=False,tournament_kind:str|None=None)->dict[str,Any]:
    """Open a self-funded exact-combat tournament with empty prize escrow."""
    if format_ref!='individual': raise KeyError(format_ref)
    kind=str(tournament_kind or ('great_jianghu_tournament' if great else 'regional_martial_tournament'))
    profile=event_profile(kind)
    return {
        'event_id':event_id,'tournament_kind':kind,'format':format_ref,'organizer_ref':organizer_ref,
...
def begin_next_match(tournament:Mapping[str,Any], *, usable_refs:set[str]|None=None)->dict[str,Any]:
    ...
    while True:
        ...
        if b is None:
            # A bye is not a resurrection mechanism.  Normal bracket-only
            # callers may omit ``usable_refs`` and retain pure bracket
            # behavior, while live competition supplies current exact-person
            # usability so a dead/missing/unavailable entrant cannot advance
            # without appearing.
            if usable_refs is None or str(a) in usable_refs:
...
def record_match_forfeit(tournament:Mapping[str,Any],*,winner_ref:str|None)->dict[str,Any]:
    """Close one active bracket slot without manufacturing a fought loss.

    One usable entrant may advance when the opponent cannot appear.  If neither
    entrant is usable, nobody advances.  A forfeit loser is not inserted into
    the championship losers bracket and cannot earn runner-up/fourth merely by
    being absent.  Match-performance points and sportsmanship evidence are
    intentionally handled only by actually fought matches.
    """
    out={**tournament}
    pair=out.get('active_pair')
    if not isinstance(pair,(list,tuple)) or len(pair)!=2:
        raise ValueError('tournament forfeit without active pair')
    refs=[str(pair[0]),str(pair[1])]
    if winner_ref is not None and str(winner_ref) not in refs:
        raise ValueError('tournament forfeit winner not in active pair')
...
def advance_individual_competition(
    ...
    max_exchanges:int=240, max_matches:int|None=None, social_state:Mapping[str,Any]|None=None,
)->dict[str,Any]:
    ...
    def result_payload(*,waiting:bool,completed:bool,champion:Any=None,combat_ref:Any=None,continuation:bool=False)->dict[str,Any]:
        return {
            ...
            'combats_state_after':combats,'social_state_after':social,'waiting_for_player':waiting,'completed':completed,
    ...
    while True:
        usable_refs={ref for ref,person in persons.items() if _combatant_usable(person)}
        nxt=begin_next_match(out,usable_refs=usable_refs); out=nxt['tournament_after']
        if nxt['completed']:
        ...
        if not usable_a or not usable_b:
            winner=b if usable_b and not usable_a else a if usable_a and not usable_b else None
            out=record_match_forfeit(out,winner_ref=winner)
            # A bracket forfeit consumes competition throughput, but it is not
            # a fought match and therefore earns no match-performance point or
            # sportsmanship/rivalry evidence.
            match_count+=1
            if session_exhausted():
                out, completed_now = _settle_bracket_frontier(out)
                if completed_now:
        ...
        if player_ref and player_ref in {a,b}:
            ...
            if combat_ref not in combat_map:
                combat_map[combat_ref]=initialize_combat(
                    ...
                    zone_ref=zone_ref,started_at=at_iso,objective={'kind':'tournament_match','tournament_ref':out.get('tournament_ref') or out.get('event_id'),'phase':phase,'ring_out_enabled':True,'ring_radius_mm':event_profile(_resolved_tournament_kind(out))['ring_radius_mm']},
        ...
        result=simulate_exact_combat(
            ...
            objective={'kind':'tournament_match','tournament_ref':out.get('tournament_ref') or out.get('event_id'),'phase':phase,'ring_out_enabled':True,'ring_radius_mm':event_profile(_resolved_tournament_kind(out))['ring_radius_mm']},
            targeting_intent='disable',max_exchanges=max_exchanges,social_state=social,
            authored_combat_policy={
                'decision_origin':'tournament_entry',
                'engagement_intent':'compete_under_tournament_rules',
        ...
        social=copy.deepcopy(dict(result.get('social_state_after',social)))
