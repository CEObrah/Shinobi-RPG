...
def _retinue_assignment_variants(
    leader: Mapping[str, Any], people: Sequence[Mapping[str, Any]], *, year: int,
    unavailable_refs: Sequence[str], requested_count: int,
) -> list[dict[str, Any]]:
    """Return a small bounded set of feasible teams without selecting one."""
    variants: list[dict[str, Any]] = []
    seen: set[tuple[tuple[str, str], ...]] = set()

    def add(excluded: Sequence[str]) -> None:
        try:
            refs, roles = select_retinue_members(
                leader, people, year=year,
                unavailable_refs=[*unavailable_refs, *excluded],
                target_count=requested_count,
            )
        except ValueError:
            return
        if len(refs) != requested_count:
            return
        key = tuple(sorted((str(ref), str(roles.get(ref) or "")) for ref in refs))
        if key in seen:
            return
        seen.add(key)
        variants.append({
            ...
            "member_roles": {str(ref): str(roles[ref]) for ref in refs},
        })

    add(())
    if variants:
        # Produce bounded alternatives by removing one member of the strongest
        # scored composition at a time. Python supplies feasible/ranked
        # envelopes; it does not pick which human staffing judgment is used.
        for ref in list(variants[0]["member_refs"]):
            add((str(ref),))
    return variants[:6]
...
def _settle_retinue_assignments(
    *, read_json: Callable[[str], Mapping[str, Any]], writes: dict[str, Any],
    ...
    committed_events = [
        ...
        and row.get("judgment_kind") == "retinue_assignment_policy"
    ]
    if not review_events and not committed_events:
        return []
    ...
    def current_context(retinue_ref: str) -> tuple[dict[str, Any], Mapping[str, Any], list[Mapping[str, Any]], list[str], int, list[str]] | None:
        ...
        try:
            ...
            return None
        people = [row for row in (roster.get("people", []) if isinstance(roster.get("people"), list) else []) if isinstance(row, Mapping)]
        requested_count = max(1, int(current.get("requested_count", 3)))
        unavailable = [*base_unavailable, *sorted(reserved_elsewhere), *chooser_refs]
        return copy.deepcopy(dict(current)), leader, people, unavailable, requested_count, chooser_refs

    def apply_assignment(retinue_ref: str, current: Mapping[str, Any], choice: Mapping[str, Any]) -> dict[str, Any]:
        member_refs = [str(ref) for ref in choice.get("member_refs", []) if isinstance(ref, str)]
        roles_raw = choice.get("member_roles")
        roles = {str(ref): str(role) for ref, role in roles_raw.items()} if isinstance(roles_raw, Mapping) else {}
        ...
        notice = {
            ...
            "chooser_refs": list(assigned.get("chooser_refs", [])),
            "leader_ref": str(assigned.get("leader_ref") or ""),
    ...
    # First consume already-authored same-timestamp staffing judgments.
    for event in sorted(committed_events, key=lambda row: str(row.get("event_id") or "")):
        context = event.get("decision_context")
        choice = event.get("selected_option")
        if not isinstance(context, Mapping) or not isinstance(choice, Mapping):
            raise ValueError("retinue assignment judgment invalid")
        retinue_ref = str(context.get("retinue_ref") or event.get("owner_ref") or "")
        ctx = current_context(retinue_ref)
        if ctx is None:
            continue
        current, leader, people, unavailable, requested_count, chooser_refs = ctx
        intent = str(choice.get("intent") or "")
        if intent == "defer":
            schedule_raw = writes.get(_SCHEDULER_PATH)
            schedule = copy.deepcopy(dict(schedule_raw)) if isinstance(schedule_raw, Mapping) else _read_or(read_json, _SCHEDULER_PATH, {"schema":"jianghu-scheduler-1.0","one_off":{}})
            schedule = upsert_one_off_event(schedule, {
                "event_id": f"retinue_assignment_review:{retinue_ref}",
                "kind": "retinue_assignment_review", "owner_ref": retinue_ref,
                "retinue_ref": retinue_ref, "chooser_refs": chooser_refs,
                "due_at": (at + timedelta(hours=12)).isoformat(),
                "requires_player_decision": False,
            })
            writes[_SCHEDULER_PATH] = schedule
            continue
        variants = _retinue_assignment_variants(
            leader, people, year=at.year, unavailable_refs=unavailable,
            requested_count=requested_count,
        )
        selected = {
            "intent": "assign",
            "member_refs": [str(ref) for ref in choice.get("member_refs", []) if isinstance(ref, str)],
            "member_roles": {str(ref): str(role) for ref, role in (choice.get("member_roles", {}).items() if isinstance(choice.get("member_roles"), Mapping) else [])},
        }
        if selected not in variants:
            raise ValueError("retinue assignment choice no longer feasible")
        settled.append(apply_assignment(retinue_ref, current, selected))

    # A scheduled review computes feasibility and bounded alternatives only.
    for event in sorted(review_events, key=lambda row: str(row.get("event_id") or "")):
        ...
        if not variants:
            ...
            notice = {
                ...
                "retinue_ref": retinue_ref, "chooser_refs": list(chooser_refs),
                "leader_ref": str(current.get("leader_ref") or ""),
        ...
        judgment = build_npc_judgment_event(
            judgment_kind="retinue_assignment_policy", at_iso=at.isoformat(),
            actor_ref=faction_ref or str(chooser_refs[0] if chooser_refs else current.get("leader_ref") or ""),
            owner_ref=retinue_ref,
            options=[*variants, {"intent": "defer"}],
            decision_context={
                "retinue_ref": retinue_ref, "leader_ref": str(current.get("leader_ref") or ""),
                "chooser_refs": list(chooser_refs), "requested_count": requested_count,
        ...
        schedule_raw = writes.get(_SCHEDULER_PATH)
        schedule = copy.deepcopy(dict(schedule_raw)) if isinstance(schedule_raw, Mapping) else _read_or(read_json, _SCHEDULER_PATH, {"schema":"jianghu-scheduler-1.0","one_off":{}})
        writes[_SCHEDULER_PATH] = upsert_one_off_event(schedule, judgment)
        notice = {
            "kind": "retinue_assignment_judgment_due",
            "event_id": judgment["event_id"], "retinue_ref": retinue_ref,
            "leader_ref": str(current.get("leader_ref") or ""), "chooser_refs": list(chooser_refs),
            "decision_ref": judgment["decision_ref"], "candidate_variant_count": len(variants),
            "delivered_to_player": False, "requires_player_decision": False,
        }
        handoffs.append({**notice, "handoff": classify_handoff(notice)})
...
def augment_frontier_with_progression(
    *, read_json: Callable[[str], Mapping[str, Any]], frontier: Mapping[str, Any], at: datetime,
    ...
    mission_settlements = settle_closed_mission_records(
        ...
        pending_one_off_events=offer_delivery_events, events=events,
    )
