...
from shinobi_runtime.commands.paths import INFORMATION_REGISTRY_PATH
...
def _deliver_player_front_report(owner: Any, *, result: Mapping[str, Any], decision: AutonomousDecision, front_ref: str, assignment: Mapping[str, Any], at: CampaignTime, command: Any, world_events: Dict[str, Any], record_writes: Dict[str, Dict[str, Any]]) -> list[Mapping[str, Any]]:
    ...
    if not isinstance(claim_id, str):
        return []
    registry = record_writes.get(INFORMATION_REGISTRY_PATH)
    if registry is None:
        try:
            loaded = owner.repository.read_json(INFORMATION_REGISTRY_PATH)
        ...
        registry = copy.deepcopy(dict(loaded))
        record_writes[INFORMATION_REGISTRY_PATH] = registry
    claims = registry.get("claims")
    knowledge = registry.get("knowledge")
    deliveries = registry.get("deliveries")
    if not isinstance(claims, dict) or not isinstance(knowledge, dict) or not isinstance(deliveries, list):
        raise CommandRejectedError("information_registry_invalid")
    ...
    sender_known = knowledge.get(sender_ref)
    ...
    existing = next((row for row in deliveries if isinstance(row, Mapping) and row.get("delivery_id") == delivery_id), None)
    if existing is None:
        try:
    ...
    recipient_known = knowledge.setdefault(recipient_ref, [])
    if not isinstance(recipient_known, list):
        raise CommandRejectedError("information_registry_invalid")
    ...
    event_id = owner._append_internal_event(
        ...
        affected_owner_refs=(INFORMATION_REGISTRY_PATH,),
