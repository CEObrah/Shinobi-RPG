...
Development history is not campaign state. This campaign lineage was explicitly **rebaselined at revision 1 on 2026-09-02** from a repaired pre-rebaseline snapshot, and that prior state is preserved only under `docs/forensics/` for failure analysis. The live/package save is allowed to advance normally after that baseline; revision 1 identifies the lineage origin, not a permanent current-revision invariant. Gameplay idempotency receipts and WAL state remain runtime-private.
...
- Campaign revision and world time are read from authoritative `state/meta.json`; documentation never hard-codes the mutable live revision. The lineage baseline is revision 1, and the pre-rebaseline snapshot is forensic only.
- Player: Tang Wei.
...
This repository package is source plus the current campaign state. Git push, Railway deployment, MCP schema refresh and ChatGPT Skill installation are separate operations. Do not reset persistent campaign/recovery volumes during an ordinary source deployment.

## Final audit status

The packaged campaign save is whatever authoritative revision and world time are present in `state/meta.json`. It descends from the revision-1 rebaseline recorded in `docs/CAMPAIGN_REBASELINE_20260902.json`. Source and gameplay durability both use the single canonical Git branch `main`. Normal deployments preserve the active campaign's matching private WAL/receipt lineage. The pre-rebaseline snapshot remains available under `docs/forensics/` strictly for forensics and regression reconstruction.
