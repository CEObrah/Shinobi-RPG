...
class RouteAwareCampaignOperations(CampaignOperations):
    ...
    def _player_exact_team_refs(self, player_id: str) -> tuple[str, ...]:
        ...
        if not isinstance(team_ids, list) or len(team_ids) > _MAX_PLAYER_TEAM_REFS:
            raise OperationError(503, "object_access_policy_invalid")
        result: list[str] = []
        for team_ref in team_ids:
            ...
            if (
                ...
                and all(isinstance(ref, str) for ref in members)
                and player_id in members
            ):
                result.append(team_ref)
    ...
    def _project_play_context(
        ...
        object_reads["exact_team_ref_count"] = len(refs)
