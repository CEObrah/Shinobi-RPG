...
def _world_event_execution_has_durable_provenance(execution: Mapping[str, Any]) -> bool:
    """Accept normal transaction provenance or an explicit campaign-rebaseline anchor."""
    baseline_ref = execution.get("baseline_ref")
    has_baseline = isinstance(baseline_ref, str) and bool(baseline_ref)
    receipt_refs = execution.get("receipt_refs")
    has_transaction = (
        isinstance(execution.get("transaction_ref"), str)
        and bool(execution.get("transaction_ref"))
        and isinstance(receipt_refs, list)
        and bool(receipt_refs)
        and all(isinstance(value, str) and bool(value) for value in receipt_refs)
    )
    return has_baseline or has_transaction
...
class MissionCommandsMixin:
    ...
    def _mission_creation(
        ...
        candidate_team_refs: set[str] = set()
        try:
            for participant_ref in participant_refs:
                candidate_team_refs.update(team_refs_for_member(self.repository, participant_ref))
        ...
        authority_decision = self._domain_authority(cache=owner_cache).mission_tasking(
            ...
            candidate_team_refs=sorted(candidate_team_refs),
        )
    ...
    def _mission_objective_evidence(
        ...
        *,
        ...
        if (
            ...
            or not _world_event_execution_has_durable_provenance(execution)
        ):
            raise CommandRejectedError("mission_objective_evidence_invalid")
    ...
    @staticmethod
    def _mission_field_domains(owner: MissionOwner) -> tuple[str, ...]:
        domains: set[str] = {"teamwork"}
        for objective in owner.mission.objectives:
            kind = objective.kind
            if kind in ("protect", "preserve", "escort", "hold", "secure"):
                domains.add("protection")
            if kind in ("rescue",):
                domains.add("medicine")
            if kind in ("capture", "restrain", "intercept"):
                domains.add("capture")
            if kind in ("observe", "identify", "investigate"):
                domains.add("recon")
            if kind in ("negotiate",):
                domains.add("social")
            if kind in ("defeat", "destroy", "prevent"):
                domains.add("combat")
            if kind in ("reach", "extract", "escape", "survive", "deliver"):
                domains.add("fieldcraft")
            if kind in ("sabotage", "conceal"):
                domains.add("infiltration")
        return tuple(sorted(domains))

    def _mission_house_field_evidence(
        self, *, owner: MissionOwner, current_time: CampaignTime
    ) -> tuple[dict[str, bytes], tuple[str, ...]]:
        """Persist bounded field evidence for rostered House people.

        This never grants immediate stat points.  It records what a real
        mission exercised; the House training host may later consolidate that
        evidence. Exactified people retain the same lite service ledger as a
        cheap projection/history while exact mechanics remain authoritative for
        overlapping numerical capability.
        """
        if owner.mission.state not in _TERMINAL_MISSION_STATES:
            return {}, ()
        roster_path = "state/person-core/house-tang.json"
        raw = self.repository.read_optional_bytes(roster_path)
        if raw is None:
            return {}, ()
        try:
            roster = copy.deepcopy(self.repository.read_json(roster_path))
        except (FileNotFoundError, ValueError) as exc:
            raise CommandRejectedError("house_roster_profile_invalid") from exc
        profiles = roster.get("profiles") if isinstance(roster, Mapping) else None
        if not isinstance(profiles, dict):
            raise CommandRejectedError("house_roster_profile_invalid")
        refs = [ref for ref in owner.mission.participant_refs if ref in profiles]
        if not refs:
            return {}, ()
        rank_units = {"D": 2, "C": 4, "B": 7, "A": 11, "S": 16}
        base_units = rank_units.get(owner.mission_rank, 2)
        outcome_milli = {"succeeded": 1000, "failed": 700, "aborted": 450, "expired": 300}.get(owner.mission.state, 500)
        units = max(1, base_units * outcome_milli // 1000)
        domains = list(self._mission_field_domains(owner))
        evidence_ref = f"{owner.mission_id}:settlement:{owner.mission.state}"
        touched: list[str] = []
        for ref in refs:
            entry = profiles.get(ref)
            if not isinstance(entry, MutableMapping):
                raise CommandRejectedError("house_roster_profile_invalid")
            institutional = entry.get("institutional_progression")
            if not isinstance(institutional, MutableMapping):
                raise CommandRejectedError("house_roster_profile_invalid")
            history = institutional.get("service_history")
            already_recorded = (
                isinstance(history, list)
                and any(isinstance(row, Mapping) and row.get("evidence_ref") == evidence_ref for row in history)
            )
            try:
                record_rostered_field_evidence(
                    ...
                    kind="mission",
                    evidence_units=float(units),
                    domains=domains,
                    at=str(current_time),
                    details={
                        "mission_ref": owner.mission_id,
                        "mission_rank": owner.mission_rank,
                        "outcome": owner.mission.state,
        ...
        if not touched:
            return {}, ()
        return {roster_path: _json_bytes(roster)}, tuple(touched)
    ...
    def _mission_built_plan(
        ...
        *,
        ...
        merged_extra_writes = dict(extra_writes or {})
        house_evidence_refs: tuple[str, ...] = ()
        # Terminal mission settlement is field evidence, not instant character
        # advancement.  Rostered House people receive a bounded evidence ledger
        # that later training can consolidate.
        if owner.mission.state in _TERMINAL_MISSION_STATES and command.command_type == "mission_derive_and_settle":
            house_writes, house_evidence_refs = self._mission_house_field_evidence(
                owner=owner, current_time=current_time
            )
            for evidence_path, evidence_bytes in house_writes.items():
                if evidence_path in merged_extra_writes and merged_extra_writes[evidence_path] != evidence_bytes:
                    raise CommandRejectedError("mission_extra_write_conflict")
                merged_extra_writes[evidence_path] = evidence_bytes
        world_events = self._world_events()
        ...
        event_id = self._append_semantic_event(
            ...
            affected_owner_refs=(path, self.scheduler_path, *tuple(sorted(merged_extra_writes.keys()))),
        ...
        scheduler_writes = self._scheduler_write_images(scheduler)
        writes = {
            self.meta_path: _json_bytes(
                self._meta_after(meta, command, world_time=current_time)
            ),
        ...
        for extra_path, extra_bytes in sorted(merged_extra_writes.items()):
            if extra_path in writes and writes[extra_path] != extra_bytes:
                raise CommandRejectedError("mission_extra_write_conflict")
            writes[extra_path] = extra_bytes
        writes.update(scheduler_writes)
        writes = self._prune_noop_writes(writes)
        ...
        if house_evidence_refs:
            enriched_result["house_field_evidence_refs"] = list(house_evidence_refs)
        return _BuiltPlan(
            code=code,
            affected_refs=expected_paths,
            writes=writes,
