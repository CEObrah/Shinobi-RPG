...
def test_generic_disable_intent_targets_function_denial_for_people_without_personal_doctrine():
    ...
    # Generic restraint must aim at a real function-denial limb structure, but
    # actor-conditioned variation prevents every fighter from selecting the same
    # exact family/side merely because a global list is deterministic.
    choice=intent_target(actor,intent='disable',target=target)
    assert any(name in choice for name in ('wrist','elbow','knee','ankle'))
    assert intent_target(actor,intent='lethal',target=target) is None


def test_generic_disable_intent_preserves_discipline_bias_without_universal_exact_targeting():
    target=_person(); target['person_id']='generic-target'
    ...
    for discipline,expected_primary in primary_family.items():
        counts={name:0 for name in ('wrist','elbow','knee','ankle')}
        for index in range(64):
            actor=copy.deepcopy(base)
            actor['person_id']=f'{discipline}-{index}'
            actor['martial_skills']={key:0 for key in ('sword','spear','unarmed','bow','hidden_weapons')}
            actor['martial_skills'][discipline]=90
            choice=intent_target(actor,intent='disable',target=target)
            family=next(name for name in counts if name in choice)
            counts[family]+=1
        assert max(counts,key=counts.get) == expected_primary, (discipline,counts)
        assert sum(1 for value in counts.values() if value>0) >= 3, (discipline,counts)


def test_generic_spear_disable_targeting_is_actor_conditioned_not_formation_wide_knee_spam():
    target=_person(); target['person_id']='formation-target'; target['health']={'status':'ready','injuries':[],'blood_lost_ml':0,'shock':0,'consciousness':100}
    choices=[]
    for index in range(16):
        actor=_person(); actor.pop('combat_doctrine_ref',None); actor['person_id']=f'generic-spear-{index}'
        actor['martial_skills']={key:0 for key in ('sword','spear','unarmed','bow','hidden_weapons')}
        actor['martial_skills']['spear']=90
        choices.append(intent_target(actor,intent='disable',target=target))
    families={next(name for name in ('wrist','elbow','knee','ankle') if name in choice) for choice in choices if choice}
    assert len(families) >= 3
    assert len(set(choices)) >= 4
    assert not all('knee' in choice for choice in choices if choice)
