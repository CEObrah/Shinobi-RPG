...
def _person(ref: str, *, birth_year: int = 30, sword: int = 50, scouting: int = 30,
            ...
            joined_year: int | None = None, offices=()):
    row = {
...
def test_delegated_retinue_is_exactly_three_complementary_permanent_members():
    ...
    refs, roles = select_retinue_members(people[0], people, requested_count=0, year=61)
    assert len(refs) == 3
...
def test_permanent_team_excludes_probationary_recruits_even_when_they_are_role_best():
    ...
    refs, _ = select_retinue_members(
        ...
        [leader, probationary, trusted, _person("guard", birth_year=38, sword=70),
         _person("scout", birth_year=37, scouting=70)], requested_count=3, year=61)
...
def test_permanent_team_excludes_same_year_recruit_even_after_junior_promotion():
    ...
    refs, _ = select_retinue_members(
        ...
        [leader, newcomer, established, _person("medic", birth_year=39, medicine=80),
         _person("guard", birth_year=37, sword=80)], requested_count=3, year=61)
...
def test_retinue_excludes_critical_house_officers_minors_and_unavailable_people():
    ...
    refs, _ = select_retinue_members(people[0], people, requested_count=3, year=61,
...
def test_capability_outranks_age_closeness_once_candidate_is_inside_lawful_cohort():
    ...
    refs, roles = select_retinue_members(
        ...
        [leader, _person("medic", birth_year=39, medicine=90), near_weak, older_strong,
         _person("scout", birth_year=38, scouting=85)], requested_count=3, year=61)
...
def test_legacy_exact_two_member_selection_remains_readable_but_is_not_live_default():
    people = [_leader(), _person("medic", birth_year=39, medicine=90),
              _person("guard", birth_year=38, sword=90), _person("scout", birth_year=37, scouting=90)]
    refs, roles = select_retinue_members(people[0], people, requested_count=2, year=61)
    assert len(refs) == 2
    assert len(roles) == 2
