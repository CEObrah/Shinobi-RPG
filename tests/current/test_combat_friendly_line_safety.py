...
def test_autonomous_schedule_fails_closed_when_first_contact_side_is_unresolved():
    actor, friend, target, people, ledger, combat = _fixture()
    # Keep the friendly body physically in the lane while corrupting the exact
    # side authority that autonomous safety must consult. This must not degrade
    # into "no blocker" and authorize a fresh AI attack.
...
def test_autonomous_schedule_fails_closed_when_control_geometry_is_malformed():
    actor, friend, target, people, ledger, combat = _fixture()
    combat["positions"] = []
    with pytest.raises(ValueError, match="autonomous_attack_control_state_invalid"):
        exact._schedule_action(
            combat=combat,
            actor_ref=actor["person_id"],
            target_ref=target["person_id"],
...
def test_autonomous_projectile_is_withheld_if_friendly_crosses_lane_before_resolution():
    actor, friend, target, people, ledger, combat = _fixture()
    ledger.setdefault("person_loadouts", {})[actor["person_id"]] = {"items": {"weapon_needle": 12}}
    combat["positions"][friend["person_id"]].update(x_mm=2500, y_mm=2000)
    ...
    # The lane was clear at declaration. A same-side body then crosses the
    # projectile path before the attack resolves on the shared combat clock.
    combat["positions"][friend["person_id"]].update(x_mm=2500, y_mm=0)
    ...
    assert event["friendly_blocker_ref"] == friend["person_id"]
    assert event["safety_phase"] == "pre_resolution_dynamic_lane_recheck"


def test_autonomous_melee_is_withheld_if_friendly_crosses_lane_before_resolution():
    actor, friend, target, people, ledger, combat = _fixture()
    # Declaration lane is clear. The ally crosses the committed melee line after
    # scheduling but before contact; autonomous AI must give up the opening
    # rather than letting the ally become a defensive counter-contact victim.
    combat["positions"][friend["person_id"]].update(x_mm=550, y_mm=1200)
    action = exact._schedule_action(
        combat=combat,
        actor_ref=actor["person_id"],
        target_ref=target["person_id"],
    ...
    assert event["friendly_blocker_ref"] == friend["person_id"]
    assert event["safety_phase"] == "pre_resolution_dynamic_lane_recheck"
    assert not friend["health"]["injuries"]
    assert not actor["health"]["injuries"]
