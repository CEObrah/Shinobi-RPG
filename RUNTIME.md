...
Treat live play as continuous architecture testing. When gameplay exposes a systemic simulation, validation, routing, data-consistency, narration, maintainability, or repository-design defect or improvement opportunity, surface it briefly OOC. Repair or implement it immediately when the change is safe, deterministic, reduces unnecessary complexity/context/bloat or strengthens authority, routing, validation, or play quality, and does not rewrite a lawful campaign outcome or invent Wei's intent. If the change is invasive or uncertain, identify it OOC and continue play from authoritative state rather than silently normalizing bad state. Do not interrupt play for cosmetic nits.
