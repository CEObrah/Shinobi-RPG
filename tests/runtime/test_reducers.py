...
from shinobi_runtime.reducers.training import point_cost
...
def test_training_executes_declared_formula_and_is_representation_neutral():
    ...
    assert outcomes[0].ending_value == 80
    assert outcomes[0].residual_units == Decimal("4.160")


def test_training_mastery_cost_rises_sharply_after_professional_bands():
    assert point_cost(40) == 1
    assert point_cost(60) == 2
    assert point_cost(80) == 5
    assert point_cost(100) == 10
    assert point_cost(120) == 19
    assert point_cost(140) == 34
    assert point_cost(160) == 55
    assert point_cost(180) == 82
    assert point_cost(200) == 115
...
def test_routine_training_does_not_create_legendary_progress_above_ceiling():
    inputs = training("exact")
    capped = TrainingInputs(**{
        **inputs.__dict__,
        "current_value": 205,
        "residual_units": "250",
    })
    result = settle_training(capped)
    assert result.ending_value == 205
    assert result.points_gained == 0
    assert Decimal("0") <= result.residual_units < point_cost(205)


def test_repeated_low_volume_training_does_not_rush_the_200_reference_band():
    value = 100
    residual = "0"
    for _week in range(52 * 10):
        outcome = settle_training(
            TrainingInputs(
                scheduled_hours="4", attendance="1",
                available_instructor_hours="0", required_instructor_hours="0",
                facility_slots="1", required_slots="1",
                equipment_sets="1", required_sets="1",
                instructor_quality_factor="1", facility_quality_factor="1",
                equipment_factor="1", health_factor="1", recovery_factor="1",
                relevance_factor="1", difficulty_fit_factor="1",
                aptitude=130, experience_modifier="1", current_value=value,
                residual_units=residual, representation="exact",
            )
        )
        value = outcome.ending_value
        residual = str(outcome.residual_units)
    assert value < 180
