...
def _effective_location_ref(person: Mapping[str, Any], faction: Mapping[str, Any]) -> str:
    """Resolve sparse at-home people to the institution's real local site."""
    return str(person.get("location_ref") or faction.get("local_site_ref") or faction.get("headquarters") or "")
...
class JianghuInstitutionalCommandsMixin:
    ...
    def _jianghu_institutional_operation_resolution(self, command: CommandEnvelope, meta: Mapping[str, Any], current_time: Any):
        ...
        if action == "convene":
            ...
            actor_location = _effective_location_ref(actor, faction)
            actor_place = person_place(actor, local_sites=sites_doc, home_place=str(faction.get("headquarters") or ""), home_site_ref=str(faction.get("local_site_ref") or ""))
            ...
            for ref in attendee_refs:
                ...
                effective_ref = _effective_location_ref(person, faction)
                place = person_place(person, local_sites=sites_doc, home_place=str(faction.get("headquarters") or ""), home_site_ref=str(faction.get("local_site_ref") or ""))
                # A House hall meeting requires physical co-presence in the same
                # settlement. Sparse at-home records resolve to the House site.
                if place != actor_place:
            ...
            row["updated_at"] = now_iso
            active[op_ref] = row
            return self._simple_plan(command, meta, current_time, writes_records={OPERATIONS_PATH: state}, code="jianghu_institutional_council_convened", result={"command_type": command.command_type, "operation_ref": op_ref, "attendee_refs": attendee_refs, "approval_authority_ref": row["council"].get("approval_authority_ref")})
        ...
        if action == "submit_plan":
            ...
            for ref in member_refs:
                ...
                locations.add(person_place(person, local_sites=sites_doc, home_place=str(faction.get("headquarters") or ""), home_site_ref=str(faction.get("local_site_ref") or "")))
            if len(locations) != 1:
            ...
            effective_place = next(iter(locations)) if locations else ""
            ready_home = 0
            for person in all_people:
                if not isinstance(person, Mapping):
                    continue
                health = person.get("health", {}) if isinstance(person.get("health"), Mapping) else {}
                if health.get("status") == "dead" or int(health.get("consciousness", 100)) <= 0 or bool(person.get("retired_from_field", False)):
                    continue
                if person_place(person, local_sites=sites_doc, home_place=str(faction.get("headquarters") or ""), home_site_ref=str(faction.get("local_site_ref") or "")) == effective_place:
                    ready_home += 1
        ...
        if action == "dispatch":
            ...
            source_ref = _effective_location_ref(commander, faction)
