...
# Per-settlement work budgets, never lifetime/world cardinality limits.
# A larger scheduler resumes across additional settlement calls instead of
# becoming invalid merely because more hosts or queued events exist.
...
class CatchUpEngine:
    ...
    def settle(
        ...
        *,
        ...
        initial_queue_size = len(initial_events)
        ...
        while len(processed) < event_budget:
            ...
            try:
                ...
                # These are work budgets, not validity ceilings.  Preserve the
                # current event and return a resumable frontier when this
                # settlement would touch too many distinct hosts or grow the
                # pending queue too far beyond its starting size.
                prospective_causal_hosts = touched_host_ids | affected_host_ids
                if (
                    len(prospective_causal_hosts) > causal_host_budget
                    or len(prospective_events) > initial_queue_size + pending_event_budget
                ):
                    # A work budget may stop only between causal timestamps. If
                    # more events share the already-entered boundary, finish that
                    # boundary coherently even when it overruns the soft target;
                    # otherwise a committed world clock could strand overdue work.
                    if event.due_at > reached:
                        queue.add(event)
            ...
            touched_host_ids = prospective_causal_hosts
        ...
        budget_exhausted = work_budget_exhausted or (
            len(processed) >= event_budget
            and queue.peek() is not None
            and queue.peek().due_at <= target
        )
        unsafe = tuple(sorted(
