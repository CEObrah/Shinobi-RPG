...
def test_outlaw_rivalry_uses_predatory_affordance_instead_of_honor_challenge():
    ...
    envelopes = [row for row in actions if row is not None]
    assert envelopes
    options = [option for envelope in envelopes for option in envelope.get("options", [])]
    assert options
    assert all(option["action"] != "formal_challenge" for option in options)
    assert all(option.get("advisory_operation_intent") in {"robbery", "revenge_strike"} for option in options)
    assert all("operation_intent_options" in option for option in options)
...
def test_semantic_resolution_methods_have_one_mechanical_owner_or_explicit_staging_wrapper():
    methods = {}
    ...
    # Production route-contact compatibility stages owner records around the
    # canonical repository build; it must never define a second semantic combat
    # reducer with the same command name.
    campaign_text = (ROOT / "runtime/shinobi_runtime/commands/campaign_planner.py").read_text(encoding="utf-8")
    assert "def _jianghu_combat_resolution" not in campaign_text
    assert "def _combat_build_with_staged_records" in campaign_text
    assert "return base_planner._build(command)" in campaign_text
    assert "plan = self._combat_build_with_staged_records(command, staged)" in campaign_text
