...
from shinobi_runtime.commands.core import _BuiltPlan, _campaign_datetime, _declared_payload, _json_bytes, _stable_id
...
def _install_manufacturing_command_spec() -> None:
    COMMAND_SPECS.setdefault(
        ...
        CommandSpec(
            ...
            ("schedule_ref", "recipe_ref", "place_ref", "stock_ref"),
            "Create or cancel one conserved standing institutional manufacturing schedule.",
        ),
    )
...
class CampaignCommandPlanner(_Base):
    ...
    def _institution_manufacturing_resolution(
        ...
        if action == "schedule":
            ...
            if any(command.payload.get(key) is not None for key in ("recipe_ref", "place_ref", "stock_ref")):
                raise CommandRejectedError("institution_manufacturing_fields_invalid")
