...
def advance_npc_relationships(
    ...
    if not started:
        ...
        eligible_refs = [
            ref for ref, person in people.items()
            if ref not in married_refs and ref not in active_courtship_refs
            and (not player_ref or ref != player_ref) and _age(person, now.year) >= min_age
        ...
        desired_pairs = max(0, len(eligible_refs) * max(0, min(1000, target_share)) // 2000)
        current_pairs = sum(
            1 for row in marriages.values()
            if isinstance(row, Mapping) and row.get("status") == "married" and row.get("faction_ref") == faction_ref
        )
        deficit = max(0, desired_pairs - current_pairs)
...
def review_conceptions(
    ...
    """Create only causally eligible deterministic pregnancies for one faction.
    ...
    for marriage_ref in sorted(marriages):
        ...
        if marriage.get("status") != "married" or marriage.get("faction_ref") != faction_ref:
            continue
...
def apply_family_death_status(
    family: Mapping[str, Any], *, dead_refs: Sequence[str], faction_ref: str,
    roster_people: Sequence[Mapping[str, Any]] = (),
) -> dict[str, Any]:
    """Close current family authorities invalidated by death.

    Parentage and marriage identity remain durable kinship facts. Household
    membership and succession claims are current authorities, so dead members
    are removed and a household head is chosen only from that same household.
    """
    out = copy.deepcopy(dict(family)); dead = {str(x) for x in dead_refs}
    if not dead:
        return out
    living = {
        str(p.get("person_id")): p for p in roster_people
        if isinstance(p, Mapping) and isinstance(p.get("person_id"), str)
        and str(p.get("person_id")) not in dead and _usable(p)
    }
    ...
    if isinstance(marriages, dict):
        for key, raw in list(marriages.items()):
            if not isinstance(raw, Mapping):
                continue
            same_faction = raw.get("faction_ref") == faction_ref
            cross_factions = raw.get("faction_refs", []) if isinstance(raw.get("faction_refs"), list) else []
            if not same_faction and faction_ref not in cross_factions:
                continue
            refs = raw.get("spouse_refs", [])
            if isinstance(refs, list) and any(str(x) in dead for x in refs):
                row = copy.deepcopy(dict(raw)); row["status"] = "widowed"; row.pop("pregnancy", None); marriages[key] = row
    claims = out.get("succession_claims", {})
    if isinstance(claims, dict):
        for claim_ref, raw in list(claims.items()):
            if isinstance(raw, Mapping) and raw.get("faction_ref") == faction_ref and str(raw.get("person_ref")) in dead:
                claims.pop(claim_ref, None)
    households = out.get("households", {})
    if isinstance(households, dict):
        for hid, raw in list(households.items()):
            if not isinstance(raw, Mapping) or raw.get("faction_ref") != faction_ref:
                continue
            row = copy.deepcopy(dict(raw))
            members = [
                str(ref) for ref in row.get("member_refs", [])
                if isinstance(ref, str) and ref not in dead
            ]
            row["member_refs"] = members
            if not members:
                households.pop(hid, None)
                continue
            if str(row.get("head_ref")) in dead or str(row.get("head_ref")) not in members:
                # Oldest living member of this household succeeds as household
                # head; stable ID breaks same-year ties. Never import an
                # unrelated political claimant from elsewhere in the faction.
                eligible_heads = [ref for ref in members if ref in living] or members
                row["head_ref"] = min(
                    eligible_heads,
                    key=lambda ref: (int(living.get(ref, {}).get("birth_year", 10**9)), ref),
                )
            households[hid] = row
    return out
...
__all__ = [
    "advance_annual_life_course", "apply_family_death_status", "apply_recognized_succession", "newborn_person",
