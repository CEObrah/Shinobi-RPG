...
from shinobi_runtime.martial_world.institutional_progression import (
    _View, _player_message_target, close_expired_contract_dossiers,
)
from shinobi_runtime.store.repository import RepositoryStore
from fixture_support import make_person_activity_ready_fixture
...
def test_courier_targets_underlying_route_destination_while_combat_owns_presence():
    files = {
        "state/martial-world/custody.json": {"active": {}},
        ...
        "state/martial-world/combats.json": {
            "combats": {
                "combat.test": {
                    ...
                    "elapsed_ms": 500,
                    "sides": {"side_a": ["pc_wei_tang"], "side_b": ["enemy"]},
                    "combatants": {
                        "pc_wei_tang": {"status_families": []},
                        "enemy": {"status_families": []},
                    },
                    "positions": {
                        "pc_wei_tang": {"zone_ref": "route.changan.huashan"},
                        "enemy": {"zone_ref": "route.changan.huashan"},
                    },
                }
            }
        },
    }
    at = datetime.fromisoformat("0061-09-27T21:15:00")
    view = _View(_reader(files), {})
    target, eta = _player_message_target(
        view, "pc_wei_tang", {"person_id": "pc_wei_tang", "location_ref": "changan"}, at
    )
    assert target == "huashan"
    assert int((eta - at).total_seconds()) == 600
...
def _prepare_institutional_fixture(root, player_ref="pc_wei_tang"):
    ...
    council_refs = {player_ref, "char.zhu", "char.ling"}
    for person in roster.get("people", []):
        if isinstance(person, dict) and person.get("person_id") in council_refs:
            make_person_activity_ready_fixture(person)
...
def test_house_assignment_offer_requires_house_gm_judgment_before_player_offer():
    from shinobi_runtime.martial_world.institutional_progression import (
    ...
    handoffs = []; pending = []
    reviews = [{
        "kind": "faction_review", "faction_ref": "house_tang",
        "executed_actions": [{
            "action": "respond_known_captivity", "result": "player_decision_required",
            "person_ref": "mw.person.house_tang.1000", "holder_faction_ref": "shaolin",
    ...
    added = stage_house_assignment_offers(
        ...
        pending_one_off_events=pending, reviews=reviews, at=at,
    )
    assert added == []
    assert writes[OPERATIONS_PATH]["active"] == {}
    assert handoffs == []
    assert len(pending) == 1 and pending[0]["kind"] == "npc_judgment_due"
    assert pending[0]["judgment_kind"] == "house_assignment_offer_policy"
    assert pending[0]["requires_gm_judgment"] is True
    rescue = next(row for row in pending[0]["options"] if row.get("intent") == "offer" and row.get("proposal", {}).get("mission_kind") == "rescue")
    committed = {
        "event_id": "npc_judgment_committed:test-house-offer", "kind": "npc_judgment_committed",
        "due_at": at.isoformat(), "owner_ref": "house_tang", "actor_ref": "house_tang",
        "judgment_kind": "house_assignment_offer_policy", "selected_option": copy.deepcopy(rescue),
        "decision_context": copy.deepcopy(pending[0]["decision_context"]),
    }
    delivery_pending = []
    added = stage_house_assignment_offers(
        read_json=repository.read_json, writes=writes, handoffs=handoffs,
        pending_one_off_events=delivery_pending, reviews=reviews, at=at, events=[committed],
    )
    assert len(added) == 1
    ...
    # Wei is traveling in the current save; even an authored House offer must
    # travel physically before it becomes the player's protected decision.
    assert handoffs == []
    assert row["offer_delivery_status"] == "in_transit"
    assert len(delivery_pending) == 1
    assert delivery_pending[0]["kind"] == "house_assignment_offer_delivery"
    assert delivery_pending[0]["requires_player_decision"] is False
    ...
    combats = copy.deepcopy(repository.read_json("state/martial-world/combats.json"))
    for combat_row in combats.get("combats", {}).values():
        if not isinstance(combat_row, dict) or combat_row.get("status") != "active":
            continue
        sides = combat_row.get("sides", {}) if isinstance(combat_row.get("sides"), dict) else {}
        if any("pc_wei_tang" in refs for refs in sides.values() if isinstance(refs, list)):
            combat_row["status"] = "resolved"
    writes["state/martial-world/combats.json"] = combats
    writes["state/martial-world/people/house_tang.json"] = roster
    delivered_handoffs = []; replacements = []
    delivered = settle_house_assignment_offer_deliveries(
        ...
        events=delivery_pending, at=datetime.fromisoformat(delivery_pending[0]["due_at"]),
...
def test_delegated_prisoner_exchange_releases_only_exact_authorized_captives(tmp_path):
    ...
    built = execute("jianghu_diplomacy_resolution", {
        ...
        "source_captive_refs": [shaolin_captive], "target_captive_refs": [house_captive],
        "institutional_operation_ref": op_ref, "target_response": "accept",
